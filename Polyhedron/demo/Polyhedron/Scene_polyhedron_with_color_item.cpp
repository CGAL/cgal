...
Input_facets_AABB_tree* get_aabb_tree(Scene_polyhedron_with_color_item* item)
{
...
void delete_aabb_tree(Scene_polyhedron_with_color_item* item)
{
  QVariant aabb_tree_property = item->property(aabb_property_name);
  if(aabb_tree_property.isValid()) {
    void* ptr = aabb_tree_property.value<void*>();
...
Scene_polyhedron_with_color_item::Scene_polyhedron_with_color_item()
...
Scene_polyhedron_with_color_item::Scene_polyhedron_with_color_item(Polyhedron* const p)
  : Scene_item_with_display_list(),
    poly(p),
    show_only_feature_edges_m(false),
    facet_picking_m(false),
...
Scene_polyhedron_with_color_item::Scene_polyhedron_with_color_item(const Polyhedron& p)
  : Scene_item_with_display_list(),
...
Scene_polyhedron_with_color_item::clone() const {
  return new Scene_polyhedron_with_color_item(*poly);
}

// Load polyhedron from .OFF file
bool
Scene_polyhedron_with_color_item::load(std::istream& in)
{
  in >> *poly;
...
bool 
Scene_polyhedron_with_color_item::save(std::ostream& out) const
{
  out.precision(13);
  out << *poly;
  return out;
...
QString 
Scene_polyhedron_with_color_item::toolTip() const
{
  if(!poly)
    return QString();
...
QMenu* Scene_polyhedron_with_color_item::contextMenu()
{
...
void Scene_polyhedron_with_color_item::show_only_feature_edges(bool b)
{
  show_only_feature_edges_m = b;
  emit itemChanged();
}

void Scene_polyhedron_with_color_item::enable_facets_picking(bool b)
{
  facet_picking_m = b;
}

void Scene_polyhedron_with_color_item::set_erase_next_picked_facet(bool b)
{
  erase_next_picked_facet_m = b;
}

// Points/Wireframe/Flat/Gouraud OpenGL drawing in a display list
void Scene_polyhedron_with_color_item::direct_draw() const {
  //gl_render_facets(*poly,colors_);
}
void Scene_polyhedron_with_color_item::draw() const {
	//for(std::vector<QColor>::const_iterator it = colors_.begin(); it != colors_.end(); ++it)
	//	qDebug() << it->red();
  gl_render_facets(*poly,colors_);
}
// Points/Wireframe/Flat/Gouraud OpenGL drawing in a display list
void Scene_polyhedron_with_color_item::direct_draw_edges() const {
...
Polyhedron* 
Scene_polyhedron_with_color_item::polyhedron()       { return poly; }
const Polyhedron* 
Scene_polyhedron_with_color_item::polyhedron() const { return poly; }

bool
Scene_polyhedron_with_color_item::isEmpty() const {
  return (poly == 0) || poly->empty();
}

Scene_polyhedron_with_color_item::Bbox
Scene_polyhedron_with_color_item::bbox() const {
...
void 
Scene_polyhedron_with_color_item::select(double orig_x,
                              double orig_y,
                              double orig_z,
                              double dir_x,
                              double dir_y,
...
void Scene_polyhedron_with_color_item::set_color_vector(std::vector<QColor>& colors)
{
	colors_ = colors;
	//for(std::vector<QColor>::const_iterator it = colors_.begin(); it != colors_.end(); ++it)
	//	qDebug() << it->red();
}
#include "Scene_polyhedron_with_color_item.moc"
