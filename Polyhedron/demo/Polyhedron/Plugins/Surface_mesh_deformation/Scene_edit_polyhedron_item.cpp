...
void Scene_edit_polyhedron_item_priv::apply_reset_drawing_data(Mesh* mesh, M_Deform_mesh* m_deform_mesh)
{
  ...
  CGAL_FOREACH(mesh_vd vb, vertices(*mesh))
  {
  ...
  CGAL_FOREACH(mesh_fd fb, faces(*mesh))
  {
...
void Scene_edit_polyhedron_item_priv::compute_normals_and_vertices(Mesh* mesh)
{
  typedef typename boost::graph_traits<Mesh>::vertex_descriptor mesh_vd;
    ...
    CGAL_FOREACH(mesh_vd vd, fs.get_deform_mesh(mesh)->roi_vertices())
    {
...
struct Is_constrained_map<Polyhedron>
{
  ...
  Is_constrained_map( std::vector<int>* vec, Polyhedron* mesh)
  {
    ...
    CGAL_FOREACH(mesh_vd v, vertices(*mesh))
    {
      (*icmap)[v] = (*vec)[v->id()];
    }
  }
...
void Scene_edit_polyhedron_item_priv::remesh(Mesh* mesh)
{
      ...
      hgb_data != fs.get_ctrl_vertex_frame_map(mesh).end(); ++hgb_data)
  {
    ...
    CGAL_FOREACH(mesh_vd vd, hgb_data->ctrl_vertices_group)
    {
  ...
  CGAL_FOREACH(mesh_vd v, fs.get_deform_mesh(mesh)->roi_vertices())
  {
    CGAL_FOREACH(mesh_fd fv, CGAL::faces_around_target(halfedge(v, g), g))
    {
      if(fv == boost::graph_traits<Mesh>::null_face())
        continue;
      bool add_face=true;
      CGAL_FOREACH(mesh_vd vfd, CGAL::vertices_around_face(halfedge(fv,g),g))
  ...
  CGAL_FOREACH(mesh_fd f, faces(*mesh))
  ...
  {
    double sum_len = 0.;
    CGAL_FOREACH(mesh_ed e, roi_border)
    {
...
//re-create ctrl_groups
  ...
  {
    item->create_ctrl_vertices_group();
    CGAL_FOREACH(mesh_vd vd, control_groups[i]){
      item->insert_control_vertex<Mesh>(vd, mesh);
    }
  }

  CGAL_FOREACH(mesh_fd f, faces(g))
  {
    if (get(roi_faces_pmap, f) == 0/*false*/)
      continue;
    CGAL_FOREACH(mesh_hd h, halfedges_around_face(halfedge(f, g), g))
    {
...
void Scene_edit_polyhedron_item::save_roi(const char* file_name) const
{
  ...
  if(d->poly_item)
  {
    ...
    CGAL_FOREACH(poly_vertex_descriptor vd, d->deform_mesh->roi_vertices())
    {
      out << vd->id() << " ";
    }
  }
  else
  {
    ...
    CGAL_FOREACH(sm_vertex_descriptor vd, d->deform_sm_mesh->roi_vertices())
    {
