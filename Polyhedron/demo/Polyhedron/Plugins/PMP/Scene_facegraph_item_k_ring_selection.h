...
public Q_SLOTS:
  ...
  void lasso_selection()
  {
    ...
    CGAL_FOREACH(fg_face_descriptor f, faces(poly))
    {
      CGAL_FOREACH(fg_vertex_descriptor v, CGAL::vertices_around_face(halfedge(f, poly), poly))
      {
    ...
    CGAL_FOREACH(fg_halfedge_descriptor h, boundary_edges)
    ...
    CGAL_FOREACH(fg_face_descriptor f, face_sel)
    {
    ...
    case Active_handle::EDGE:
    {
      ...
      CGAL_FOREACH(fg_face_descriptor f, final_sel)
      {
        CGAL_FOREACH(fg_halfedge_descriptor h, CGAL::halfedges_around_face(halfedge(f, poly), poly))
        {
