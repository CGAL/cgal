...
void Polyhedron_demo_mean_curvature_flow_skeleton_plugin::on_actionSegment()
{
  ...
  CGAL_FOREACH(boost::graph_traits<Skeleton>::vertex_descriptor v, vertices(item->skeleton_curve) )
  {
    const Point& skel_pt = item->skeleton_curve[v].point;
    CGAL_FOREACH(vertex_descriptor mesh_v, item->skeleton_curve[v].vertices)
    {
  ...
  CGAL_FOREACH(boost::graph_traits<Face_graph>::face_descriptor f, faces(*item->input_triangle_mesh))
  {
    double dist = 0;
    CGAL_FOREACH(boost::graph_traits<Face_graph>::halfedge_descriptor hd, halfedges_around_face(halfedge(f, *item->input_triangle_mesh), *item->input_triangle_mesh))
  ...
  CGAL_FOREACH(boost::graph_traits<Face_graph>::face_descriptor fd, faces(*segmented_polyhedron))
  {
