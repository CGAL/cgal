  ...
  bool applicable(QAction* a) const {
    ...
    else if(a==actionExport )
    {
      for(int i = 0, end = scene->numberOfEntries();
          i < end; ++i)
      {
        if(qobject_cast<Scene_edit_path_item*>(scene->item(i)))
        {
          return true;
        }
      }
...
public Q_SLOTS:
  ...
  void connectNewViewer(QObject* o)
  {
    ...
    {
      Scene_edit_path_item* item = qobject_cast<Scene_edit_path_item*>(
            scene->item(i));
      if(item)
...
void Locally_shortest_path_plugin::init(QMainWindow* mainWindow, CGAL::Three::Scene_interface* scene_interface, Messages_interface*)
{
  ...
  mw = mainWindow;
  actionBbox = new QAction(tr("Create Locally Shortest Path"), mainWindow);
  connect(actionBbox, SIGNAL(triggered()),
          this, SLOT(bbox()));
  actionExport = new QAction(tr("PIPO"), mainWindow);
...
void Locally_shortest_path_plugin::bbox()
{
  for(int i = 0, end = scene->numberOfEntries();
      i < end; ++i)
  {
  ...
  Scene_edit_path_item* item = new Scene_edit_path_item(scene);
...
void Locally_shortest_path_plugin::exportToPoly()
{
  int id =0;
  const CGAL::qglviewer::Vec v_offset = Three::mainViewer()->offset();
  EPICK::Vector_3 offset(v_offset.x, v_offset.y, v_offset.z);
  Scene_edit_path_item* item = nullptr;
  for(int i = 0, end = scene->numberOfEntries();
      i < end; ++i)
  {
    item = qobject_cast<Scene_edit_path_item*>(scene->item(i));
    if(item)
    {
      id = i;
      break;
