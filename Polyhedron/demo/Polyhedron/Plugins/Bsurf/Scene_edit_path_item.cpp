#include "Scene_edit_path_item.h"
...
typedef Scene_edit_path_item_priv Priv;

struct Scene_edit_path_item::vertex{
  int id;
  double *x;
  double *y;
  double *z;
...
struct Scene_edit_path_item::edge{
  vertex* source;
  vertex* target;
};
struct Scene_edit_path_item::face{
  vertex* vertices[4];
};

struct Scene_edit_path_item_priv{
  ...
  Scene_edit_path_item_priv(const Scene_interface *scene_interface, Scene_edit_path_item* ebi)
  {
  ...
  mutable Scene_edit_path_item::vertex vertices[8];
  mutable Scene_edit_path_item::edge edges[12];
  mutable Scene_edit_path_item::face faces[6];

  std::vector<Scene_edit_path_item::vertex*> selected_vertices;
...
Scene_edit_path_item::Scene_edit_path_item()
{
  d = nullptr;
}
Scene_edit_path_item::Scene_edit_path_item(const Scene_interface *scene_interface)
{
  d = new Scene_edit_path_item_priv(scene_interface, this);
...
QString Scene_edit_path_item::toolTip() const {

  return QString();
}

void Scene_edit_path_item::drawSpheres(Viewer_interface *viewer, const QMatrix4x4 f_matrix ) const
{
  GLdouble d_mat[16];
  QMatrix4x4 mv_mat;
...
void Scene_edit_path_item::draw(Viewer_interface *viewer) const
{
  if(!isInit(viewer))
    initGL(viewer);
  if ( getBuffersFilled() &&
...
void Scene_edit_path_item::drawEdges(Viewer_interface* viewer) const
{
  if(!isInit(viewer))
    initGL(viewer);
  if ( getBuffersFilled() &&
...
void Scene_edit_path_item::compute_bbox() const
{
  const CGAL::qglviewer::Vec offset = static_cast<CGAL::Three::Viewer_interface*>(CGAL::QGLViewer::QGLViewerPool().first())->offset();
...
void push_xyz(std::vector<float> &v,
              const Scene_edit_path_item::Kernel::Point_3& p,
...
void Scene_edit_path_item_priv::computeElements() const
{
  vertex_edges.clear();
  vertex_faces.clear();
  normal_faces.clear();
...
bool Scene_edit_path_item::supportsRenderingMode(RenderingMode m) const {
  return (m==Wireframe || m==FlatPlusEdges);
}

Scene_item::ManipulatedFrame*
Scene_edit_path_item::manipulatedFrame()
{
  return d->frame;
}

double Scene_edit_path_item::point(short i, short j) const
{
...
void Scene_edit_path_item::highlight(Viewer_interface *viewer)
{
  d->ready_to_hl = true;
  viewer->makeCurrent();
  int type = -1, id = -1;
  ...
  if(type !=-1)
  {
    ...
    case 0:
    {
      ...
      //draw
      d->hl_type = Scene_edit_path_item_priv::VERTEX;
      Q_FOREACH(CGAL::QGLViewer* v, CGAL::QGLViewer::QGLViewerPool())
      {
    ...
    case 1:
    {
      ...
      //draw
      d->hl_type = Scene_edit_path_item_priv::EDGE;
      Q_FOREACH(CGAL::QGLViewer* v, CGAL::QGLViewer::QGLViewerPool())
      {
        CGAL::Three::Viewer_interface* viewer =
            static_cast<CGAL::Three::Viewer_interface*>(v);
    ...
    case 2:
    {
      ...
      //draw
      d->hl_type = Scene_edit_path_item_priv::FACE;
      Q_FOREACH(CGAL::QGLViewer* v, CGAL::QGLViewer::QGLViewerPool())
      {
        CGAL::Three::Viewer_interface* viewer =
            static_cast<CGAL::Three::Viewer_interface*>(v);
    ...
    default:
      d->hl_type = Scene_edit_path_item_priv::NO_TYPE;
      break;
    }
  }
  else
...
void Scene_edit_path_item::clearHL()
{
...
void Scene_edit_path_item_priv::reset_selection()
{
  selection_on = false;
  CGAL::QGLViewer* viewer = *CGAL::QGLViewer::QGLViewerPool().begin();
...
bool Scene_edit_path_item::eventFilter(QObject *obj, QEvent *event)
{
  if(!visible())
    return false;
...
void Scene_edit_path_item_priv::draw_picking(Viewer_interface* viewer)
{
...
void Scene_edit_path_item_priv::remodel_box(const QVector3D &dir)
{
  CGAL::qglviewer::AxisPlaneConstraint::Type prev_cons = constraint.translationConstraintType();
  constraint.setTranslationConstraintType(CGAL::qglviewer::AxisPlaneConstraint::FREE);
  Q_FOREACH(Scene_edit_path_item::vertex*  selected_vertex, selected_vertices )
  {
...
double Scene_edit_path_item_priv::applyX(int id, double x, double dirx)
{
  switch(id)
  {
  case 0:
...
double Scene_edit_path_item_priv::applyY(int id, double y, double diry)
{
  switch(id)
  {
  case 0:
...
double Scene_edit_path_item_priv::applyZ(int id, double z, double dirz)
{
  switch(id)
  {
  case 1:
...
void Scene_edit_path_item_priv::picking(int& type, int& id, Viewer_interface *viewer)
{
  viewer->makeCurrent();
  type = -1;
  id = -1;
...
void Scene_edit_path_item::drawHl(Viewer_interface* viewer)const
{
  ...
  if(d->hl_type == Scene_edit_path_item_priv::VERTEX)
  {
    Tc* tc = getTriangleContainer(Priv::S_Spheres);
  ...
  else if(d->hl_type == Scene_edit_path_item_priv::EDGE)
  {
    Ec* ec = getEdgeContainer(Priv::S_Edges);
    if(viewer->isOpenGL_4_3())
    {
  ...
  else if(d->hl_type == Scene_edit_path_item_priv::FACE)
  {
...
void Scene_edit_path_item::drawTransparent(CGAL::Three::Viewer_interface*viewer)const
{
...
void Scene_edit_path_item::invalidateOpenGLBuffers()
{
  compute_bbox();
...
void Scene_edit_path_item::computeElements() const
{
  d->computeElements();
...
void Scene_edit_path_item::initializeBuffers(Viewer_interface *v) const
{
...
void Scene_edit_path_item::connectNewViewer(QObject *o)
{
