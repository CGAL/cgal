...
protected:
  ...
  */
  ...
  CGAL::Oriented_side oriented_side(const Plane_3 &p,
				    const T &s) const {
    CGAL::Object o= geom_traits_object().intersect_3_object()(p, line_through(s));
    Point_3 pt;
    Line_3 l;
    if (CGAL::assign(pt, o)){
      ...
      int ptindex=0;
      do {
	lpt= line_through(s).point(++ptindex);
	os= p.oriented_side(lpt);
      } while (os == ON_ORIENTED_BOUNDARY);
      
      for (unsigned int i=0; i< 3; ++i) {
	Comparison_result csp= compare(lpt[i], pt[i]);
	...
	if (csl== CGAL::EQUAL) return ON_ORIENTED_BOUNDARY;
      ...
      return p.oriented_side(line_through(s).point());
    }
  }
...
private:
  ...
  const Line_3& line_through(const Sphere_point_3 &pt) const {
    return pt.line();
  }
  Line_3 line_through(const Center_point_3 &pt) const {
