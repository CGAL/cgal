...
to_double(const Root_of_2<RT> &x)
{
  ...
  bool using_my_to_double;
  CGAL_assertion(is_valid(x));

  const double a = 1; //CGAL::to_double(x[2]);
  const double b = CGAL::to_double(FT(x[1])/FT(x[2]));
  //std::cout << "b= " << b << std::endl;
  double d = std::sqrt(CGAL_NTS to_double(FT(x.discriminant())/FT(CGAL::square(x[2]))));
