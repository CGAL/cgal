...
bool Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::intersects(Sphere_3_key a,
  ...
  try {
    ...
    CGAL_LOG(Log::LOTS, eqpt  << std::endl);
    ...
    CGAL_ERROR("Equal centers for " << a << " and " << b);
    return false;
  }
  /*}
    return ret;*/
...
bool Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::intersects(Sphere_3_key a,
  ...
  if (CGAL::assign(l,o)){
    ...
    CGAL_LOG(Log::LOTS, "Degeneracy for " << a << " " << b << " " << c
...
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::compare_to_rule_c(const Sphere_point_3 &pt,
  ...
  return pt.compare_c(table_->center(rule)[C.index()], C);
}
...
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::sphere_events(Sphere_3_key ind) const {
  ...
  return Event_pair(Sphere_point_3(table_->sphere(ind), 
		    ...
		    Line_3(table_->center(ind), Vector_3(-v[0], -v[1], -v[2]))));*/
  return event_pair(ind, Line_3(table_->center(ind),
...
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::intersection_2_events(Sphere_3_key a, Sphere_3_key b) const {
  CGAL_LOG(Log::LOTS, "Intersection events for " << a << " and " << b << std::endl);
  if (!intersects(a,b)) {
    return Event_pair();
  }
...
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::circle_cross_rule_event(Sphere_3_key a, 
  ...
  if (ed.index_==-1) {
    CGAL_LOG(Log::LOTS, "Initializing AAR for " << a << " " << b << " " << rs << " " << C << std::endl);
  ...
  if (ed.index_ < 2) {
    CGAL_LOG(Log::LOTS, "Returning " << ed.index_ << " of AAR" << std::endl);
...
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::sphere_intersect_rule_rule_events(Sphere_3_key s, 
  ...
  CGAL_LOG(Log::LOTS, "SRR event for " << s << " " << x << " " << y << std::endl);
...
CGAL_AOS3_TEMPLATE
CGAL_AOS3_TYPENAME Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::Event_pair
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::event_pair(Sphere_3_key k,
							   const Plane_3 &a,
							   const Plane_3 &b) const {
  CGAL::Object o= geom_traits_object().intersect_3_object()(a,b);
  Line_3 l;
  if (CGAL::assign(l,o)) return event_pair(k,l);
  else return Event_pair();
}
...
Arrangement_of_spheres_traits_3 CGAL_AOS3_TARG::event_pair(Sphere_3_key k,
  ...
  Sphere_point_3 sp0(table_->sphere(k), l);
  if (!sp0.is_valid()) return Event_pair();
  Sphere_point_3 sp1(table_->sphere(k), l.opposite());
  if (sp0.compare_c(sp1, CGAL_AOS3_INTERNAL_NS::sweep_coordinate()) == SMALLER){
    return Event_pair(sp0, sp1);
  } else return Event_pair(sp1, sp0);
}
