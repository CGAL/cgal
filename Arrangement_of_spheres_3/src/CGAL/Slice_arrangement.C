...
void Slice_arrangement::build_arrangement(const std::vector<Circular_k::Circle_2> &circles, 
  ...
  std::vector<Rule<0> > horizontal_rules;
  std::vector<Rule<1> > vertical_rules;

  //std::vector<std::vector<Line_3> > lines;
  std::cout << "Constructing circles..." << std::flush;
  for (unsigned int i=0; i< circles.size(); ++i){
    horizontal_rules.push_back(Rule<0>(circles[i], 
				       Curve(names[i], Curve::L_RULE), inf_));
    horizontal_rules.push_back(Rule<0>(circles[i],
				       Curve(names[i], Curve::R_RULE), inf_));
    vertical_rules.push_back(Rule<1>(circles[i],
...
bool Slice_arrangement::Line_arc_less::operator()(Circular_k::Line_arc_2 a, 
...
bool Slice_arrangement::Circular_arc_less::operator()(Circular_k::Circular_arc_2 a, 
...
Slice_arrangement::Vertex_iterator Slice_arrangement::vertices_begin() {
  return arr_.vertices_begin();
}

Slice_arrangement::Vertex_iterator Slice_arrangement::vertices_end()  {
  return arr_.vertices_end();
}

void Slice_arrangement::audit() const {
...
/*Slice_arrangement::Curve_handle*/ 
void Slice_arrangement::insert(const Circular_k::Line_arc_2 &la, Curve f){
  //std::cout << "Inserting: ";
  //write(std::cout, la);
  //std::cout << std::endl;
...
//Arr::Curve_handle
void Slice_arrangement::insert(const Circular_k::Circular_arc_2 &la, Curve f) {
  //std::cout << "Inserting: ";
  //write(std::cout, la);
  //std::cout << std::endl;
  CArr::Curve_handle h= CGAL::insert_curve(arr_, Arc(la), pl_);
...
std::size_t Slice_arrangement::number_of_faces() const {
  return arr_.number_of_faces()-1;
}
std::size_t Slice_arrangement::number_of_halfedges() const {
  return arr_.number_of_halfedges();
}
std::size_t Slice_arrangement::number_of_vertices() const {
  return arr_.number_of_vertices();
}

void Slice_arrangement::write(std::ostream& out, const Circular_k::Circular_arc_2 &k) const{
...
void Slice_arrangement::write(std::ostream& out, const Circular_k::Line_arc_2 &k) const{
...
Slice_arrangement::Point Slice_arrangement::point(CArr::Vertex_const_handle h) const{
...
void Slice_arrangement::Face_iterator::increment() {
  ++c_;
  cache_.clear();
...
Slice_arrangement::Face_iterator::Face_iterator(CArr::Face_const_iterator c, 
						CArr::Face_const_iterator e,
						const Slice_arrangement *a): c_(c),
									     e_(e),
									     a_(a){
  if (c_ != e_) {
...
void Slice_arrangement::Face_iterator::fill_cache() {
...
Slice_arrangement::Face_iterator Slice_arrangement::faces_begin() const {
  return Face_iterator(arr_.faces_begin(), arr_.faces_end(), this);
}
Slice_arrangement::Face_iterator Slice_arrangement::faces_end() const {
  return Face_iterator(arr_.faces_end(), arr_.faces_end(), this);
}

 struct Slice_arrangement::Get_circle {
...
struct Slice_arrangement::Get_segment {
...
void Slice_arrangement::draw(Qt_examiner_viewer_2 *qtv) const {
