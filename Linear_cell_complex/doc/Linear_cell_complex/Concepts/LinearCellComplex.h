...
The concept `LinearCellComplex` represents a linear cell complex in dimension `d`, in an ambient space of dimension `d2`. This is a model of the concept of `BasicMap` adding a requirement to ensure that each vertex of the map is associated with a model of `CellAttributeWithPoint`.
...
(a shortcut for \link BasicMap::Attribute_type `Attribute_type<0>::type` \endlink).
*/
typedef unspecified_type Vertex_attribute;

/*!
%Handle through 0-attributes
(a shortcut for \link BasicMap::Attribute_handle `Attribute_handle<0>::type` \endlink).
*/
typedef unspecified_type Vertex_attribute_handle;

/*!
Const handle through 0-attributes
(a shortcut for \link BasicMap::Attribute_const_handle `Attribute_const_handle<0>::type` \endlink).
*/
typedef unspecified_type Vertex_attribute_const_handle;

/*!
%Range of all the 0-attributes, a model of the `Range` concept
(a shortcut for \link BasicMap::Attribute_range `Attribute_range<0>::type` \endlink).
...
a shortcut for \link BasicMap::Attribute_const_range `Attribute_const_range<0>::type` \endlink).
%Iterator inner type is bidirectional iterator and value type is \link LinearCellComplex::Vertex_attribute `Vertex_attribute`\endlink.
*/
...
(a shortcut for \link BasicMap::attributes `attributes<0>()`\endlink).
*/
Vertex_attribute_range& vertex_attributes();

/*!
Returns a const range of all the 0-attributes in this linear cell complex
(a shortcut for \link BasicMap::attributes `attributes<0>() const`\endlink).
*/
...
A linear cell complex `lcc` is valid if it is a valid basic map (cf. `BasicMap::is_valid()`), and if for each dart handle <I>dh</I> such that `*dh`\f$\in\f$\link BasicMap::darts `darts()`\endlink: \link BasicMap::attribute `dh->attribute<0>()`\endlink`!=NULL`.
*/
bool is_valid() const;

/*!
Returns the number of 0-attributes in this linear cell complex
(a shortcut for \link BasicMap::number_of_attributes `number_of_attributes<0>()`\endlink).
*/
...
/*!
Associates the 0-attribute of all the darts of the 0-cell containing `dh` to `vh` (a shortcut for \link BasicMap::set_attribute `set_attribute<0>(dh,vh)`\endlink).
\pre `*dh`\f$ \in\f$\link BasicMap::darts `darts()`\endlink and `*vh`\f$ \in\f$`vertex_attributes()`.
...
\pre 1\f$ \leq\f$<I>i</I>\f$ \leq\f$\link BasicMap::dimension `dimension`\endlink and `*dh`\f$ \in\f$\link BasicMap::darts `darts()`\endlink.
...
\pre <I>i</I>\f$ \leq\f$\link BasicMap::dimension `dimension`\endlink \f$ \leq\f$ 2 and `*dh`\f$ \in\f$\link BasicMap::darts `darts()`\endlink.

If \link BasicMap::are_attributes_automatically_managed `are_attributes_automatically_managed()`\endlink`==true`, if <I>i</I>-attributes are non void, \link CellAttribute::On_split `Attribute_type<i>::type::On_split`\endlink(<I>a</I>,<I>a'</I>) is called, with <I>a</I> the original <I>i</I>-attribute associated with <I>dh</I> and <I>a'</I> each new <I>i</I>-attribute created during the operation.

\cgalAdvancedBegin
If \link BasicMap::are_attributes_automatically_managed `are_attributes_automatically_managed()`\endlink`==false`, non void attributes are not updated; thus the combinatorial map can be no more valid after this operation.
\cgalAdvancedEnd
...
\pre 2\f$ \leq\f$\link BasicMap::dimension `dimension`\endlink and `*dh`\f$ \in\f$\link BasicMap::darts `darts()`\endlink.
...
\pre \link BasicMap::dimension `dimension`\endlink \f$ \geq\f$ 2.
...
\pre \link BasicMap::dimension `dimension`\endlink \f$ \geq\f$ 1.
