...
model of \ccc{CGAL::Point_2} or \ccc{CGAL::Point_3} or
\ccc{CGAL::Point_d}) is associated to each vertex of the combinatorial
map.
...
% because these tests are complex, too slow (for example to detect a
% self intersection, we have to simulate a Boolean operation), and often
% false for inexact kernels.
...
As explained in the \ccc{Combinatorial_map} user manual (see
...
coordinates of the new point is here given by \ccc{p} instead of being
