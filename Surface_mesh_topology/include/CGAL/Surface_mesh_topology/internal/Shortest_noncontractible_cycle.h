...
public:
  ...
  Shortest_noncontractible_cycle(const Mesh_original& gmap, bool display_time=false) : m_cycle(gmap)
  {
    CGAL::Timer t;
    if (display_time)
    { t.start(); }
    ...
    // std::cerr << '\n';
    Gmap_wrapper::copy(m_gmap, const_cast<Mesh_original&>(gmap), m_origin_to_copy, m_copy_to_origin);
  ...
  Path compute_cycle(Dart_handle_original root_vertex,
...
protected:
  ...
  void compute_noncon_edges(const Dart_container& spanning_tree, Dart_container& noncon_edges)
  {
    ...
    {
      if (m_gmap.is_marked(dh, edge_deleted)) continue; // TODO negate
    ...
    {
      if (m_gmap.is_marked(it, face_deleted)) continue; // TODO negate
