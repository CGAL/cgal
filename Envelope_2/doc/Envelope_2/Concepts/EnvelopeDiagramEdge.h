 ...
 * of the \f$ x\f$-axis. It is associated with a (possibly empty) set of curves
 * that induce the envelope over this portion of the \f$ x\f$-axis. Note that
 * all curves in this set overlap over the interval represented by the edge.
...
/*! returns whether `e` represents an empty interval, namely, whether the set
 * of \f$x\f$-monotone curves associated with it is empty.
 */
bool is_empty() const;
 ...
 */
void set_left(Vertex_const_descriptor v);

/*! sets the vertex lying to the right of `e` to be `v`.
 */
void set_right(Vertex_const_descriptor v);
