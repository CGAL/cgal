...
\begin{enumerate}
   ...
   \item  \ccc{centroid} is the centroid computed. This parameter can be
          omitted.
   \item  \ccc{tag} is the tag identifying the dimension of the objects in the object set. It should be one of \ccc{Dimension_tag<0>}, \ccc{Dimension_tag<1>}, \ccc{Dimension_tag<2>} or \ccc{Dimension_tag<3>}. Also, it shouldn't be of a greater dimension than the geometry of the object. For example, a \ccc{Triangle} can't have a \ccc{Dimension_tag<3>} tag, or a \ccc{Segment} can't have a \ccc{Dimension_tag<2>} nor a \ccc{Dimension_tag<3>} tag.
\end{enumerate}
