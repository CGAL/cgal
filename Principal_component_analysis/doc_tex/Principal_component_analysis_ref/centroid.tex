...
{ computes the centroid of a non-empty set of 2D objects having a manifold described by t. \ccc{K} is \ccc{Kernel_traits<std::iterator_traits<InputIterator>::value_type>::Kernel}.  The value type must be either \ccc{K::Point_2}, \ccc{K::Segment_2}, \ccc{K::Triangle_2}, \ccc{K::Rectangle_2} or \ccc{K::Circle_2}. To fit a set of disks the user must call the function with value type \ccc{K::Circle_2} and with dimension tag of 2. The tag must range between \ccc{CGAL::Dimension_tag<0>} and \ccc{CGAL::Dimension_tag<2>}.
\ccPrecond{first != beyond.} }
