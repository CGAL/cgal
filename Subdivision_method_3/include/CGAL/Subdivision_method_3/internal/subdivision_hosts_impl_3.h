...
void PQQ_1step(Poly& p, VertexPointMap vpm, Mask mask) {
  ...
  CGAL_FOREACH(vertex_descriptor vh, p_vertices){
    v_index[vh]= i++;
  }
  ...
  {
    std::size_t i = 0;
    CGAL_FOREACH(edge_descriptor ed, p_edges){
      if(is_border(ed,p)){
...
void DQQ_1step(Poly& p, VertexPointMap vpm, Mask mask) {
  ...
  CGAL_FOREACH(vertex_descriptor vd, p_vertices){
    CGAL_FOREACH(halfedge_descriptor hd, halfedges_around_target(vd,p)){
      if (! is_border(hd,p)){
  ...
  CGAL_FOREACH(halfedge_descriptor eeh, border_halfedges){
...
void Sqrt3_1step(Poly& p, VertexPointMap vpm, Mask mask,
  ...
  CGAL_FOREACH (face_descriptor fd, p_faces) {
    //ASSERTION_MSG(circulator_size(fitr->facet_begin())==3, "(ERROR) Non-triangle facet!");
    if(refine_border) {
      CGAL_FOREACH(halfedge_descriptor hd, halfedges_around_face(halfedge(fd, p), p)) {
