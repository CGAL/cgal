...
\label{secFirstSub}
Assuming your familiarity of \ccc{Polyhedron_3},
...
on a \ccc{Polyhedron_3}. The polyhedron is restricted within the Cartesian
...
of the Catmull-Clark subdivision in \ccc{Subdivision_method_3}.
...
the control mesh.
\ccc{Subdivision_method_3} supports a function that glues all 
...
\ccc{Polyhedron} is a model of \ccc{Polyhedron_3}, which
is a generic mesh data structure for arbitrary 
2-manifolds. \ccc{PQQ()} provides the process refining 
the control mesh \ccc{p}. During the refinement, 
\ccc{PQQ()}, the \emph{refinement host}, computes and assigns
...
\ccc{depth} specifies the steps of the refinement 
on the control mesh. 
%To make the call to
...
Each class function (called policy) in \ccc{PQQ_stencil_3} 
computes a new point with the primitive handle (which 
accesses the stencil), and 
assigns the new point to \ccc{Point& pt}. 
...
within the \ccc{Polyhedron_3}. It is designed
to work on a \ccc{Polyhedron_3} with the \ccc{Cartesian} 
...
are designed to work on a \ccc{Polyhedron_3}. If you application 
