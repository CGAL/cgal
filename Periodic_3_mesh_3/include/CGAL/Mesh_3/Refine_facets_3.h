...
compute_facet_properties(const Facet& facet) const
{
  ...
  if (facet.first->vertex(1)->point() == typename Gt::Point_3(0.092943758607192783483, 0.46291153068381452051, 0.68117446929756453322)
   && facet.first->vertex(2)->point() == typename Gt::Point_3(0.060896535856585033708, 0.49890962191731347275, 0.58512922815073431515)
   && facet.first->vertex(3)->point() == typename Gt::Point_3(0.081371862983233025912, 0.38490320302267944097, 0.60652049523753581717)
   && facet.second == 0)
  {
  	   std::cout << "facet index : " << facet.second << std::endl;
  	   std::cout << "offset 1 : " << facet.first->vertex(1)->offset() << std::endl;
  	   std::cout << "offset 2 : " << facet.first->vertex(2)->offset() << std::endl;
  	   std::cout << "offset 3 : " << facet.first->vertex(3)->offset() << std::endl;
  	   std::cout << std::endl;
  }
                    ...
                    const Point& point) const
{
  ...
  for( int i = 0; i < 3; i++ ) {
    for( int j = 0; j < 3; j++) {
      for( int k = 0; k < 3; k++ ) {
        surface_centers[9*i+3*j+k] = r_tr_.point(
  ...
  // facet is encroached if the new point is near from center than
  // one vertex of the facet
  return ( /*compare_distance(center, reference_point, point) != CGAL::SMALLER*/!(min_distance_to_ref_point < min_distance_to_point) );
}
