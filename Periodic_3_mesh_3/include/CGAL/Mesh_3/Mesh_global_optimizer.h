...
//
// $URL: svn+ssh://mbogdanov@scm.gforge.inria.fr/svn/cgal/branches/features/Mesh_3_fix_optimization_for_periodic-yvinec/Mesh_3/include/CGAL/Mesh_3/Mesh_global_optimizer.h $
// $Id: Mesh_global_optimizer.h 63087 2011-04-21 14:28:33Z yvinec $
//
//
...
operator()(int nb_iterations, Visitor visitor)
{
  ...
  // Changed for peridic meshes
  Vertex_set moving_vertices;
  for (Cells_in_complex_iterator cit=c3t3_.cells_in_complex_begin();
       cit != c3t3_.cells_in_complex_end();
       ++cit)
  {
    for ( int i=0 ; i<4 ; ++i )
    {
      moving_vertices.insert(cit->vertex(i)); 
    }
  }  
...
{    
  ...
  // The case c3t3_.in_dimension(v) == 2 is note yet adapted.
  if(c3t3_.in_dimension(v) == 2) {
    return CGAL::NULL_VECTOR;
  }
