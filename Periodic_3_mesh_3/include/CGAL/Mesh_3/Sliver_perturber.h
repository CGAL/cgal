...
//
// $URL: svn+ssh://mbogdanov@scm.gforge.inria.fr/svn/cgal/branches/features/Mesh_3_fix_optimization_for_periodic-yvinec/Mesh_3/include/CGAL/Mesh_3/Sliver_perturber.h $
// $Id: Sliver_perturber.h 63087 2011-04-21 14:28:33Z yvinec $
//
//
...
class Sliver_perturber
{
  ...
  // typedef Sliver_perturber<C3T3,MeshDomain,SliverCriterion,Visitor> Self;
  // friend class Self::PVertex;
...
private:
  ...
  public:
    ...
    // Is perturbable // I Moved this test into outer class Sliver_perturber 
    // to add test for vertices outside domain 
    // bool is_perturbable() const
    //     {
    //       bool outside = ( vertex_handle_->in_dimension() == 3
    // 		       && vertex_handle_->index() == domain_.index_from_subdomain_index (Subdomain_index())); 
...
private:
  ...
  /**
   * Test if a pvertex is perturbable. I deplaced this test from nested class Pvertex 
   * to add testing if vertex is outside the domain
   */
  bool is_perturbable( const PVertex& pv) const; 
...
perturb(const FT& sliver_bound, PQueue& pqueue, Visitor& visitor) const
{
  ...
  while ( !is_time_limit_reached() && !pqueue.empty() )
  {
    ...
    CGAL_assertion(is_perturbable(pv));
...
bool
Sliver_perturber<C3T3,Md,Sc,V_>::
is_perturbable( const PVertex& pv) const 
{
  bool outside = ( pv.vertex()->in_dimension() == 3
		   && pv.vertex()->index() == domain_.index_from_subdomain_index (Subdomain_index())); 
  return (   (pv.vertex()->in_dimension() > 1)
	     && (!outside)
             && (NULL != pv.perturbation()) 
             && (pv.sliver_nb() != 0) );
}
