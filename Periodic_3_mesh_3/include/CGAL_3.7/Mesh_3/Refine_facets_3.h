...
//
// $URL: svn+ssh://mbogdanov@scm.gforge.inria.fr/svn/cgal/trunk/Mesh_3/include/CGAL/Mesh_3/Refine_facets_3.h $
// $Id: Refine_facets_3.h 53822 2010-01-27 13:50:01Z hemmer $
//
//
...
private:
  ...
  typename Gt::FT find_distance_surface_center(const Facet& facet, const Point& point) const;
  typename Gt::FT find_distance_surface_center_and_point(const Point& center, const Point& point) const;
...
template<class Tr, class Cr, class MD, class C3T3_, class P_, class C_>
typename Refine_facets_3<Tr,Cr,MD,C3T3_,P_,C_>::Gt::FT 
Refine_facets_3<Tr,Cr,MD,C3T3_,P_,C_>::
find_distance_surface_center(const Facet& f, const Point& point) const
{
  ...
  Point surface_center =  r_tr_.canonicalize_point( point );
...
typename Refine_facets_3<Tr,Cr,MD,C3T3_,P_,C_>::Gt::FT 
Refine_facets_3<Tr,Cr,MD,C3T3_,P_,C_>::
find_distance_surface_center_and_point(const Point& center, const Point& point) const
{
  typedef typename Gt::FT FT;
  ...
  Point surface_center =  r_tr_.canonicalize_point( center );
  ...
  FT min_distance = distance(point, surface_centers[0]);
  for( int i = 1; i < 27; i++ ) {
    FT current_distance = distance(point, surface_centers[i]);
    if ( current_distance < min_distance )
    {
      min_distance = current_distance;
    }
                    ...
                    const Point& point) const
{
  ...
  return ( find_distance_surface_center_and_point(center, point)
          <= find_distance_surface_center(facet, center) );
}
