...
// $Id: Mesh_periodic_cell_criteria_3.h 53957 2010-02-01 12:55:28Z spion $
// 
//
// Author(s)     : Mikhail Bogdanov


#ifndef CGAL_MESH_PERIODIC_CELL_CRITERIA_3_H
#define CGAL_MESH_PERIODIC_CELL_CRITERIA_3_H

#include <iostream>
#include <CGAL/Mesh_3/periodic_mesh_standard_cell_criteria.h>

namespace CGAL {
  
template <typename Tr, typename Visitor_ = Mesh_3::Cell_criterion_visitor<Tr> >
class Mesh_periodic_cell_criteria_3
{
...
public:
   ...
   */
  Mesh_periodic_cell_criteria_3(const Iso_cuboid& periodic_domain,
                       const FT& radius_edge_bound,
    const FT& radius_bound) :
    helper_tr(periodic_domain)
  {
	initialize(radius_edge_bound, radius_bound);
  }
   ...
   */
  template<class PMD>
  Mesh_periodic_cell_criteria_3(const PMD& periodic_mesh_domain,
                       const FT& radius_edge_bound,
                       const FT& radius_bound) :
...
private:
  ...
  void initialize(const FT& radius_edge_bound,
      const FT& radius_bound)
  {
    typedef Mesh_3::Periodic_mesh_3::Cell_radius_criterion<Tr,Visitor> Radius_criterion;
    typedef Mesh_3::Periodic_mesh_3::Cell_radius_edge_criterion<Tr,Visitor> Radius_edge_criterion;
    
    if ( 0 != radius_bound )
      criteria_.add(new Radius_criterion(helper_tr, radius_bound));
    
    if ( 0 != radius_edge_bound )
      criteria_.add(new Radius_edge_criterion(helper_tr, radius_edge_bound));
  }

  Criteria criteria_;
  
  Tr helper_tr;
};  // end class Mesh_periodic_cell_criteria_3

}  // end namespace CGAL


#endif // CGAL_MESH_PERIODIC_CELL_CRITERIA_3_H
