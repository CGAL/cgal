...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_NOX_2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_NOX_2_H
...
#include <CGAL/Segment_Delaunay_graph_2/nox/Traits_wrapper_2.h>
...
protected:
  // LOCAL TYPES
  //------------
  typedef Segment_Delaunay_graph_nox_2<Gt,D_S,LTag>      Self;

  typedef Segment_Delaunay_graph_traits_wrapper_2<Gt>    Modified_traits;
  typedef Delaunay_triangulation_2<Modified_traits,D_S>  DG;
...
public:
  // CREATION
  //---------
  Segment_Delaunay_graph_nox_2(const Geom_traits& gt = Geom_traits())
    : DG(gt), only_points(true) {}

  template< class Input_iterator >
  Segment_Delaunay_graph_nox_2(Input_iterator first, Input_iterator beyond,
  ...
  Segment_Delaunay_graph_nox_2(const Self& other);
...
public:
  ...
  inline Vertex_handle insert(const Point_2& p0, const Point_2& p1) {
    ...
    Vertex_handle v = insert_segment(Site_2(p0, p1), Vertex_handle());
    //    if ( v == Vertex_handle() ) {
  ...
  inline Vertex_handle insert(const Vertex_handle& v0,
    ...
    CGAL_precondition( v0->site().is_point() && v1->site().is_point() );
    ...
    Site_2 s = Site_2::construct_site_2(v0->site().point(),
					v1->site().point());

    if ( number_of_vertices() == 2 ) {
      return insert_third(s, v0, v1);
    }

    Vertex_handle v = insert_segment_interior(s, v0);
    //    if ( v == Vertex_handle() ) {
    //      unregister_input_site(php.first, php.second);
    //    }
    return v;
  ...
  inline Vertex_handle insert(const Point_2& p0, const Point_2& p1, 
    ...
    //    Storage_site_2 ss =
    //      st_.construct_storage_site_2_object()(php.first, php.second);
    Vertex_handle v = insert_segment(Site_2(p0, p1), vnear);
    //    if ( v == Vertex_handle() ) {
    //      unregister_input_site(php.first, php.second);
    //    }
    return v;
  ...
  Vertex_handle insert(const Site_2& t, Vertex_handle vnear)
  {
    ...
    if ( t.is_segment() ) {
      ...
      //      Storage_site_2 ss =
      //	st_.construct_storage_site_2_object()(php.first, php.second);
      Vertex_handle v = insert_segment(t, vnear);
      //      if ( v == Vertex_handle() ) {
      //	unregister_input_site( php.first, php.second );
      //      }
      return v;
...
protected:
  ...
  Vertex_handle  insert_first(const Site_2& s);
  Vertex_handle  insert_second(const Site_2& s);
  Vertex_handle  insert_third(const Site_2& t);
  Vertex_handle  insert_third(const Site_2& s, Vertex_handle v0,
			      Vertex_handle v1);

  inline
  Vertex_handle insert_point(const Point_2& p, Vertex_handle vnear)
  {
    return insert_point(Site_2::construct_site_2(p), vnear);
  }

  Vertex_handle insert_point(const Site_2& t, Vertex_handle vnear);
  Vertex_handle insert_point2(const Site_2& t, Vertex_handle vnear);
...
public:
  ...
  void swap(Self& sdg) {
    DG::swap(sdg);
    std::swap(only_points, sdg.only_points);
    //    pc_.swap(sdg.pc_);
    //    isc_.swap(sdg.isc_);
  }
...
protected:
  ...
  inline Vertex_handle insert_degree_2(Edge e, const Site_2& s) {
  ...
  inline Vertex_handle create_vertex(const Site_2& s) {
    Vertex_handle v = this->_tds.create_vertex();
    v->set_site(s);
    return v;
  }

  inline Vertex_handle create_vertex_dim_up(const Site_2& s) {
...
protected:
  ...
  //----------------------------------
  inline
  bool same_points(const Point_2& p, const Point_2& q) const {
  ...
  bool is_endpoint_of_segment(const Site_2& p, const Site_2& s) const
  {
    ...
    return ( same_points(p.point(), s.source()) ||
	     same_points(p.point(), s.target()) );
  }

  inline
  bool is_degenerate_segment(const Site_2& s) const {
    CGAL_precondition( s.is_segment() );
    return same_points(s.source(), s.target());
  }
...
std::istream& operator>>(std::istream& is,
			 Segment_Delaunay_graph_nox_2<Gt,D_S,LTag>& sdg)
{
  sdg.file_input(is);
  return is;
}
...
std::ostream& operator<<(std::ostream& os,
			 const Segment_Delaunay_graph_nox_2<Gt,D_S,LTag>& sdg)
{
  sdg.file_output(os);
  return os;
}
...
#include <CGAL/Segment_Delaunay_graph_2/nox/Segment_Delaunay_graph_nox_2_impl.h>


#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_NOX_2_H
