...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_VORONOI_VERTEX_RING_C2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_VORONOI_VERTEX_RING_C2_H
...
private:
  ...
  compute_ppp(const Site_2& sp, const Site_2& sq, const Site_2& sr)
  {
    ...
    const Point_2& p = sp.point();
    const Point_2& q = sq.point();
    const Point_2& r = sr.point();
  ...
  compute_pss(const Site_2& p, const Site_2& q, const Site_2& r)
  {
    ...
    bool pq = is_endpoint_of(p, q);
    bool pr = is_endpoint_of(p, r);
    ...
    if ( sgn_c1_ == NEGATIVE ) {
      ...
      CGAL_assertion( pq );
      if ( same_points(p.point(), q.target()) ) {
	a1 = -a1;  b1 = -b1;  c1_ = -c1_;
      }
    }
    ...
    if ( sgn_c2_ == NEGATIVE ) {
      ...
      CGAL_assertion( pr );
      if ( same_points(p.point(), r.source()) ) {
	a2 = -a2;  b2 = -b2;  c2_ = -c2_;
      }
    }
			   ...
			   const Site_2& s, const Line_2& l) const
  {
    ...
    if ( same_segments(supp, s) ) {
      return false;
    }

    if ( is_endpoint_of(s.source(), supp) ) {
      return oriented_side_of_line(l, s.target()) == ON_POSITIVE_SIDE;
    }

    if ( is_endpoint_of(s.target(), supp) ) {
  ...
  Sign incircle_p_no_easy(const Site_2& st, SSS_Type ) const
  {
    ...
    RT a1, b1, c1;
    compute_supporting_line(p_, a1, b1, c1);
  ...
  Sign incircle_s(const Site_2& t, Type type) const
  {
    ...
    if ( v_type == PSS ) {
      if ( p_.is_segment() && same_segments(p_, t) ) {
	return POSITIVE;
      }
      if ( q_.is_segment() && same_segments(q_, t) ) {
	return POSITIVE;
      }
      if ( r_.is_segment() && same_segments(r_, t) ) {
	return POSITIVE;
      }
    }
  ...
  {
    Sign d1, d2;
    if (  ( p_.is_point() && same_points(p_.point(), t.source()) ) ||
	  ( q_.is_point() && same_points(q_.point(), t.source()) ) ||
	  ( r_.is_point() && same_points(r_.point(), t.source()) )  ) {
      d1 = ZERO;
    } else {
      // MK::ERROR: I need to modify the incircle_p methods to take a
      // point as an argument
      d1 = incircle_p(Site_2::construct_site_2(t.source()));
    }
    if ( d1 == NEGATIVE ) { return NEGATIVE; }

    if (  ( p_.is_point() && same_points(p_.point(), t.target()) ) ||
	  ( q_.is_point() && same_points(q_.point(), t.target()) ) ||
	  ( r_.is_point() && same_points(r_.point(), t.target()) )  ) {
      d2 = ZERO;
    } else {
      // MK::ERROR: I need to modify the incircle_p methods to take a
      // point as an argument
      d2 = incircle_p(Site_2::construct_site_2(t.target()));
    }
  ...
  Sign incircle_s(const Site_2& t) const 
  {
    ...
    if ( is_degenerate_Voronoi_circle() ) {
      ...
      if (  !same_points( p_ref().point(), t.source() ) &&
	    !same_points( p_ref().point(), t.target() )  ) {
	return POSITIVE;
      }
	...
	sq = q_;
      ...
      Point_2& pq = const_cast<Point_2&>(sq.source());
      Point_2& pp = const_cast<Point_2&>(sp.source());
      Point_2& pt = const_cast<Point_2&>(t.source());

      const Point_2& pr_ = pr.point();

      if ( same_points(sp.source(), pr_) ) {
	pp = const_cast<Point_2&>(sp.target());
      }
      if ( same_points(sq.source(), pr_) ) {
	pq = const_cast<Point_2&>(sq.target());
      }
      if ( same_points( t.source(), pr_) ) {
	pt =  const_cast<Point_2&>(t.target());
      }
...
public:
  ...
  FT squared_radius() const {
    ...
    case SSS:
      {
	Line_2 l = compute_supporting_line(p_);
  ...
  const Point_2& degenerate_point() const
  {
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_VORONOI_VERTEX_RING_C2_H
