...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_FINITE_EDGE_INTERIOR_CONFLICT_C2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_FINITE_EDGE_INTERIOR_CONFLICT_C2_H
...
private:
				    ...
				    const Site_2& st, Method_tag ) const
  {
    ...
    Line_2 lq = compute_supporting_line(sq);

    if ( oriented_side_of_line(lq, p) == ON_NEGATIVE_SIDE ) {
      lq = opposite_line(lq); 
    }

    if ( is_endpoint_of(sp, st) ) {
	 ...
	 ((opqr == ON_POSITIVE_SIDE) & (oqps == ON_NEGATIVE_SIDE));
      ...
      Point_2& t1 = const_cast<Point_2&>(st.source());
      if ( same_points(sp.point(), st.source()) ) {
	t1 = const_cast<Point_2&>(st.target());
      }

      Oriented_side o_t1;

      if ( same_points(t1, sq.source()) ||
	   same_points(t1, sq.target()) ) {
	o_t1 = ON_ORIENTED_BOUNDARY;
      } else {
	o_t1 = oriented_side_of_line(lq, t1);
      }
	...
	return true;
      ...
      Comparison_result res = compare_squared_distances_to_line(lq, p, t1);

      return ( res == LARGER );
    }

    Line_2 lt = compute_supporting_line(st);
				  ...
				  const Site_2& t, Method_tag ) const
  {
    ...
    if ( vpqr.is_degenerate_Voronoi_circle() ) {
      const Point_2& tpqr = vpqr.degenerate_point();

      if ( is_endpoint_of(tpqr, p) ) {
...
public:
		       ...
		       const Site_2& t, Sign sgn) const
  {
    ...
    return ( is_endpoint_of(p, t) && is_endpoint_of(q, t) );
  }
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_FINITE_EDGE_INTERIOR_CONFLICT_C2_H
