...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_CONSTRUCTIONS_C2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_CONSTRUCTIONS_C2_H
...
{
private:
  typedef Is_endpoint_of_segment_C2<Gt>  Is_endpoint_of_segment_2;
...
public:
		   ...
		   const Site_2& r) const
  {
    ...
    if ( p.is_point() && q.is_point() ) {
      ...
      p1 = are_same_points(p.point(), q.source()) ? q.target() : q.source();
      p2 = p.point();
    } else {
      // p is a segment and q a point
      p1 = q.point();
      p2 = are_same_points(q.point(), p.source()) ? p.target() : p.source();
    }
    Line_2 l(p1, p2);
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_CONSTRUCTIONS_C2_H
