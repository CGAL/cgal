...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_INFINITE_EDGE_INTERIOR_CONFLICT_C2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_INFINITE_EDGE_INTERIOR_CONFLICT_C2_H
...
public:
		       ...
		       const Site_2& t, Sign sgn) const
  {
    ...
    if ( s.is_segment() && r.is_segment() && same_segments(s, r) ) {
      CGAL_assertion( is_endpoint_of(q, s) );

      Point_2& ss = const_cast<Point_2&>(s.source());
      if ( same_points(q.point(), s.source()) ) {
	ss = s.target();
      }
      // MK::ERROR: write this code using the compare_x_2 and
      //    compare_y_2 predicates instead of computing the inner
      //    product...
      RT dtssx = ss.x() - t.point().x();
      RT dtssy = ss.y() - t.point().y();
      RT dtqx = q.point().x() - t.point().x();
      RT minus_dtqy = -q.point().y() + t.point().y();
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_INFINITE_EDGE_INTERIOR_CONFLICT_C2_H
