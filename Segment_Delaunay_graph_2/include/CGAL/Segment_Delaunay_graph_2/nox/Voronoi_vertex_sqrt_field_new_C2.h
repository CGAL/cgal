...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_VORONOI_VERTEX_SQRT_FIELD_NEW_C2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_VORONOI_VERTEX_SQRT_FIELD_NEW_C2_H
...
private:
  ...
  inline
  const Point_2& other_point(const Site_2& sp, const Site_2& seg) const
  {
    CGAL_precondition( sp.is_point() && seg.is_segment() );

    if ( same_points(sp.point(), seg.source()) ){
      return seg.target();
    }
    return seg.source();
  }
	     ...
	     const PPS_Type&) const
  {
    ...
    FT a, b, c;
    compute_supporting_line(sr, a, b, c);

    const Point_2& pp = sp.point();
    const Point_2& qq = sq.point();

    FT c_ = a * pp.x() + b * pp.y() + c;
    FT cq_ = a * qq.x() + b * qq.y() + c;

    if ( is_endpoint_of(pp, sr) ) {
      c_ = FT(0);
    }
    if ( is_endpoint_of(qq, sr) ) {
      cq_ = FT(0);
    }
    ...
    const Point_2& r_src = sr.source();
    const Point_2& r_trg = sr.target();
    ...
    FT ux = J - FT(2) * y_ * sqrt_S;
    FT uy = I + FT(2) * x_ * sqrt_S;

    vv = Point_2(ux / X + pp.x(), uy / X + pp.y());
#else
    std::cerr << "vvvvvvvvvvvvvvvvvvv" << std::endl;
    FT e1 = a * x_ + b * y_;
    std::cerr << "e1: " << e1 << std::endl;
    FT e4 = c_ * (b * x_ - a * y_);
    std::cerr << "e4: " << e4 << std::endl;
    FT C = (e4 - CGAL::sqrt(n_ * nl * c_ * cq_)) / e1;
    std::cerr << "c : " << c_ << std::endl;
    std::cerr << "cq: " << cq_ << std::endl;
    std::cerr << "S : " << (n_ * nl * c_ * cq_) << std::endl;
    std::cerr << "C : " << C << std::endl;

    FT ux = (a * n_ / FT(2) - C * y_) / e1 + pp.x();
    std::cerr << "ux: " << ux << std::endl;
    FT uy = (b * n_ / FT(2) + C * x_) / e1 + pp.y();
    std::cerr << "uy: " << uy << std::endl;
	     ...
	     const PSS_Type&) const
  {
    ...
    bool pq = is_endpoint_of(pp, sq);
    bool pr = is_endpoint_of(pp, sr);
    ...
    if ( sgn_c1_ == NEGATIVE ) {
      ...
      CGAL_assertion( pq );
      if ( same_points(sp.point(), sq.target()) ) {
	a1 = -a1;  b1 = -b1;  c1_ = -c1_;
      }
    }
    ...
    if ( sgn_c2_ == NEGATIVE ) {
      ...
      CGAL_assertion( pr );
      if ( same_points(sp.point(), sr.source()) ) {
	a2 = -a2;  b2 = -b2;  c2_ = -c2_;
      }
    }
	       ...
	       const Site_2& sr, FT a[], FT b[], FT c[]) const
  {
    ...
    Line_2 l[3];
    l[0] = compute_supporting_line(sp);
    l[1] = compute_supporting_line(sq);
    l[2] = compute_supporting_line(sr);
		  ...
		  const Site_2& t, const Type& type) const
  {
    ...
    return incircle_p(p, q, r, t.point(), type);
  }
#endif
  ...
  Sign incircle_p(const Point_2& vv,
		  const Site_2& p, const Site_2& q, const Site_2& r,
		  const Point_2& t, const Type& type) const
  {
    FT r2 = squared_radius(vv, p, q, r, type);
    FT d2 = CGAL::square(vv.x() - t.x()) + CGAL::square(vv.y() - t.y());
		  ...
		  const Site_2& t, const PPS_Type& type) const
  {
    ...
    return incircle_p(vv, p, q, r, t.point(), type);
  }
		     ...
		     const Site_2& t, const Type& type) const
  {
    ...
    Sign d1, d2;
    if (  ( p.is_point() && same_points(p.point(), t.source()) ) ||
	  ( q.is_point() && same_points(q.point(), t.source()) ) ||
	  ( r.is_point() && same_points(r.point(), t.source()) )  ) {
      d1 = ZERO;
    } else {
      d1 = incircle_p(p, q, r, Site_2::construct_site_2(t.source()), type);
    }

    if (  certainly(d1 == NEGATIVE)  ) { return NEGATIVE; }
    if (  !is_certain(d1 == NEGATIVE)  ) { return indeterminate<Sign>(); }

    if (   ( p.is_point() && same_points(p.point(), t.target()) ) ||
	   ( q.is_point() && same_points(q.point(), t.target()) ) ||
	   ( r.is_point() && same_points(r.point(), t.target()) )  ) {
      d2 = ZERO;
    } else {
      d2 = incircle_p(p, q, r, Site_2::construct_site_2(t.target()), type);
    }
		  ...
		  const Site_2& t, const PPP_Type& type) const
  {
    ...
    if ( pp1 != NULL ) {
      ...
      const Point_2& p1 = pp1->point();
      const Point_2& p2 = other_point(*pp1, t);
      compute_vv(p, q, r, type);
		  ...
		  const Site_2& t, const PPS_Type& type) const
  {
    ...
    if ( is_endpoint_of(*pp2, t) ) {
      const Point_2& p1 = other_point(*pp2, t);
      const Point_2& p2 = pp2->point();
      compute_vv(p, q, r, type);
		  ...
		  const Site_2& t, const PSS_Type& type) const
  {
    ...
    if ( is_endpoint_of(p, q) && is_endpoint_of(p, r) ) {
      ...
      if ( same_segments(q, t) ) {
	return ZERO;
      }

      if ( same_segments(r, t) ) {
	return ZERO;
      }


      const Point_2& p_ = p.point();
      Point_2& r_ = const_cast<Point_2&>(r.source());
      Point_2& q_ = const_cast<Point_2&>(q.source());
      Point_2& t_ = const_cast<Point_2&>(t.source());

      if ( same_points(q.source(), p_) ) {
	q_ = const_cast<Point_2&>(q.target());
      }
      if ( same_points(r.source(), p_) ) {
	r_ = const_cast<Point_2&>(r.target());
      }
      if ( same_points(t.source(), p_) ) {
	t_ =  const_cast<Point_2&>(t.target());
      }
    ...
    if ( is_endpoint_of(p, t) ) {
      const Point_2& p1 = p.point();
      const Point_2& p2 = other_point(p, t);
      compute_vv(p, q, r, type);
      Compute_scalar_product_2 csp;
      return -CGAL::sign( csp(vv - p1, p2 - p1) );
    }
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_VORONOI_VERTEX_SQRT_FIELD_NEW_C2_H
