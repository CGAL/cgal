...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_ORIENTED_SIDE_OF_BISECTOR_C2_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_ORIENTED_SIDE_OF_BISECTOR_C2_H
...
private:
		       ...
		       const Site_2& q) const
  {
    ...
    if ( is_endpoint_of(q, s) ) { return SMALLER; }


    bool is_src = same_points(p.point(), s.source());
    bool is_trg = same_points(p.point(), s.target());
		       ...
		       const Site_2& q) const
  {
    ...
    bool is_on_s1 = is_endpoint_of(q, s1);
    bool is_on_s2 = is_endpoint_of(q, s2);
    ...
    Line_2 ls1 = compute_supporting_line(s1);
    ...
    Line_2 ls2 = compute_supporting_line(s2);
	...
	return CGAL::compare(d2_s1 * d2_s2.second, d2_s2.first);
      ...
      if ( idx2 == -1 ) {
	if ( same_points(s1.target(), s2.source()) ) {
	  return EQUAL;
	}
	RT d2_s2 = compute_squared_distance(qq, ssrc2);
	return CGAL::compare(d2_s1, d2_s2);
      } else if ( idx2 == 1 ) {
	if ( same_points(s1.target(), s2.target()) ) {
	  return EQUAL;
	}
	RT d2_s2 = compute_squared_distance(qq, strg2);
	return CGAL::compare(d2_s1, d2_s2);
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_2_NOX_ORIENTED_SIDE_OF_BISECTOR_C2_H
