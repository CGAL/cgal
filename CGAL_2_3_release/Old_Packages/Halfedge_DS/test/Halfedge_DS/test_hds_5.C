...
void test_Halfedge_data_structure_min_vector() {
    ...
    HDS hds(1,2,1);
    hds.new_edge();
    (*(hds.halfedges_begin())).set_next(&*(hds.halfedges_begin()+1));
    (*(hds.halfedges_begin()+1)).set_next(&*(hds.halfedges_begin()));
    CGAL_assertion( hds.size_of_vertices() == 0);
    CGAL_assertion( hds.size_of_halfedges() == 2);
    CGAL_assertion( hds.size_of_facets() == 0);
...
int main() {
    test_Halfedge_data_structure_min_vector();
    return 0;
}
// EOF //
