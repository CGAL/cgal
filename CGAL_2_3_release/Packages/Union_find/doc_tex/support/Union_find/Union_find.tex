...
partition of values of type \ccc{T} into disjoint sets.  The type
\ccc{A} has to be a model of the allocator concept as defined in the
C++ standard.
...
\ccNestedType{value_type}{values stored in items (equals \ccc{T}).  }

\ccNestedType{handle}{handle to values.  }

\ccNestedType{iterator}{iterator over values.  }

There are also constant versions \ccc{const_handle} and
\ccc{const_iterator}.
...
\ccMethod{size_t number_of_sets() ;}{returns the number of disjoint
  sets of \ccc{P}.  }

\ccMethod{size_t size() ;}{returns the number of values of \ccc{P}.  }

\ccMethod{size_t bytes() ;}{returns the memory consumed by \ccc{P}.  }

\ccMethod{size_t size(handle p) ;}{returns the size of the set
  containing $p$.  }
...
\ccMethod{handle find(handle p) ;}{returns a canonical handle of the
  set that contains \ccc{p}, i.e., \ccc{P.same_set(p,q)} iff
  \ccc{P.find(p)} and \ccc{P.find(q)} return the same handle.
...
\ccMethod{bool same_set(handle p, handle q) ;}{returns true iff $p$
  and $q$ belong to the same set of \ccc{P}. \ccPrecond $p$ and $q$
  are in \ccc{P}.  }
...
is $O(n \alpha(m,n))$ where $\alpha(m,n)$ is the extremly slow growing
