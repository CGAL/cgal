                                    ...
                                    std::map<edge_descriptor, EdgeWPtr>& e2e)
{
    ...
    auto weight_pmap = sm.property_map<Face_index, double>("f:weight");
   ...
   // @fixme factorize with OBJ/PLY save()
    ...
    for (VertexSPtr v : polyhedron->vertices()) {
        ...
        if (convert_to_double) {
            p = Point3(CGAL::to_double(p.x()), CGAL::to_double(p.y()), CGAL::to_double(p.z()));
        }
        vertex_descriptor idx = sm.add_vertex(p);
        v_map[v] = idx;
    }

    // Write facets
    auto weight_pmap = sm.add_property_map<face_descriptor, double>("f:weight", 1).first;
