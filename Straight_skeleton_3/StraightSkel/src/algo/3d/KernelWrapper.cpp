...
Point3SPtr KernelWrapper::intersection(Plane3SPtr plane1, Plane3SPtr plane2, Plane3SPtr plane3) {
    ...
    } else if (const CGAL::Point3* ipoint = std::get_if<CGAL::Point3>(&*res)) {
...
Line3SPtr KernelWrapper::intersection(Plane3SPtr plane1, Plane3SPtr plane2) {
    ...
    } else if (const CGAL::Line3 *iline = std::get_if<CGAL::Line3>(&*res)) {
        result = KernelFactory::createLine3(*iline);
    } else {
        CGAL_SS3_TRAITS_TRACE_CODE(if (const CGAL::Plane3* iplane = std::get_if<CGAL::Plane3>(&*res)) {)
        CGAL_SS3_TRAITS_TRACE_CODE(CGAL_USE(iplane);)
...
// @speed could return num / den and re-use the denominator for point computation
CGAL::FT KernelWrapper::intersectionTimeOffsetPlanes(Plane3SPtr plane_0, const CGAL::FT& w0,
                                                     ...
                                                     const std::optional<CGAL::FT>& past_bound,
                                                     const std::optional<CGAL::FT>& future_bound)
{
    CGAL_precondition(!(is_zero(w0) && is_zero(w1) && is_zero(w2) && is_zero(w3)));
...
KernelWrapper::intersectionPointAndTimeOffsetPlanes(Plane3SPtr plane_0,
                                                    const CGAL::FT& w0,
                                                    Plane3SPtr plane_1,
                                                    const CGAL::FT& w1,
                                                    Plane3SPtr plane_2,
