...
def plot_data_from_directories(root_dir):
    ...
    for subdir_name in os.listdir(root_dir):
        ...
        face_count_file_path = os.path.join(subdir_path, 'face_count.txt')

        if os.path.exists(runtime_file_path) and os.path.exists(face_count_file_path):
            try:
                with open(runtime_file_path, 'r') as runtime_file:
                    runtime = float(runtime_file.readline().strip())

                with open(face_count_file_path, 'r') as face_count_file:
                    face_count = int(face_count_file.readline().strip())

                x_values.append(face_count)
    ...
    if x_values and y_values:
         ...
         ax.set_title(f'Runtime vs. Face Count (Data Points: {len(x_values)})')  # Add the number of data points to the title
