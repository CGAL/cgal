...
#ifndef CGAL_STRAIGHT_SKELETON_3_INTERNAL_ALGORITHM_GENERIC_VERTEX_EVENT_H
#define CGAL_STRAIGHT_SKELETON_3_INTERNAL_ALGORITHM_GENERIC_VERTEX_EVENT_H
...
class Generic_vertex_event
  : public Abstract_event<GeomTraits>
{
  using Base = Abstract_event<GeomTraits>;
  using Generic_vertex_event_sptr = std::shared_ptr<Generic_vertex_event<GeomTraits> >;
...
public:
  Generic_vertex_event()
    : Base(Base::GENERIC_VERTEX_EVENT)
  { }

  virtual ~Generic_vertex_event()
  { }

  static Generic_vertex_event_sptr create()
  {
    return std::make_shared<Generic_vertex_event>();
  }
  ...
  std::string to_string() const
  {
    ...
    sstr << "\t(facet B=" << facet_2->id() << "\n"
         << "\t vertex B2=" << vertex_2->to_string() << ")";
    return sstr.str();
  }

  bool operator==(const Generic_vertex_event& other) const
  {
...
#endif /* CGAL_STRAIGHT_SKELETON_3_INTERNAL_ALGORITHM_GENERIC_VERTEX_EVENT_H */
