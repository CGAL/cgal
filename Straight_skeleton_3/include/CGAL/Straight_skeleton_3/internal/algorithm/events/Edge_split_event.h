...
class Edge_split_event
  : public Abstract_event<GeomTraits>
{
  using Base = Abstract_event<GeomTraits>;
  using Edge_split_event_sptr = std::shared_ptr<Edge_split_event<GeomTraits> >;
...
public:
  ...
  virtual ~Edge_split_event()
  { }

  static Edge_split_event_sptr create()
  {
    return std::make_shared<Edge_split_event>();
  }
  ...
  std::string to_string() const
  {
    ...
    sstr << "\t(edgeA=" << edge1->get_ID() << "[" << edge1->source()->get_ID() << "-"
                                                  << edge1->target()->get_ID() << "]"
         << "; edgeB=" << edge2->get_ID() << "[" << edge2->source()->get_ID() << "-"
                                                 << edge2->target()->get_ID() << "])";
    return sstr.str();
  }

  bool operator==(const Edge_split_event& other) const
  {
