  ...
  {
    {
      ...
      CGAL_FOREACH(const Corner_index_and_point& pair, corners_tmp)
      {
    ...
    CGAL_FOREACH(const typename Corners_indices::value_type& pair,
      corners_indices)
    {
      ...
      // possible.
      CGAL_FOREACH(Curve_index curve_index, incident_curves) {
...
// 	std::cerr << "lt=" << lt << std::endl;
      ...
      if(ids_it == corners_indices.end()) {
        ...
        if(projection_traits.found()) {
...
#ifdef CGAL_MESH_3_PROTECTION_HIGH_VERBOSITY
          {
            ...
            CGAL_FOREACH(Patch_index i, ids) {
              s << CGAL::oformat(i) << " ";
            }
            s << "}\n";
            std::cerr << s.str();
      ...
      CGAL_FOREACH(Input_curves_AABB_tree_primitive_ prim, prims)
      {
