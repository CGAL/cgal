...
%
% Finally, following a \betaun{} pointer, we get to a
...
% we explained that when following a \betai{} pointer we change the
...
% this is the
% case for \betadeux{} for all the darts that does not belong to edge
% $e_1$, $e_2$ and $e_3$. 
...
% \betatrois{} for example for all the darts belonging to facet
...
Given a dart \emph{d0}, in general, \betai{}(\emph{d0}) (with 1$\leq$ \emph{i} $\leq$ d)
...
%  since \betazero{} is not a relation
...
\orbit{\{\betai{}\comp{}\betaj{}$|\forall$ i,j: 1$\leq$ \emph{i<j} $\leq$ \emph{d}\}}(\emph{d0}).
...
\betaj{}s for \emph{j} $\leq$ \emph{d'}.  The \cell{i} containing \emph{d0} in dimension
\emph{d'} is the orbit
\orbit{\betaun{},$\ldots$,\betaimun{},\betaipun{},$\ldots$,\betadprim{}}(\emph{d0}), and 
the 0-cell is the orbit \orbit{\{\betai{}\comp{}\betaj{}$|\forall$ i,j: 2$\leq$
  i<j $\leq$ \emph{d'}\}}(\emph{d0}).  By default, \cells{i} are considered in
...
% \begin{description}
% \item[V1] \betaun{} is injective:

% $\forall d_1 \in D$, $\forall d_2 \in D$, if $\mb{1}(d_1)=\mb{1}(d_2)$, then 
% $\mb{1}(d_1)=\varnothing$ or $d_1=d_2$;

% \item[V2] Other \betai{}s are their own inverse function that link two different darts:
...
% from two darts and using \betaun{} iteratively (which are cycles when
...
% (For example in 3D, if two darts are linked by \betatrois{}, the following
% constraints need to be satisfied:
% $\mb{0}(\mb{3}(d_0))=\mb{3}(\mb{0}^{-1}(d_0))$, and
% $\mb{1}(\mb{3}(d_0))=\mb{3}(\mb{1}^{-1}(d_0))$, which means that all the
% darts of the two facets must be consistently linked by \betatrois{}. In
...
\item \ccc{Dart_of_orbit_range<Beta...>}: range of all the darts of
  the orbit \orbit{Beta...}(\emph{d0}) for a given \emph{d0}.  \ccc{Beta...} is a
  sequence of integers \ccTexHtml{$i_1$}{i<sub>1</sub>},$\ldots$,\ccTexHtml{$i_k$}{i<sub>k</sub>}, each \ccTexHtml{$i_j$}{i<sub>j</sub>} $\in$
  \{0,\ldots,\emph{d}\}. These integers must satisfy:\ccTexHtml{$i_1$}{i<sub>1</sub>}<\ccTexHtml{$i_2$}{i<sub>2</sub>}<$\ldots$<\ccTexHtml{$i_k$}{i<sub>k</sub>},
  and (\ccTexHtml{$i_1$}{i<sub>1</sub>}$\neq$ 0 or \ccTexHtml{$i_2$}{i<sub>2</sub>}$\neq$ 1) (for example
...
% iterating over a \ccc{Dart_of_orbit_range} using all the \betai{}s in
...
\item \ccc{make_combinatorial_polygon<CMap>(cm,alg)}: creates an
  ...
  linked by \betaun{}), for \ccc{alg>0}; dimension must be greater
  or equal than one;
...
Reciprocally, unlinking a given dart \emph{d0} by \betai{}, with 2$\leq$
...
% links by \betai{} two by two all the darts of
% the orbit $\orb{\mb{1},\ldots,\mb{i-2},\mb{i+2},\ldots,\mb{d}}(d1)$
% and $\orb{\mb{0},\mb{2},\ldots,\mb{i-2},\mb{i+2},\ldots,\mb{d}}(d2)$
% by using the \betats links of each orbit in the same order
% (i.e.,  if we use the $k^{\mbox{th}}$ \betats in the first
% orbit, we use also the $k^{\mbox{th}}$ \betats in the second one).
...
Similarly, \ccc{unsew<i>} operations unlink \betai{} for all the darts
...
\begin{ccAdvanced}
  ...
  \ccc{unlink_beta<i>(dh1)} to unlink \ccc{d1} for \betai{}. In this
...
Given a set of partial permutations \emph{S}=\{\ccTexHtml{$f_1$}{f<sub>1</sub>},$\ldots$,\ccTexHtml{$f_k$}{f<sub>k</sub>}\}, we denote by \orbit{\emph{S}} the \emph{permutation group} generated by
\{\ccTexHtml{$f_1$}{f<sub>1</sub>},$\ldots$,\ccTexHtml{$f_k$}{f<sub>k</sub>}\} and whose group operation is the composition of
partial permutations.  The orbit \orbit{\ccTexHtml{$f_1$}{f<sub>1</sub>},$\ldots$,\ccTexHtml{$f_k$}{f<sub>k</sub>}}(\emph{a}) is the set of darts which can be obtained from \emph{a} by elements of \orbit{\emph{S}}:
\orbit{\ccTexHtml{$f_1$}{f<sub>1</sub>},$\ldots$,\ccTexHtml{$f_k$}{f<sub>k</sub>}}(\emph{a})=\{$\phi$(\emph{a})$|\phi \in$ \orbit{\emph{S}}\}$\setminus$\{$\varnothing$\}.
...
\orbit{\{\betai{}\comp{}\betaj{}|$\forall$ i,j: 1$\leq$ i<j $\leq$ \emph{d}\}}(\emph{d0}).

% \subsection{In 2D}
% In 2D, a 2-map is $M=(D,\beta_1,\beta_2)$ where \betaun{} is a partial
% permutation, and \betadeux{} a partial
...
% halfedges). \betaun{} gives, for each dart, the next dart by turning
% around its facet in the positive orientation; \betadeux{} gives, for each
% dart, the other dart of the same edge. This is why \betadeux{} is a
% partial involution: there are at most two darts corresponding to the same
% edge (in 2D), and applying twice \betadeux{} gives the initial dart (see
...
% \subsection{In 3D}
% In 3D, a 3-map is $M=(D,\beta_1,\beta_2,\beta_3)$ where \betaun{} is a
% partial permutation, and \betadeux{} and \betatrois{} are two partial
...
% \betaun{} is a partial permutation, and \betadeux{}, \betatrois{} and
