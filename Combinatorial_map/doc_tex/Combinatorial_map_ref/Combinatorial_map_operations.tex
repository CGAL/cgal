...
\ccFunction{template <class CMap, unsigned int i>
    bool is_removable(const CMap& cm, typename CMap::Dart_const_handle dh);}
         {Returns true iff the $i$-cell containing \ccc{dh} can be removed,
          ...
          if $i<CMap::dimension-1$ and the $i$-cell containing \ccc{dh} 
          is incident to at most two $(i+1)$-cells.
          \ccPrecond{$0 \leq i \leq CMap::dimension$ and \ccc{*dh}$\in$\ccc{cm.darts()}.}
}
...
{Removes the $i$-cell containing \ccc{dh}. 
...
% \begin{ccAdvanced} 
   ...
   \ccc{dh}, \ccc{Attribute_type<i+1>::type::On_merge}($a_1$,$a_2$) is
   called, with $a_1$ the $(i+1)$-attribute associated to \ccc{dh},
   ...
   \ccc{Attribute_type<j>::type::On_split}($a$,$a'$) is called 
...
{Returns true iff it is possible to insert a 1-cell in \ccc{cm}
  ...
  i.e. if $dh1 \neq dh2$ and $dh1 \in <\beta_1>(dh2)$.
 \ccPrecond{\ccc{CMap::dimension}$\geq 2$, 
   \ccc{*dh1}$\in$\ccc{cm.darts()}, and \ccc{*dh2}$\in$\ccc{cm.darts()}.}
}
% \ccCommentHeading{Template parameter}\\
...
{Returns true iff it is possible to insert a 2-cell in \ccc{cm} along the path
  ...
  iff each couple of consecutive darts of the path $a_1$ and $a_2$ belong to the 
  same vertex and the same volume, and if the path is closed.
  \ccPrecond{\ccc{CMap::dimension}$\geq 3$.}
  % \ccCommentHeading{Template parameter}\\
  % \ccc{Map} must be a model of the \ccc{CombinatorialMap} concept.
  % \ccCommentHeading{Parameters} \\
  % \ccc{cm}: the combinatorial map used;\\
...
{Inserts a 0-cell in the 1-cell containing \ccc{dh}. 
  ...
  \ccPrecond{\ccc{CMap::dimension}$\geq 1$ and \ccc{*dh}$\in$\ccc{cm.darts()}.}\\
  See example in Figure~\ref{fig-insert-vertex}.\\
%  \begin{ccAdvanced}
    If 1-attributes are non void, 
    \ccc{Attribute_type<1>::type::On_split}($a$,$a'$) is called, with $a$ the original 1-attribute associated
...
{Inserts a 0-cell in the 2-cell containing \ccc{dh}.
 ...
 \ccPrecond{\ccc{CMap::dimension}$\geq 2$ and \ccc{*dh}$\in$\ccc{cm.darts()}.}\\
  See example in Figure~\ref{fig-triangulate}.\\
%  \begin{ccAdvanced}
    If 2-attributes are non void, 
    \ccc{Attribute_type<2>::type::On_split}($a$,$a'$) is called, with $a$ the original 2-attribute associated
...
%  \begin{ccAdvanced}
    ...
    \ccc{Attribute_type<3>::type::On_split}($a$,$a'$) is called, with $a$ the original 3-attribute associated
