...
\ccMethod{template<unsigned int ... Beta> Dart_of_orbit_range darts_of_orbit(Dart_handle dh);}
   {Returns a range of all the darts of the orbit \ccc{<Beta...>(dh)}.
     ...
     $\{0,$\ldots$,\emph{dimension}\}, with \ccTexHtml{$i_1$}{i<sub>1</sub>} <\ccTexHtml{$i_2$}{i<sub>1</sub>} <$\ldots$<\ccTexHtml{$i_k$}{i<sub>k</sub>} , and (\ccTexHtml{$i_1$}{i<sub>1</sub>}$\neq$
     0 or \emph{i2} $\neq$ 1).}}
\ccGlue
...
\ccMethod{template <unsigned int i> void sew(Dart_handle dh1,
  ...
  \emph{D1}=$\orb{\mb{1},\ldots,\mb{i-2},\mb{i+2},\ldots,\mb{d}}$(\ccc{dh1}) and
  \emph{D2}=$\orb{\mb{0},\mb{2},\ldots,\mb{i-2},\mb{i+2},\ldots,\mb{d}}$(\ccc{dh2})
