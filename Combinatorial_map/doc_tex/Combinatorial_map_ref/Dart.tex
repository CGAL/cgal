...
\ccc{cm.create_dart();} A new dart is initialized to be $i$-free,
...
\ccMethod{bool is_free(unsigned int i) const;} 
         {Returns true iff \ccc{d0} is $i$-free.
          \ccPrecond{$0 \leq i \leq dimension$.}}

\ccMethod{int highest_nonfree_dimension() const;} 
         {Returns the highest dimension $i$ such that \ccc{d0} is not $i$-free.
...
\ccMethod{template <unsigned int i> Attribute_handle<i>::type attribute();}
         {Returns the $i$-attribute associated to \ccc{d0}.
