...
struct Combinatorial_map_helper
{
  ...
  #else
  // This one cannot be moved in Combinatorial_map_utility_novariadic.h because
  // this is an inner struct which uses inner type Attributes.
  template <class Functor>
  struct Foreach_enabled_attributes
  {
    ...
    {Foreach_static_restricted<Functor,Attributes >::run(t1,t2,t3,t4,t5,t6,t7);}
