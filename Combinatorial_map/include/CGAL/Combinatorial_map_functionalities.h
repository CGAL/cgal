...
namespace CGAL {
  ...
  public:
    ...
    UFTree_handle get_face(Dart_handle dh)
    {
      ...
      return uftrees.find(faces.find(dh));
    }
            ...
            (dangling || !m_map.is_marked(currentdart, treated)))
        {
          ...
          else
          {
                ...
                (dangling || get_face(currentdart)!=get_opposite_face(currentdart)))
            {
              ...
              // remove_cell<2>(currentdart) for optimisation reasons.
              ...
              if (!dangling)
              { uftrees.merge(get_face(currentdart), get_opposite_face(currentdart)); }

              remove_cell<2>(currentdart);
            }
          }
        }
      }
