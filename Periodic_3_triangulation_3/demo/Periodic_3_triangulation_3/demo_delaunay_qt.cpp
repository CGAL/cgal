...
class Demo_periodic_qt : public QMainWindow
{

public:
  Demo_periodic_qt() {
    ui = new Ui::periodicClass;
    ui->setupUi(this);
    s = new Scene(ui);
...
int main(int argc, char *argv[])
{
  ...
  Demo_periodic_qt dpq;
  
  dpq.ui->viewer->restoreStateFromFile();
  
  dpq.show();

  a.connect(&a, SIGNAL(lastWindowClosed()), &a, SLOT(quit()));
  a.connect(dpq.ui->actionExit, SIGNAL(triggered()), &a, SLOT(quit()));
