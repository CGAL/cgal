...
In addition to the \ccc{Point_2} , \ccc{X_monotone_curve_2} types and the
\ccc{Has_infinite_category} category tag defined in the arrangement-traits concept ,
it also defines the types \ccc{Surface_3} and \ccc{Xy_monotone_surface_3}
...
{provides the operator (templated by the \ccc{OutputIterator} type)~:
 ...
 The value-type of \ccc{OutputIterator} is \ccc{std::pair<CGAL::Object , CGAL::Oriented_side>}
 where Object stores either \ccc{X_monotone_curve_2} or \ccc{Point_2}.
 The operator returns a past-the-end iterator for the output sequence.
 \end{itemize}}
