...
Main(int argc, const char* argv[])
{
  ...
  const char* selection_file = (argc > 4) ? argv[4]
    : "data-remeshing/joint-patch.selection.txt";
  ...
  if (selection_file != nullptr && sharp_angle == 0.)
  {
    ...
    if (!facets.empty())
    {
      ...
      if (filename == CGAL::data_file_path("meshes/joint_refined.off") == 0)
        assert(false);
      return;
    }
    else
  ...
  test_precondition(CGAL::data_file_path("meshes/joint_refined.off"),
    "data-remeshing/joint-patch-toolargeconstraints.selection.txt");
}
};
...
int main(int argc, const char* argv[])
{
  ...
  const char* param[6] = { "remesh",//command
                           "data-remeshing/cheese_transformed-facets.off",//input
  ...
  const char* param_bis[6] = { param[0],
                               "data-remeshing/cheese_transformed-facets-2.off",
