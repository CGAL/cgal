...
struct My_sizing_field
{
  ...
  My_sizing_field(double min_size, double max_size, double ymin, double ymax)
    : min_size(min_size), max_size(max_size), ymin(ymin), ymax(ymax)
  {}
  ...
  double at(const Mesh::Vertex_index v, const Mesh& mesh) const
  {
    return at(mesh.point(v));
  }
...
{
  const std::filesystem::path filename = (argc > 1) ? argv[1] : CGAL::data_file_path("meshes/elk.off");
