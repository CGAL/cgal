...
inline std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> getSegmentOuterCircle(typename Kernel::Segment_2 seg,	CGAL::Orientation orientation)
{
	...
	if(orientation ==CGAL::Orientation::CLOCKWISE)
		return std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> (backward,forward);
	return std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> (forward,backward);
...
public :
	bool startIsClosed;
	typename Kernel::Direction_2 edgeStartEngle; //the end is the start of the next edge
	char count; //number of outer circles that cover the edge (0/1/2+)
	size_t edgeIndex;
	CircleArrangmentEdge(typename Kernel::Direction_2 edgeStartEngle,size_t edgeIndex,bool startIsClosed,bool setCountToZero=false)
	{
...
//template <typename Kernel>
//inline bool isOpenDirectionContainedInSegment(bool isFirstDirection,bool isStartClosed,bool isEndClosed,typename Kernel::Direction_2 d, std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> s)
//{
//	if(!isEndClosed)
//	{
//		if( isFirstDirection && (d == s.second))
//			return false;
//	}
//	if(!isStartClosed)
//	{
//		if( !isFirstDirection && (d == s.first))
//			return false;;
//	}
...
//}
template <typename Kernel>
inline bool isOpenDirectionContainedInSegment(typename Kernel::Direction_2 d, std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> s)
{
	return !d.counterclockwise_in_between(s.first,s.second);

}
template <typename Kernel>
inline bool isAcontainedInB(bool isAStartClosed,bool isAEndClosed,std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> A, std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> B)
{
	...
	if( ( isAStartClosed &&A.first == B.first )|| (isAEndClosed && A.second == B.second))
		return false;
	if(!A.first.counterclockwise_in_between(B.first,B.second) && !A.second.counterclockwise_in_between(B.first,B.second) &&
...
{
	std::list<CircleArrangmentEdge<Kernel> >edges;

public :
	CircleArrangment(std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> firstSegmentOuterCircle)
{
		edges.push_back(CircleArrangmentEdge<Kernel>(firstSegmentOuterCircle.first,0,false));
		edges.push_back(CircleArrangmentEdge<Kernel>(firstSegmentOuterCircle.second,0,true,true));
}
	void addSegmentOuterCircle(std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> segmentOuterCircle, size_t edge_index)
	{
		std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> edge;
		...
		typename std::list<struct CircleArrangmentEdge< Kernel> >::iterator nextIt=edges.begin();
		typename std::list<struct CircleArrangmentEdge< Kernel> >::iterator it=edges.begin();
		bool done = false;
		while(!done)
		{
			it = nextIt;
			nextIt=it;
			nextIt++;
			//			std::cout<<"len "<<edges.size()<<"\n";
			//			std::cout<<"it "<<&(*it)<<"\n";
			//			std::cout<<"next it "<<&(*nextIt)<<"\n\n";
			if(nextIt==edges.end()) {
				done =true;
				nextIt= edges.begin();
			}
			...
			std::cout<<segmentOuterCircle.first<<"\t"<<edge.first<<"\t"<<edge.second<<"\t"<<segmentOuterCircle.second<<"\n\n";

			isStartClosedSegment = !isEndClosedSegment;;
			isEndClosedSegment = nextIt->startIsClosed;
			edge.first = edge.second;
			edge.second =nextIt->edgeStartEngle;
			if( isAcontainedInB<Kernel>(isStartClosedSegment,isEndClosedSegment,edge,segmentOuterCircle))
			{
				std::cout<<"O<\n";
			...
			bool isStartContained = isOpenDirectionContainedInSegment<Kernel>(segmentOuterCircle.first,edge);
			bool isEndContained = isOpenDirectionContainedInSegment<Kernel>(segmentOuterCircle.second,edge);
			// o~~~~~~~~~~~~o  = new segment
...
//		for(typename std::list<struct CircleArrangmentEdge< Kernel> >::iterator it=edges.begin();it!=edges.end();)
//		{
//
//
//			if(it->isCovered())
...
//		}
	...
	{
		for(typename std::list<struct CircleArrangmentEdge< Kernel> >::iterator it=edges.begin();it!=edges.end();++it)
		{
			if(it->startIsClosed)
				std::cout<<")[";
			else
	...
	void getAll1Edges(OutputIterator oi)
	{
		for(typename std::list<struct CircleArrangmentEdge< Kernel> >::iterator it=edges.begin();it!=edges.end();)
		{
			...
			if((*it).count == 1)
			{
				std::pair<size_t, 	std::pair<typename Kernel::Direction_2,typename Kernel::Direction_2> > edge;
