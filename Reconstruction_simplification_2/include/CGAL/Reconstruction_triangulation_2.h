...
/// The Reconstruction_triangulation_2 class (corresponding to Triangulation in prototype)
...
template <class Kernel,
          class Tds_ = Triangulation_data_structure_2<Reconstruction_vertex_base_2<Kernel>, Reconstruction_face_base_2<Kernel> > >
class Reconstruction_triangulation_2 : public Delaunay_triangulation_2<Kernel,Tds_> {
public:
	...
	public:
	    ...
	    {
	        for (Finite_edges_iterator ei = Rt_2::finite_edges_begin(); ei != Rt_2::finite_edges_end(); ++ei)
	        {
	            Edge edge = *ei;
	            Edge twin = twin_edge(edge);
	            collect_samples_from_edge(edge, samples);
	        ...
	        for (Finite_vertices_iterator vi = Rt_2::finite_vertices_begin(); vi != Rt_2::finite_vertices_end(); ++vi)
	        {
	            Vertex_handle v = vi;
	    ...
	    {
	        for (Finite_faces_iterator fi = Rt_2::finite_faces_begin(); fi != Rt_2::finite_faces_end(); ++fi)
	        {
	    ...
	    {
	        Edge twin = twin_edge(edge);
	        if (Rt_2::is_infinite(twin.first)) return false;
	        if (Rt_2::is_infinite(edge.first)) return false;
	    ...
	    bool make_collapsible(Edge& edge, Iterator begin, Iterator end, int verbose = 0)
	    {
	        ...
	        while (!pqueue.empty())
	        {
	            ...
	            {
	                // TODO: IV comment in std::cerr << red << "--- No flips available ---" << white << std::endl;
	            ...
	            if (std::max(Dac, Dbd) + EPS < Dbc)
	            {
	                ...
	                // TODO: IV comment in std::cerr << red << "--- Flip makes kernel worse ---" << white << std::endl;
