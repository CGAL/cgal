...
%%%%%%%%%%%%%%%%%%%%%%%
\section{Notes for reviewers ---section to be removed after the review}
\label{sec:notes-reviewers}
%%%%%%%%%%%%%%%%%%%%%%%
 
\begin{enumerate}
\item
The main function is provided as the constructor of the
Monge\_via\_jet\_fitting class for two reasons
\begin{itemize}

\item  the class defines the types Monge\_form which is a canonical
object of independent interest

%\item  the class defines the types Monge\_form and
%Monge\_form\_condition\_numbers types which are of independent interest

\item the class is used as a container for the various fields which
are passed in between the several functions performing the calculation
\end{itemize}

\item 
Notice also the constructor modifies two arguments through
references. These two objects are of independent interest the former
provides the geometric information, the latter assesses the quality of
the construction and may be discarded if the user is happy with the
condition numbers.
...
\cite{cgal:cp-edqpf-05}, ---\ it should be stressed 
...
no analysis can be carried out, and it is up to the user to check the
results match his/her needs.
...
function $z$, and is called its $d$-jet. Notice that a $d$-jet contains
...
frames. If $v_1$ is a unit vector of direction $d_1$, there exists
...
may not be good. The nearer this direction to the tangent plane the
worse the estimation. 

\item
As observed in \cite{cgal:cp-edqpf-05}, the interpolating problem is
not {\em poised} if the points project, into the fitting frame,
...
\ccc{SvdTraits::FT} must be compatible.
...
\begin{itemize}
\item
a human readable text file featuring the \ccc{Monge_form} and
...
\begin{ccTexOnly}
\centerline{
\includegraphics[width=.5\linewidth]{Jet_fitting_3/david-dirmaxkmin-jpg-white-back.pdf}}
\end{ccTexOnly}
\caption{Principal directions associated with $k_{max}$ scaled by $k_{min}$.}
...
Let us note $P_{W\rightarrow F}$ the matrix to change coordinates from the
...
basis of the tangent plane using the Gram-Schmidt algorithm, and then
we compute Weingarten in this basis (applying a change of basis with
the matrix $W'=P^{-1}WP$). In this orthonormal basis, the matrix of
the Weingarten map is symmetric and we diagonalize it.  One finally
gets the principal curvatures which are the eigenvalues of $W$, and
the associated principal directions. This gives an orthonormal direct
basis $(d_1,d_2,n)$. Let us note $P_{F
