...
by Eq. (\refs{eq-answer}). Computations are done in the fitting-basis.
...
FC: dependence fig here????????????

FC: rapid description of main classes

Monge\_rep Monge\_info

In addition, we assume implicit cast between Data, Local and Linalg
are well defined.
...
types support the dotprodut, wedge prod, sum, scalar
...
\begin{itemize}
\item
a human readable text file featuring ??????????
