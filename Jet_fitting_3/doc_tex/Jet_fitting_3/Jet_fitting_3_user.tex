...
the sample points. Such naturally accommodate meshes, but also point
clouds.
...
this internal number type is for example the \ccc{double} type.
...
coefficients which are stored in a \ccc{Monge_form} class. In
...
numerical information on the computation~: condition number and pca
basis;
\item
...
The fitting consists of finding the coefficients
...
seeks the vector $A$ such that $A = \arg \min_A  ||MA-Z||_2$.
...
$W'=P^{-1}WP$). It is then symmetric and we diagonalize it.  One
finally gets the principal curvatures which are the eigenvalues of $W$
and the principal directions. This  gives an orthonormal
direct basis $(d_1,d_2,n)$. Let's note $P_{F
