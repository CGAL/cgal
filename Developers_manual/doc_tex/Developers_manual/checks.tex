...
The default behavior of these checks is to throw exceptions for reporting
failures.  It is also forbidden to call \ccc{std::abort}, \ccc{std::exit}
...
The flag \ccc{CGAL_NDEBUG} disables all checks in \cgal\ but does not affect
the standard \ccc{assert} macro.
Setting \ccc{NDEBUG} in turn sets \ccc{CGAL_NDEBUG}.
