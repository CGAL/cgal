...
\item Use pre- and postcondition checkers wherever it is possible in the
      main code.  In the test-suite code itself, the macro \ccc{CGAL_test_assert}
      (defined in \ccc{<CGAL/Testsuite/assert.h>}) should be used in place of
      \ccc{assert} to check all conditions, since the former is not disabled by
      \ccc{NDEBUG}. 
\end {itemize}
