...
\begin{itemize}
   \item {\tt -c} Uses an already untared internal release instead of
                  downloading and untaring a new one. You can use this option
                  to {\em resume} immediately the test suite process in case it
                  failed before. In this case, the current contents of RELEASE\_NR 
                  is used to identify the directory containing the release to test. 
                  If the symbolic link 'CGAL-I' is currently targetting a different
                  directory (as is the case normally, without the -c option),
                  the release on that CGAL-I directory is used as a reference
                  and its configuration copied into the new release. However,
                  if CGAL-I points to the release whose test is being resumed,
                  there is no reference release and no configuration is copied.
                  In this case, the tested release must have some subdirectory
                  under {\tt cmake/platforms}.    

   \item {\tt -n} Skips the testing phase, finishing after the libraries has been built.
                  You can use this option to verify that your system contains all the
                  necessary elements for the libraries to be build with cmake.
