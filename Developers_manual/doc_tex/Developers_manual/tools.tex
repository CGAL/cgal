...
CMake scrits get created automatically when a release is made. This ensures
that the most up-to-date CMakeLists are created. Experience has shown
...
(the \_RELEASE and \_DEBUG variables are added to the general variable, so for each case 
...
allowing those {\tt TESTSUITE\_*} enviroment variables to append compiler and linker
flags as neccesary.
...
such as {\tt -DCMAKE\_BUILD\_TYPE=Debug}.
...
creates and runs temporary build and test scripts in each configuration folder to build an test the libraries. 
Those temporary scripts are created with a verbatim copy of the contents of the {\tt setup} 
file, if it exists, at the beggining, allowing you to boosttrap the process as needed.
...
the enviroment for the command line compiler. This can be done
by setting up the VC command line tools in the {\tt setup} script
that is used to boostrap the build and test process, as explained before.
Merly executing the {\tt vcvars32.bat} does not work since the variables
in there will be defined only the enviroment for the batch file and not
in the enviroment of the building/testing script (this is why
the contents of 'setup' is copied into the script instead of just
executing it)

Furthermore, within the but {\tt setup} script, the following must be
considered: 
...
Thus, the values for these variales must be a verbatim copy of the values defined in
{\tt vcvars32.bat} of your installation.
...
POSIX paths and properly appended to the cygwin's PATH variable.
