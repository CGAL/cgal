...
The \ccStyle{intersection} function used to return a
...
Here, \ccStyle{Intersect_23} means either \ccStyle{Intersect_2} or \ccStyle{Intersect_3}, depending on the arguments.
}}

The same functionality is also available through the functors
\ccc{Kernel::Intersect_2} and \ccc{Kernel::Intersect_3}.
...
where both \ccStyle{Type1} and \ccStyle{Type2} can be either
\begin{itemize}
...
elements of type \ccStyle{result_of< Intersect_23(A, B) >}, in lexicographic order,
...
three objects. \ccc{intersections} iterates over
...
An example showing the use of the intersection function as a plain function call and with \ccc{CGAL::Dispatch_output_iterator} combined with a standard library algorithm.
