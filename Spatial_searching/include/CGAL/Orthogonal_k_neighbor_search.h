...
private:
  ...
  void compute_neighbors_orthogonally(typename Base::Node_const_handle N, FT rd)
  {
    if (!(N->is_leaf())) 
    {
      Tree::Internal_node_const_handle node = static_cast<Tree::Internal_node_const_handle>(N);
    ...
    {
      // n is a leaf
      Tree::Leaf_node_const_handle node = static_cast<Tree::Leaf_node_const_handle>(N);
