...
\ccNestedType{Polygon_2}{represents a general polygon. Must be a model of the \ccc{GpsTraits::GeneralPolygon_2} concept.}
\ccGlue
\ccNestedType{Polygon_with_holes_2}{represents a general polygon with holes. Must be a model of the \ccc{GpsTraits::GeneralPolygonWithHoles_2} concept.}
...
{a functor that returns a pair that consists of the begin and
 ...
 \ccc{std:pair<Curve_const_iterator, Curve_const_iterator>
  operator() (const Polygon_2 & pgn)}.}

\ccNestedType{Is_valid_2}
{provides the operators~: \\
 \ccc{bool operator() (Polygon_2 & pgn)} \\
 which returns \ccc{true} if the \ccc{pgn} is valid, and \ccc{false}
 otherwise; \\ and~: \\
 \ccc{bool operator() (Polygon_with_holes_2 & pgn_with_holes)} \\
 which returns \ccc{true} if \ccc{pgn_with_holes} 
 ...
 A polygon of type \ccc{Polygon_with_holes_2} is valid, 
...
\ccMethod{Is_valid_2 is_valid_2_object();}
...
\ccMethod{Construct_general_polygon_with_holes_2 construct_polygon_with_holes_2_object() const;}
{returns a functor that obtains begin/wend iterators over a container of holes.}
