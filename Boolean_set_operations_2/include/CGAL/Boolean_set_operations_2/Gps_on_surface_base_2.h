...
protected:
  ...
  // Start tagging ccbs
    ...
    do{
      ...
      CGAL_FOREACH(Halfedge_handle h, halfedges_that_was_on_an_outer_ccb)
      {
        if (h->flag()!=NOT_VISITED) continue;
  ...
  // End tagging ccbs
    ...
    CGAL_FOREACH(Outer_ccb_and_face& ccb_and_face, outer_ccb_and_new_face_pairs)
      ccb_and_face.first->set_face(ccb_and_face.second);
    CGAL_FOREACH(Inner_ccb_and_face& ccb_and_face, inner_ccb_and_new_face_pairs)
      ccb_and_face.first->set_face(ccb_and_face.second);
