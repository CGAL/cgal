...
%%xfc-removed: installed to compile the example code.
...
\ccc{MIN_HYPERBOLIC_RIDGE}, see figure \ref{ellipsoid-ridges}.
%%
...
\ccc{MAX_CREST_RIDGE} and \ccc{MIN_CREST_RIDGE}, see figure \ref{david-crest}.

%%xfc-proposal: next to fig 1.2, put the crest of the ellipsoid?
...
%%xfc-problem: how come the 3rd condition is not 'otherwise'. it would
%%be easier to grasp
...
%%xfc:for umbilics, I liked the fact that both functions (class
%%member, external) are
%%mentioned. I think we should get the same here.
...
type (ENUMERATE), the vertex of the mesh they are associated to and the list of
half-edges representing the contour of the neighborhood.

%%xfc-prev-sentence: replace ENUMERATE by ellip, hyper, unknown !??
...
\section{Illustrations - Examples} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{examples}

%%xfc: $a$ and not a !!!

%%%xfc  when giving command line example, -m 4 instead of -m4 ... even if
%%%both work!
