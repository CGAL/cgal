...
private:
  ...
  void build_triangle_aabb_tree(const C3t3& c3t3)
  {
    ...
    const CGAL::Bbox_3& bb = m_triangles_aabb_tree.bbox();
    m_aabb_epsilon = 1e-3 * (std::min)(bb.xmax() - bb.xmin(),
                                       (std::min)(bb.ymax() - bb.ymin(), bb.zmax() - bb.zmin()));
  }
  ...
  void reset_free_vertices(const Tr& tr)
  {
    ...
    for(const Cell_handle c : tr.finite_cell_handles())
    {
      ...
      for(auto vi : tr.vertices(c))
      {
        ...
        switch(dim)
        {
        case 3: m_free_vertices[idi] = true; break;
        case 2: m_free_vertices[idi] = !m_protect_boundaries; break;
        case 1: m_free_vertices[idi] = !m_protect_boundaries && m_smooth_constrained_edges; break;
        case 0: m_free_vertices[idi] = false; break;
    ...
    for(const auto& fn : fnormals)
    {
      ...
      while(!facets.empty())
      {
        ...
        for(const Edge& ei : facet_edges(ff.first, ff.second, tr))
        {
          ...
          {
            const Facet neigh = *neighbor;
            if(fnormals[neigh] == CGAL::NULL_VECTOR)
            {
              fnormals[neigh] = compute_normal(neigh, ref, gt);
              facets.push_back(neigh);
            }
  ...
  std::optional<Facet> find_adjacent_facet_on_surface(const Facet& f, const Edge& edge, const C3t3& c3t3)
  {
    ...
    do
    {
      ...
      if(f != fi && mf != fi && is_boundary(c3t3, fi, m_cell_selector) && patch == c3t3.surface_patch_index(fi))
        return canonical_facet(fi);
    } while(++fcirc != fend);
    return {};
  }

  template <typename Gt_>
  Vector_3 compute_normal(const Facet& f, const Vector_3& reference_normal, const Gt_& gt)
  {
    typename Gt_::Construct_opposite_vector_3 opp = gt.construct_opposite_vector_3_object();
    typename Gt_::Compute_scalar_product_3 scalar_product = gt.compute_scalar_product_3_object();
    Vector_3 n = CGAL::Tetrahedral_remeshing::normal(f, gt);
    if(scalar_product(n, reference_normal) < 0.)
      n = opp(n);
    return n;
...
protected:
  ...
  template <typename CellRange, typename Tr_>
  bool check_inversion_and_move(const typename Tr_::Vertex_handle v,
                                const typename Tr_::Geom_traits::Point_3& final_pos,
                                const CellRange& inc_cells,
                                const Tr_& tr,
#ifdef CGAL_TETRAHEDRAL_REMESHING_VERBOSE
                                FT& total_move) const
...
#endif
  {
    const typename Tr_::Point backup = v->point();
    const typename Tr_::Geom_traits::Point_3 pv = point(backup);

    bool valid_orientation = false;
    bool angles_improved = true;
    double frac = 1.0;
    typename Tr_::Geom_traits::Vector_3 move(pv, final_pos);
    ...
    do
    {
      v->set_point(typename Tr_::Point(pv + frac * move));
      valid_try = true;
      valid_orientation = true;
      angles_improved = true;

      for(const typename Tr_::Cell_handle& ci : inc_cells)
      {
        ...
        else if(m_context->m_flip_smooth_steps)
        {
          if(is_selected(ci))
          {
            Dihedral_angle_cosine max_cos_ci = max_cos_dihedral_angle(tr, ci, false);
...
public:
  ...
  bool execute_operation(const ElementType& v, C3t3& c3t3) override
  {
    ...
    return BaseClass::check_inversion_and_move(v, new_pos, inc_cells, c3t3.triangulation(), m_context->m_total_move);
  }
