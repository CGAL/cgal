...
namespace debug_helpers {
  ...
  std::string point_to_string(const Point3& p) {
  ...
  std::string vector_to_string(const Vector3& v) {
    std::ostringstream oss;
  oss<< std::setprecision(25);
    oss << v.x() << " " << v.y() << " " << v.z();
    return oss.str();
...
public:
  ...
  void init(const C3t3& c3t3) {
    ...
    // Build MLS Surfaces
    createMLSSurfaces(subdomain_FMLS, subdomain_FMLS_indices, vertices_normals, vertices_surface_indices, c3t3);
#else
    // Build AABB tree
...
private:
  ...
  template <typename IncCellsVector> void collect_incident_cells(const Tr& tr, IncCellsVector& inc_cells) {
    for(const Cell_handle c : tr.finite_cell_handles()) {
      for(auto vi : tr.vertices(c)) {
        const std::size_t idi = vertex_id(vi);
        if(is_free(idi))
  ...
  template <typename Patch_index> std::string debug_to_string(const Patch_index i) { return std::to_string(i); }

  template <typename Patch_index> std::string debug_to_string(const std::pair<Patch_index, Patch_index>& pi) {
    std::string str = std::to_string(pi.first);
    str.append("_").append(std::to_string(pi.second));
    return str;
  }

  template <typename VertexNormalsMap> void compute_vertices_normals(const C3t3& c3t3, VertexNormalsMap& normals_map) {
    ...
    std::unordered_map<Surface_patch_index, std::vector<typename Tr::Geom_traits::Segment_3>,
                       boost::hash<Surface_patch_index>>
        ons_map;
#endif

    // normalize the computed normals
    for(typename VertexNormalsMap::iterator vnm_it = normals_map.begin(); vnm_it != normals_map.end(); ++vnm_it) {
      // value type is map<Surface_patch_index, Vector_3>
      for(typename VertexNormalsMap::mapped_type::iterator it = vnm_it->second.begin(); it != vnm_it->second.end();
          ++it)
      {
    ...
    for(auto& kv : ons_map) {
      std::ostringstream oss;
      oss << "dump_normals_normalized_[" << debug_to_string(kv.first) << "].polylines.txt";
  ...
  Dihedral_angle_cosine max_cosine(const Tr& tr, const boost::container::small_vector<Cell_handle, 40>& cells) const {
...
public:
  void smooth_vertices(C3t3& c3t3)
  {
    ...
    #ifndef CGAL_TETRAHEDRAL_REMESHING_USE_INTERNAL_VERTEX_SMOOTHING
    ////////////// INTERNAL VERTICES ///////////////////////
#ifdef CGAL_TETRAHEDRAL_REMESHING_VERBOSE
    nb_done_3d =
#endif
