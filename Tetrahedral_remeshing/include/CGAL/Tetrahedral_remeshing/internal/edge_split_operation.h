  ...
  bool execute_operation(const ElementType& vp, C3t3& c3t3) override {
    ...
    if(tr.tds().is_edge(e.first, e.second, cell, i1, i2)) {
      ...
      if(!splittable) {
        ...
        edge_split_logger.log("[SPLIT_FAILED] edge not splittable. length:", vp.first,
...
#ifdef CGAL_TETRAHEDRAL_REMESHING_EDGE_SPLIT_DEBUG
        // Debug log for split failure
        edge_split_logger.log("[SPLIT_FAILED] operation failed. length:", vp.first,
                              " - Thread: ", std::this_thread::get_id());
#endif
        return false;
      }
      ...
      edge_split_logger.log("[SPLIT_SUCCESS] on edge with length:", vp.first,
                            " - Thread: ", std::this_thread::get_id());
#endif
    ...
    edge_split_logger.log("[SPLIT_FAILED] edge not found. length:", vp.first,
                          " - Thread: ", std::this_thread::get_id());
#endif
    return false;
  }
