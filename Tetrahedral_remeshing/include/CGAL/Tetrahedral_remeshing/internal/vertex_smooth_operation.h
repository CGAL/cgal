...
template <typename C3t3, typename SizingFunction, typename CellSelector> class Vertex_smoothing_context
{
public:
  ...
  std::unordered_map<Vertex_handle, std::unordered_map<Surface_patch_index, Vector_3, boost::hash<Surface_patch_index>>>
      m_vertices_normals;
...
private:
  ...
  void build_triangle_aabb_tree(const C3t3& c3t3) {
    ...
    m_aabb_epsilon = 1e-3 * (std::min)(bb.xmax() - bb.xmin(), (std::min)(bb.ymax() - bb.ymin(), bb.zmax() - bb.zmin()));
  }
  ...
  void compute_vertices_normals(const C3t3& c3t3) {
    ...
    typename Tr::Geom_traits::Construct_opposite_vector_3 opp = gt.construct_opposite_vector_3_object();
    ...
    for(const auto& fn : fnormals) {
      ...
      if(c3t3.triangulation().is_infinite(mf.first) || c3t3.subdomain_index(mf.first) < c3t3.subdomain_index(f.first))
      ...
      while(!facets.empty()) {
        ...
        for(const Edge& ei : facet_edges(ff.first, ff.second, tr)) {
          if(std::optional<Facet> neighbor = find_adjacent_facet_on_surface(ff, ei, c3t3)) {
  ...
  std::optional<Facet> find_adjacent_facet_on_surface(const Facet& f, const Edge& edge, const C3t3& c3t3) {
    CGAL_expensive_assertion(is_boundary(c3t3, f, m_cell_selector));
    ...
    do {
      ...
      if(f != fi && mf != fi && is_boundary(c3t3, fi, m_cell_selector) && patch == c3t3.surface_patch_index(fi)) {
  ...
  template <typename Gt> Vector_3 compute_normal(const Facet& f, const Vector_3& reference_normal, const Gt& gt) {
    typename Gt::Construct_opposite_vector_3 opp = gt.construct_opposite_vector_3_object();
...
template <typename C3t3, typename SizingFunction, typename CellSelector> class VertexSmoothOperationBase
{
protected:
...
protected:
  ...
  template <typename CellRange> Dihedral_angle_cosine max_cosine(const Tr& tr, const CellRange& cells) const {
...
class InternalVertexSmoothOperation : public VertexSmoothOperationBase<C3t3, SizingFunction, CellSelector>,
...
public:
  ...
  bool execute_operation(const ElementType& v, C3t3& c3t3) override {
    ...
    bool result =
        BaseClass::check_inversion_and_move(v, new_pos, inc_cells, c3t3.triangulation(), m_context->m_total_move);
    return result;
  }
...
template <typename C3t3, typename SizingFunction, typename CellSelector>
class SurfaceVertexSmoothOperation : public VertexSmoothOperationBase<C3t3, SizingFunction, CellSelector>,
                                     public ElementaryOperation<C3t3,
                                                                typename C3t3::Triangulation::Vertex_handle,
                                                                typename C3t3::Triangulation::Finite_vertex_handles,
                                                                typename C3t3::Triangulation::Vertex_handle>
...
template <typename C3t3, typename SizingFunction, typename CellSelector>
class ComplexEdgeVertexSmoothOperation : public VertexSmoothOperationBase<C3t3, SizingFunction, CellSelector>,
                                         public ElementaryOperation<C3t3,
                                                                    typename C3t3::Triangulation::Vertex_handle,
                                                                    typename C3t3::Triangulation::Finite_vertex_handles,
                                                                    typename C3t3::Triangulation::Vertex_handle>
...
public:
  ...
  using Base =
      ElementaryOperation<C3t3, Vertex_handle, typename C3t3::Triangulation::Finite_vertex_handles, Vertex_handle>;
