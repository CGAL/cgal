...
####################################################################################################
def run_benchmark(execname, filename, target_edge_length, num_iterations, max_time, test_ID, outpath):
  ...
  exit_code = 0
  output = ""
  # Command line for Tetrahedral_remeshing
  cmd = [
    "/usr/bin/time", "-v",
    execname,
    xml_filename,
  ...
  print("EXIT CODE: ", exit_codes.get(exit_code, exit_code))

  outfile = outpath + "/Robustness/results/" + test_ID + "/" + raw_filename + ".txt"
  print("writing to", outfile)
  file = open(outfile, "w")
  file.write(exit_codes.get(exit_code, str(exit_code)))
  file.close()
...
def main(argv):
  ...
  outpath=""
  try:
    opts, args = getopt.getopt(sys.argv[1:], 'i:t:', ["exec=", "target_edge_length=", "num_iterations=", "test_ID=", "out="])
  ...
  for opt, arg in opts:
    if opt == "--exec": # executable
      ...
      filename = arg
    elif opt == "--target_edge_length":
      target_edge_length = arg
  ...
  print("target_edge_length = ", target_edge_length)
  print("num_iterations = ", num_iterations)
  print("max_time = ", max_time)
  print("test_ID = ", test_ID)
  print("outpath = ", outpath)

  exit_code = run_benchmark(execname, filename, target_edge_length, num_iterations, max_time, test_ID, outpath)
  ...
  # These tags must match what Tetrahedral_remeshing writes in its XML
