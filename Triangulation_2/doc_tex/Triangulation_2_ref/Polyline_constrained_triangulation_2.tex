...
The class \ccc{Polyline_constrained_triangulation_2<Tr>} inherits from
its template parameter Tr, which has to be instantiated by a
\ccc{CGAL::Constrained_triangulation_2<Traits,Tds>} or a
\ccc{CGAL::Constrained_Delaunay_triangulation_2<Traits,Tds>}, whose
\ccStyle{Vertex} type must be a model of
\ccc{PolylineConstrainedTriangulationVertexBase_2}.
...
\ccc{std::pair<const Vertex_handle, Vertex_handle>}
...
\ccPrecond The \ccc{value_type} of \ccc{first} and \ccc{last}
 is either \ccc{Constraint}, \ccc{Polygon_2} or a \ccc{Range} of \ccc{Point}.
}
...
\ccMethod{ Constraint_id insert_constraint(const Constraint& c);}
...
\ccMethod{Vertex_handle push_back(const Constraint& c);}
{Equivalent to \ccc{insert_constraint(c)}.}
...
\ccMethod{ template < class Range >
           Constraint_id
           insert_constraint(const Range& range);}
{ Inserts the constraint defined by the \ccc{range}.
\ccPrecond The \ccc{value_type} of \ccc{Range} is \ccc{Point}.}
...
$\left[\right.$\ccc{first}, \ccc{last}$\left.\right)$.
\ccPrecond The \ccc{value_type} of \ccc{first} and \ccc{last}
 is either \ccc{Constraint}, or \ccc{Polygon_2}, or a \ccc{Range} of \ccc{Point}. 
 Equivalent to \ccStyle{while(first != last) insert_constraint(*first++);} }
...
{Removes vertex at \ccc{viq} from the constraint and the triangulation.
Only the vertex but not the point is removed from the constraint \ccc{cid}.
\ccPrecond The vertex at \ccc{viq} must not be the first or last vertex, must not be fixed, and the line segment between the predecessor \ccc{vip} and 
the successor \ccc{vir} of \ccc{viq} must not intersect any constraint.} 
