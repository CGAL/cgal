...
{Split a constraint in two constraints, so that \ccc{vcit} becomes the first
...
\ccMethod{template <class OutputIterator>
  ...
  remove_vertex_from_constraint(Constraint_id cid, Vertices_in_constraint_iterator pos, 
				OutputIterator out);}
{Removes the vertex at \ccc{pos} from the constraint, and writes the faces that
have changed into \ccc{out}.}. 
...
operates on \ccRefName{}.  Application code can exclude vertices in
polyline constraints from being removed with the following functions.
