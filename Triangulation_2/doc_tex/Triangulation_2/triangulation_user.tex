...
\ccIndexMainItem{Cropped_voronoi diagram}
...
\ccIncludeExampleCode{Triangulation_2/print_cropped_voronoi.cpp}
...
\ccIncludeExampleCode{Triangulation_2/polygon_triangulation.cpp}
...
\section{Polylines as Constraints \label{Section_2D_Polyline_Constrainted_Triangulation}}

The class \ccc{Polyline_constrained_triangulation_2<Tr>} is
similar to the class  \ccc{Constrained_triangulation_plus_2<Tr>}.
It stores how constraints are split in subconstraints when
they intersect, and it allows to find the enclosing constraint
for a subconstraint.

It is more powerful, as constraints are not limited to segments, but
may be arbitrary polylines. These polyline constraints can be concatenated,
and split, and vertices can be removed and inserted in a polyline
constraint. The polyline constraints can even be simplified with
the polyline simplification algorithm described in 
Chapter~\ref{chapter-polylinesimplification}.
