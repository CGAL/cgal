...
wrap around the (unshrunk) input balls, we can first shrink the
weights of the input balls with a factor $1/s$ (which means that their
weight grows) and then compute the skin surface.
...
mesh and \ccc{Skin_surface_3} (\ccc{Union_of_balls_3}) object and
refines the mesh.
...
surface wraps around the input balls instead of shrunk input balls.
The second optional argument specifies the geometric traits class to
be used.
