...
namespace dsrpdb {
  ...
  double no_align_cRMS(It ab, It ae, It bb, It be, F f){
    ...
    if (std::distance(ab, ae) != std::distance(bb, be)){
      dsrpdb_internal::error_logger.new_fatal_error("Protein chains used for computing cRMS must have equal lengths.\n");
    }
    double ret=0;
    int num=0;
  ...
  double cRMS(It ab, It ae, It bb, It be){
    ...
    dsrpdb::Transform tr
      = dsrpdb_internal::transform_taking_first_to_second( bb, be, ab, ae);
    /*{
