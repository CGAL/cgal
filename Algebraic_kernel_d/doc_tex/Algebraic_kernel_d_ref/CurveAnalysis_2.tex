...
Observe that there is no need to have an analogue member function
for arcs approaching the top or bottom boundary, as in this cases it 
suffices to call \ccc{AlgebraicKernel_2::Compare_x_2} to compare their 
asymptotic values and by the $y$-per-$x$-view (perturbed infinite rectangle)
only arcs approaching the left or right boundary can reach the corners of the
rectangle. 
