...
class Prime_polynomial:public Prime,public Page_alloc{
    protected:
        static
        int pp_from_poly(pn *pp,mpz_t *poly,int n){
            int i;
        ...
        static
        void pp_out_str(FILE *stream,pn* pp,int n){
            int i;
            for(i=n;i;--i)
        ...
        // Knuth 2; m>=n
        static
        int pp_pdivrem(pn *r,pn *u,int m,pn *v,int n){
            int k,j;
            for(k=0;k<=m;++k)
                r[k]=u[k];
            // division p. 402
        ...
        // GCL, page 280; da>=db
        static
        int pp_gcd(pn *g,pn *a,int da,pn *b,int db){
            pn *r0,*r1,*r2;
            int i,d0,d1,d2;
            d0=da;
            r0=(pn*)palloc((1+da)*sizeof(pn));
            //for(i=0;i<=da;++i)
            //  r0[i]=a[i];
            pp_pp(r0,a,da);
            d1=db;
            r1=(pn*)palloc((1+da)*sizeof(pn));
            //for(i=0;i<=db;++i)
            //  r1[i]=b[i];
            pp_pp(r1,b,db);
            r2=(pn*)palloc((1+da)*sizeof(pn));
