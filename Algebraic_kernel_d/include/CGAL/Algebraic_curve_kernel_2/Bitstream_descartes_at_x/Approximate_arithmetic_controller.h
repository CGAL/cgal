...
public:
    ...
    {
#if CGAL_ACK_USE_DERIVATIVE_OPTION || CGAL_ACK_USE_BISECTION_OPTION
        f_x = CGAL::diff(_m_f);
        flag = false;
#endif

#if CGAL_ACK_USE_BISECTION_OPTION
        flag_r = false;
#endif
        iter_refine_steps = 0;
    }
    ...
    Eval_interval interval_approximation() const {
...
#if CGAL_ACK_USE_DERIVATIVE_OPTION || CGAL_ACK_USE_BISECTION_OPTION
        //! CGAL_ACK_USE_DERIVATIVE_OPTION On or CGAL_ACK_USE_BISECTION_OPTION On;
        //! flag request, because to save runtime
        if(!flag)
        {
            interval_f_x = evaluate_iv(f_x, interval);
            CGAL::Sign sign_f_x = CGAL::sign(interval_f_x.lower()) * 
                                  CGAL::sign(interval_f_x.upper());
...
#if CGAL_ACK_USE_BISECTION_OPTION
                // help-variables, needed to save the old/new state 
                // of the intervals
                low = _m_alpha.low();
                high = _m_alpha.high();
        ...
        if(flag)
        {
            ...
            if(sign_low == CGAL::POSITIVE)
            {
                ...
                interval_f = Eval_interval( _m_f.evaluate(_m_alpha.low()), 
                                            _m_f.evaluate(_m_alpha.high()) );
            }
            else
            {
                //! slope of _m_f is negative => switch interval-boundaries
                interval_f = Eval_interval( _m_f.evaluate(_m_alpha.high()), 
                                            _m_f.evaluate(_m_alpha.low()) );
            }
#else
            if(!flag_r)
            {
            ...
            {
                // request if the low boundary hasn't change 
                ...
                // request if the high boundary hasn't change 
                // after the refinement
                else if(high == high_r)
                {
                    low = low_r;
        ...
        return interval_f;
#else
        //! CGAL_ACK_USE_DERIVATIVE_OPTION Off or CGAL_ACK_USE_BISECTION_OPTION Off;
