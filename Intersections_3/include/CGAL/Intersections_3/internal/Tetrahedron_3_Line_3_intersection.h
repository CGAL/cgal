    ...
    const K&)
{
  typedef typename Intersection_traits<K,
      ...
      typename K::Line_3>::result_type Result_type;

  typedef typename Intersection_traits<K,
      typename K::Triangle_3,
      typename K::Line_3>::result_type Inter_type;

  int res_id = -1;

  Inter_type tr_line[4];
  for(std::size_t i = 0; i < 4; ++i)
  {
   const typename K::Triangle_3 triangle(tet.vertex((i+1)%4),
                           tet.vertex((i+2)%4),
                           tet.vertex((i+3)%4));
    tr_line[i] = CGAL::intersection(line, triangle);
    if(tr_line[i])
      if( boost::get<typename K::Segment_3>(&*tr_line[i]) != nullptr)
        res_id = i;
  }
  //if there is a segment in the intersections, then we return it
  if(res_id !=-1)
    return tr_line[res_id];
  //else, if all intersections are the same or none, we return this single point.
  ...
  for(std::size_t i = 0; i< 4; ++i)
  {
    if(tr_line[i])
    {
      if (const typename K::Point_3* p = boost::get<typename K::Point_3>(&*tr_line[i]))
      {
        if(res_points.empty())
        {
          res_id = i;
