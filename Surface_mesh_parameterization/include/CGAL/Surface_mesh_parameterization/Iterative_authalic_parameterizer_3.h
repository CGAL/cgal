...
class Iterative_authalic_parameterizer_3
  : public Fixed_border_iterative_parameterizer_3<
             TriangleMesh_,
             typename Default::Get<BorderParameterizer_,
                                   Square_border_arc_length_parameterizer_3<TriangleMesh_> >::type,
             typename Default::Get<SolverTraits_,
#if defined(CGAL_EIGEN3_ENABLED)
                                   CGAL::Eigen_solver_traits<
                                     Eigen::BiCGSTAB<Eigen_sparse_matrix<double>::EigenType,
...
protected:
  ...
  virtual NT compute_faceWise_L2(TriangleMesh& mesh, Vertex_point2_map &uvmap)
  {
    ...
    for(face_descriptor fd : faces(mesh))
    {
      ...
      int i = 0;
      for(vertex_descriptor vd : CGAL::vertices_around_face(halfedge(fd, mesh), mesh))
      {
                                                  ...
                                                  Vertex_Int_map& vimap)
  {
    ...
    {
      std::cout << "Encountered inner border with valency-2 vertex (" << vertex << "), "
    ...
    else
    {
      ...
      for(int n=0; n<neighborsCounter; ++n)
        NeighborList_[n].uv = Point_2(NeighborList_[n].length*cos(NeighborList_[n].angle),
                                      NeighborList_[n].length*sin(NeighborList_[n].angle));
  ...
  virtual double compute_sig_ij(TriangleMesh& mesh,
                                Vertex_point2_map& uvmap,
                                vertex_descriptor v_i,
                                vertex_descriptor v_j,
                                double gamma)
  {
...
private:
  ...
  double getvL2(const TriangleMesh& mesh, const vertex_descriptor vertex) const
  {
  ...
  {
    double A = (((uv_points[1].x() - uv_points[0].x()) * (uv_points[2].y()-uv_points[0].y()))
                - ((uv_points[2].x()-uv_points[0].x())*(uv_points[1].y()-uv_points[0].y()))) / 2.;
    if(A == 0.0)
      return 1.0;
    return A;
  }
  ...
  Point_3 getSs(Point_3 mesh_points[3], Point_2 uv_points[3], const double &A) const
  {
    ...
    Point_3 Ss((mesh_points[0].x()*dt0 + mesh_points[1].x()*dt1 + mesh_points[2].x()*dt2 )/(2.0*A),
  ...
  Point_3 getSt(Point_3 mesh_points[3], Point_2 uv_points[3], const double &A) const
  {
    ...
    Point_3 St((mesh_points[0].x()*ds0 + mesh_points[1].x()*ds1 +mesh_points[2].x()*ds2)/(2.0*A),
