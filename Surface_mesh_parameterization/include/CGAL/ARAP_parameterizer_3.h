...
private:
  ...
  void output_ct_exterior_face(const CT& ct) const
  {
...
//      std::cout << i << " is fixed in A " << std::endl;
    ...
    // vh_i is NOT fixed
    fill_linear_system_matrix_mvc_from_points(vh_i->point(), i,
                                              vh_j->point(), j,
                                              vh_k->point(), k, A);
  }
                                                  ...
                                                  Matrix& A) const
  {
    ...
    fill_linear_system_matrix_mvc_from_ct_vertices<CT>(vh0, vh1, vh2, vimap, vpmap, A);
    fill_linear_system_matrix_mvc_from_ct_vertices<CT>(vh1, vh2, vh0, vimap, vpmap, A);
    fill_linear_system_matrix_mvc_from_ct_vertices<CT>(vh2, vh0, vh1, vimap, vpmap, A);
  }
                                ...
                                Matrix& A) const
  {
    ...
    for(; fit!=fend; ++fit)
    {
      ...
      fill_linear_system_matrix_mvc_from_ct_face(ct, fit, vimap, vpmap, A);
    }
                                               ...
                                               const VertexParameterizedMap mvc_vpmap) const
  {
    ...
    assign_solution(Xu, Xv, vertices, uvmap, vimap);
    output_uvmap("ARAP_post_processing.txt", mesh, faces, uvmap);
...
//    typedef CGAL::Constrained_Delaunay_triangulation_2<Kernel, TDS, Itag>   CT;
    ...
    CGAL_postcondition(is_param_border_simple); // not sure how to handle non-simple yet
