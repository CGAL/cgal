...
{
Initialize {\em A$\ast$X = B} linear system after (at least 2) border vertices are parameterized.
Preconditions:\begin{itemize}
\item vertices must be indexed.\item X and B must be allocated and empty.\item (at least 2) border vertices must be parameterized. \end{itemize}
}
\ccGlue
\ccMethod{void project_triangle (const Point_3 & p0, const Point_3 & p1, const Point_3 & p2, Point_2 & z0, Point_2 & z1, Point_2 & z2);}
{
Utility for setup\_triangle\_relations(): Computes the coordinates of the vertices of a triangle in a local 2D orthonormal basis of the triangle's plane.
}
\ccGlue
...
\item {\em A$\ast$X = B} system is solvable (in the least squares sense) with a good conditioning.\item 3D -$>$ 2D mapping is 1 to 1. \end{itemize}
}
\ccGlue
