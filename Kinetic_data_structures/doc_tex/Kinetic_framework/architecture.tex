...
concept is discussed separately in Section \ref{sec:kds_algebraic_kernel}.
...
\caption{\label{fig:kds_uml_usage_architecture} The figure, identical to the one in the
  ...
  \ccc{Kinetic::Sort<Traits, Visitor>} kinetic data structure and the
  various pieces of our package.  Other, more complicated, kinetic
  data structures will also use the \ccc{Kinetic::InstantaneousKernel}
...
\label{sec:kds_active_objects_table}
...
\label{sec:kds_instantaneous_kernel}
...
kernels' \ccc{Root} objects (see Section \ref{sec:kds_algebraic_kernel}),
...
\label{sec:kds_algebraic_kernel}
