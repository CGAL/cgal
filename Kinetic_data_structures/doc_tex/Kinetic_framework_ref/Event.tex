...
\ccMethod{CGAL::Comparison_result compare_concurrent(Key a, Key b) const;}{The two events \ccc{a} and \ccc{b} occur at the same time (\ccc{this} has key \ccc{a}). This method returns a \ccc{CGAL::Comparison_result} which is used to order the two equal events. If \ccc{CGAL::EQUAL} is returned then \ccc{merge} will be called.}
