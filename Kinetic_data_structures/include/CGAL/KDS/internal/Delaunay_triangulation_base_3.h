...
#ifndef CGAL_KDS_INTERNAL_DELAUNAY_BASE_3_H
#define CGAL_KDS_INTERNAL_DELAUNAY_BASE_3_H
...
public:
  ...
  */
  ...
  Edge flip(const Facet &flip_facet) {
    ...
    Certificate ore= extract_root_stack(failed_key);
    CGAL_KDS_LOG(LOG_LOTS, "Next root of solver is " << ore.failure_time() << std::endl);
...
private:
  ...
  void make_certificate( const Edge &e) {
    ...
    //typename Simulator::Time t= s.next_time_negative();
    //if (!s.empty()) {
    CGAL_KDS_LOG(LOG_LOTS,"Failure time is " << s.failure_time() << std::endl);
    typename Simulator::Time t= s.failure_time();
    s.pop_failure_time();
    CGAL_KDS_LOG(LOG_LOTS, "Next root of this cert is " << s.failure_time() << std::endl);
