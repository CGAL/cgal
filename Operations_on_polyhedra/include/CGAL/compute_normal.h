 ...
 * @param f facet
 */
template <class Traits, class Facet>
typename Traits::Vector_3 compute_facet_normal(const Facet& f)
{
 ...
 */
// normalize = false
template <class Traits, class Vertex>
typename Traits::Vector_3 compute_vertex_normal(const Vertex& v)
{
  typedef typename Traits::Vector_3 Vector;
  ...
  CGAL_For_all(he,end)
  {
    if(!he->is_border())
    {
      Vector n = compute_facet_normal<Traits>(*he->facet());
