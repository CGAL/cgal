...
int main() {
  ...
  benchmark<Deformation_Eigen_closest_rotation_traits_3>();
  ...
  benchmark<Deformation_Eigen_polar_closest_rotation_traits_3>();

  std::cerr << "Benchmark for Deformation_fast_SVD_closest_rotation_traits_3 (fast SVD code with SSE): " << std::endl;
  benchmark<Deformation_fast_SVD_closest_rotation_traits_3>();
...
void benchmark()
{
  ...
  std::cerr << "Reading matrices from file is completed, (number of matrices read: " << matrices.size() << " )." << std::endl;
  ...
  std::cerr << "done: " << task_timer.time() << std::endl;
