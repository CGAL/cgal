...
DeleteCurveModeItemEditor( QWidget* parent ):
    QComboBox( parent )
{
    this->setFrame( false );

    QVariant deleteCurveOption = QVariant::fromValue( DeleteCurveMode( ) );
    QVariant deleteEdgeOption = QVariant::fromValue( DeleteCurveMode( DeleteCurveMode::DELETE_EDGE ) );
...
mode( ) const
{
    return qVariantValue< DeleteCurveMode >( this->itemData( this->currentIndex( ), Qt::UserRole ) );
}
