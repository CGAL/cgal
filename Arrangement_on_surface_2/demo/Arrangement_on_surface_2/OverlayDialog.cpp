...
OverlayDialog::
OverlayDialog( ArrangementDemoWindow* parent, Qt::WindowFlags f ):
    QDialog( parent, f ),
    ui( new Ui::OverlayDialog )
{
    ...
    {
        QListWidgetItem* item = new QListWidgetItem( this->ui->arrangementsListWidget );
        ...
        {
            icon.addFile(QString::fromUtf8(":/icons/red_icon.xpm"), QSize(), QIcon::Normal, QIcon::Off);
        }
        item->setIcon( icon );
    }
}
...
void 
OverlayDialog::
on_pickPushButton_pressed( )
{
    int currentIndex = this->ui->arrangementsListWidget->currentRow( );
    if ( currentIndex == -1 )
        return;

    if ( !(this->ui->arrangementsListWidget->item( currentIndex )->flags( ) & Qt::ItemIsEnabled) )
    {
        return;
    }
...
void 
OverlayDialog::
restrictSelection( QListWidgetItem* item )
{
