...
public:
   ...
   * arrangement. A halfedge is directed from its <I>source</I> vertex to its
   * <I>target</I> vertex, and has an <I>incident face</I> lying to its
   * left. Each halfedge has a <I>twin</I> halfedge directed in the opposite
   * direction, where the pair of twin halfedges form together an arrangement
   * edge, that is - a \f$ 1\f$-dimensional cell, associated with planar \f$
   * x\f$-monotone curve.
   *
  ...
  public:
    ...
    /*! obtains whether the halfedge is fictitious (i.e., connects two vertices
     * at infinity and is not associated with a valid curve).
     */
     ...
     * `e`'s source vertex is lexicographically smaller than it target (so the
     * halfedge is directed from left to right), and `ARR_RIGHT_TO_LEFT` if it
     * is lexicographically larger than the target (so the halfedge is directed
     * from right to left).
     */
    Arr_halfedge_direction direction() const;

    /*! obtains the face that `e` is incident to (The face lies to the left of
     * `e`).
     */
    Face_handle face();
  ...
  public:
    ...
    /*! obtains a Boolean indicating whether this is the fictitious face, which
     * contain the entire arrangement (and does not have an outer CCB).  An
     * arrangement that supports only bounded curves does not have a fictitious
     * face at all.
     */
    bool is_fictitious() const;
    ...
    /*! obtains a Boolean indicating whether the face has an outer CCB.  (The
     * fictitious face and the unbounded face of an arrangement that does not
    ...
    /*! obtains an iterator for traversing all the isolated vertices contained
     * in the interior of `f`.
     */
    Isolated_vertex_iterator isolated_vertices_begin();

    /*! obtains a past-the-end iterator for the isolated vertices contained
     * inside `f`.
     */
  ...
  /*! obtains a handle for an unbounded face of the arrangement.  In case the
   * arrangement comprises only bounded curves, there is a single unbounded face
   * and the function returns a handle to it. Otherwise, a handle to an
   * arbitrary unbounded face is returned.
   */
  ...
  /*! obtains a handle to the fictitious face of the arrangement.  If the
   * arrangement is not unbounded, there is no fictitious face. In this case the
   * result is not deterministic. A const version is also available.
   */
   ...
   * vertex that correspond to `c`'s right endpoint is created and connected to
   * `v` with a newly created halfedge pair. If `c` has an unbounded right end,
   * the new vertex lies at infinity and the unbounded face that contains the
   * interior of the curve is split.  The function returns a handle for one of
  ...
  /*! inserts the curve `c` into the arrangement, such that its right endpoint
   * corresponds to a given arrangement vertex. As a result, a new vertex that
   * correspond to `c`'s left endpoint is created and connected to `v` with a
   * newly created halfedge pair. If `c` has an unbounded left end, the new
   ...
   * vertex at infinity that corresponds to the unbounded end of `c`. If both
   * ends of `c` are unbounded, `fict_pred1` indicated the place for its left
  ...
  */
   ...
   * pairs), associated with the given subcurves `c1` and `c2`, and creates a
  ...
  */
   ...
   * whether they should be left as isolated vertices in the arrangement.  If
  ...
  */
   ...
   * structure of the arrangement and assures that it is valid. In addition, the
   * function performs several simple geometric tests to ensure the validity of
   * some of the geometric properties of the arrangement. Namely, it checks that
   * all arrangement vertices are associated with distinct points, and that the
   * halfedges around every vertex are ordered clockwise.
  */
  bool is_valid() const;
