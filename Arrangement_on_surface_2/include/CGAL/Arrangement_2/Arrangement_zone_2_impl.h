...
void Arrangement_zone_2<Arrangement, ZoneVisitor>::compute_zone()
{
  ...
  if ((vh = std::get<Vertex_const_handle>(&m_obj)) != nullptr) {
  ...
  else if ((hh = std::get<Halfedge_const_handle>(&m_obj)) != nullptr) {
    if (m_has_left_pt) {
    ...
    const Face_const_handle* fh = std::get<Face_const_handle>(&m_obj);
                           ...
                           bool& intersection_on_right_boundary)
{
  ...
  if (iter != m_inter_map.end()) {
    ...
    while (! inter_list.empty()) {
      ...
      ip = std::get<Intersection_point>(&(inter_list.front()));
      if (ip != nullptr) {
        ...
        icv = std::get<X_monotone_curve_2>(&(inter_list.front()));
                       ...
                       bool& leftmost_on_right_boundary)
{
  ...
  if (iobj) {
    ...
    const Intersection_point* int_p = std::get<Intersection_point>(&*iobj);
    if (int_p != nullptr) {
      ...
      // endpoint.
      const X_monotone_curve_2* icv = std::get<X_monotone_curve_2>(&*iobj);
      CGAL_assertion(icv != nullptr);
