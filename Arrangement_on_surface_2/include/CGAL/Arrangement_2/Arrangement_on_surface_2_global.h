            ...
            std::is_same<int, double>::type)
{
  ...
  for (const auto& x_obj : x_objects) {
    ...
    const auto* x_curve = std::get<X_monotone_curve_2>(&x_obj);
    if (x_curve != nullptr) {
    ...
    const auto* iso_p = std::get<Point_2>(&x_obj);
 ...
 const PointLocation& pl)
{
  ...
  if ((bx1 == ARR_INTERIOR) && (by1 == ARR_INTERIOR)) {
    ...
    CGAL_precondition_msg(std::get<Halfedge_const_handle>(&obj1) == nullptr,
  ...
  if ((bx2 == ARR_INTERIOR) && (by2 == ARR_INTERIOR)) {
    ...
    CGAL_precondition_msg(std::get<Halfedge_const_handle>(&obj2) == nullptr,
                          "The curve must not intersect an existing edge.");
  }
  else {
  ...
  if (vh1 != nullptr) {
    ...
    if (vh2 != nullptr) {
      ...
      // directed from left to right.
      const Face_const_handle* fh1 = std::get<Face_const_handle>(&obj1);
      const Face_const_handle* fh2 = std::get<Face_const_handle>(&obj2);
             ...
             const PointLocation& pl)
{
  ...
  const Face_const_handle* fh = std::get<Face_const_handle>(&obj);
  if (fh != nullptr) {
    ...
    const Halfedge_const_handle* hh = std::get<Halfedge_const_handle>(&obj);
    if (hh != nullptr) {
      ...
      const Vertex_const_handle* vh = std::get<Vertex_const_handle>(&obj);
...
is_valid(const Arrangement_on_surface_2<GeometryTraits_2, TopologyTraits>& arr)
{
  ...
  for (auto vf_iter = vf_list.begin(); vf_iter != vf_list.end(); ++vf_iter) {
    ...
    if (auto* he_below_p = std::get<Halfedge_const_handle>(&obj)) {
    ...
    else if (auto* v_below_p = std::get<Vertex_const_handle>(&obj)) {
      ...
      auto* in_face_p = std::get<Face_const_handle>(&obj);
             ...
             const PointLocation& pl, std::is_same<int, double>::type)
{
  ...
  for (const auto& x_obj : x_objects) {
    // Act according to the type of the current object.
    const X_monotone_curve_2* x_curve = std::get<X_monotone_curve_2>(&x_obj);
    if (x_curve != nullptr) {
    ...
    const Point_2* iso_p = std::get<Point_2>(&x_obj);
    CGAL_assertion(iso_p != nullptr);
    ...
    if (std::get<Face_const_handle>(&x_obj) != nullptr) return true;
  }
