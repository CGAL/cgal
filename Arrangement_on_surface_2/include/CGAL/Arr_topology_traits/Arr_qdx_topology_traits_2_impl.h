...
#if 0 // has been removed from the concept

//-----------------------------------------------------------------------------
// Compare the x-coordinate of a given vertex (which may lie at infinity)
//
template <class GeomTraits, class Dcel_>
Comparison_result
Arr_qdx_topology_traits_2<GeomTraits, Dcel_>::compare_x
    (const Point_2& p, const Vertex* v) const
{
    //std::cout << "Arr_qdx_topology_traits_2 compare_x"  << std::endl;

    // First check if the vertex v has a boundary condition in x
    // implying that it is smaller/greater than p
    const Boundary_type          bx = v->boundary_in_x();
    
    if (bx < CGAL::NO_BOUNDARY) {
        return (CGAL::LARGER);
    } else if (bx > CGAL::NO_BOUNDARY) {
        return (CGAL::SMALLER);
    }

#if 0
    // Check if the vertex has boundary in y
    const Boundary_type          by = v->boundary_in_y();
...
#endif  
    // else
    // v is a concrete point and we can use concrete compare:
    return (this->m_traits->compare_x_2_object() (p, v->point()));
}
...
template <class GeomTraits, class Dcel_>
Comparison_result
Arr_qdx_topology_traits_2<GeomTraits, Dcel_>::compare_xy
    (const Point_2& p, const Vertex* v) const
{
    //std::cout << "Arr_qdx_topology_traits_2 compare_xy(p,v)"  << std::endl;

    // First check if the vertex v has a boundary condition in x
    // implying that it is smaller/greater than p
    const Boundary_type bx = v->boundary_in_x();
    
    if (bx == CGAL::MINUS_INFINITY) {
        //std::cout << "larger" << std::endl;
        return CGAL::LARGER;
    }
    if (bx == CGAL::PLUS_INFINITY) {
    ...
    if (by != NO_BOUNDARY) {
        CGAL_assertion(!v->has_null_point());
        CGAL_assertion(by == CGAL::BEFORE_DISCONTINUITY ||
                       by == CGAL::AFTER_DISCONTINUITY);
        ...
        // In case of equality, consider whether v has boundary
    ...
    CGAL::Comparison_result res = 
        (this->m_traits->compare_xy_2_object() (p, v->point()));
    //std::cout << "res: " << res << std::endl;
    return res;
}

//-----------------------------------------------------------------------------
// Compare the two given vertices.
//
template <class GeomTraits, class Dcel_>
Comparison_result
Arr_qdx_topology_traits_2<GeomTraits, Dcel_>::compare_xy
...
{
    //std::cout << "Arr_qdx_topology_traits_2 compare_xy(v1, v2)"  
    //          << std::endl;
    // If the two vertices are equal, the result is obvious:
    if (v1 == v2) {
        return (CGAL::EQUAL);
    }
    ...
    if (! v1->has_null_point()) {
        if (! v2->has_null_point()) {
            // If the two vertices are associated with valid points, 
            // simply compare these points.
            res = this->m_traits->compare_xy_2_object() (v1->point(), 
                                                         v2->point());
        } else {
            // Compare the valid point associated with v1 to the vertex v2.
            res = compare_xy (v1->point(), v2);
        }
        
        return (res);
    } else if (! v2->has_null_point()) {
        // Compare the valid point associated with v2 to the vertex v1.
        res = compare_xy (v2->point(), v1);
        res = CGAL::opposite (res);
        
        return (res);
    }
    
    // In this case, both vertices are not associated with valid points.
    // First, we get the boundary conditions in x.
    const Boundary_type   bx1 = v1->boundary_in_x();
    const Boundary_type   bx2 = v2->boundary_in_x();
    
    if (bx1 != bx2) {
        // In this case, the comparison is straightforward:
        if (bx1 < CGAL::NO_BOUNDARY || bx2 > CGAL::NO_BOUNDARY) {
            return (CGAL::SMALLER);
        }
        
        CGAL_assertion (bx1 > CGAL::NO_BOUNDARY || bx2 < CGAL::NO_BOUNDARY);
        return (CGAL::LARGER);
    } else if (bx1 != CGAL::NO_BOUNDARY) { 
        // Both vertices have a negative boundary condition
        // so we have to compare their y-position.
        
        // Obtain the incident curves and compare them at the boundary in x
        Curve_end      ind1, ind2;
...
#if 0
    // else
    // Both vertices have boundary condition in y
    ...
    if (res != CGAL::EQUAL) {
        // if we now know the answer
        return (res);
    }
    
    // otherwise points have equal x and we compare y
    // but for y - we only can have boundary conditions
    const Boundary_type           by1 = v1->boundary_in_y();
    const Boundary_type           by2 = v2->boundary_in_y();
    
    if (by1 != by2) {
        return ((by1 < CGAL::NO_BOUNDARY) ? CGAL::SMALLER : CGAL::LARGER);
    }
#endif
    // We should never reach here, since otherwise v1 == v2
    CGAL_assertion (false);
    return (CGAL::EQUAL);
}
     ...
     Curve_end& ind) const
{
    ...
    // there are not fictious halfedges to skip!
    ...
    // minimal end.
    ind = (he->direction() == LEFT_TO_RIGHT) ? CGAL::MAX_END : CGAL::MIN_END;
...
//-----------------------------------------------------------------------------
// Get the clockwise order around a common boundary vertex on line of disc
//
template <class GeomTraits, class Dcel_>
CGAL::Comparison_result
Arr_qdx_topology_traits_2<GeomTraits, Dcel_>::
_cw_order_at_boundary_vertex(
        const X_monotone_curve_2& cv1,
        const CGAL::Curve_end& ind1,
        const X_monotone_curve_2& cv2,
        const CGAL::Curve_end& ind2
) const {
#if 0
    CGAL::Boundary_type by1 = 
        this->m_traits->boundary_in_y_2_object()(cv1, ind1);
    CGAL::Boundary_type by2 = 
        this->m_traits->boundary_in_y_2_object()(cv2, ind2);
    // preconds on by1 and by2 later in if-construct

    // Attention: we have to introduce a special order around the
    // vertex: all halfedges whose curve-end is AFTER_DISC is sorted
    // w.r.t. increasing compare_x, all curve-ends with BEFORE_DISC
    // are sorted with decreasing compare_x, 
    // and AFTER_DISC < BEFORE DISC
    // 
    // So the "smallest" one is the leftmost curve-end with AFTER_DISC
    // The "greatest" one is the leftmost curve-end with BEFORE_DISC

    if (by1 == CGAL::AFTER_DISCONTINUITY) {
        if (by2 == CGAL::AFTER_DISCONTINUITY) {
            return this->m_traits->compare_x_2_object()(cv1, ind1, cv2, ind2);
        } else {
            CGAL_assertion(by2 == CGAL::BEFORE_DISCONTINUITY);
            return CGAL::SMALLER;
        }
    } else {
        CGAL_assertion(by1 == CGAL::BEFORE_DISCONTINUITY);
        if (by2 == CGAL::AFTER_DISCONTINUITY) {
            return CGAL::LARGER;
        } else {
            CGAL_assertion(by2 == CGAL::BEFORE_DISCONTINUITY);
            CGAL::Comparison_result res = 
                this->m_traits->compare_x_2_object()(cv1, ind1, cv2, ind2);
...
//-----------------------------------------------------------------------------
// Get the precedessing halfedge
//
template <class GeomTraits, class Dcel_>
typename Arr_qdx_topology_traits_2<GeomTraits, Dcel_>::Halfedge*
Arr_qdx_topology_traits_2<GeomTraits, Dcel_>::
_find_predecessor_at_boundary_vertex(
        Vertex *v,
        const X_monotone_curve_2& cv,
        CGAL::Curve_end ind,
        bool& overlaps) const {
    ...
    // To locate the halfedge you need, we have to apply geometric 
    // comparisons; new curve is cv, the we look for a halfedge he 
    // such that 
    // compare_y_at_x_2 (cv, curr->curve(), MIN/MAX_END) 
    // is not the same as 
    // compare_y_at_x_2 (cv, next->curve(), MIN/MAX_END) ... 
    // compare_x, respectively.
    bool initial = true;
    CGAL::Comparison_result cmp_curr;
    CGAL::Comparison_result cmp_next;
    CGAL::Comparison_result cmp_he;
    CGAL::Curve_end indc;
    CGAL::Curve_end indn;

    // TASK implement this using a comparison functor

    bool check_curr_next = true;
    do {
        // circulating in cw-order over all incident halfedges

        // to implement: cw_around singular point

        if (v->boundary_in_x() != CGAL::NO_BOUNDARY) {
            
            CGAL_assertion(v->boundary_in_x() == CGAL::MINUS_INFINITY ||
                           v->boundary_in_x() == CGAL::PLUS_INFINITY);
            
            // compute new compare
            cmp_curr = this->m_traits->compare_y_at_x_2_object()(
                    cv, curr->curve(), ind
            );
            // exceptional test
            if (cmp_curr == CGAL::EQUAL) {
                overlaps = true;
                return curr;
            }
            if (initial) {
            ...
            // normal test
            if (cmp_curr != cmp_next) {
                return curr;
            }
            
            if (check_curr_next) {
                cmp_he = this->m_traits->compare_y_at_x_2_object()(
                        curr->curve(), 
                        next->curve(), 
                        ind
                );
                
                CGAL::Comparison_result exp     = CGAL::LARGER;
                CGAL::Comparison_result exp_rev = CGAL::SMALLER;
                if (v->boundary_in_x() == CGAL::MINUS_INFINITY) {
                    exp     = CGAL::SMALLER;
                    exp_rev = CGAL::LARGER;
                }
                if (cmp_he == exp) {
                    check_curr_next = false;
                    
                    // curr reached top-end (bottom-end)
                    if (cmp_curr == exp || cmp_next == exp_rev) {
                        // curr is the previous one, since new curve is higher
                        // than the highest or smaller than the smallest:
                        // (higher means with resp. to arc numbers, and not the
                        // ordering around the vertex. For this we have to 
                        // introduce exp and exp_rev
                        return curr;
                    }
                }
                // TASK can also be unified as in the following case:
            }
        ...
        // not needed anymore
            ...
            // now the same for vertices on line of disc
            
            CGAL_assertion(v->boundary_in_y() == CGAL::AFTER_DISCONTINUITY ||
                           v->boundary_in_y() == CGAL::BEFORE_DISCONTINUITY);
            
            // compute new compare
            indc = (curr->direction() == LEFT_TO_RIGHT ?
                    CGAL::MAX_END : CGAL::MIN_END);
            
            cmp_curr = _cw_order_at_boundary_vertex(cv, ind, 
                                                    curr->curve(), indc);
            // exceptional test
            if (cmp_curr == CGAL::EQUAL) {
                overlaps = true;
            ...
            if (initial) {
                indn = (next->direction() == LEFT_TO_RIGHT ?
                        CGAL::MAX_END : CGAL::MIN_END);
                
                cmp_next = 
                    _cw_order_at_boundary_vertex(cv, ind, next->curve(), indn);
                // exceptional test
                if (cmp_next == CGAL::EQUAL) {
                    overlaps = true;
                    return curr;
            ...
            if (check_curr_next) {
                cmp_he = _cw_order_at_boundary_vertex(curr->curve(), indc,
                                                      next->curve(), indn);
                
                if (cmp_he == CGAL::LARGER) {
                    check_curr_next = false;
                    // curr reached symbolic highest end
                    if (cmp_curr == CGAL::LARGER || 
                        cmp_next == CGAL::SMALLER) {
                        // curr is the previous one, since new curve is greater
                        // than the greatest or smaller than the smallest:
                        return curr;
                    }
                }
            }
