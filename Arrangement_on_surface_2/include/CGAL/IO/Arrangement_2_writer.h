...
protected:
  ...
  void _write_edge(Formatter& formatter, Edge_const_iterator hit)
  {
    ...
    formatter.write_vertex_index((he->direction() == LEFT_TO_RIGHT) ? 0 : 1);
  ...
  {
#if 0
    if (v == m_arr_access.bottom_left_fictitious_vertex()) return -1;
    else if (v == m_arr_access.top_left_fictitious_vertex()) return -2;
    else if (v == m_arr_access.bottom_right_fictitious_vertex()) return -3;
    else if (v == m_arr_access.top_right_fictitious_vertex()) return -4;
#endif
