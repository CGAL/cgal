...
\ccMethod{template<class OutputIterator> OutputIterator 
          operator() (Curve_2 cv, Point_2 end_min, Point_2 end_max,
                      OutputIterator out);}
  {Writes a sequence of \ccc{X_monotone_curve_2} objects (terminal
  segments) into \ccc{out}. These terminal segments compose an
  $x$-monotone (or vertical) segment of the curve \ccc{cv} that
  starts in \ccc{end_min}, and ends in \ccc{end_max}.
  \ccPrecond(\ccc{end_min} must have a unique $x$-monotone segment
             ...
             by an $x$-monotone segment of \ccc{cv})}
