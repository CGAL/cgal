...
\ccMethod{Arr_parameter_space operator()(const X_monotone_curve_2& xcv,
  ...
  \ccPrecond{If \ccc{Left_side_category} is not convertible to
    \ccc{Arr_open_side_tag} then \ccc{ce} $\neq$ \ccc{ARR_MIN_END}.}
  \ccPrecond{If \ccc{Right_side_category} is not convertible to
    \ccc{Arr_open_side_tag} then \ccc{ce} $\neq$ \ccc{ARR_MAX_END}.}
...
\ccMethod{Arr_parameter_space operator()(const X_monotone_curve_2& xcv,
  ...
  \ccc{ARR_INTERIOR}, accordingly.
  \ccPostcond{If \ccc{Bottom_side_category} is not convertible to
    \ccc{Arr_open_side_tag} then the result is not \ccc{ARR_BOTTOM_BOUNDARY}.}
  \ccPostcond{If \ccc{Top_side_category} is not convertible to
