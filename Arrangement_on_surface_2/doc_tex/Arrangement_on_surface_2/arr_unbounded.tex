...
\item
 The \ccc{Vertex} class provides three-valued predicates
  ...
  Both return \ccc{ARR_INTERIOR} if the point associated with the
  vertex has a finite $x$-coordinate or a finite $y$-coordinate,
  respectively. The former returns \ccc{ARR_LEFT_BOUNDARY} if the
  $x$-coordinate of the point approaches $-\infty$ (the vertex lies
  on the left boundary-side) and \ccc{ARR_RIGHT_BOUNDARY} if the
  $x$-coordinate approaches $\infty$ (the vertex lies on the right
  boundary-side). The latter returns \ccc{ARR_BOTTOM_BOUNDARY} if the
  $y$-coordinate of the point approaches $-\infty$ (the vertex lies
  on the bottom boundary-side) and \ccc{ARR_TOP_BOUNDARY} if the
  $y$-coordinate approaches $\infty$ (the vertex lies on the top
  boundary-side).
  The Boolean predicate \ccc{is_at_open_boundary()} is also supported.
  You can access the point associated with a vertex only if it is not
  a vertex at an open boundary (recall that a vertex at an open
  boundary is not associated with a \ccc{Point_2} object).
%
\item
