...
computes the separation required between a polygon and the outer frame used to obtain an exterior skeleton
...
If such a distance can be approximately computed, returns an `optional<FT>` with the value `d + (offset * 1.05)`.
If the distance cannot be computed, not even approximately, due to overflow for instance,
returns an empty `optional<FT>` (an <I>absent result</I>).
...
This will occur whenever the polygon has a vertex with an internal angle approaching `0`
(because the offset vertex of a vertex whose internal angle equals 0 is at <I>infinity</I>).
...
\tparam InputIterator must be a model of `InputIterator` with `value_type` equivalent to `Traits::Point_2`.
