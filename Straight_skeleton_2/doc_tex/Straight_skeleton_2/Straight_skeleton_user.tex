...
Contours parition the plane in two open regions: one bounded and one unbounded. If the bounded region of a contour is a \textit{singly-connected set}, the contour is said to be {\em strictly-simple}.\\
The \ccc{Orientation} of a contour is given by the order of the vertices around the region they bound. It can be \ccc{Clockwise} (CCW) or \ccc{Counter-clockwise} (CW).\\
...
\begin{ccTexOnly}
\begin{center}
\includegraphics{Straight_skeleton_2/valid_polygons} % omit suffix .eps to supprt PS and PDF
\end{center}
\end{ccTexOnly}
\begin{ccHtmlOnly}
...
\begin{ccTexOnly}
\begin{center}
\includegraphics{Straight_skeleton_2/invalid_polygons} % omit suffix .eps to supprt PS and PDF
\end{center}
\end{ccTexOnly}
\begin{ccHtmlOnly}
<TABLE CELLSPACING=40>
...
\begin{ccTexOnly}
\begin{center}
\includegraphics{Straight_skeleton_2/complex_offset} % omit suffix .eps to supprt PS and PDF
\end{center}
\end{ccTexOnly}
\begin{ccHtmlOnly}
<TABLE CELLSPACING=40>
...
\begin{ccTexOnly}
\begin{center}
\includegraphics{Straight_skeleton_2/complex_skeleton} % omit suffix .eps to supprt PS and PDF
\end{center}
\end{ccTexOnly}
...
\begin{ccTexOnly}
\begin{center}
\includegraphics{Straight_skeleton_2/simple_skeleton_0} % omit suffix .eps to supprt PS and PDF
\end{center}
\begin{center}
\includegraphics{Straight_skeleton_2/simple_skeleton_1} % omit suffix .eps to supprt PS and PDF
\end{center}
\begin{center}
\includegraphics{Straight_skeleton_2/validly_touching} % omit suffix .eps to supprt PS and PDF
\end{center}
\end{ccTexOnly}
...
by testing each inner contour (which is identified by being clockwise) against each outer contour (identified as being counter-clocwise) for insideness.
...
To construct an offset contour you need to construct an straight skeleton. Thus, to construct exterior offset contours for a polygon with holes, you need to construct, {\em separatedly}, the exterior skeleton of the outer contour and the interior skeleton of each hole.\\
...
Constructing exterior offset contours in the outside of the outer contour is just a little bit more involved: Since the contour is placed as a hole of a frame, you will alwyas obtain 2 offset contours for any given distance; one is the offseted frame and the other is the offseted contour. Thus, from the resulting offset contour sequence, you always need to discard the offsetted frame, easily identified as the 4-points contour with the biggest area.\\
It is neccesary to place the frame sufficiently away from the contour. If it is not, it could ocurr that the offset contour collides and merges with the offset frame, resulting in not 2 with just 1 (conceptually incorrect) offset contour.\\
Thus, If you wish to obtain an offset contour at some distance \ccc{t}, place the frame at least 100*t away from the bbox of the contour. A bit more than \ccc{2*t}, as you would have thought, won't be nearly enough. The reason is that an offset contour is at a distance \ccc{t} from the lines supporting the sides, not from the sides themselves (unlike rounded offsets as the ones you can obtain from a voronoi diagram instead of a straight skeleton). It's easy to see that for a reflex vertex, the corresponding offset reflex vertex can be at a distance much much greater than \ccc{t}.

\begin{figure}[htbp]
\begin{ccTexOnly}
\begin{center}
\includegraphics{Straight_skeleton_2/exterior_skeleton} % omit suffix .eps to supprt PS and PDF
\end{center}
\begin{center}
\includegraphics{Straight_skeleton_2/exterior_offset} % omit suffix .eps to supprt PS and PDF
\end{center}
\end{ccTexOnly}
