...
\end{description}
\begin{description}
\item[Returns:]\ccc{CGAL::LARGER} if x(p1) $>$ x(p2); \ccc{CGAL::SMALLER} if x(p1) $<$ x(p2); \ccc{CGAL::EQUAL} if x(p1) = x(p2). \end{description}
\ccGlue
...
\ccConstructor{Compare_x_2();}{default constructor.}
