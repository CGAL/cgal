...
\end{description}
\begin{description}
\item[Returns:]\ccc{CGAL::LARGER} if x($\ast$this) $>$ x(q), or if x($\ast$this) = x(q) and y($\ast$this) $>$ y(q); \ccc{CGAL::SMALLER} if x($\ast$this) $<$ x(q), or if x($\ast$this) = x(q) and y($\ast$this) $<$ y(q); \ccc{CGAL::EQUAL} if the two points are equal. \end{description}
\begin{description}
\item[Precondition:]Compared points are in the interior of parameter space \end{description}
\ccGlue
