...
//
//
// Author(s)     : Stephane Tayeb, Clement Jamin, Jane Tournois
//
//******************************************************************************
// File Description :
//******************************************************************************

#ifndef CGAL_MESH_COMPLEX_2_IN_TRIANGULATION_3_H
#define CGAL_MESH_COMPLEX_2_IN_TRIANGULATION_3_H
...
/*!
  ...
  \brief A data structure to represent and maintain a 2D complex embedded
  in a 3D triangulation.

  The class `Mesh_complex_2_in_triangulation_3` implements a data structure
  to store the surface restricted Delaunay triangulation used by a mesh
  generation process.

  This class is a model of the concept
  `MeshComplexWithFeatures_2InTriangulation_3`.
  ...
  \cgalModels{MeshComplexWithFeatures_2InTriangulation_3}
*/
template <typename Tr,
          typename CornerIndex,
          typename CurveIndex>
class Mesh_complex_2_in_triangulation_3
...
private:
  typedef Mesh_complex_2_in_triangulation_3<
...
/// @{
   ...
   */
  Mesh_complex_2_in_triangulation_3();
  /**
   * Copy constructor
   */
  Mesh_complex_2_in_triangulation_3(const Self& rhs);

  /**
   * Move constructor
   */
  Mesh_complex_2_in_triangulation_3(Self&& rhs);
...
private:
  ...
  public:
    ...
    Facet_iterator_not_in_complex(const Self& c2t3,
    ...
    {
      if (index_ == Surface_patch_index()) { return !c2t3_->is_in_complex(*it); }
      else { return !(c2t3_->surface_patch_index(*it) == index_); }
    }
  };
  ...
  public:
    Edge_iterator_not_in_complex(const Self& c2t3,
    ...
    bool operator()(Iterator it) const
    {
      if ( index_ == Curve_index() ) { return ! c2t3_.is_in_complex(*it); }
      else { return c2t3_.curve_index(*it) != index_;  }
    }
  };
  ...
  public:
    Vertex_iterator_not_in_complex(const Self& c2t3,
...
public:
  ...
  std::istream& operator>>(std::istream& is,
    Mesh_complex_2_in_triangulation_3<Tr2, CoI, CuI>& c3t3);
...
Mesh_complex_2_in_triangulation_3<Tr, CI_, CSI_>::
Mesh_complex_2_in_triangulation_3()
  : Base()
  , tr_()
...
operator<< (std::ostream& os,
            const Mesh_complex_2_in_triangulation_3<Tr,CI_,CSI_> &c3t3)
{
...
operator>> (std::istream& is,
            Mesh_complex_2_in_triangulation_3<Tr,CI_,CSI_> &c3t3)
{
...
#endif // CGAL_MESH_COMPLEX_2_IN_TRIANGULATION_3_H
