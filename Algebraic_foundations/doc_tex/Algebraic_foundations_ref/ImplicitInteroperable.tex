...
The types \ccc{A} and \ccc{B} are a model of the concept 
...
this type. This type is \ccc{Coercion_traits<A,B>::Coercion_type}. 

The type \ccc{Coercion_traits<A,B>::Coercion_type} is required to be 
implicit constructible from \ccc{A} and \ccc{B}. 

%From this it follows that all binary functors (and their global functions)
%provided by \ccc{Algebraic_structure_traits< Coercion_traits<A,B> :: Coercion_type> }  
%and \ccc{Real_embeddable_traits< Coercion_traits<A,B> :: Coercion_type> } also 
...
%Note that \ccc{Coercion_traits<A,B>::Coercion_type} may be equal to \ccc{A} or \ccc{B}.\\
