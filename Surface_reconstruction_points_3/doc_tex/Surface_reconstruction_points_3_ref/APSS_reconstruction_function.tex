...
\ccCommentHeading{Parameters}
\begin{description}
\item \ccc{first}: iterator over the first input point. \item \ccc{beyond}: past-the-end iterator. \item \ccc{point_pmap}: property map InputIterator -$>$ \ccc{Point_3} (access to the position of an input point). \item \ccc{normal_pmap}: property map InputIterator -$>$ \ccc{Vector_3} (access to the {\bf oriented} normal of an input point). \item \ccc{radius_pmap}: property map InputIterator -$>$ FT (access to the local point spacing of an input point). \item \ccc{smoothness}: smoothness factor. Typical choices are in the range 2 (clean datasets) and 8 (noisy datasets). \end{description}
}
\ccGlue
