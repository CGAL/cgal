...
#ifndef ALPHA_SHAPE_3_H
#define ALPHA_SHAPE_3_H
...
public:
  typedef typename Dt::GT Gt;   // A corriger avec Monique
  typedef typename Dt::Tds Tds; // A corriger avec Monique
...
private:
  ...
  typedef std::vector< Interval_cell > Interval_cell_map;

  typedef typename Tds::Cell Cell_tds;                // A corriger avec Monique
  typedef typename Cell_tds::Cell Cell_base;          // A corriger avec Monique
  typedef typename Cell_base::Interval_3 Interval3;   // A corriger avec Monique

  typedef std::pair< Interval3, Facet > Interval_facet;
  typedef std::vector< Interval_facet > Interval_facet_map;
...
#else
  typedef std::hash_set< Key > Marked_cell_set;
#endif
...
public:
  ...
  Alpha_shape_3( InputIterator first,  
		 InputIterator last,  
...
public:
  ...
  void initialize_interval_cell_map(void);

  void initialize_interval_facet_map(void);
...
private:
  ...
  Coord_type find_interval(Cell_handle s) const
    ...
    {
      return Cell_handle->get_alpha();
    }
  
  const Interval3& find_interval(const_facet f) const
    {
      return (e.first)->get_facet_ranges(e.second);    
    }

  const Interval3& find_interval(const Edge e) const
    {
      return (Edge.first)->get_edge_ranges(Edge.second, Edge.third);
    }
  
  const Interval2& find_interval(Vertex_handle v) const
    {
      return v->get_ranges();
    }
...
public: 
  ...
  Classification_type  classify(Cell_handle s) const
  ...
  Classification_type  classify(Cell_handle s, 
				const Coord_type& alpha) const
    // Classifies the cell `f' of the underlying Delaunay
    // tetrahedralization with respect to `A'.
    ...
    // problem the operator [] is non-const
  {
    ...
    return ((*(_cell_interval_map.find((Key)s))).second < alpha) ? 
                                                             INTERIOR :
                                                             EXTERIOR;
...
//---------------------------------------------------------------------
  ...
  Classification_type  classify(pair<Cell_handle,  int> f) const
  {  
  ...
  Classification_type  classify(pair<Cell_handle,  int> f,
				 const Coord_type& alpha) const
  {  
  ...
  { 
    ...
    Interval3 interval = find_interval(const_facet(s, i));
 ...
 //---------------------------------------------------------------------
  ...
  Classification_type  classify(Triple<Cell_handle, int, int> e) const
  {  
  ...
  Classification_type  classify(Triple<Cell_handle, int, int> e,
				 const Coord_type& alpha ) const
  {  
...
number_solid_components(const Coord_type& alpha) const
    ...
    //            O(#alpha_shape log n) otherwise
  {
    ...
    for( cell_it = simplices_begin(); 
	 cell_it != simplices_end(); 
	 ++cell_it)
    {
	...
	if (! is_infinite(*vertex_it))
	  {
	    ...
	    Coord_type alpha_min_v = _interval_cell_map.back().first;
...
private:
  ...
  bool is_attached(Cell_handle s, int i) const
  {
    int i0, i1, i2;
    reindex(i0, i1, i2, i);
    Bounded_side b = traits().side_of_bounded_sphere(
						      s->vertex(i0)->point(),
						      s->vertex(i1)->point(),
						      s->vertex(i2)->point(),
						      s->vertex(i)->point());
  ...
  //EXEMPLE D'ANDREAS
  Coord_type squared_radius(Cell_handle s) const
  {
    return traits().squared_radius_smallest_circumsphere_object()(
						      s->vertex(0)->point(),
						      s->vertex(1)->point(),
						      s->vertex(2)->point(),
						      s->vertex(3)->point());
  ...
  Coord_type squared_radius(Cell_handle s, int i) const
  {
    // test which one is faster TBC

    int i0, i1, i2;
    reindex(i0, i1, i2, i);
    return traits().squared_radius_smallest_circumsphere(
						     s->vertex(i0)->point(),
						     s->vertex(i1)->point(),
						     s->vertex(i2)->point()) ;
    // return traits().squared_radius_smallest_circumsphere(
    //						     s->vertex((i+1)%4)->point(),
    //						     s->vertex((i+2)%4)->point(),
    //						     s->vertex((i+3)%4)->point()) ;
  }

 Coord_type squared_radius(Cell_handle s, int i, int j) const
  {
    return traits().squared_radius_smallest_circumsphere(
						     s->vertex(i)->point(),
						     s->vertex(j)->point()) ;
...
template <class Dt>
void 
Alpha_shape_3<Dt>::initialize_interval_cell_map(void)
{
 
  _interval_cell_map.reserve(number_of_simplices());

  // size equals number of finite simplices
  // actually number of finite + infinite simplices TBC !!!
  
  Coord_type alpha_f;
  Cell_iterator cell_it;

  // at the moment all finite + infinite simplices
  ...
  for( cell_it = simplices_begin(); 
       ...
       ++cell_it)
    {
      ...
      if (!is_infinite(pCell)) // TBC
	{
	  alpha_f = squared_radius(pCell);

	  _interval_cell_map.push_back(Interval_cell(alpha_f, (pCell)));
  ...
  sort(_interval_cell_map.begin(), _interval_cell_map.end());
...
template <class Dt>
void 
Alpha_shape_3<Dt>::initialize_interval_facet_map(void)
{

  // _interval_facet_map.reserve(3*number_of_simplices()-6); TBC
  // derive from number of simplices and vertices using Euler's formula
  // e <= 3*f -6

  Facet_iterator face_it(faces_begin());  // TBC
  Facet f;

  // at the moment all finite + infinite faces
  for( ; 
       face_it != faces_end(); 
       ++face_it)
    {
      ...
      if (! is_infinite(f))              // TBC
	{
	  ...
	  if(!is_infinite(pCell) && !is_infinite(pNeighbor))
	    { 
	      ...
	      interval = (is_attached(pCell, i) || 
			  is_attached(pNeighbor, pNeighbor->index(pCell))) ?
		make_triple(UNDEFINED,
			    std::min(squared_radius_Cell, 
				     squared_radius_Neighbor),
			    std::max(squared_radius_Cell, 
				     squared_radius_Neighbor)):
	      ...
	      if(is_infinite(pCell))
		{
		  if (! is_infinite(pNeighbor))
			...
			make_triple(UNDEFINED,
				    _cell_interval_map[(Key)pNeighbor], 
				    INFINITY):
	  ...
	  _interval_facet_map.push_back(Interval_facet(interval, f));
  ...
  sort(_interval_facet_map.begin(), _interval_facet_map.end());
...
template <class Dt>
void 
Alpha_shape_3<Dt>::initialize_interval_edge_map(void)
{
  ...
  for( edge_it = edges_begin(); 
      ...
      if (! is_infinite(e))              // TBC
	{
	  ...
	  Interval3 interval = make_triple(alpha_min_e, 
						alpha_mid_e, 
						alpha_max_e);
	  _interval_edge_map.push_back(Interval_edge(interval, e));
  ...
  sort(_interval_edge_map.begin(), _interval_edge_map.end());
...
template <class Dt>
void 
Alpha_shape_3<Dt>::initialize_interval_vertex_map(void)
{
  ...
  for( vertex_it = vertices_begin(); 
      ...
      if (!is_infinite(v))         // TBC
	{
	  ...
	  _interval_vertex_map.push_back(Interval_vertex(interval, 
							 (*vertex_it)));

	  // cross references
	  vertex_it->handle()->set_ranges(interval);
	}
    }

  sort(_interval_vertex_map.begin(), _interval_vertex_map.end());
...
template <class Dt>
void 
Alpha_shape_3<Dt>::initialize_alpha_spectrum(void)
// merges the alpha thresholds of the simplices faces (and edges)
{
  ...
  Interval_facet_map::iterator
    face_it;
  face_it = upper_bound(_interval_facet_map.begin(),
			_interval_facet_map.end(),
			UNDEFINED,
			Less());
  ...
  while (face_it != _interval_facet_map.end() ||
    ...
    {
      if (face_it == _interval_facet_map.end() ||
	...
	{
	  assert (face_it != _interval_facet_map.end() &&
		  (*cell_it).first >= (*face_it).first.first);
...
{ 
  ...
  Interval_vertex_map::const_iterator vertex_alpha_it;

  const Alpha_shape_3<Dt>::Interval2* pInterval2;

  typedef long Key;
#ifndef STL_HASH_TABLES
  map< Key, int, less< Key > > V;
#else
  hash_map< Key, int > V;
	   ...
	   ++face_alpha_it)
	{
	  ...
	  if(pInterval->second < A.get_alpha() &&
	    ...
	    // visualize the boundary
	    {
	      ...
	      if (A.classify(s) == Alpha_shape_3::EXTERIOR)
		{ 
		  // take the reverse cell
		  Cell_handle pNeighbor = s->neighbor(i);
		  i = pNeighbor->index(s);
		  s = pNeighbor;
		}
	  
	      assert(A.classify(s) == Alpha_shape_3::INTERIOR);

	      assert(A.classify(s, i) ==
		     Alpha_shape_3<Dt>::REGULAR);

	      int i0, i1, i2;
	      A.reindex(i0, i1, i2, i);
	   ...
	   ++face_alpha_it)
	{
	  ...
	  if(pInterval->third >= A.get_alpha()
		  ...
		  pInterval->second < A.get_alpha())
		{
		  ...
		  assertion(A.classify(s) == Alpha_shape_3::INTERIOR);
					    ...
					    result) const;
{
  ...
  for (vertex_alpha_it = _interval_vertex_map.begin(); 
	...
	{
	  v = (*vertex_alpha_it).second;
	  assertion(classify(v) == Alpha_shape_3<Dt>::REGULAR);
	  *result++ = v;
	}
    }
  ...
  if (get_mode() == Alpha_shape_3<Dt>::GENERAL)
	...
	{
	  v = (*vertex_alpha_it).second;
	  assertion(classify(v) == Alpha_shape_3<Dt>::SINGULAR);
					  ...
					  result) const
{
  ...
  if (get_mode() == Alpha_shape_3<Dt>::REGULARIZED)
	   ...
	   ++face_alpha_it)
	{
	  ...
	  assertion(pInterval->second != INFINITY);
	  // since this happens only for convex hull of dimension 2
	  // thus singular
	  ...
	  if(pInterval->second < get_alpha() &&
	    ...
	    // visualize the boundary
	    {
	      assertion(classify((*face_alpha_it).second.first,
				 (*face_alpha_it).second.second) ==
			Alpha_shape_3<Dt>::REGULAR);
