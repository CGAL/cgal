...
void BigFloatRep::approx(const BigFloatRep& B, const extLong& r, const extLong& a) {
  if (B.err) {
...
void BigFloatRep::div(const BigInt& N, const BigInt& D, const extLong& r, const extLong& a) {
  if (sign(D)) {
    if (sign(N)) {
...
void BigFloatRep::mul(const BigFloatRep& x, const BigFloatRep& y) {
  ...
  /* Logically, there is nothing wrong with the following, but
...
void BigFloatRep::sqrt(const BigInt& x, const extLong& a) {
  if (sign(x) == 0) {
    ...
    BigFloatRep q(CORE_BIGINT_ZERO, 0, 0);
    BigFloatRep z(CORE_BIGINT_ZERO, 0, 0);
    extLong     aa;
  ...
  if (sign(x.m) >= 0) {          //  x.m >= 0
    ...
    if (x.isZeroIn()) {        //  x.m <= x.err
	 ...
	 ppp = defBFsqrtAbsPrec;
      ...
      BigFloatRep z(CORE_BIGINT_ZERO,0 ,0);
      extLong  aa = - flrLg(x.err) + (lg(x.m) + 1 - (bits(delta) >> 1)) + 3;
  ...
  if (sign(x.m) >= 0) {          //  x.m >= 0
    ...
    if (x.isZeroIn()) {         //  x.m <= x.err
      ...
      if (!x.err) {             //  x.m > x.err = 0 (ERROR FREE CASE)
        ...
        BigFloatRep z(CORE_BIGINT_ZERO,0 ,0);
        extLong  absp = - flrLg(x.err) + (lg(x.m) + 1 - (bits(delta) >> 1)) + 3;
...
BigFloatRep::toDecimal(unsigned int width, bool Scientific) const {
  ...
  long lm = lg(M);               // binary length of mantissa
  ...
  if (err != 0) {
    unsigned long valid = floorlg10(m) - (long)::floor(log10(float(err))); // the number of significant
                                                // digits
    // decM = dec(M, valid); // ??? usage of dec
    if ((decM = (char *)malloc(M.bit_length()/3 + 10)) == NULL) {
      core_error("Cannot allocate suffcient memory in bigint-to-string conversion", __FILE__, __LINE__, true);
    }
    bigint_to_string(M, decM);
    if (strlen(decM) > valid) {
      decM[valid] = '\0';
    }
  } else {
    if ((decM = (char *)malloc(M.bit_length()/3 + 10)) == NULL) {
  ...
  std::string decRep = decM; // copy to the work string
  ...
  if (Scientific ||
    ...
    if (L10 != 0) {
      ...
      if ((ne = sprintf(eBuf, "%ld", labs(L10))) >= 0) {
        ...
        perror("BigFloat.cc: Problem in outputing the exponent!");
      }
      decRep += eBuf;
    ...
    if (L10 >= 0) { // x >= 1 or x == 0 and L10 + 1 <= width
      ...
      decRep.insert(1, ".");
      // decRep.insert(0, "."); // Bug??? (11/25/02) Zilin
      // sqrt(2)-1 prints as .0414... instead of 0.414...
    }
    decOut.isScientific = false;
  }
#ifdef DEBUG
...
void BigFloatRep :: fromString(const char *str, const extLong & prec ) {
  ...
  if (prec.isInfty()) error("infinite precision not allowed");
...
double BigFloatRep :: toDouble() const
{
  ...
  assert (le >= 0);
#endif
  BigInt M = m >> le;   // remove error bits in mantissa
  ...
  if (M == 0) return ( 0.0/foolCompilerZero ) ; // return NaN
  e2 += le;             // adjust exponent corresponding to error bits

  int len = M.bit_length() - 52;  // this is positive if M is too large
  ...
  int ee = e2 + M.bit_length() - 1; // real exponent. 
...
//  class BigFloat
void BigFloat::read_from_file (std::istream& in, long maxLength) {
  char *buffer;
  long length;
  long exponent;
  BigInt mantissa;
  ...
  if (strcmp(buffer, "Float") != 0)
    lidia_error_handler("BigFloat::read_from_file()", "type name expected");
  delete[] buffer;
  ...
  if (strcmp(buffer, "(16384)") != 0)
    lidia_error_handler("BigFloat::read_from_file()", "base expected");
  delete[] buffer;
  ...
  // read mantissa
  BigInt::read_base_number(in, mantissa, length, maxLength);
...
void BigFloat::write_to_file(std::ostream& out, int base, int charsPerLine) {
  BigInt c = rep->m;
  c.abs();
  ...
  // write exponent
  out << (rep->exp) << std::endl;
  
  // write mantissa
  if ( (rep->m).is_negative() ) {
    out << '-';
  }
...
void BigFloat::read_from_file2(std::istream& in, long maxLength) {
  ...
  if (strcmp(buffer, "NFloat") != 0)
    lidia_error_handler("BigFloat::read_from_file2()", "type name expected");
  delete[] buffer;
  ...
  if (strcmp(buffer, "(16)") != 0)
    lidia_error_handler("BigFloat::read_from_file2()", "base expected");
  delete[] buffer;
