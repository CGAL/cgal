...
static const double log_5 = log(5)/log(2);
...
BigInt floor(const Expr& e, Expr &sub) {
  BigInt f = e.approx(CORE_INFTY, 2).getBigFloat().toBigInt();
  sub = e-f;
  // Adjustment
  if (sub<0)
    ++sub, --f;
...
NodeInfo::NodeInfo() : appValue(CORE_REAL_ZERO), appComputed(false),
     ...
     length(0), measure(0), high(0), low(1), 
...
extLong ExprRep::computeBound() {
  ...
  if (bfmsskBd.isNaN()) // since we might compute \infty - \infty for this bound
	  bfmsskBd = CORE_INFTY;
...
extLong ExprRep::degreeBound() {
  ...
  d_e() = (c > 31) ? CORE_posInfty : (1 << c);
  clearFlag();
  return d_e();
}

#ifdef _MSC_VER
#pragma warning(disable: 4541) // warning message "'dynamic_cast' used on polymorphic type 'class CORE::ConstRealRep' with /GR-; unpredictable behavior may result"
#endif
//  class ConstRealRep
//  constructor
ConstRealRep::ConstRealRep(const Real & r) : value(r) {
  if (!value.isExact()) {
    RealBigFloat *p = NULL;
    if ((p = dynamic_cast<RealBigFloat *>(value.get_rep())) != NULL) {
      // clone the BigFloat and set its error to zero.
      value = BigFloat((p->get_ker()).m(), 0, (p->get_ker()).exp());
    } else {
...
unsigned long NegRep::count() {
  if (d_e() == 1) return 0; // no radicals below.
  if (!visited()) {
    visited() = true;
    unsigned long c = child->count();
    d_e() = (c > 31) ? CORE_posInfty : (1 << c);
    return c;
  } else
    return 0;
...
unsigned long SqrtRep::count() {
  if (!visited()){
    visited() = true;
    unsigned long c = 1 + child->count();
    d_e() = (c > 31) ? CORE_posInfty : (1 << c);
    return c;
  } else return 0;
}

unsigned long BinOpRep::count() {
  if (d_e() == 1) return 0; // no radicals below
  if (!visited()){
    visited() = true;
    unsigned long c = first->count() + second->count();
    d_e() = (c > 31) ? CORE_posInfty : (1 << c);
    return c;
  } else return 0;
}
...
void computeExactFlags_temp(ConstRep* t, const Real &value) {
  ...
  if (rationalReduceFlag) {
    t->ratFlag() = 1;
    t->ratValue() = new BigRat(value.toBigRat());
  }
...
void AddSubRep<Operator>::computeExactFlags() {
  ...
  if (Op(sf, ss) != 0) {     // can't possibly cancel out
	...
	extLong  ua = lowBound + 1; 	// need one additional bit for children
	...
	for (extLong i = 1; i<ua ; i*=2) {
	  ...
	  if (i> EscapePrec) { 
	    ...
	    core_error("AddSubRep: Escape Precision triggered",
	           __FILE__, __LINE__, false);
#ifdef DEBUG
...
void SqrtRep::computeApproxValue(const extLong& relPrec,
  ...
  if (incrementalEvalFlag) {
    ...
    appValue() = val.sqrt(p, appValue().getBigFloat());
  } else
    appValue() = val.sqrt(p);
}
