...
\begin{ccRefFunctionObjectConcept}{AdaptableFunctor}
  ...
  \ccTwo{Arity}{}
  \ccNestedType{Arity}{defines the arity of the functor, i.e. the
    number of arguments it takes. The class has to be a specialization
    of \ccc{CGAL::Arity_tag<int>}, where the template parameter
    corresponds to the arity of the functor, e.g.
  ...
  \ccHeading{Notes} Alternatively, the type can be defined in a
  specialization of \ccc{CGAL::Arity_traits<>} for the functor. This
...
\begin{ccRefClass}{Arity_traits<F>}
  ...
  functor, i.e. the number of arguments it takes. It is used as a
  compile time tag only, that is objects of this type are never
  created anywhere. Specializations of \ccRefName\ can be defined,
  where existing functors cannot be changed easily to contain their
  \ccc{Arity} type.
...
\begin{ccRefFunction}{compose}
  ...
  \ccFunction{template < class F0, class F1 > Compose< F0, F1 >::Type
    compose(const F0& f0, const F1& f1);}{returns the functor
    \ccc{f0}(\ccc{f1}($\cdot$)) with the same arity as \ccc{f1}.
    \ccCommentHeading{Requirements} \ccc{f0} is unary function
    (arity~1).}
  
  \ccFunction{template < class F0, class F1, class F2 > Compose< F0,
    F1, F2 >::Type compose(const F0& f0, const F1& f1, const F2&
    f2);}{returns the functor
    \ccc{f0}(\ccc{f1}($\cdot$),$\,$\ccc{f2}($\cdot$)) with arity equal
    to $ar(\ccc{f1})+ar(\ccc{f2})$. \ccCommentHeading{Requirements}
    \ccc{f0} is binary function (arity~2).}
  
  \ccFunction{template < class F0, class F1, class F2, class F3 >
    Compose< F0, F1, F2, F3 >::Type compose(const F0& f0, const F1&
    f1, const F2& f2, const F3& f3);}{returns the functor
