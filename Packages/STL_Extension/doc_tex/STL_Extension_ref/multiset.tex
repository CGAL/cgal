...
\ccc{SMALLER}, \ccc{EQUAL}, or \ccc{LARGER}). Thus, it is possible to maintain
...
possible to use a key of type \ccc{Type} and to employ the default
\ccc{Compare} functor for the look-up.
...
\ccNestedType{Type}{the type of the stored elements.}

\ccNestedType{Compare}{the comparison-functor type.
    If the \ccc{==} and the \ccc{<} operators are defined on two \ccc{Type}
    elements, it is possible not supply a comparison-functor type when
    instantiating the template, so a default functor that relies on the above
    mentioned comparison operators will be used instead.}

\ccNestedType{Allocator}{the allocator type.
    \ccc{CGAL_ALLOCATOR} is used by default.}

In compliance with \stl, the types \ccc{value_type} and \ccc{key_type}
(both equivalent to \ccc{Type}), \ccc{key_compare} and \ccc{value_compare}
(both equivalent to \ccc{Compare}), and \ccc{size_type} are defined as well.
...
\ccc{find(x)}, \ccc{count(x)}, \ccc{lower_bound(x)} and \ccc{upper_bound(x)}) 
