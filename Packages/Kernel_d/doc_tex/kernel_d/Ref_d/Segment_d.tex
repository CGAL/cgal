...
\ccFunction{bool weak_equality(const Segment_d<R>& s, const Segment_d<R>& t) ;}{Test for equality as unoriented segments. 
}

\ccFunction{bool parallel(const Segment_d<R>& s1, const Segment_d<R>& s2) ;}{return true if one of the segments is trivial or
if the unoriented supporting lines are parallel.  
}

\ccFunction{bool common_endpoint(const Segment_d<R>& s1, const Segment_d<R>& s2,   Point_d<R>& common)  ;}{if \ccc{s1} and \ccc{s2} touch in a common end point, this point
