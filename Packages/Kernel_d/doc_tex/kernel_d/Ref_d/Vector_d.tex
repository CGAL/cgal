...
\ccConstructor{Vector_d<R>(int d, Null_vector)}{
introduces the zero vector \ccc{v} of type \ccc{Vector_d<R>} in 
$d$-dimensional space. For the creation flag \ccc{CGAL::NULL_VECTOR}
can be used.}
...
\ccConstructor{Vector_d<R>(int d, Base_vector, int i)}{
returns a variable \ccc{v} of type \ccc{Vector_d<R>} initialized  
...
\ccFunction{Vector_d<R> operator*(const RT& n, const Vector_d<R>& v)  ;}{returns the vector with Cartesian coordinates $n v_i$. 
}

\ccFunction{Vector_d<R> operator*(const FT& r, const Vector_d<R>& v) ;}{returns the vector with Cartesian coordinates 
$r v_i, 0 \leq i < d$. 
}
