#ifndef CGAL_AFF_TRANSFORMATIONCD_C
#define CGAL_AFF_TRANSFORMATIONCD_C
...
Aff_transformationCd(int d, const DirectionCd<FT,LA>& dir, 
   const FT& num, const FT& den, int e1, int e2) : Base( Rep(d) )
{
  ...
  FT cos;
  FT denom;
  FT dx = CGAL_NTS abs(dir.dx());
  FT dy = CGAL_NTS abs(dir.dy());
  FT sq_hypotenuse = dx*dx + dy*dy;
  FT common_part;
  FT diff_part;
  FT rhs;
  bool lower_ok;
  ...
  if (dy * dy * den * den < sq_hypotenuse * num * num) {
    ...
    for(;;) {
      ...
      common_part = (CGAL_NTS square(sin)*CGAL_NTS square(den) + 
                     ...
                     sq_hypotenuse;
      diff_part   = FT(2)*num*sin*den*denom*sq_hypotenuse;
...
CGAL_END_NAMESPACE
#endif // CGAL_AFF_TRANSFORMATIONCD_C
