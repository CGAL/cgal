#ifndef CGAL_FUNCTION_OBJECTSHD_HA
#define CGAL_FUNCTION_OBJECTSHD_HA
...
{ TUPLE_DIM_CHECK(first,last,Barycentric_coordinates_d);
  ...
  int d = p.dimension();
  std::vector< Point_d > V(n+1);
  std::copy(first,last,V.begin()); V[n]=p;
  CGAL_assertion(affine_dimension(v.begin(),v.end())==n);
...
{ TUPLE_DIM_CHECK(first,last,linear_base);
  ...
  LA::independent_columns(M,indcols); 

  for (int l=0; l < int(indcols.size()); l++) {
