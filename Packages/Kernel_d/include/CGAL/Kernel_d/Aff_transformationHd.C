   ...
   const RT& num, const RT& den, int e1, int e2) : Base( Rep(d) )
{
  ...
  CGAL_assertion_msg((0<=num && num < den),
  ...
  if (dy * dy * den * den < sq_hypotenuse * num * num) {
    ...
    for(;;) {
      ...
      if ( lower_ok && upper_ok ) {
         if ( CGAL_NTS square(p)%2 + CGAL_NTS square(q)%2 > 1) {
             sin = p*q;
             cos = (CGAL_NTS square(q) - CGAL_NTS square(p))/2;     
             // exact division
             denom = (CGAL_NTS square(p) + CGAL_NTS square(q))/2;   
             // exact division
         } else {
