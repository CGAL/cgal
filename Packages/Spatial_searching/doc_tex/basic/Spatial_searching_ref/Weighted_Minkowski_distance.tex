...
\ccc{SearchTraits}, for example \ccc{CGAL::Kd_tree_traits_2<Kernel>}.
...
\ccTypedef{Traits::Point Point;}{Point type.}
...
\ccMethod{FT max_distance_to_rectangle(Point q, Kd_tree_rectangle<Traits> r;);}
{Returns $d^{power}$, where $d$ denotes the distance between the query item \ccc{q} and
...
\ccMethod{FT transformed_distance(FT d);} {Returns for $0 < p <\infty$ $d^p$. Returns for $p=\infty$ $d$.}

\ccMethod{FT inverse_of_transformed_distance(FT d);} {Returns for $0 < p <\infty$ $d^{1/p}$}.
