...
\begin{ccRefClass}{Point_container<SpatialPoint>}  %% add template arg's if necessary
...
Expects for the parameter \ccc{SpatialPoint} an implementation of 
a $d$-dimensional point, for example \ccc{CGAL::Point_d<R>}.

\ccTypes

\ccTypedef{Kernel_traits<SpatialPoint>::Kernel::FT NT;}{Number type.}
\ccTypedef{std::list<SpatialPoint*> Point_list;}{(Sorted) list of pointers to points.}
...
{\bf Non-Member functions related to \ccc{Point_container<SpatialPoint>}}

\ccGlueBegin
\ccGlobalFunction{ template<class SpatialPoint>
std::ostream& operator<<(std::ostream& s, Point_container<SpatialPoint> c);}
