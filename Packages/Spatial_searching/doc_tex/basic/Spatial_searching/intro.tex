...
Often the number of the neighbours to be computed is not know beforehand, e.g. because the number
...
\ccIncludeExampleCode{Using_fair_splitting_rule.C}
...
\leavevmode
\vspace*{-6.5cm}
\scalebox{.3}{\includegraphics{Fig1.ps}}
\end{center}
\vspace*{6cm}
\end{ccTexOnly}
...
\item
\ccc{Container} denoting a container, that stores points and
provides method to support the implementation of splitting rules. 
\ccc{CGAL::Point_container<Point>} provides an implementation.


\ccc{Separator} denoting a $d-1$-dimensional
subspace separating a $d$-dimensional spaces in two parts,
as implemented by \ccc{CGAL::Plane_separator<NT>}.

\item 
\ccc{Splitter} denoting a splitting rule as implemented by the classes 
...
\ccc{CGAL::Fair< Point, Container, Separator>}, and
\ccc{CGAL::Sliding_Fair< Point, Container, Separator>}.

\item
\ccc{Traits} denoting a traits class for the construction of a tree.
\ccc{CGAL::Kd_tree_traits_point< Point, Splitter >} provides an implementation. 
...
\ccc{CGAL::Iso_rectangle_2<R>}, \ccc{CGAL::Iso_cuboid_3<R>}, and \ccc{CGAL::Iso_rectangle_d<R>}.

\item
\ccc{Distance} denoting a distance object. A distance object should define distances between a query object
...
by \ccc{CGAL::Euclidean_distance<Point>}, and
\ccc{CGAL::Weighted_Minkowski_distance<Point>} implementing a 
...
\ccc{CGAL::Euclidean_distance_sphere_point<Query_item,Point>}.

\end{itemize}
\ccc{Tree} denoting a search tree.  
\ccc{CGAL::Kd_tree<Traits>} provides an implementation of $k$-$d$ trees.
...
\item
The class \ccc{CGAL::General_standard_search<Traits, Distance, Query_Item, Tree>} 
...
\item
The class \ccc{CGAL::General_priority_search<Traits, Distance, Query_Item, Tree>} implementing the priority search 
...
\item
The class \ccc{CGAL::Orthogonal_standard_search<Traits, Distance, Tree>} implementing the standard search strategy for 
...
\item
The class \ccc{Orthogonal_priority_search<Traits, Distance, Query_Item, Tree>} implementing the priority search strategy for general 
distances
...
\item
The class \ccc{Kd_tree_rectangle} implementing $d$-dimensional iso-rectangles, 
methods to compute bounding boxes
of point sets and methods to split iso-rectangles.

\item
The class \ccc{Kd_tree_node} implementing $k$-$d$ tree nodes.
...
the class \ccc{CGAL::Fuzzy_iso_box_d<Point, Iso_Box_d>}  
and the class \ccc{CGAL::Fuzzy_sphere_d<Point>}. 
...
$k$-$d$ trees have been implemented by the class \ccc{CGAL::Kdtree_d} (see Section \ref{KDT_section}, that is parameterized with one of the
traits classes 
...
class \ccc{R} as illustrated by the following code fragment:
