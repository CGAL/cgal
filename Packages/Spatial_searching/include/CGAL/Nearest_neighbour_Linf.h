...
template <class NT, class Point> 
  NT Min_distance_linf_to_box(const Point& p,
    ...
    for (int i = 0; i < r.dimension(); ++i) {
      h=p[i];
      if (r.lower(i) - h > distance)  distance = r.lower(i)-h;
	  if (h - r.upper(i) > distance)  distance = h-r.upper(i);
	}
    return distance;
  }
  ...
  NT Max_distance_linf_to_box(const Point& p,
    ...
    for (int i = 0; i < r.dimension(); ++i) {
      ...
      if (h >= (r.lower(i)+r.upper(i))/2.0) 
		  if (h - r.lower(i) > distance)  distance = h-r.lower(i); 
	  else
		  if (r.upper(i) - h > distance)  distance = r.upper(i)-h;
	}
    return distance;
  }
			...
			N->separator()->cutting_value();
                ...
                for (Item_iterator it=N->begin(); it != N->end(); it++) {
                        ...
                        for (int i=0; i< (*it)->dimension(); i++)       {
                                if (fabs((*query_point)[i] - (*(*it))[i]) 
				    > distance_to_query_point)
                                distance_to_query_point = 
				fabs((*query_point)[i] - (*(*it))[i]);
                        }
