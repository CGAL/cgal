// file: examples/Planar_map/example15.C

/*! \file
 * Constructions of a house-like-shaped Planar_map using default- and copy-.
 * constructors.
...
typedef CGAL::Pm_trapezoid_ric_point_location<Planar_map>
  Trap_point_location;
...
#define NUM_CONSTRUCTORS        2
  
  Planar_map * pms[NUM_PLS][NUM_CONSTRUCTORS];
  
  // Trap:
  Trap_point_location trap_pl;
  Planar_map pm_trap(&trap_pl);
  pms[PL_TRAP][DEFAULT_CONSTRUCTOR] = &pm_trap;
    
    // Naive:
  Naive_point_location naive_pl;  
  Planar_map pm_naive(&naive_pl);
  pms[PL_NAIVE][DEFAULT_CONSTRUCTOR] = &pm_naive;

  // Walk:
  Walk_point_location walk_pl;
  Planar_map pm_walk(&walk_pl);
  pms[PL_WALK][DEFAULT_CONSTRUCTOR] = &pm_walk;

  for (int i = 0; i < NUM_PLS; i++) {
    pms[i][DEFAULT_CONSTRUCTOR]->insert(&cv[0], &cv[6]);
    pms[i][COPY_CONSTRUCTOR] = new Planar_map(*(pms[i][DEFAULT_CONSTRUCTOR]));

    for (unsigned j = 0; j < NUM_CONSTRUCTORS; j++) {
      std::cout << ((pms[i][j]->is_valid()) ? "map valid!" : "map invalid!")
                << std::endl;
      print_point_locate(Point_2(1, 4, 2), pms[i][j]);
      print_point_locate(Point_2(1, 2, 2), pms[i][j]);
      print_point_locate(Point_2(1, 5, 4), pms[i][j]);
      print_point_locate(Point_2(1, 0, 2), pms[i][j]);
      print_point_locate(Point_2(2, 0), pms[i][j]);
      std::cout << std::endl;
    }
  }
