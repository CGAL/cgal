...
int main(int argc, char *argv[])
{
  ...
  while (n--) {
    inputt x1,y1,x2,y2;
    std::cin >> x1 >> y1 >> x2 >> y2;
    Halfedge_handle e = Pm.insert(Segment(Point_2(x1,y1),Point_2(x2,y2)));
  ...
  while (n--) {
    inputt x1, y1, x2, y2;
    std::cin >> x1 >> y1 >> x2 >> y2;
    Halfedge_handle e = Pm.insert(Ray(Point_2(x1,y1),Point_2(x2,y2)));
  ...
  while (n--) {
    ...
    std::cin >> a >> b;
    Locate_type lt;
    Halfedge_handle e = Pm.locate(Point_2(a,b),lt);
    std::cout << "\nLocate("<< a << "," << b << ")=";
