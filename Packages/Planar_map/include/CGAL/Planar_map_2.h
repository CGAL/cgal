...
public:
  ...
  Planar_map_2(const Traits &tr_, Point_location_base *pl_ptr, Bounding_box_base *bb_ptr)
  {
  ...
  // set NULLs for defaults
  Planar_map_2(Traits_wrap *tr_ptr, Point_location_base *pl_ptr, Bounding_box_base *bb_ptr)
  {
    if (tr_ptr == NULL)
      {
        traits = new Traits_wrap();
...
private:
	...
	bool prev1_inside_hole(Halfedge_const_handle previous1,Halfedge_const_handle previous2,
		const X_curve& cv) 
	{
		/*
                             ...
                             const X_curve& c1, const X_curve& c2)
  {
		...
		CGAL_precondition(
			traits->point_is_same(
				traits->curve_source(c1),e->source()->point()) &&
			traits->point_is_same(	
				traits->curve_target(c2),e->target()->point()) ||
			traits->point_is_same(
				traits->curve_source(c1),e->target()->point()) &&
			traits->point_is_same(
				traits->curve_target(c2),e->source()->point()) );
	...
	Halfedge_handle vertical_ray_shoot(
	...
	Halfedge_const_handle vertical_ray_shoot(
