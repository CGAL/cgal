		...
		bool & cv_equal_cv2) const
	{
			...
			if (dir1 == DIR_UP)
			{
				if (dir2 == DIR_LEFT) {
					if (dir == DIR_RIGHT || dir == DIR_DOWN)
						return (true);
					if (dir == DIR_LEFT) {
						res2 = curves_compare_y_at_x_left (cv2, cv, p);
						if (res2 == EQUAL)
							cv_equal_cv2 = true;
						return (res2 == LARGER);
					}
					...
					if (dir == DIR_RIGHT) {
						res2 = curves_compare_y_at_x_right (cv2, cv, p);
						if (res2 == EQUAL)
							cv_equal_cv2 = true;
						return (res2 == SMALLER);
					}
		...
		if (dir2 == DIR_UP || dir2 == DIR_DOWN)
		{
			...
			if (dir2 == DIR_UP)
			{
				if (dir1 == DIR_LEFT) {
					if (dir == DIR_LEFT) {
						res1 = curves_compare_y_at_x_left (cv1, cv, p);\
							if (res1 == EQUAL)
								cv_equal_cv1 = true;
						return (res1 == SMALLER);
					}
					...
					if (dir == DIR_LEFT || dir == DIR_DOWN) 
						return (true);
					if (dir == DIR_RIGHT) {
						res1 = curves_compare_y_at_x_right (cv1, cv, p);
						if (res1 == EQUAL)
							cv_equal_cv1 = true;
						return (res1 == LARGER);
					}
...
/*! curves_compare_y_at_x_cw() returns as follows:	
	...
	* if one curve is vertical up - it is the largest
	* if one curve is vertical down - it is the smallest 
	* if both curves are vertical up or both vertical down - return equal
	* you can't have one curve defined to the left and the other defined to the right !
	* \pre The point q is in the x range of the two curves, and both
	* of them must be also be defined to the same size (i.e., both left, both right, or one is vertical). 
    * The two curves must also intersect at x(q).
	*/
	Comparison_result curves_compare_y_at_x_cw(const X_monotone_curve_2 & cv1,
		const X_monotone_curve_2 & cv2, 
		const Point_2 & p, 
		bool intersecting = true) const 
	{
		Curve_dir_at_point     dir1 = _curve_direction_at_point (cv1, p);
		Curve_dir_at_point     dir2 = _curve_direction_at_point (cv2, p);
		...
		if (dir1 == DIR_UP || dir1 == DIR_DOWN) 
		{
			if (dir1 == dir2)  //both vertical to the same side
				return EQUAL; 	

			CGAL_precondition( ! ( dir1 == DIR_UP && dir2 == DIR_DOWN  )); 
			CGAL_precondition( ! ( dir2 == DIR_UP && dir1 == DIR_DOWN  )); 
		...
		//none of the curves is vertical
		CGAL_precondition( ! ( dir1 == DIR_RIGHT && dir2 == DIR_LEFT  )); 
		CGAL_precondition( ! ( dir2 == DIR_RIGHT && dir1 == DIR_LEFT  )); 
		
		if (dir1 == DIR_LEFT) 
		{
			if (! intersecting) //halfedges - non intersecting (except end points)
			{ 
				//get the leftmost points from both curves
				Point_2 left1 = curve_leftmost(cv1);
				Point_2 left2 = curve_leftmost(cv2);
				//get the rightmost of them, and compare it to the other curve
				Comparison_result res = compare_x(left1,left2);
				if (res ==LARGER) 
				{
					return curve_compare_y_at_x(left1, cv2);
				}
				else if (res == SMALLER) 
				{
					Comparison_result res = curve_compare_y_at_x(left2, cv1);
					//switch the result because we compared 2 with 1 (and not 1 with 2)
					if (res == SMALLER) 
						return LARGER;
					if (res == LARGER)
						return SMALLER;
			...
			//may be intersecting or second point equal  - no choice but to compare left
			return curves_compare_y_at_x_left(cv1,cv2,p);
		}	
		else //both curves are to the RIGHT
		{
			if (! intersecting) 
			{ 
				//get the rightmost point from both curves
				Point_2 right1 = curve_rightmost(cv1);
				Point_2 right2 = curve_rightmost(cv2);
				//get the leftmost of them, and compare it to the other curve
				Comparison_result res = compare_x(right1, right2);
				if (res == LARGER) 
				{
					//no need to switch results because we compared 2 with 1
					return curve_compare_y_at_x(right2, cv1);
				}
				else if (res == SMALLER)
				{
					Comparison_result res = curve_compare_y_at_x(right1, cv2);
					//switch the result because we're on the right side
					if (res == SMALLER) 
						return LARGER;
					if (res == LARGER)
						return SMALLER;
			...
			//if intersecting, or if second endpoint is equal
			//switch order of cv2 and cv1 to get opposite result. 
			//(instead of converting the result);
			//this is beacuse we're on the right side - the larger right is smaller cw
			return curves_compare_y_at_x_right(cv2,cv1,p);			
		}
	}
