...
%
  ...
  See concepts \ccc{Planar_map_DCEL}\lcTex{ (\ccRefPage{Planar_map_DCEL})},
  \ccc{Planar_map_DCEL::Vertex} \lcTex{(\ccRefPage{Pm_Dcel_Vertex})}
  and \ccc{Planar_map_DCEL::Halfedge} \lcTex{(\ccRefPage{Pm_Dcel_Halfedge})}.
%\ccc{Dcel::Vertex}\\
%\ccc{Dcel::Halfedge}\\
...
\ccc{Point_location_base<Planar_map>} whose description is given on
\ccRefPage{Pm_point_location_base<Planar_map>}.
...
\ccMethod{bool read (istream &in);}{reading to \ccRefName\ from a given input stream.} 

\ccStyle{template <class Scanner>}
\ccMethod{bool read (istream &in, Scanner& scanner);}{reading to \ccRefName\ from a given input stream when taking the scanner class as a parameter.}
...
%structure). 
    ...
    \ccMethod{Halfedge_handle insert_in_face_interior( const X_curve &
       cv, Face_handle f);}{inserts \ccStyle{cv} as a new inner component of
       \ccStyle{f}.  \ccPrecond{ \ccStyle{cv} is contained completely
          in the interior of \ccStyle{f} (no vertex of \ccStyle{cv}
          meets any vertex on the map).} 
       \ccPrecond{\ccc{cv} is not equivalent to a point. }}
\end{ccAdvanced}              
