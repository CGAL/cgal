...
\ccMethod{bool read (istream &in);}
         ...
         the \ccStyle{Point} and \ccStyle{X_curve} types of
...
\ccModifiers
  ...
  \ccMethod{Halfedge_handle insert( const X_curve & cv );} 
  ...
  \ccMethod{Halfedge_handle insert(const X_curve_2_iterator & begin, 
...
%structure). 
  ...
  \ccMethod{Halfedge_handle insert_in_face_interior( const X_curve & cv, 
                                                     Face_handle f);}
  ...
  {inserts the curve \ccc{cv} into the map. One endpoint of \ccc{cv} is the 
   point of the given vertex \ccc{v}, that is already in the map. 
