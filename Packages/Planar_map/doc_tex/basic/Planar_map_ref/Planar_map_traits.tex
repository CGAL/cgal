...
%       \item[]\ccStyle{CURVE_VERTICAL} - $cv$ is a vertical segment,
       ...
       \ccMethod{bool curve_is_same(const X_curve_2 &cv1, const X_cur
...
\ccMethod{Point_2 point_to_left(const Point_2& p);}{returns a point to 
   the left of \ccc{p}.}
\ccMethod{Point_2 point_to_right(const Point_2& p);}
         {returns a point to the right of \ccc{p}.} %}
...
\ccHasModels
  \ccc{Pm_segment_traits_2<R>}\\
  \ccc{Pm_leda_segment_traits_2}
