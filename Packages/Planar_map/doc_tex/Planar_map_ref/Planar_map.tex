...
\ccMethod{Halfedge_handle locate(const Point_2& p , 
   Locate_type& lt );}{
...
\ccModifiers
  ...
  \ccMethod{template <X_curve_2_iterator>
            Halfedge_handle insert(const X_curve_2_iterator & begin, 
...
%structure). 
  ...
  \ccMethod{Halfedge_handle insert_from_vertex(const X_curve_2 & cv,
                                               Vertex_handle v);}
  {inserts the curve \ccc{cv} into the map. The given vertex \ccc{v},
  that is already in the map, holds one endpoint of \ccc{cv}. 
