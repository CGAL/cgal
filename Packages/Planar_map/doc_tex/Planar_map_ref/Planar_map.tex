...
\ccDefinition
An object $pm$ of the class \ccClassTemplateName\ is the subdivision
of the planar induced by a set of $x$-monotone curves, such that no
curve intersects the interior of any other curve. The available traits
and dcel classes are described below. \ccc{Self} is an abbreviation
of the \ccRefName\ type hereafter.
...
%  \ccTypedef{typedef Planar_map_2<Dcel,Traits> Self;}{planar map class}
  ...
  \ccGlue
  \ccNestedType{Change_notification}{}
...
%  \ccConstructor{Planar_map<I>( list<X_monotone_curve_2> & l
...
%                   is \ccStyle{X_monotone_curve_2} objects}
%                \ccPrecond{The curves are $x$-monotone} 
...
{reads \ccRefName\ from the given input stream \ccc{in}. The input
 ...
 the \ccStyle{Point_2} and \ccStyle{X_monotone_curve_2} types of
...
{reads \ccRefName\ from the given input stream \ccc{in} using 
\ccc{scanner}. The input stream should support the
extractor operator (\ccc{>>}) for the \ccStyle{Point_2} and
\ccStyle{X_monotone_curve_2} types of \ccc{Planar_map} }
...
%structure). 
  ...
  \ccMethod{Halfedge_handle insert_in_face_interior(const X_monotone_curve_2 & cv, 
                                                    Face_handle f,
						    Change_notification * en = NULL);}
  ...
  \ccMethod{Halfedge_handle insert_at_vertices(const X_monotone_curve_2 & cv,
                                               Vertex_handle v1, 
                                               Vertex_handle v2,
					       Change_notification * en = NULL);}
  ...
  {inserts the curve \ccc{cv} into the map. One endpoint of \ccc{cv} is the 
   ...
   \ccPrecond{no vertex of the map has a point that coincides with the other 
    endpoint of \ccc{cv}. }
   \ccPrecond{In the clockwise order of curves around the common point
   of \ccc{v}, \ccc{cv} succeeds the curve of \ccc{h}.} 
   \ccPrecond{no curve of \ccc{pm} intersects \ccc{cv} in the
    interiors of the curve itself and \ccc{cv}. }
   \ccPrecond{\ccc{cv} is not equivalent to a point. }
  }
  ...
  {inserts the curve \ccc{cv} into the map. The two endpoints of \ccc{cv} are
   ...
   \ccPrecond{the points of \ccc{v1} and \ccc{v2} coincide with the two 
    endpoints of \ccc{cv} respectively. } 
   \ccPrecond{In the clockwise order of curves around the common point
   of \ccc{v1}, \ccc{cv} succeeds the curve of \ccc{h1}.} 
   \ccPrecond{In the clockwise order of curves around the common point
   of \ccc{v2}, \ccc{cv} succeeds the curve of \ccc{h2}.} 
   \ccPrecond{no curve of \ccc{pm} intersects \ccc{cv} in the
    interiors of the curve itself and \ccc{cv}. }
   \ccPrecond{\ccc{cv} is not equivalent to a point. }
  }
...
{splits the edge \ccStyle{e} into \ccStyle{e1} and  \ccStyle{e2} , and
   ...
   is identical to the source of the curve \ccStyle{c1} then
   \ccStyle{c1} abd \ccStyle{c2} will be assigned to \ccStyle{e1} and
   to \ccStyle{e2} respectively and to their respective twin halfedges. 
...
{merges the edges represented by \ccStyle{e1} and \ccStyle{e2} into
   ...
   \ccStyle{cv}. The return value is the halfedge with the same
