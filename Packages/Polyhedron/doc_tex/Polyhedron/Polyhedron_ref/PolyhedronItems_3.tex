...
\ccNestedType{Point}{point type stored in vertices. Note, \ccc{HalfedgeDS}
    is dimensionless, so this type is named \ccc{Point} and not \ccc{Point_3}.}
...
an arbitrary type, such as \ccc{void*} or \ccc{Tag_false}.

%\ccNestedType{Normal}{surface normal vector stored in faces.}
%\ccGlue

\ccNestedType{Plane}{plane equation stored in faces.  Note, \ccc{HalfedgeDS}
    is dimensionless, so this type is named \ccc{Plane} and not \ccc{Plane_3}.}
...
\ccRefIdfierPage{CGAL::HalfedgeDS_halfedge_base}\\
\ccRefIdfierPage{CGAL::HalfedgeDS_face_base}
