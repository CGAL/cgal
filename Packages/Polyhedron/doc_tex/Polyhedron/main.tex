...
output simplifies to a single statement by using \ccc{std::copy} and
...
defined function \texttt{compute\_plane\_equations}.  Depending on the
arithmetic (exact/inexact) and the shape of the facets
...
homogeneous representation with \texttt{int} coordinates is sufficient.
...
restrictions onto these formats. They must represent a permissible 
