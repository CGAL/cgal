...
\texttt{for} loop simplifies to a single statement by using \ccc{std::copy} and
the ostream iterator adaptor.
...
tetrahedron.  The actual plane computation is provided in the
user-defined function \texttt{compute\_plane\_equations}.  Depending
on the arithmetic (exact/inexact) and the shape of the facets
(convex/non-convex) different methods are useful. We assume here
strictly convex facets and exact arithmetic. In our example a
homogeneous representation with \texttt{int} coordinates is
sufficient. The four plane equations are the output of the program.
...
an ASCII and a binary format exists. The format can be selected with
...
a subdivision surface~\ref{k-s-00}. Each step subdivides a facet
...
The following figure shows three example objects, each 
...
actually a hole whose vertex coordinates let the hole collapse to an edge.
The example directory \texttt{examples/Polyhedron/} contains the 
OFF files used here.
