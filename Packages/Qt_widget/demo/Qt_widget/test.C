...
class My_Window : public QCGALWidget {
public:
...
private:
  ...
  {
    this->lock();
    *this << BackgroundColor(CGAL::ORANGE) << CGAL::RED <<
      LineWidth(3) << PointSize(3) << PointStyle(CGAL::DISC);
    *this << Segment(Point(10,20),Point(300,400));
    *this << LineWidth(5) << CGAL::GREEN << FillColor(CGAL::BLACK) <<
      Circle(Point(400,400),50*50);
    *this << LineWidth(1) << noFill << Circle(Point(300,300),300*300);
    *this << CGAL::BLUE << LineWidth(2);
    *this << Segment(Point(200,200),Point(400,400));
    *this << Segment(Point(200,400),Point(400,200));
    this->setFilled(true);
    *this << CGAL::RED << Triangle(Point(150,300),
				   Point(150,350),
				   Point(100,325));
    *this << FillColor(CGAL::RED) << Rectangle(Point(320,220),
					       Point(350,240));
    *this << CGAL::DEEPBLUE << BBox(100,80,260,140);
    Polygon p;
    ...
    *this << PointSize(10)
	  << PointStyle(CGAL::PIXEL) << Point(120,100)
	  << PointStyle(CGAL::CROSS) << Point(140,100)
	  << PointStyle(CGAL::PLUS) << Point(160,100)
	  << PointStyle(CGAL::CIRCLE) << Point(180,100)
	  << PointStyle(CGAL::DISC) << Point(200,100)
	  << PointStyle(CGAL::RECT) << Point(220,100)
	  << PointStyle(CGAL::BOX) << Point(240,100);
    *this << PointSize(5)
	  << PointStyle(CGAL::PIXEL) << Point(120,120)
	  << PointStyle(CGAL::CROSS) << Point(140,120)
	  << PointStyle(CGAL::PLUS) << Point(160,120)
	  << PointStyle(CGAL::CIRCLE) << Point(180,120)
	  << PointStyle(CGAL::DISC) << Point(200,120)
	  << PointStyle(CGAL::RECT) << Point(220,120)
	  << PointStyle(CGAL::BOX) << Point(240,120);
    *this << LineWidth(1) << CGAL::RED <<
