...
This line is meant to call the redraw() from the \ccStyle{Qt\_widget}
...
\end{ccExampleCode}
This time you see that W is no longer an instance of \ccStyle{Qt\_widget} but is
an instance of \ccStyle{QMainWindow}. \ccStyle{QMainWindow} is also a widget but provides
...
The class My\_Widget is like in the previous example an intance of
\ccStyle{Qt\_widget}, that you need in the class My\_Window.

The only difference between this example and the previous one is the
use of standard toolbar. It is declared as private in My\_Window class:\\
CGAL::Standard\_toolbar *stoolbar;\\
To use it, in the constructor of My\_Window, it is added:\\
stoolbar = new CGAL::Standard\_toolbar(widget, this);\\
...
select the first point, also a new CGAL::Point\_2 is inserted in the
...
class My\_Tool derived from \ccStyle{Qt\_widget\_tool}, that is used to create a
...
In My\_Window class an instance of My\_Tool is created :\\
My\_Tool	t;\\
...
tool calls the new\_object() member function from \ccStyle{Qt\_widget}, that emits
...
This connect tells the application that every time \ccStyle{Qt\_widget} emits the
...
\ccStyle{Qt\_widget}, the last tool attached, is detached. Two tools will
...
by a kernel of CGAL. In this tutorial the generic tool \ccStyle{CGAL::Qt\_widget\_get\_point} it is
