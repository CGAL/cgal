...
The line \ccc{//moc_source_file : tutorial2.C} is for users that want
to use the script create_makefile with Qt (with option -q). It tells
to the \cgal\ makefile generator that \ccc{moc} has to be run on
\ccc{tutorial2.C} to generate \ccc{tutorial2.moc} that is included at
the next line. If you are not using makefiles you should not be
