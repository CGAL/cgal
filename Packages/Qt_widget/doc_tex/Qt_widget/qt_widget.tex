...
The class \ccStyle{Qt_widget} is derived from the \cgalqt\ class \ccStyle{QWidget}%
...
method \ccc{redraw} is called. This method implemented in
\ccc{Qt_widget} emits a signal, which is connected to the slot 
\ccc{My_window::redraw_win()} in this example. In this
...
\begin{ccAdvanced}
There are a couple of ways to draw something with \ccc{Qt_widget}. One way
...
other drawings. The other option is to use the \ccc{QPainter} instance 
