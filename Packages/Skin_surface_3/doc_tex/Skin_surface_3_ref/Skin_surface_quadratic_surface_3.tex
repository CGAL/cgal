...
\ccConstructor{Skin_surface_quadratic_surface_3();}{}

\ccOperations
\ccThree{virtual Vector}{Triangulated}{}
\ccThreeToTwo

\ccMethod{Point to_surface(Point const &p0, Point const &p1);}{Returns
  a point close to the intersection point of the line segment \ccc{p0
    p1} with the surface. } \ccGlue \ccMethod{Point to_surface(Segment
  const &s);}{Equal to \ccc{to_surface(s.source(), s.target())}.}
...
\ccMethod{virtual RT value(Point const &p) const = 0;}{Returns the
  function value of the quadric.}

\ccGlue \ccMethod{virtual Vector gradient(Point const &p) = 0;
