...
\ccConstructor{Sphere_circle(Sphere_point p, Sphere_point
  q)}{ If $p$ and $q$ are
...
\ccConstructor{Sphere_circle(CGAL::Plane_3 h)}{ creates the
  circle corresponding to the plane \ccc{h}.  \ccPrecond \ccc{h}
  contains the origin.  }

\ccConstructor{Sphere_circle(RT x, RT y, RT z)}{ creates the circle
  orthogonal to the vector $(x,y,z)$.  }

\ccConstructor{Sphere_circle(Sphere_circle c, Sphere_point
...
\ccMethod{Sphere_segment_pair split_at(const Sphere_point& p) ;}{
...
\ccFunction{bool equal_as_sets(const
  Sphere_circle c1, const Sphere_circle c2) ;}{
  returns true iff \ccc{c1} and \ccc{c2} are equal as unoriented
  circles.  }

\ccFunction{Sphere_point intersection(const
  Sphere_circle c1, const Sphere_circle c2) ;}{ returns one of
