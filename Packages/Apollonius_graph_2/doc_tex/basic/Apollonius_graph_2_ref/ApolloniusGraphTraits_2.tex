...
\ccc{Apollonius_site_2}. It also provides
...
\ccGlue
\ccNestedType{Ray_2}{A type for a ray. Only required if the primal or
  dual diagram are inserted in a stream.}
\ccGlue
\ccNestedType{Segment_2}{A type for a segment. Only required if the
  primal or dual diagram are inserted in a stream.}
\ccGlue
...
\ccNestedType{Construct_object_2}{Must provide \ccc{template <class T>
    Object_2 operator()( T t)} that constructs an object that contains \ccc{t}
  and returns it.}
\ccGlue
...
equidistant from three sites). Must provide
\ccc{Point_2 operator()(Site_2 s1,
Site_2 s2, Site_2 s3)}, which constructs a
point equidistant from the sites \ccc{s1}, \ccc{s2} and \ccc{s3}.}
%
\ccGlue
...
a line bitangent to \ccc{s1} and \ccc{s2}. This line is the dual site
of \ccc{s1}, \ccc{s2} and the site at infinity; its center is at
infinity and its weight is infinite.
}
%
...
corresponding to \ccc{s2} is contained inside the circle corresponding
to \ccc{s1}, \ccc{false} otherwise.}
%
\ccGlue
...
contains \ccc{p}. The orientation of the bisector is such that the
positive halfspace corresponds to \ccc{s1}.}
%
\ccGlue
...
vertices defined by the triples \ccc{s1}, \ccc{s2}, \ccc{s3} and
...
defined by the triples \ccc{s1}, \ccc{s2}, \ccc{s3} and \ccc{s1},
...
$s_\infty$ define an Apollonius edge that lies on the bisector of
\ccc{s1} and \ccc{s2} and has as endpoints the Apollonius vertices
defined by the triples \ccc{s1}, \ccc{s2}, $s_\infty$ and \ccc{s1},
