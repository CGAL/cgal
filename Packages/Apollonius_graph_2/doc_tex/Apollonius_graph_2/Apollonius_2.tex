...
\item the {\bf geometric traits} class. The user can specify the
  ...
  Apollonius diagram.
\item a {\bf boolean variable} that indicates if the hidden weighted
  points are to be stored or not. Its value should \ccc{true} if the
  user wants to keep track of the weighted points or false
...
discuss them in detail. Te interested reader may refer to the papers
by Karavelas and Emiris for the details
\cite{ke-ppawv-02,ke-rctac-03}. However, we would like to give a brief
overview what they 
compute. There are several predicates needed for this algorithm. We
will discuss the most important/complicated ones. It turnes out that
...
Fig. \ref{fig:ag2vertexconcfict}) define a tritangent circle (yellow
...
$K_2=(c_2, r_2$ is defined to be the distance of their centers minus
their radii:
...
inseerted in the diagram (the green circle). In the Apollonius graph
...
\caption{The \ccc{Vertex_conflict_2} predicate. The red circles define
  ...
  Left: the predicate returns \ccc{NAGATIVE}. Right: the predicate
...
What we essentially want to compute when we contruct incrementally a
...
the same then we need additional test which determines if the interior
of the Apollonius diagram edge is destroyed by the new circle. This
...
is infinite is its dual edge in the Apollonius graph connects the
...
\end{center}\includegraphics[width=0.4\textwidth]%
...
\caption{The \ccc{Finite_edge_interior_conflict_2} predicate. The red
  circles define two the tritangent yellow circles, whose centers are
...
The \ccc{ApolloniusGraphHierachy_2} concept is nothing but the
equivalent of the \ccc{TriangulationHierarchy_2} applied to the
...
is a model of the \ccc{ApolloniusGraphHierachy_2} concept and has
