...
concept. A vertex stores an Apollonius site and
provides access to one of its incident faces through a
\ccc{Face_handle}. In addition to that it maintains a container of
...
%
\ccGlue
\ccNestedType{Store_hidden}{An enumeration type that indicates if
  hidden sites are actually stored or not. Its value is \ccc{true} if
  hidden site are stored, \ccc{false} otherwise.}
%
\ccGlue
...
iterates over the hidden site in the hidden site
...
\ccGlue
\ccMethod{Face_handle face();}{Returns a handle to the incident face.}
\ccGlue
