...
% | Package: HalfedgeDS
% | 
\RCSdef{\RCSHalfedgeDSdecoratorRev}{$Revision$}
...
\ccMethod{void vertices_erase( Vertex_handle first, Vertex_handle last);}{
    ...
    are supported. \ccPrecond \ccc{Supports_removal} 
...
\ccMethod{void faces_erase( Face_handle first, Face_handle last);}{
    ...
    supported. \ccPrecond \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}
...
\ccMethod{void erase_face( Halfedge_handle h);} {removes the
   ...
   halfedge data structure if they were already border edges.  See
   \ccc{make_hole(h)} for a more specialized variant.
   \ccPrecond \ccc{h != Halfedge_handle()}. If faces are supported, 
...
\ccMethod{void erase_connected_component( Halfedge_handle h);}
    {removes the  vertices, halfedges, and facets that belong to the 
     connected component of $h$. \ccPrecond \ccc{Supports_removal} $\equiv$
     \ccc{CGAL::Tag_true}. For all halfedges $g$ in the 
...
\ccMethod{Halfedge_handle make_hole( Halfedge_handle h);}
   {removes the face incident to \ccc{h} from \ccc{hds} and creates a hole.
    ...
    If faces are supported, \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}
...
\ccMethod{Halfedge_handle add_face_to_border( Halfedge_handle h, 
   ...
   {extends the surface with a new face from \ccc{hds} into the hole 
    ...
    of $g$ with the tip of $h$ and fills this separated part of the hole 
    with a new face, such that the new face is incident to $g$. Returns 
...
\begin{ccTexOnly}
    \begin{center}
      \parbox{\textwidth}{%
          \includegraphics[width=\textwidth]{fig/euler_hds.ips}%
      }
    \end{center}
\end{ccTexOnly}
...
\ccMethod{Halfedge_handle split_face( Halfedge_handle h, Halfedge_handle g);}
    {splits the face incident to \ccc{h} and \ccc{g} into two faces
     ...
     \ccc{hds} is a copy of the first face. The new diagonal is
     returned. The new face is to the right of the new diagonal, 
     the old face is to the left. The time is proportional to the 
     distance from \ccc{h} to \ccc{g} around the face.} 
...
\ccMethod{Halfedge_handle create_center_vertex( Halfedge_handle h);}
    {barycentric triangulation of \ccc{h->face()}. Creates a new vertex,
     ...
     are copies of this face. Returns the halfedge next to \ccc{h}
...
These operations are the same as for \ccc{CGAL::HalfedgeDS_const_decorator<HDS>}.
\begin{ccTexOnly}
