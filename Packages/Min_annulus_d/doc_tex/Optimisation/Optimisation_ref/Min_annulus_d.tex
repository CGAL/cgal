...
\ccMemberFunction{ Center_coordinate_iterator
        ...
        returns an iterator referring to the coordinate of the first
        coordinate of the center of \ccVar.
...
\ccMemberFunction{ template < class InputIterator >
                   void  set( InputIterator first,
                              InputIterator last );}{
        sets \ccVar\ to the $ma(P)$, where $P$ is the set of points in
...
%  If \ccc{p} lies inside the current
%  annulus, this is a constant-time operation, otherwise it might take
%  longer, but in any case substantially less than recomputing the
%  smallest enclosing annulus from scratch.
        \ccPrecond  The dimension of \ccc{p} equals
        \ccVar\ccc{.ambient_dimension()} if \ccVar\ is not empty.}
...
%  , by calling \ccc{insert} for all points in the range.
        \ccPrecond  The value type of \ccc{InputIterator} is \ccc{Point},
        and all points have the same dimension.
        If \ccVar\ is not empty, this dimension must be equal to
        \ccVar\ccc{.ambient_dimension()}.}

% When applied to an empty \ccVar, \ccc{set( first, last)} and
% \ccc{insert( first, last)} produce the same result. However, the
% latter method is usually much slower, because it processes the
% points on-line, while the \ccc{set} method takes advantage of
% knowing the whole point set in advance.
