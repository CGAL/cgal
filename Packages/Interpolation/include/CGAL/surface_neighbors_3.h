		    ...
		    OutputIterator out, const Kernel& K)
{
  ...
  return 
    surface_neighbors_3(first, beyond, p, out, I_gt(p,normal));
}
			      ...
			      OutputIterator out, const Kernel& K)
{
  typedef Voronoi_intersection_2_traits_3<Kernel> I_gt;
  return 
    surface_neighbors_certified_3(first, beyond, p, out, I_gt(p,normal));
}
			      ...
			      const Kernel& K)
{
  typedef Voronoi_intersection_2_traits_3<Kernel> I_gt;
  return 
    surface_neighbors_certified_3(first, beyond, p, radius,
				  out, I_gt(p,normal));
}
