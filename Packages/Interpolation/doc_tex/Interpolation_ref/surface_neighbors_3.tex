...
dense, the neighbors re provably close to the point \ccc{p} on the
surface (cf.\ the manual pages and
\cite{bf-lcss-02},\cite{prisme-these-flototto}). They are defined to
be the neighbors of \ccc{p} in the regular triangulation that is dual
to the power diagram that is equivalent to the intersection of the
