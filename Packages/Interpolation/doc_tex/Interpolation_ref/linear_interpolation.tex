...
\ccFunction{template < class ForwardIterator, class Functor> typename
  ...
  function value and boolean. The boolean indicates wether the
  ...
  \item \ccc{Functor::argument_type} must be equivalent to
    ...
    multiplication and addition operation with the type
    \ccc{ForwardIterator::value_type::second_type}. A model of the
    functor is provided by the struct \ccc{Data_access}. It must be
    instantiated accordingly with an associative container (e.g.\ 
    \stl\ \ccc{std::map}) having the point type as \ccc{key_type} and
    the function value type as \ccc{mapped_type}.
  \end{enumerate}
