...
Given a set of sample points issued from the surface and a query point
\ccc{p}, the function \ccRefName\ computes the neighbors of \ccc{p} on
a surface within the sample points. If the sampling is sufficiently
dense, the neighbors meets the properties described in the manual
pages and in \cite{bf-lcss-02},\cite{prisme-these-flototto}. Notably,
they are provably close to the point \ccc{p} on the surface.  The
neighbors are defined to be the neighbors of \ccc{p} in the regular
triangulation that is dual to the power diagram representing the
intersection of the Voronoi cell of the query point \ccc{p} with the
tangent plane to the surface at \ccc{p}.  
...
\ccFunction{template <class OutputIterator, class InputIterator, class
  ...
  neighbors of \ccc{p} in the regular triangulation that is dual to the
