...
the natural neighbor coordinates have the following properties:
  ...
  \item[(ii)] For any $i,j \leq n, \lambda_i(\mathbf{p_j})= \delta_{ij}$, where
...
vertices in conflict with the query point are determined. The area $\pi_i(\mathbf{x})$ are computed by triangulating the Voronoi sub-cells. The
