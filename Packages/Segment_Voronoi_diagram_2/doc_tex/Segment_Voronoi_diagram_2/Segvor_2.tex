...
\item the {\bf segment Voronoi diagram data structure}. This is
  ...
  \ccc{SegmentVoronoiDiagramDataStructure_2}, which in fact is a refinement
  of the \ccc{ApolloniusGraphDataStructure_2} concept. The class
  \ccc{Segment_Voronoi_diagram_data_structure_2<Vb,Fb>} is a model of
  the concept \ccc{SegmentVoronoiDiagramDataStructure_2}. A default value
...
\begin{figure}[!t]
  \begin{ccTexOnly}
    \begin{center}
      ...
      {Segment_Voronoi_diagram_2/svd-rep}
    \end{center}
  \end{ccTexOnly}
  \begin{ccHtmlOnly}
    <center>
  ...
  \begin{center}
    \caption{Site representation. The point $s_1$ is represented by the
      ...
      represented similarly.}\label{fig:segvor2-siterep}
  \end{center}
  \begin{ccHtmlOnly}
    </font>
  \end{ccHtmlOnly}
...
The \ccc{SegmentVoronoiDiagramSite_2} concept encapsulates the ideas
...
class \ccc{Segment_Voronoi_diagram_site_2<K>} implements this
concept.
...
%% \ccc{SegmentVoronoiDiagramSite_2} concept that implements the concept
%% in a custom way, when we only have weakly intersecting sites. The
%% corresponding class is called
%% \ccc{Segment_Voronoi_diagram_simple_site_2<K>} and it only stores 2
%% points, in contrast to the 6 points stored by the
%% \ccc{Segment_Voronoi_diagram_2<K>} class.
...
\ccc{Segment_Voronoi_diagram_storage_site_2<Gt>} class which is a
model of the corresponding concept:
\ccc{SegmentVoronoiDiagramStorageSite_2}. This concept enforces a site
...
\item One that supports the full-fledged sites, and is suitable when
  ...
  consists of the 
  \ccc{Segment_Voronoi_diagram_traits_2<K,MTag>} and
  \ccc{Segment_Voronoi_diagram_filtered_traits_2<CK,CM,EK,EM,FK,FM>} 
  classes.
\item One that is customized for input that contain only weakly
  intersecting sites. This series consists of the
  \ccc{Segment_Voronoi_diagram_traits_without_intersections_2<K,MTag>} and
  \ccc{Segment_Voronoi_diagram_filtered_traits_without_intersections_2<CK,CM,EK,EM,FK,FM>}
  classes.
\end{itemize}
...
\ccc{SegmentVoronoiDiagramTraits_2} concept. These models are the
classes:
\ccc{Segment_Voronoi_diagram_traits_2<K,MTag>},
\ccc{Segment_Voronoi_diagram_traits_without_intersections_2<K,MTag>},
\ccc{Segment_Voronoi_diagram_filtered_traits_2<CK,CM,EK,EM,FK,FM>} and
\ccc{Segment_Voronoi_diagram_filtered_traits_without_intersections_2<CK,CM,EK,EM,FK,FM>}.
...
\item The user can define his/her kernel using as number type, a
  ...
  \ccc{Segment_Voronoi_diagram_2<K,MTag>} or
  \ccc{Segment_Voronoi_diagram_with_intersections_2<K,MTag>} class.
\item The user can define up to three different kernels \ccc{CK},
  \ccc{FK} and \ccc{EK} (some default values are provided). The first
  kernel \ccc{CK} will be used only for constructions. The second
  kernel \ccc{FK} is the filtering kernel: the traits class will
  attempt to compute the predicates using this kernel. If the
  filtering kernel fails to successfully compute a predicate, the
  exact kernel \ccc{EK} will be used. These three kernels are then
  used in the 
  \ccc{Segment_Voronoi_diagram_filtered_traits_2<CK,CM,EK,EM,FK,FM>} and
  \ccc{Segment_Voronoi_diagram_filtered_traits_without_intersections_2<CK,CM,EK,EM,FK,FM>}
...
\ccc{Segment_Voronoi_diagram_2<K,MTag>} and 
\ccc{Segment_Voronoi_diagram_with_intersections_2<K,MTag>}.
...
operations exactly. The second choices is \ccc{CGAL::Field_tag} for
the \ccc{Segment_Voronoi_diagram_2<K,MTag>}, and \ccc{CGAL::Ring_tag}
for the \ccc{Segment_Voronoi_diagram_with_intersections_2<K,MTag>}
...
\ccc{Segment_Voronoi_diagram_filtered_traits_2<CK,CM,EK,EM,FK,FM>} is
...
\ccc{Segment_Voronoi_diagram_2<K,MTag>} and 
\ccc{Segment_Voronoi_diagram_with_intersections_2<K,MTag>} classes.
...
The
\ccc{Segment_Voronoi_diagram_hierarchy_2<SegmentVoronoiDiagramTraits_2,SSTag,SegmentVoronoiDiagramDataStructure_2>}
class is the analogue of the
\ccc{Triangulation_hierarchy_2} or the \ccc{Apollonius_graph_hierarchy_2}
...
hierarchies by means of the template parameter \ccc{SSTag}. If
\ccc{SSTag} is set to \ccc{false} (which is also the default value),
the upper levels of the hierarchy consist of point Voronoi
diagrams. If \ccc{SSTag} is set to \ccc{true}, we have segment Voronoi
diagrams at all levels of the hierarchy.

The class \ccc{Segment_Voronoi_diagram_hierarchy_2<SegmentVoronoiDiagramTraits_2,SSTag,SegmentVoronoiDiagramDataStructure_2>}
