...
\begin{ccRefClass}{Segment_Voronoi_diagram_2<Gt,PC,DS>}
...
\ccc{PC} which must be a model of the concept \ccc{SimpleContainer},
and \ccc{DS},
...
whereas the third template argument defaults to 
...
{A type for the segment Voronoi diagram site, defined in the geometric
  traits.}
\ccGlue
\ccTypedef{typedef PC Point_container;}{A type for the container of points.}
\ccGlue
...
%
\ccTwo{Semgent_Voronoi_diagram_2<Gt,PC,DS>::Finite_vertices_iterator+}
{}
%
\ccGlue
...
times, every {\bf should this be this way, or just one instance?}
instance of this site will be reported. The set of output
...
\ccCreation
\ccThree{Segment_Voronoi_diagram_2<Gt,PC,DS>}{svd = other other}{}
\ccThreeToTwo
%
...
\ccConstructor{Segment_Voronoi_diagram_2(const
  Segment_Voronoi_diagram_2<Gt,PC,DS>& other)}
...
%
\ccMethod{Segment_Voronoi_diagram_2<Gt,PC,DS>
operator=(const Segment_Voronoi_diagram_2<Gt,PC,DS>& other);}
...
[\ccc{first},\ccc{beyond}) after performing a random shuffle on
  them. The input iterator range is not invalidated since the sites
  are copied locally before the random shuffling is performed.
  The number of additional sites inserted in the diagram is
  returned.
\ccPrecond{\ccc{Input_iterator} must be a model of
  \ccc{InputIterator} and its value type must be
  either \ccc{Point_2} or \ccc{Site_2}.}}
...
\ccMethod{Vertex_handle insert(Site_2 s);}{Inserts the site \ccc{s} in the
  ...
  semantics with the vertex handle returned by the methods
...
\ccMethod{void swap(Segment_Voronoi_diagram_2<Gt,PC,DS>
