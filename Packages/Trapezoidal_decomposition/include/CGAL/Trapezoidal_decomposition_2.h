 ...
 There exist non degenerate trapezoids between the roots of the input DS's
	...
	*/
		...
		)
	{
		...
		Data_structure leftDS= Data_structure(X_trapezoid(curr.left(),p,curr.bottom(),curr.top(),
		...
		Data_structure rightDS=Data_structure(X_trapezoid(p,curr.right(),curr.bottom(),curr.top(),
...
*/
		...
		const Point& p)
	{
		...
		CGAL_assertion(tt_left->is_right_unbounded()==tt_right->is_right_unbounded());
	...
	*/
	...
	optimization:
	Each degenerate X_curve trapezoid eminating from the point p holds a pointer to the next
	...
	For the trapezoid representing p rt and lb hold the previous X_curve degenerate trapezoid
	...
	*/
	void replace_curve_using_geometry(In_face_iterator& it,const X_curve& old_cv,const X_curve& new_cv,pointer& end)
	{
		pointer last=0;
  ...
  */
  ...
  Data_structure container2data_structure(map_nodes& ar,int left,int right) const
  {
	...
	*/
	const X_trapezoid insert_in_face_interior(curve_const_ref cv)
	{
	 ...
	 write(std::cout,*DS,*traits) << std::endl;
		...
		// locate and insert end points of the input X_curve to the X_trapezoid Dag if needed
		...
		CGAL_postcondition(traits->curve_is_same((const X_curve)old_output->top(),cv));
...
/* calculate the immediate lower central and upper neighbourhood of the curve 
		...
		do
		{
			...
			if (!bottom_it || bottom && !traits->trapezoid_bottom_curve_is_same(*old,*it))
			{
	...
	*/
	...
	void split_edge(curve_const_ref cv,curve_const_ref cv1, curve_const_ref cv2)
	{
	 ...
	 write(std::cout,*DS,*traits) << std::endl;
		...
		const Data_structure
			&new_left_tt=Data_structure(X_trapezoid(old_t.left(),p,left_cv,left_cv),old_left,old_right),
			&new_right_tt=Data_structure(X_trapezoid(p,old_t.right(),right_cv,right_cv),old_left,old_right),
			&new_tt=Data_structure(X_trapezoid(p,p,left_cv,right_cv),new_left_tt,new_right_tt);
		reference
	...
	*/
	...
	{
		return !ds || ds->is_valid(traits) && ds->get_node() && is_valid(ds.left()) && is_valid(ds.right());
	}
  ...
  */
  ...
  unsigned long container(X_curve_container& content)
  {
    ...
    return sz;
		...
		DepthThreshold times log of the X_curve count or the Trapezoidal Tree's size 
		...
		*/
		...
		bool
		Description:
		uses needs_update to determine whether the Trapezoidal_decomposition_2<Traits> needs update
...
private:
	...
	void init()
	{
		...
		/*		Point tmp;
		if (!traits->point_is_left_low(TD_X_trapezoid<Traits, X_curve>::POINT_AT_LEFT_TOP_INFINITY,tmp=TD_X_trapezoid<Traits, X_curve>::POINT_AT_RIGHT_BOTTOM_INFINITY))
		TD_X_trapezoid<Traits, X_curve>::POINT_AT_LEFT_TOP_INFINITY=traits->point_to_left(tmp);
		*/
