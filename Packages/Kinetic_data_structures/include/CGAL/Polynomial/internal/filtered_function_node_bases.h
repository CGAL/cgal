...
public:
  ...
  Filtered_function_node(const Interval_function_converter &ifc): has_exact_(false), ifc_(ifc) {
...
public:
  Filtered_function_node_binary_operation(const typename P::Pointer &lc, 
				    const typename P::Pointer &rc)
    : P(lc->interval_function_converter()), lc_(lc), rc_(rc){}
  virtual ~Filtered_function_node_binary_operation(){}

  const typename P::Pointer& left_child() const {
    return lc_;
  }
  const typename P::Pointer& right_child() const {
    return rc_;
  }
protected:
  ...
  mutable typename P::Pointer lc_, rc_;
};
...
class Filtered_function_node_unary_operation: public Filtered_function_node<Traits> {
  ...
  typedef Filtered_function_node<Traits> P;
public:
  Filtered_function_node_unary_operation(const typename P::Pointer &c):
    P(c->interval_function_converter()), c_(c){}
  virtual ~Filtered_function_node_unary_operation(){}
  const typename P::Pointer& child() const {
    return c_;
  }
protected:
