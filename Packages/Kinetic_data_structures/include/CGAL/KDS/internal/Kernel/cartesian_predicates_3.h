...
//#include <CGAL/KDS/kernel/Cartesian_moving_sphere_3.h>
...
struct Cartesian_side_of_oriented_sphere_3 {
  ...
  result_type operator()(const first_argument_type &a, 
    ...
    FT ret= CGAL::det4x4_by_formula(dpx, dpy, dpz, dpt,
				    ...
				    drx, dry, drz, drt,
				    dsx, dsy, dsz, dst);
    /*  CGAL_KDS_MAPLE_LOG( << std::endl << std::endl;
	CGAL_KDS_MAPLE_LOG( << "pt3\n";
	CGAL_KDS_MAPLE_LOG( << "m:=matrix(5,5,[[";
	CGAL_KDS_MAPLE_LOG(
	...
	CGAL_KDS_MAPLE_LOG( << "det(m)-( " << ret << ");\n";
	::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << "m2:= matrix(4,4,[[";
	::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << dpx << "," << dpy << "," << dpz << "," << dpt << "], [";
	::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << dqx << "," << dqy << "," << dqz << "," << dqt << "], [";
	::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << drx << "," << dry << "," << drz << "," << drt << "], [";
	::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << dsx << "," << dsy << "," << dsz << "," << dst << "]]);\n";
    */
    return ret;
  }
};
...
typename Pt::Coordinate co3(const Pt &a, const Pt &b, const Pt &c, const Pt &d){
  ...
  CGAL_KDS_LOG_MAPLE( d.x() << ","<<d.y()<<","<<d.z() << "]]);"<< std::endl);
  CGAL_KDS_LOG_MAPLE( "det(m)- (" << ret << ");" << std::endl);
...
typename CC::result_type co3(const Pt &a, const Pt &b, const Pt &c, const Pt &d, CC cc){
  ...
  RT ret= CGAL::det3x3_by_formula(a00, a01, a02,
				  a10, a11, a12,
				  a20, a21, a22);
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << std::endl << std::endl;
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << "co3\n";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << "m:= matrix(4,4, [[1," << a.x()<<","<<a.y() <<","<<a.z() << "], [1," << b.x()<<","<<b.y()<<","<<b.z();
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << "], [1," << c.x() <<","<<c.y()<<","<<c.z() << "], [1,";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << d.x() << ","<<d.y()<<","<<d.z() << "]]);"<< std::endl;
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << "det(m)- (" << ret << ");" << std::endl;
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << "m2:= matrix(3,3,[[,";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << a00 << "," << a01 << "," << a02 << "], [";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << a10 << "," << a11 << "," << a12 << "], [";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << a20 << "," << a21 << "," << a22 << "]]);";
...
typename CC::result_type pt3(const Cartesian_moving_point_3<C>  &a,
  ...
  FT ret= CGAL::det4x4_by_formula(dpx, dpy, dpz, dpt,
				  ...
				  dsx, dsy, dsz, dst);
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << std::endl << std::endl;
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << "pt3\n";
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << "m:=matrix(5,5,[[";
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE)
    << a.x() << ", " << a.y() << ", " << a.z() << ", " << a.x()*a.x()+a.y()*a.y()+a.z()*a.z() << ", 1], ["
    << a.x() << ", " << b.y() << ", " << b.z() << ", " << b.x()*b.x()+b.y()*b.y()+b.z()*b.z() << ", 1], ["
    << c.x() << ", " << c.y() << ", " << c.z() << ", " << c.x()*c.x()+c.y()*c.y()+c.z()*c.z() << ", 1], ["
    << d.x() << ", " << d.y() << ", " << d.z() << ", " << d.x()*d.x()+d.y()*d.y()+d.z()*a.z() << ", 1], ["
    << e.x() << ", " << e.y() << ", " << e.z() << ", " << e.x()*e.x()+e.y()*e.y()+e.z()*e.z() << ", 1]]);\n";
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << "det(m)-( " << ret << ");\n";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << "m2:= matrix(4,4,[[";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << dpx << "," << dpy << "," << dpz << "," << dpt << "], [";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << dqx << "," << dqy << "," << dqz << "," << dqt << "], [";
  ::CGAL::KDS::log()->stream(::CGAL::KDS::Log::MAPLE) << drx << "," << dry << "," << drz << "," << drt << "], [";
...
#if 0
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << std::endl << std::endl;
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << "pt3 lifted\n";
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE) << "m:=matrix(5,5,[[";
  CGAL::KDS::log()->stream(CGAL::KDS::Log::MAPLE)
