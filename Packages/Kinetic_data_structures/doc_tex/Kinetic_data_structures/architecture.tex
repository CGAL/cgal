...
\item[\ccc{NotifyingTable}] a container which stores kinetic geometric
...
off a wall, then the \ccc{NotifyingTable} notifies the kinetic Delaunay data
...
models, the
\ccc{CGAL::KDS::Cartesian_instantaneous_kernel<NotifyingTable,
Kernel>} and
\ccc{CGAL::KDS::Regular_triangulation_instantaneous_traits<NotifyingTable,
Kernel>}.
...
triangulation queries the \ccc{NotifyingTable} for all the moving points on
initialization. Later, when the simulation is running, the
\ccc{NotifyingTable} notifies the kinetic Delaunay data structure whenever a point's
trajectory changes.

The \ccc{NotifyingTable} allows multiple kinetic data structures to
...
\ccc{NotifyingTable}, \ccc{Notifying_table} is actually a generic
...
primitive a particular instance of the \ccc{NotifyingTable} model
...
\ccc{NotifyingTable} containing points of appropriate
dimensionality. Like the \ccc{Simulator} they \ccc{NotifyingTable}
models are explicitly shared and accessed through reference counted
pointers.

The \ccc{NotifyingTable} uses a notification system which will be
...
\ccc{NotifyingTable} and handle them accordingly. We provide helper
...
The \ccc{NotifyingTabke} model provided will not meet the needs of
...
the \ccc{NotifyingTabke} model can be extended to handle many such
...
\ccc{NotifyingTabke} can check that the trajectories are
...
The \ccc{NotifyingTabke} only knows about one type of kinetic
...
detailed discussion of how to use the \ccc{NotifyingTabke} appears
...
\ccc{NotifyingTable} and the \ccc{Simulator} to the kinetic data structures to
