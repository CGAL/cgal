...
\ccc{set_editing(true)} and ended by calling \ccc{set_editing(false)}.
...
\ccConstructor{Moving_object_table();}{default constructor.}

\ccOperations

\ccMethod{Moving_object operator[](Key key) const;}{Access the object referenced by the key.}

\ccMethod{Moving_object object(Key key) const;}{Access the object referencd by the key.}
...
\ccMethod{void set(Key key, Moving_object object);}{This method changes
  ...
  in the \ccRefName.
    If \ccc{is_editing()} is not true, then it is as if the calls \ccc{set_editing(true)}, \ccc{set_object(key, value)} and finally \ccc{set_editing(false)} were made. If it is true, then no notifications are created.}

  \ccMethod{Key insert(Moving_object ob);}{Insert a new object into the
    table and return a \ccc{Key} which can be used to refer to it. See
    \ccc{set_object} for a description of editing modes.}
 ...
 \ccMethod{void erase(Key key);}{ Delete an object from the
   ...
   fact, it is unlikely to do so.  See \ccc{set_object()} for an
