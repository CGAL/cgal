...
a support set $R$ (which remains fixed until the next \ccc{update()}
...
compute the minsphere.  The instance is called \emph{up-to-date} if
and only if the minsphere of the inserted spheres $S$ has been
computed (e.g.\ if \ccc{update()} has been called after an
\ccc{insert()}).
...
\ccNestedType{Result}{is the type of the radius and of the center
  ...
  member function \ccc{disciminant()} of class
...
%
                                 {creates a variable \ccVar\ of type
                                   ...
                                   After this call \ccVar\ is not
                                   up-to-date, so you will have to
                                   call \ccc{update()} in order to
                                   actually compute $ms(S)$.
%
\ccRequire The value type of \ccc{first} and \ccc{last} is
\ccc{Sphere}. If the traits parameter is not supplied, the class
\ccc{Traits} must provide a default constructor.  Furthermore, the
...
\ccMemberFunction{ Support_iterator support_begin() const;}{ returns
  an iterator referring to the first support sphere of \ccVar.  If
  \ccVar\ is not up-to-date, then \ccc{update()} is called internally
  before returning the iterator.}
%
\ccMemberFunction{ Support_iterator support_end() const;}{ returns the
  corresponding past-the-end iterator.  If \ccVar\ is not up-to-date,
  then \ccc{update()} is called internally before returning the
  iterator.}
...
\ccMemberFunction{ const FT& discriminant( ) const;}{ returns the
  ...
  returned by this function.  \ccPrecond \ccVar\ is up-to-date and not
  empty, and \ccc{FT} is an exact number type.}
...
\ccMemberFunction{Coordinate_iterator center_begin( ) const;}{ returns
  a const-iterator to the first of the \ccc{Traits::D} center
  coordinates of \ccVar.  The iterator returns objects of type
  \ccc{Result}.  If \ccc{FT} is an exact number type, then a center
  coordinate is represented by a pair $(a,b)$ describing the real
  number $a+b\sqrt{t}$, where $t$ is the minsphere's discriminant (cf.
  \ccc{discriminant()}).  \ccPrecond \ccVar\ is up-to-date and not
  empty.}
...
\ccMemberFunction{ bool is_empty( ) const;}{ returns \ccc{true}, iff
  \ccVar\ is empty, i.e.\ iff $ms(S)=\emptyset$.  If \ccVar\ is not
  up-to-date, then \ccc{update()} is called before the emptiness
  check.}
...
\ccUnchecked \ccMemberFunction{ template < class InputIterator > void
  ...
  \ccRequire The value type of \ccc{first} and \ccc{last} is
  \ccc{Sphere}.  Furthermore, the spheres in the given range must not
  change during the lifetime of variable \ccVar.  (The algorithm uses
  pointers to the inserted spheres internally in order to save space
  and time, and consequently, the referenced spheres must not change.)
  \ccPostcond \ccVar\ is up-to-date.  }
...
\ccMemberFunction{ void insert( const Sphere& s );}{ inserts the
  ...
  \ccc{update()} to actually compute $ms(S)$.  \ccRequire The sphere
