...
the placement of streamlines and some functions for answaring
different queries.
...
\ccCreation
\ccCreationVariable{vector_field_2}  %% choose variable name
\ccConstructor{VectorField_2();}{only a default constructor.}
\ccGlue

\ccHeading{Query Functions}

\ccMethod{CGAL::Iso_rectangle_2<Geom_traits> iso_rectangle();}{return
the bounding box of the whole domain.}
\ccGlue
\ccMethod{std::pair<Vector_2,FT> get_field(Point_2 p);}{returns the
vector field value and the local density.}
\ccGlue
\ccMethod{bool is_in_domain(Point_2 p);}{returns true if the point p
is inside the domain boundaries, false otherwise.}
\ccGlue
\ccMethod{FT get_integration_step(Point_2 p);}{returns the integration 
step at the point p.}
\ccGlue
