...
\ccHtmlNoLinksFrom{difference} $P = (P \setminus Q) \cup (Q \setminus P)$
...
\ccHtmlNoLinksFrom{difference} between
\lcTex{%
  \setlength{\widthRight}{1.4cm}
  \setlength{\widthLeft}{\widthLineReal}
  \addtolength{\widthLeft}{-\widthRight}
...
lexicographically.
\end{itemize}
The main \ccHtmlNoLinksFrom{difference} between the
\ccc{ArrangementXMonotoneTraits_2} and
\ccc{ArrangementDirectionalXMonotoneTraits} concepts  is that a curve
...
\ccHtmlNoLinksFrom{difference} of a set of general polygons
respectively. The symmetric \ccHtmlNoLinksFrom{difference} of a set of
\lcTex{%
  \end{minipage}\hspace{\minipageSpace}
  \begin{minipage}{\widthRight}
    \begin{center}
...
general polygons. The class \ccc{General_polygon_set_2} has the equivalent 
...
\ccHtmlNoLinksFrom{difference}, the general polygons represented by
