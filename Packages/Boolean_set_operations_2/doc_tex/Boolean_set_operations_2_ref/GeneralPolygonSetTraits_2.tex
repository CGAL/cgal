...
% =======================
\ccMethod{bool is_valid(Polygon_2 & pgn);}
{returns \ccc{true} if the general polygon \ccc{pgn} is valid, 
and \ccc{false} otherwise. \ccc{pgm} is valid, if it is strictly simple.
Used as precondition for some of the operations.}
\ccGlue
\ccMethod{bool is_valid(Polygon_with_holes_2 & pgn_with_holes);}
{returns \ccc{true} if the general polygon with holes \ccc{pgn_with_holes} 
is valid, and \ccc{false} otherwise. \ccc{pgn_with_holes} is valid, if
