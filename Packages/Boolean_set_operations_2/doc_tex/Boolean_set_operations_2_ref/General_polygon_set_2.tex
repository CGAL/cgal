...
\ccConstructor{General_polygon_set_2<Traits>(const Traits::Polygon_with_holes_2 & pgn_with_holes);}
  {constructs a set of general polygons that consists of the single
   general polygon with holes \ccc{pgn_with_holes}.}
...
%   general polygon with holes in the second range are pairwise disjoint,
...
void insert(InputIterator begin, InputIterator end);}
  ...
  \ccPrecond{All general polygons (resp. general polygons with holes)
  in the given range and the point set represented by \ccVar\ are
  pairwise disjoint.}}
% , except maybe at the vertices
...
void insert(InputIterator1 pgn_begin, InputIterator1 pgn_end,
  ...
  represented by \ccVar\ are pairwise disjoint.}}
% , except maybe at the vertices

\ccMethod{void complement();}
  {computes the complement of \ccVar.}

\ccMethod{void complement(const General_polygon_set_2 & other);}
  {computes the complement of \ccc{other}.}

\ccMethod{void complement(const General_polygon_2 & pgn);}
  {computes the complement of \ccc{pgn}.}
  
\ccMethod{void complement(const General_polygon_with-holes_2 & pgn);}
  {computes the complement of \ccc{pgn}.}
...
void intersection(InputIterator begin, InputIterator end);}
  ...
  input iterator is used to distinguish between the two options.)}
\ccGlue
\ccMethod{template <class InputIterator1, class InputIterator2>
...
\ccMethod{bool oriented_side(const Point_2 & q);}
