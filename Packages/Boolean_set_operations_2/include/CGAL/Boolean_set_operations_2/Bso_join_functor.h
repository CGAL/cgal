...
class Bso_join_functor : public Bso_base_functor<Traits_>
{
public:
  ...
  {
    CGAL_assertion(h1->direction() == LARGER);
    if(!f2->contained())
  ...
  void create_edge(Face_const_handle f1,
                   ...
                   Halfedge_handle res_h)
  {
    CGAL_assertion(h2->direction() == LARGER);
    if(!f1->contained())
