...
$S_P$ and $S_Q$ are the \emph{support points}. A pair of support sets has
size at most $d+2$ (by size we mean $|S_P|+|S_Q|$). The distance between
the two polytopes is \emph{realized} by a pair of points $p$ and $q$
lying in the convex hull of $S_P$ and $S_Q$, repectively,
i.e.~$\sqrt{||p-q||}=pd(P,Q)$. In general, neither the support sets nor
the realizing points are necessarily unique.
...
\ccc{Polytope_distance_d_traits} as its template argument. We provide the
models \ccc{Polytope_distance_d_traits_2},
\ccc{Polytope_distance_d_traits_3}, and \ccc{Polytope_distance_d_traits_d}
for two-, three-, and $d$-dimensional input points, respectively.
...
\ccConstructor{ template < class InputIterator1, class InputIterator2 >
        ...
        It is initialized to $pd(P,Q)$ with $P$ and $Q$ being the sets of
        points in the range [\ccc{p_first},\ccc{p_last}) and
        [\ccc{q_first},\ccc{q_last}), respectively.
        \ccPrecond  The value type of \ccc{InputIterator1} and
        \ccc{InputIterator2} is \ccc{Point}, and all points have the same
        dimension.}
...
\ccMemberFunction{ template < class InputIterator >
        ...
        and all points in $P$ have dimension \ccVar\ccc{.ambient_dimension()}
        if $Q$ is not empty.}
...
\ccMemberFunction{ template < class InputIterator >
        ...
        \ccPrecond  The value type of \ccc{InputIterator} is \ccc{Point},
        and all points in $Q$ have dimension \ccVar\ccc{.ambient_dimension()}
        if $P$ is not empty.}
...
\ccMemberFunction{ template < class InputIterator1, class InputIterator2 >
        ...
        \ccPrecond  The value type of \ccc{InputIterator1} and
        \ccc{InputIterator2} is \ccc{Point}, and all points have the same
        dimension. If \ccVar\ is not \PDempty, this dimension must be equal to
...
objective function. The solution is obtained using our exact solver for
