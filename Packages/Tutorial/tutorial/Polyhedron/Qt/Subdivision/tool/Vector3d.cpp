...
//********************************************
void
CVector3d::Get(float& x, float& y, float& z) const
{
  x = vec[0];
  y = vec[1];
  z = vec[2];
...
//********************************************
CVector3d
operator*(float s, const CVector3d& u)
{
...
//********************************************
int
CVector3d::Equals(const CVector3d& v, float tolerence) const
{
...
//********************************************
double
CVector3d::Normalize(float value)
{
  double len = Length();
  if(len != 0.0f)
    (*this) *= float(value/len);
  else
    Set(0.f,0.f,0.f);
...
// by Haeyoung Lee
//********************************************
CVector3d CVector3d::Rotate(float angle, 
														CVector3d Around) 
{
	double f1, f2, f3;
	CVector3d t1, t2;
	...
	f1 = (float)cos((double)angle);
	f2 = (float)sin((double)angle);
	t1 = Projection(&Around);
	t2 = Around.Cross(this);
	f3 = Dot(Around);
	
	return CVector3d((float)(f1*t1.x()+f2*t2.x()+f3*Around.x()),
		(float)(f1*t1.y()+f2*t2.y()+f3*Around.y()),
		(float)(f1*t1.z()+f2*t2.z()+f3*Around.z()));
