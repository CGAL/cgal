...
//
// implementation: Polynomial_3s in one variable
// ======================================================================
...
template <class NT> class Polynomial_3_rep;
...
template <class NT>  Polynomial_3<NT>
  operator - (const Polynomial_3<NT>&);
template <class NT>  Polynomial_3<NT>
  operator + (const Polynomial_3<NT>&, const Polynomial_3<NT>&);
template <class NT>  Polynomial_3<NT>
  operator - (const Polynomial_3<NT>&, const Polynomial_3<NT>&);
template <class NT>  Polynomial_3<NT>
  operator * (const Polynomial_3<NT>&, const Polynomial_3<NT>&);
template <class NT> inline Polynomial_3<NT>
  operator / (const Polynomial_3<NT>&, const Polynomial_3<NT>&);

#if ! defined(_MSC_VER)
template<class NT> CGAL::Sign 
  sign(const Polynomial_3<NT>& p);
#endif // collides with global CGAL sign

template <class NT> double 
  to_double(const Polynomial_3<NT>& p) ;
template <class NT> bool 
  is_valid(const Polynomial_3<NT>& p) ;
template <class NT> bool 
  is_finite(const Polynomial_3<NT>& p) ;

template<class NT>  
  std::ostream& operator << (std::ostream& os, const Polynomial_3<NT>& p);
template <class NT>  
  std::istream& operator >> (std::istream& is, Polynomial_3<NT>& p);

#if ! defined(_MSC_VER) || _MSC_VER >= 1300
  // lefthand side
template<class NT> inline Polynomial_3<NT> operator + 
  (const NT& num, const Polynomial_3<NT>& p2);
template<class NT> inline Polynomial_3<NT> operator - 
  (const NT& num, const Polynomial_3<NT>& p2);
template<class NT> inline Polynomial_3<NT> operator * 
  (const NT& num, const Polynomial_3<NT>& p2);
template<class NT> inline Polynomial_3<NT> operator / 
  ...
  // righthand side
template<class NT> inline Polynomial_3<NT> operator + 
  (const Polynomial_3<NT>& p1, const NT& num);
template<class NT> inline Polynomial_3<NT> operator - 
  (const Polynomial_3<NT>& p1, const NT& num);
template<class NT> inline Polynomial_3<NT> operator * 
  (const Polynomial_3<NT>& p1, const NT& num);
...
template<class NT> inline bool operator ==  
  (const NT& num, const Polynomial_3<NT>& p);
template<class NT> inline bool operator != 
  (const NT& num, const Polynomial_3<NT>& p);
template<class NT> inline bool operator <  
  (const NT& num, const Polynomial_3<NT>& p);
...
template<class NT> inline bool operator >=  
  ...
  // righthand side
template<class NT> inline bool operator ==
  (const Polynomial_3<NT>& p, const NT& num);
template<class NT> inline bool operator !=
  (const Polynomial_3<NT>& p, const NT& num);
template<class NT> inline bool operator < 
  (const Polynomial_3<NT>& p, const NT& num);
...
template <class pNT> class Polynomial_3_rep { 
  typedef pNT NT;
  #ifndef CGAL_SIMPLE_NEF_INTERFACE
  ...
  Polynomial_3_rep() : coeffy(), coeffz{}
  Polynomial_3_rep(const NT& n) : coeffy(1), coeffz(1) { coeff[0]=n; }
  Polynomial_3_rep(const NT& n, const NT& m) : coeff(2)
    { coeff[0]=n; coeff[1]=m; }
  Polynomial_3_rep(const NT& a, const NT& b, const NT& c) : coeff(3)
    { coeff[0]=a; coeff[1]=b; coeff[2]=c; }
  Polynomial_3_rep(size_type s) : coeff(s,NT(0)) {}

  #ifndef CGAL_SIMPLE_NEF_INTERFACE

  template <class Forward_iterator>
  Polynomial_3_rep(Forward_iterator first, Forward_iterator last SNIHACK) 
    : coeff(first,last) {}
...
template <class pNT> class Polynomial_3 : 
  public Handle_for< Polynomial_3_rep<pNT> >
{
...
(2). In both cases there's a polynomial division operation defined.}*/
  ...
  typedef Handle_for< Polynomial_3_rep<NT> > Base;
  typedef Polynomial_3_rep<NT> Rep;
  ...
  Polynomial_3(size_type s) : Base( Polynomial_3_rep<NT>(s) ) {}
  ...
  value. }*/
    : Base( Polynomial_3_rep<NT>() ) {}
   
  Polynomial_3(const NT& a0)
  /*{\Mcreate introduces a variable |\Mvar| of type |\Mname| representing
  the constant polynomial $a_0$.}*/
    : Base(Polynomial_3_rep<NT>(a0)) { reduce(); }

  Polynomial_3(NT a0, NT a1) //af 
  /*{\Mcreate introduces a variable |\Mvar| of type |\Mname| representing
  the polynomial $a_0 + a_1 x$. }*/
    : Base(Polynomial_3_rep<NT>(a0,a1)) { reduce(); }

  Polynomial_3(const NT& a0, const NT& a1,const NT& a2)
  /*{\Mcreate introduces a variable |\Mvar| of type |\Mname| representing
  the polynomial $a_0 + a_1 x + a_2 x^2$. }*/
    : Base(Polynomial_3_rep<NT>(a0,a1,a2)) { reduce(); }

  #ifndef CGAL_SIMPLE_NEF_INTERFACE
  template <class Forward_iterator>
  Polynomial_3(Forward_iterator first, Forward_iterator last) 
  /*{\Mcreate introduces a variable |\Mvar| of type |\Mname| representing
  ...
  \ldots a_d x^d$.}*/
    : Base(Polynomial_3_rep<NT>(first,last SNIINST)) { reduce(); }

  #else
  #define RPOL(I)\
  Polynomial_3(I first, I last) : \
  Base(Polynomial_3_rep<NT>(first,last SNIINST)) { reduce(); }
  RPOL(const NT*)
  // KILL int START
  RPOL(const int*)
  ...
  // KILL double START
  Polynomial_3(double n) : Base(Polynomial_3_rep<NT>(NT(n))) { reduce(); }
  Polynomial_3(double n1, double n2) 
    : Base(Polynomial_3_rep<NT>(NT(n1),NT(n2))) { reduce(); }
  // KILL double END

  // KILL int START
  Polynomial_3(int n) : Base(Polynomial_3_rep<NT>(NT(n))) { reduce(); }
  Polynomial_3(int n1, int n2)
    : Base(Polynomial_3_rep<NT>(NT(n1),NT(n2))) { reduce(); }
  // KILL int END

  Polynomial_3(const Polynomial_3<NT>& p) : Base(p) {}
  ...
  static void pseudo_div
    (const Polynomial_3<NT>& f, const Polynomial_3<NT>& g, 
     Polynomial_3<NT>& q, Polynomial_3<NT>& r, NT& D);
  ...
  static void euclidean_div 
    (const Polynomial_3<NT>& f, const Polynomial_3<NT>& g, 
     Polynomial_3<NT>& q, Polynomial_3<NT>& r);
  /*{\Mstatic implements division with remainder on polynomials of 
  ...
  Polynomial_3<NT>& operator += (const NT& num)
  { copy_on_write();
...
// SPECIALIZING_MEMBERS FOR const int& 
  ...
  Polynomial_3<NT>& operator += (const int& num)
  { copy_on_write();
    coeff(0) += (NT)num; return *this; }
...
// SPECIALIZING_MEMBERS FOR const double& 
  ...
  Polynomial_3<NT>& operator += (const double& num)
  { copy_on_write();
    coeff(0) += (NT)num; return *this; }
  ...
  void minus_offsetmult(const Polynomial_3<NT>& p, const NT& b, int k)
  { CGAL_assertion(!is_shared());
    Polynomial_3<NT> s(size_type(p.degree()+k+1)); // zero entries
...
(2). In both cases there's a polynomial division operation defined.}*/
  ...
  typedef Handle_for< Polynomial_3_rep<int> > Base;
  typedef Polynomial_3_rep<int> Rep;
  ...
  Vector& coeffs() { return ptr()->coeff; }
  const Vector& coeffs() const { return ptr()->coeff; }
  Polynomial_3(size_type s) : Base( Polynomial_3_rep<int>(s) ) {}
  // creates a polynomial of degree s-1
  ...
  value. }*/
    : Base( Polynomial_3_rep<int>() ) {}
   
  Polynomial_3(const int& a0)
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| representing
  the constant polynomial $a_0$.}*/
    : Base(Polynomial_3_rep<int>(a0)) { reduce(); }

  Polynomial_3(int a0, int a1) // af: was const&
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| representing
  the polynomial $a_0 + a_1 x$. }*/
    : Base(Polynomial_3_rep<int>(a0,a1)) { reduce(); }

  Polynomial_3(const int& a0, const int& a1,const int& a2)
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| representing
  the polynomial $a_0 + a_1 x + a_2 x^2$. }*/
    : Base(Polynomial_3_rep<int>(a0,a1,a2)) { reduce(); }
  ...
  where |++it == last| then |\Mvar| stores the polynomial $a_1 + a_2 x + 
  \ldots a_d x^d$.}*/
    : Base(Polynomial_3_rep<int>(first,last SNIINST)) { reduce(); }
  ...
  // KILL double START
  Polynomial_3(double n) : Base(Polynomial_3_rep<int>(int(n))) { reduce(); }
  Polynomial_3(double n1, double n2) 
    : Base(Polynomial_3_rep<int>(int(n1),int(n2))) { reduce(); }
  // KILL double END

  Polynomial_3(const Polynomial_3<int>& p) : Base(p) {}
  ...
  static void pseudo_div
    (const Polynomial_3<int>& f, const Polynomial_3<int>& g, 
     Polynomial_3<int>& q, Polynomial_3<int>& r, int& D);
  ...
  static void euclidean_div 
    (const Polynomial_3<int>& f, const Polynomial_3<int>& g, 
     Polynomial_3<int>& q, Polynomial_3<int>& r);
  /*{\Xstatic implements division with remainder on polynomials of 
  ...
  Polynomial_3<int>& operator += (const int& num)
  { copy_on_write();
...
// SPECIALIZING_MEMBERS FOR const double& 
  ...
  Polynomial_3<int>& operator += (const double& num)
  { copy_on_write();
    coeff(0) += (int)num; return *this; }
  ...
  void minus_offsetmult(const Polynomial_3<int>& p, const int& b, int k)
  { CGAL_assertion(!is_shared());
    Polynomial_3<int> s(size_type(p.degree()+k+1)); // zero entries
    for (int i=k; i <= s.degree(); ++i) s.coeff(i) = b*p[i-k];
    operator-=(s);
  }
...
(2). In both cases there's a polynomial division operation defined.}*/
  ...
  typedef Handle_for< Polynomial_3_rep<double> > Base;
  typedef Polynomial_3_rep<double> Rep;
  typedef  Rep::Vector    Vector;
  typedef  Rep::size_type size_type;
  typedef  Rep::iterator  iterator;
  ...
  Vector& coeffs() { return ptr()->coeff; }
  const Vector& coeffs() const { return ptr()->coeff; }
  Polynomial_3(size_type s) : Base( Polynomial_3_rep<double>(s) ) {}
  // creates a polynomial of degree s-1
  ...
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| of undefined
  value. }*/
    : Base( Polynomial_3_rep<double>() ) {}
   
  Polynomial_3(const double& a0)
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| representing
  the constant polynomial $a_0$.}*/
    : Base(Polynomial_3_rep<double>(a0)) { reduce(); }

  Polynomial_3(const double& a0, const double& a1)
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| representing
  the polynomial $a_0 + a_1 x$. }*/
    : Base(Polynomial_3_rep<double>(a0,a1)) { reduce(); }

  Polynomial_3(const double& a0, const double& a1,const double& a2)
  /*{\Xcreate introduces a variable |\Mvar| of type |\Mname| representing
  the polynomial $a_0 + a_1 x + a_2 x^2$. }*/
    : Base(Polynomial_3_rep<double>(a0,a1,a2)) { reduce(); }
  ...
  where |++it == last| then |\Mvar| stores the polynomial $a_1 + a_2 x + 
  \ldots a_d x^d$.}*/
    : Base(Polynomial_3_rep<double>(first,last SNIINST)) { reduce(); }
  ...
  // KILL int START
  Polynomial_3(int n) : Base(Polynomial_3_rep<double>(double(n))) { reduce(); }
  Polynomial_3(int n1, int n2)
    : Base(Polynomial_3_rep<double>(double(n1),double(n2))) { reduce(); }
  // KILL int END

  Polynomial_3(const Polynomial_3<double>& p) : Base(p) {}
  ...
  static Polynomial_3<double> gcd
    (const Polynomial_3<double>& p1, const Polynomial_3<double>& p2);
  /*{\Xstatic returns the greatest common divisor of |p1| and |p2|.
  ...
  static void pseudo_div
    (const Polynomial_3<double>& f, const Polynomial_3<double>& g, 
     Polynomial_3<double>& q, Polynomial_3<double>& r, double& D);
  /*{\Xstatic implements division with remainder on polynomials of 
  ...
  static void euclidean_div 
    (const Polynomial_3<double>& f, const Polynomial_3<double>& g, 
     Polynomial_3<double>& q, Polynomial_3<double>& r);
  /*{\Xstatic implements division with remainder on polynomials of 
  ...
  Polynomial_3<double>& operator += (const double& num)
  { copy_on_write();
...
// SPECIALIZING_MEMBERS FOR const int& 
  ...
  Polynomial_3<double>& operator += (const int& num)
  { copy_on_write();
    coeff(0) += (double)num; return *this; }
  ...
  void minus_offsetmult(const Polynomial_3<double>& p, const double& b, int k)
  { CGAL_assertion(!is_shared());
    Polynomial_3<double> s(size_type(p.degree()+k+1)); // zero entries
    for (int i=k; i <= s.degree(); ++i) s.coeff(i) = b*p[i-k];
    operator-=(s);
  }
...
template <class NT> NT Polynomial_3<NT>::R_;
//int    Polynomial_3<int>::R_;
//double Polynomial_3<double>::R_;
//those 2 moved in src/Polynomial_3.C

template <class NT> double to_double 
  (const Polynomial_3<NT>& p) 
  { return (CGAL::to_double(p.eval_at(Polynomial_3<NT>::R_))); }
...
template <class NT> CGAL::io_Operator 
  io_tag(const Polynomial_3<NT>&) 
  { return CGAL::io_Operator(); }


template <class NT> 
Polynomial_3<NT> operator - (const Polynomial_3<NT>& p)
{
  CGAL_assertion(p.degree()>=0);
  Polynomial_3<NT> res(p.coeffs().begin(),p.coeffs().end()); 
  typename Polynomial_3<NT>::iterator it, ite=res.coeffs().end();
  for(it=res.coeffs().begin(); it!=ite; ++it) *it = -*it;
  return res;
}

#if defined(_MSC_VER)
Polynomial_3<int> operator - (const Polynomial_3<int>& p);
Polynomial_3<double> operator - (const Polynomial_3<double>& p);
#endif

template <class NT> 
Polynomial_3<NT> operator + (const Polynomial_3<NT>& p1, 
                            const Polynomial_3<NT>& p2)
{ 
  typedef typename Polynomial_3<NT>::size_type size_type;
  ...
  Polynomial_3<NT>  p( size_type(max + 1));
                            ...
                            const Polynomial_3<NT>& p2)
{
  typedef typename Polynomial_3<NT>::size_type size_type;
  CGAL_assertion(p1.degree()>=0 && p2.degree()>=0);
  Polynomial_3<NT>  p( size_type(p1.degree()+p2.degree()+1) ); 
...
template <class NT> 
Polynomial_3<NT> divop (const Polynomial_3<NT>& p1, 
                       const Polynomial_3<NT>& p2,
                       Tag_false)
{ CGAL_assertion(!p2.is_zero());
  if (p1.is_zero()) return 0;
  Polynomial_3<NT> q,r;
  Polynomial_3<NT>::euclidean_div(p1,p2,q,r);
...
template <class NT> 
Polynomial_3<NT> divop (const Polynomial_3<NT>& p1, const Polynomial_3<NT>& p2,
                       Tag_true)
{ CGAL_assertion(!p2.is_zero());
  if (p1.is_zero()) return 0;
  Polynomial_3<NT> q,r; NT D; 
  Polynomial_3<NT>::pseudo_div(p1,p2,q,r,D); 
  CGAL_postcondition( (p2*q+r==p1*Polynomial_3<NT>(D)) );
  return q/=D;
}


template <class NT> 
inline Polynomial_3<NT> 
gcd(const Polynomial_3<NT>& p1, const Polynomial_3<NT>& p2)
{ return Polynomial_3<NT>::gcd(p1,p2); }
...
// SPECIALIZING inline to :
  ...
  // lefthand side
  inline    Polynomial_3<int> operator + 
  (const int& num, const Polynomial_3<int>& p2)
  { return (Polynomial_3<int>(num) + p2); }
  inline    Polynomial_3<int> operator - 
  (const int& num, const Polynomial_3<int>& p2)
  { return (Polynomial_3<int>(num) - p2); }
  inline    Polynomial_3<int> operator * 
  ...
  // righthand side
  inline    Polynomial_3<int> operator + 
  (const Polynomial_3<int>& p1, const int& num)
  { return (p1 + Polynomial_3<int>(num)); }
  inline    Polynomial_3<int> operator - 
  (const Polynomial_3<int>& p1, const int& num)
  { return (p1 - Polynomial_3<int>(num)); }
  inline    Polynomial_3<int> operator * 
  ...
  (const int& num, const Polynomial_3<int>& p) 
  { return ( (Polynomial_3<int>(num)-p).sign() == CGAL::ZERO );}
  inline    bool operator != 
  (const int& num, const Polynomial_3<int>& p) 
  { return ( (Polynomial_3<int>(num)-p).sign() != CGAL::ZERO );}
  inline    bool operator <  
  (const int& num, const Polynomial_3<int>& p) 
  { return ( (Polynomial_3<int>(num)-p).sign() == CGAL::NEGATIVE );}
  inline    bool operator <=  
  (const int& num, const Polynomial_3<int>& p) 
  { return ( (Polynomial_3<int>(num)-p).sign() != CGAL::POSITIVE );}
  inline    bool operator >  
  (const int& num, const Polynomial_3<int>& p) 
  { return ( (Polynomial_3<int>(num)-p).sign() == CGAL::POSITIVE );}
  inline    bool operator >=  
  ...
  // righthand side
  inline    bool operator ==
  (const Polynomial_3<int>& p, const int& num) 
  { return ( (p-Polynomial_3<int>(num)).sign() == CGAL::ZERO );}
  inline    bool operator !=
  (const Polynomial_3<int>& p, const int& num) 
  { return ( (p-Polynomial_3<int>(num)).sign() != CGAL::ZERO );}
  inline    bool operator < 
  (const Polynomial_3<int>& p, const int& num) 
  { return ( (p-Polynomial_3<int>(num)).sign() == CGAL::NEGATIVE );}
  inline    bool operator <= 
  (const Polynomial_3<int>& p, const int& num) 
  { return ( (p-Polynomial_3<int>(num)).sign() != CGAL::POSITIVE );}
  inline    bool operator > 
  (const Polynomial_3<int>& p, const int& num) 
  { return ( (p-Polynomial_3<int>(num)).sign() == CGAL::POSITIVE );}
  inline    bool operator >=
...
// SPECIALIZING pure int params:
  ...
  // lefthand side
  template <class NT>    Polynomial_3<NT> operator + 
  (const int& num, const Polynomial_3<NT>& p2)
  { return (Polynomial_3<NT>(num) + p2); }
  template <class NT>    Polynomial_3<NT> operator - 
  (const int& num, const Polynomial_3<NT>& p2)
  { return (Polynomial_3<NT>(num) - p2); }
  template <class NT>    Polynomial_3<NT> operator * 
  ...
  // righthand side
  template <class NT>    Polynomial_3<NT> operator + 
  (const Polynomial_3<NT>& p1, const int& num)
  { return (p1 + Polynomial_3<NT>(num)); }
  template <class NT>    Polynomial_3<NT> operator - 
  (const Polynomial_3<NT>& p1, const int& num)
  { return (p1 - Polynomial_3<NT>(num)); }
  template <class NT>    Polynomial_3<NT> operator * 
  ...
  // lefthand side
  template <class NT>    bool operator ==  
  (const int& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() == CGAL::ZERO );}
  template <class NT>    bool operator != 
  (const int& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() != CGAL::ZERO );}
  template <class NT>    bool operator <  
  (const int& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() == CGAL::NEGATIVE );}
  template <class NT>    bool operator <=  
  (const int& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() != CGAL::POSITIVE );}
  template <class NT>    bool operator >  
  (const int& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() == CGAL::POSITIVE );}
  template <class NT>    bool operator >=  
  ...
  // righthand side
  template <class NT>    bool operator ==
  (const Polynomial_3<NT>& p, const int& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() == CGAL::ZERO );}
  template <class NT>    bool operator !=
  (const Polynomial_3<NT>& p, const int& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() != CGAL::ZERO );}
  template <class NT>    bool operator < 
  (const Polynomial_3<NT>& p, const int& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() == CGAL::NEGATIVE );}
  template <class NT>    bool operator <= 
  (const Polynomial_3<NT>& p, const int& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() != CGAL::POSITIVE );}
  template <class NT>    bool operator > 
  (const Polynomial_3<NT>& p, const int& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() == CGAL::POSITIVE );}
  template <class NT>    bool operator >=
...
// SPECIALIZING inline to :
  ...
  // lefthand side
  inline    Polynomial_3<double> operator + 
  (const double& num, const Polynomial_3<double>& p2)
  { return (Polynomial_3<double>(num) + p2); }
  inline    Polynomial_3<double> operator - 
  (const double& num, const Polynomial_3<double>& p2)
  { return (Polynomial_3<double>(num) - p2); }
  inline    Polynomial_3<double> operator * 
  ...
  // righthand side
  inline    Polynomial_3<double> operator + 
  (const Polynomial_3<double>& p1, const double& num)
  { return (p1 + Polynomial_3<double>(num)); }
  inline    Polynomial_3<double> operator - 
  (const Polynomial_3<double>& p1, const double& num)
  { return (p1 - Polynomial_3<double>(num)); }
  inline    Polynomial_3<double> operator * 
  ...
  // lefthand side
  inline    bool operator ==  
  (const double& num, const Polynomial_3<double>& p) 
  { return ( (Polynomial_3<double>(num)-p).sign() == CGAL::ZERO );}
  inline    bool operator != 
  (const double& num, const Polynomial_3<double>& p) 
  { return ( (Polynomial_3<double>(num)-p).sign() != CGAL::ZERO );}
  inline    bool operator <  
  (const double& num, const Polynomial_3<double>& p) 
  { return ( (Polynomial_3<double>(num)-p).sign() == CGAL::NEGATIVE );}
  inline    bool operator <=  
  (const double& num, const Polynomial_3<double>& p) 
  { return ( (Polynomial_3<double>(num)-p).sign() != CGAL::POSITIVE );}
  inline    bool operator >  
  (const double& num, const Polynomial_3<double>& p) 
  { return ( (Polynomial_3<double>(num)-p).sign() == CGAL::POSITIVE );}
  inline    bool operator >=  
  ...
  // righthand side
  inline    bool operator ==
  (const Polynomial_3<double>& p, const double& num) 
  { return ( (p-Polynomial_3<double>(num)).sign() == CGAL::ZERO );}
  inline    bool operator !=
  (const Polynomial_3<double>& p, const double& num) 
  { return ( (p-Polynomial_3<double>(num)).sign() != CGAL::ZERO );}
  inline    bool operator < 
  (const Polynomial_3<double>& p, const double& num) 
  { return ( (p-Polynomial_3<double>(num)).sign() == CGAL::NEGATIVE );}
  inline    bool operator <= 
  (const Polynomial_3<double>& p, const double& num) 
  { return ( (p-Polynomial_3<double>(num)).sign() != CGAL::POSITIVE );}
  inline    bool operator > 
  (const Polynomial_3<double>& p, const double& num) 
  { return ( (p-Polynomial_3<double>(num)).sign() == CGAL::POSITIVE );}
  inline    bool operator >=
...
// SPECIALIZING pure double params:
  ...
  // lefthand side
  template <class NT>    Polynomial_3<NT> operator + 
  (const double& num, const Polynomial_3<NT>& p2)
  { return (Polynomial_3<NT>(num) + p2); }
  template <class NT>    Polynomial_3<NT> operator - 
  (const double& num, const Polynomial_3<NT>& p2)
  { return (Polynomial_3<NT>(num) - p2); }
  ...
  // righthand side
  template <class NT>    Polynomial_3<NT> operator + 
  (const Polynomial_3<NT>& p1, const double& num)
  { return (p1 + Polynomial_3<NT>(num)); }
  template <class NT>    Polynomial_3<NT> operator - 
  (const Polynomial_3<NT>& p1, const double& num)
  { return (p1 - Polynomial_3<NT>(num)); }
  ...
  // lefthand side
  template <class NT>    bool operator ==  
  (const double& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() == CGAL::ZERO );}
  template <class NT>    bool operator != 
  (const double& num, const Polynomial_3<NT>& p) 
  { return ( (Polynomial_3<NT>(num)-p).sign() != CGAL::ZERO );}
  ...
  // righthand side
  template <class NT>    bool operator ==
  (const Polynomial_3<NT>& p, const double& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() == CGAL::ZERO );}
  template <class NT>    bool operator !=
  (const Polynomial_3<NT>& p, const double& num) 
  { return ( (p-Polynomial_3<NT>(num)).sign() != CGAL::ZERO );}
...
{ 
  ...
  { 
    case CGAL::IO::ASCII : 
      is >> d;
      if (d < 0) p = Polynomial_3<NT>();
      else {
        typename Polynomial_3<NT>::Vector coeffs(d+1);
        for(i=0; i<=d; ++i) is >> coeffs[i];
        p = Polynomial_3<NT>(coeffs.begin(),coeffs.end() ); // af removed SNIINST
      }
      break;
    case CGAL::IO::BINARY :
      CGAL::read(is, d);
...
// SPECIALIZE_FUNCTION ORIGINAL
template <class NT> 
void Polynomial_3<NT>::euclidean_div(
  const Polynomial_3<NT>& f, const Polynomial_3<NT>& g,
  Polynomial_3<NT>& q, Polynomial_3<NT>& r)
{
  r = f; r.copy_on_write();
  int rd=r.degree(), gd=g.degree(), qd;
  if ( rd < gd ) { q = Polynomial_3<NT>(NT(0)); }
  else { qd = rd-gd+1; q = Polynomial_3<NT>(std::size_t(qd)); }
  while ( rd >= gd ) {
    NT S = r[rd] / g[gd];
    qd = rd-gd;
    q.coeff(qd) += S;
...
template <class NT> 
void Polynomial_3<NT>::pseudo_div(
  const Polynomial_3<NT>& f, const Polynomial_3<NT>& g, 
  Polynomial_3<NT>& q, Polynomial_3<NT>& r, NT& D)
{
  ...
  if ( fd<gd ) 
  { q = Polynomial_3<NT>(0); r = f; D = 1; 
    CGAL_postcondition(Polynomial_3<NT>(D)*f==q*g+r); return; 
  }
  // now we know fd >= gd and f>=g
  int qd=fd-gd, delta=qd+1, rd=fd;
  { q = Polynomial_3<NT>( std::size_t(delta) ); }; // workaround for SunPRO
  NT G = g[gd]; // highest order coeff of g
  D = G; while (--delta) D*=G; // D = G^delta
  Polynomial_3<NT> res = Polynomial_3<NT>(D)*f;
  ...
  r = res;
  CGAL_postcondition(Polynomial_3<NT>(D)*f==q*g+r);
  TRACEN("  returning "<<q<<", "<<r<<", "<< D);
}


template <class NT> 
Polynomial_3<NT> Polynomial_3<NT>::gcd(
  const Polynomial_3<NT>& p1, const Polynomial_3<NT>& p2)
{ TRACEN("gcd("<<p1<<" , "<<p2<<")");
  if ( p1.is_zero() )
    if ( p2.is_zero() ) return Polynomial_3<NT>(NT(1));
    else return p2.abs();
  if ( p2.is_zero() )
    return p1.abs();

  Polynomial_3<NT> f1 = p1.abs();
  Polynomial_3<NT> f2 = p2.abs();
  NT f1c = f1.content(), f2c = f2.content();
  f1 /= f1c; f2 /= f2c;
  NT F = CGAL_NTS gcd(f1c,f2c);
  Polynomial_3<NT> q,r; NT M=1,D;
  bool first = true;
  while ( ! f2.is_zero() ) { 
    Polynomial_3<NT>::pseudo_div(f1,f2,q,r,D);
    if (!first) M*=D;
  ...
  TRACEV(f1.content());
  return Polynomial_3<NT>(F)*f1.abs();
}
