...
void transform_big(Nef_polyhedron& N,int n, int s) {
  ...
  RT y_min = vi->point().hy()/vi->point().hw();
  RT y_max = vi->point().hy()/vi->point().hw();
  RT z_min = vi->point().hz()/vi->point().hw();
  RT z_max = vi->point().hz()/vi->point().hw();
...
int main(int argc, char* argv[]) {
  ...
  tgen t1(out1);
  t1.create_tetrahedra(n,n,1,RT(s));
