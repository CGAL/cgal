...
triangulation and the other is its generalization, remplacing the natural
distance by the power to weighted points. The metric used determines the
underlying triangulation of the alpha shape and the version computed, in one
hand, the classic Alpha Shapes associated with the Delaunay triangulations
(cf. \ref{I1_Sect_Delaunay}), on the other hand, the weighted Alpha Shapes
...
simplex that does not contain any other point of $S$, for the metric used for
the computation of the underlying triangulation.  The corresponding
$\alpha$-shape is its its frontier and the underlying space correspondant. 
...
\section{General Alpha-Shape of Points in a Plane \label{I1_SectAlpha_Shape_2}}
...
$O(\mbox{ \em n } \log{\mbox{ \em n } })$, $n$ the number of points.
...
\section*{For the Triangulation Data Structure Class}
...
\subsection{Requirements for the \protect \ccc{Vertex_base} Classof an Alpha-Shape\label{I1_SectVertex}}
...
\subsection*{The underlying Triangulation}
...
\ccInclude{CGAL/Alpha_euclidean_traits_2.h}
...
\ccc{Df}, which can be instanciated with \ccc{Triangulation_face_base_2<Gt>}.
...
for the Alpha Shape. The class
\ccc{Alpha_shape_face_base_2<Gt,Df>} simply derived from 
\ccc{Df}, which can be instanciated with \ccc{Regular_triangulation_face_base_2<Gt>}.
...
\section{Additionnal functionnalities for weighted Alpha-Shape\label{I2_SectWeighted_Alpha_Shape_2}}

\begin{ccClassTemplate} {Weighted_alpha_shape_2<Rt>}

\ccDefinition
Specially for the weighted version, the computation of alpha shape often
necessite an estimation of weights for the sample of points. This class
\ccClassTemplateName\ provides some heuristic methods to initialize these
weights. They can deal with samples of points extract from a contour in a plane.
The main idea is to control an union of sphere, dual of the weighted alpha shape, which
preserve the skeletton of the original contour. The best results are in
\ccc{GENERAL} mode, with $\alpha=0$.
...
{initialize the weight of each point, stored in a container of points, with the
distance to his nearest voronoi vertex.}
...
Input_iterator last);}
{initialize the weight of each point, stored in a container of points, with the
distance to his nearest voronoi edge.}
...
Input_iterator  last);}
{initialize the weight of each point, stored in a container of points, with the
distance to his nearest vertex.} 
