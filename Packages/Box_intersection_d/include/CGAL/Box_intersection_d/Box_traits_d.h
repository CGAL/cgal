...
struct Box_traits_d {
    ...
    typedef typename Box_::Id_type Id_type;
...
struct Predicate_traits_d : public BoxTraits {
    ...
    public:
        ...
        bool operator()(Box a, Box b) const { return is_lo_less_lo(a,b,dim); }
    };
    ...
    public:
        ...
        bool operator() (Box box) const { return min_coord(box, dim) < value; }
    };
    ...
    static bool is_lo_less_lo(Box a, Box b, int dim) {
        return min_coord(a,dim)  < min_coord(b,dim) ||
               min_coord(a,dim) == min_coord(b,dim) && id(a) < id(b);
    }

    static bool is_lo_less_hi(Box a, Box b, int dim) {
        return hi_greater( max_coord(b,dim), min_coord(a,dim ));
    }
    static bool does_intersect (Box a, Box b, int dim) {
        return is_lo_less_hi(a,b,dim) && is_lo_less_hi(b,a,dim);
    }
    static bool contains_lo_point(Box a, Box b, int dim) {
