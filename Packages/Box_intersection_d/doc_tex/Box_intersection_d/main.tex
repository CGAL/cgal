...
\ccc{CGAL::Box_intersection_d::box_limits<NT>} traits class that
determines \ccc{inf()} and \ccc{sup()} values for initializing the
extreme ends of intervals. (This is probably subject to a better integration
into \cgal.)

In addition, a box has an \ccc{id}-number that is distinct for all
...
of equal \ccc{id}-number are not reported as intersecting pairs since
...
and the type \ccc{Id_type} for the \ccc{id}-number. Member functions
...
nutshell, we have to define the type \ccc{NT} for the box coordinates,
the type \ccc{Id_type} for the \ccc{id}-number, and the type \ccc{Box}
