...
nutshell, we have to define the type \ccc{NT} for the box coordinates,
the type \ccc{ID} for the \ccc{id}-number, and the type \ccc{Box_handle}
for the box handle, here \ccc{Point_3*} since we work with the pointers.
...
original C implementation of Afra Zomoridian, which helped clarifying
some questions, and we are greatful to the help of Afra Zomorodian in
...
however, we learned to late about this implementation.
