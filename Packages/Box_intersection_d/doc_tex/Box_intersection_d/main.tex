...
are proximity quesries and distance computations among such surfaces,
see ~\cite{cgal:ze-fsbi-02} for details.
...
selected with the two possible values for the \ccc{topology} parameter:
...
In addition, a box has an unique \ccc{id}-number. It is used to
distinguish between otherwise geometrically identical boxes and allows
us to guarantee that we report a pair of intersecting boxes only once,
even in the presence of geometrically equal boxes. Boxes
of equal \ccc{id}-number are not reported as intersecting since
...
assume that they are models of the \ccc{Assignable} concept. The
...
class of the \ccc{BoxIntersectionTraits_d} concept. A default traits
class is provided that works under the assumption that the box type is
a model of the \ccc{BoxIntersectionBox_d} concept, or a pointer to a
...
policy class \cite{cgal:a-mcdgp-01} for the \ccc{id}-number as
template parameter. 

So the full function signatures for the bipartite case look as
follows. The signatures for the complete case with
\ccc{box_self_intersection} function look the same except for the
...
example to create easily nine two-dimensional \ccc{boxes} in a grid layout
of $3 \times 3$ boxes. Additionally we pick the center box and the box
in the upper-right corner as our second box sequence \ccc{query}. 
...
call. The result will be that the first \ccc{query} box intersects all
nine \ccc{boxes} and the second \ccc{query} box intersects the four
boxes in the upper-right quadrant. See Section
\ref{sec:box-inters-params} for the change of the \ccc{topology}
...
\ccc{Box_with_handle_d<double,3,Iterator>} that also works nicely with
the \cgal\ bounding boxes of type \ccc{CGAL::Bbox_3}. In addition, each
...
and add an additional vector \ccc{ptr} storing pointers to the bounding
...
\ccc{BoxIntersectionBox_d} concept, which allows us to rely on the
...
The requirements for the box implementation are best looked at on
page~\ccRefPage{BoxIntersectionBox_d} in the Reference Manual. In a
...
page~\ccRefPage{BoxIntersectionTraits_d} in the Reference Manual. In a
nutshell, we have to define the type \ccc{NT} for the box coordinates,
the type \ccc{ID} for the \ccc{id}-number, and the type \ccc{Box}
for the box type, here \ccc{Point_3*} since we work with the pointers.
...
The callback function \ccc{report} computes than the Euclidean
