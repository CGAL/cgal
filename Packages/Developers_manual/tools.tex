...
-r Triangulation_module -s /cygdrive/d/CGALCVS/Scripts/scripts/
\end{verbatim}


The {\tt -a} parameter is used to specify the directory that contains the packages. The {\tt -p} parameter is used to specify the file containing the packages list. The parameter {\tt -r} is used to specify the module name. For the moment, the module name is also the module version and will go in the version.h and version.tex as the CGAL\_VERSION macro. This will be probably changed in the future.  The {\tt -s} parameter is used to specify the directory containing the scripts create\_makefile, and all the other scripts that CGAL made public for users. This is not the package Scripts, but the scripts directory in the Scripts package.  
