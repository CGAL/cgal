...
x^2 + s y^2 + t xy + u x + v y + w = 0$, where the coefficients
$\langle r, s, t, u, v, w \rangle$ completely characterize the
curve. The sign of the expression $\Delta_{C} = 4 r s - t^2$
...
t, u, v, w \rangle$ are rational. When dealing with linear
curves(line segments and polylines), similar assumptions guarantee
that all intersection points also have rational coordinates, such
that the arrangement of such curves can be constructed and
maintained using only rational arithmetic. Unfortunately, this
does not hold for conic curves, as the coordinates of intersection
points of two conic curves with rational coefficients are in
general algebraic numbers of degree $4$.\footnote{Namely, they are
roots of polynomials with integer coefficients of degree $4$.
However, in some special cases, for example when handling only
circles and circular arcs, the coordinates of the intersection
points are only of degree $2$, namely they are solutions of
quadratic equations.} In addition, conic arcs may not necessarily
be $x$-monotone, and must be split at points where the tangent to
the arc is vertical. Such points typically have coordinates that
...
and point with vertical tangents are likely to have algebraic
...
coefficients. Typically it is one of the standard \cgal\ kernels,
...
algebraic coordinates. Typically it is one of the standard \cgal\
kernels, instantiated with the number type
...
circles. We then look for the vertex of maximal degree, which is
the vertex associated with the point $(4,3)$, as all three circles
intersect at this point and it has six incident edges:
...
constructors for conic arcs. Especially note the constructor of a
...
supporting curves of the two arcs, the unit circle $x^2 + y^2 =
1$, and the hyperbola $y = \frac{x^2}{1+x}$ (RWRW: Check this)
intersect at the origin such that the intersection point has
multiplicity $3$ (note that they both have the same horizontal
tangent at $(0,0)$ and the same curvature $1$). In addtion, they
have another intersection point at $RWRW: (x?,y?)$ of multiplicity
$1$:

% RWRW: Here comes example15. (the former example6.C)
...
degrees. In particual, if $Q(x) = 1$ then the function is a simple
polynomial function. A bounded {\em rational arc} is defined by
the graph of a rational function over some internal $[x_{\rm min},
x_{\rm max}]$, where $Q$ does not have any real roots in this
interval (thus the arc does not contain any poles). Rational
...
approximating or interpolating more complicated curves --- see,
e.g.,~\cite[Chapeter~3]{XXX} (chap. 3 of Numerical Recipes in
C++).
...
Using the \ccc{Arr_rational_traits_2<RatKernel, AlgKernel,
NtTraits>} class template it is possible to construct and maintain
arrangement of rational arcs. The template parameters are very
similar to the ones used by the \ccc{Arr_conic_traits_2} class
template --- see the previous section. Also in this case it is
recommended to use the \ccc{CORE_number_type_traits} class, with a
kernels templated by its \ccc{Rational} and \ccc{Algberaic} types.
...
a rational arc is always $x$-monotone). A rational arc can be
constructed from a single vector of coefficients, specifying the
polynomial $P$ alone (and $Q(x) = 1$), or from two vectors of
coefficients, specifying both $P$ and $Q$.

The following example demonstrates the constrcution of an
arrangement of rational arcs:

% RWRW: Here comes example16. (the former example10.C ??)
...
themselves from the base-traits class and in particular inherit it
\ccc{Point_2} type. In addition:
\begin{itemize}
...
Users can create a curve (or an $x$-monotone curve) from a basic
curve and a data object. When curves are inserted to an
arrangement, they may be split, and the meta-traits class handles
their data fields automatically:
\begin{itemize}
...
curve-data traits classes is the way they handle this overlapping
curves:
\begin{itemize}
...
the arrangement of colored segments, we detect the vertices that
have both incident blue edges and incident red edges, thus they
correspond to red--blue intersection points. We also locate the
edges that correspond to overlaps between red and blue line
segments:

% RWRW: Here comes example17.  (the former example8.C ??)
...
one of the edges with a geometrically equivalent curve with a
different data field:

% RWRW: Here comes example18.
...
happens inside an arrangement instance. For example, when we
insert a new curve into an arrangement we may want to keep track
of the faces that are split due to this insertion operation. The
various point-location strategies that require auxiliary data
structures (see Section~\ref{arr_ssec:pl}) must be notifies on
various local changes in the arrangement in order to keep their
data structures up-to-date. The arrangement package offers a
mechanism that uses {\em observers} that can be attached to an
arrangement instance and receive notifications about the changes
this arrangement goes through.
...
Concrete arrangement observer classes should inherit from the
...
to the insertion of an edge, and whenever two face merge into one
due to the deletion of an edge:

% RWRW: Here comes example19. (the former example2.C)

Observers are especially useful when the \dcel\ records are
extended and store additional data, as they help updating this
data on-line. See Section~\ref{arr_sec:???} for more details and
examples.
