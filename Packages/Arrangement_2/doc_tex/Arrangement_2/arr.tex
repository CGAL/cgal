...
%=======================================
%
\boost\footnote{{\tt www.boost.org}.} is a collection of portable \Cpp\
...
\section{The Primal Arrangement Representation}
...
arrangement $(1,1)$. Note the usage of the \ccc{Arr_vertex_index_map} and
the \ccc{Arr_vertex_property_map} classes. The latter one, instantiated by
the type \ccc{double} is used to map vertices to their distances from
the vertex $(1,1)$.

\ccIncludeExampleCode{../examples/Arrangement_2/ex_bgl_primal_adapter.C}

\section{The Dual Arrangement Representation}
...
In dual represnetation, \ccc{Arrangement_2::Face_handle}
is the graph-vertex type, while \ccc{Arrangement_2::Halfedge_handle} is the
...
example \ccc{ex_bgl_primal_adapter.C} and perform breadth-first search on
the graph faces, starting from the unbounded face. As a result we obtain
the distance of each face from the unbounded face:
