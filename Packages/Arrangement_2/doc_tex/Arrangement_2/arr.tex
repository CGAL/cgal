...
incremental insertion process. Naturally, this tradeoff is irrelevant
...
decrators for geometric traits classes distributed with \cgal,
which extend other geometic traits-class by attaching auxiliary
...
The arrangement package includes to predefined traits-class decorator
templates, named \ccc{Arr_consolidated_curve_data_traits_2<BaseTraits, Data>}
and \ccc{Arr_merged_curve_data_traits_2<BaseTraits, Data, MergeFunc>}.
They are used to attach a data field to curves. Both templates
are parameterized by a base-traits class, which is one of the
geometric traits classes described in the previous subsections, or
a user-defined traits class. The decorator derives itself from the base-traits
class, and in particular inherits it \ccc{Point_2} type. In addition:
\begin{itemize}
...
\ccc{BaseTraits::X_monotone_curve_2} class, adding a container of
...
\ccc{Arrangement_with_history_2::Curve_with_edges_2}, which extends the 
...
are therefore not available for arrangements
with history and only the global \ccc{insert_curve()} and 
\ccc{insert_curves()} functions (the incremental
...
\ccc{Arrangement_2} object \ccc{arr} does not have a return value,
