...
the \ccc{ArrangementPointLocation} concept --- namely, they all
...
\ccc{ArrangementPointLocation} concept) and a query point, and
...
Mulmuley's point-location algorithm~\cite{m-fppa-90}. The
...
the \ccc{ArrangementVerticalRayShoot} concept. That is, they all
...
\ccc{ArrangementVerticalRayShoot} concept and prints out the
...
\ccc{ArrangementPointLocation} concept).} The insertion operation
...
which serves as its \ccc{Curve_2} and \ccc{X_monotone_curve_2}
types (the two are identical). Polyline curve objects can be
constructed from any range of points.
...
algebraic coordinates. Typically we use one of the standard \cgal\
...
one of the edges with a geometrically equivalent curve, but with a
