...
\item The tuple $\langle C, p_s, p_t, o \rangle$, where $C$ is the supporting
      ...
      counterclockwise direction.\footnote{In degenerate situation, $C$ may
      correspond to a pair of line. In this case $o$ may specify a
      \ccc{COLLINEAR} orientation.}
\end{itemize}
...
points) are algebraic numbers of degree $4$.\footnote{A real number $\alpha$
...
operators are provided for conic arcs. In addition, an \ccc{operator<<}
...
Note that the \ccc{X_monotone_curve_2} is mainly used internally by the
...
by external users. If necessary, users may use the \ccc{Make_x_monotone_2}
