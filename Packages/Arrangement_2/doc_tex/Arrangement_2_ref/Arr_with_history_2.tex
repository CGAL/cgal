...
\ccNestedType{Curve_handle}{a handle for an input curve (actually to an
       extended \ccc{Curve_with_edges_2} object --- see below).}
\ccGlue
\ccNestedType{Curve_iterator}{a bidirectional iterator over the
       curves that induce the arrangement. Its value-type is
       \ccc{Curve_with_edges_2} (see below).}

\ccNestedType{Curve_edge_iterator}
       {an iterator over the edges induced by an input curve.
        Its value type is \ccc{Halfedge_handle}.}
\ccGlue
\ccNestedType{Curve_edge_const_iterator}
       {a \ccc{const} iterator over the edges induced by an input curve.
	Its value type is \ccc{Halfedge_const_handle}.}
...
\ccGlue
\ccNestedType{Originating_curve_const_iterator}
    {a \ccc{const} iterator of the curves that originate a given arrangement
     edge. Its value type is \ccc{Cruve_const_handle}.}
...
\ccMethod{Originating_curve_iterator 
          originating_curves_begin (Halfedge_handle e);}
    {returns the begin-iterator of the curves originating the edge \ccc{e}.}
\ccGlue
\ccMethod{Originating_curve_iterator 
          originating_curves_end (Halfedge_handle e);}
...
\subsection*{Class 
        Arrangement\_with\_history\_2$<$Traits,Dcel$>$::Curve\_with\_edges\_2}
%=============================================================================

The \ccc{Curve_with_edges_2} nested class extends the \ccc{Curve_2} type
by storing pointers to the edges the curve induces in the arrangement.

\begin{ccClass}{Arrangement_with_history_2<Traits,Dcel>::Curve_with_edges_2}
...
\ccSeeAlso
    ...
    \ccc{insertion functions}\lcTex{ 
      (\ccRefPage{CGAL::insert (Arrangement_with_history_2)})} \\
    \ccc{removal functions}\lcTex{ 
      (\ccRefPage{CGAL::remove (Arrangement_with_history_2)})} \\
    \ccc{overlaying arrangements}\lcTex{ 
      (\ccRefPage{CGAL::overlay (Arrangement_with_history_2)})}
