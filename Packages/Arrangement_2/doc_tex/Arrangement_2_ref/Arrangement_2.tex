...
\ccMethod{bool is_valid() const;}
  {returns \ccc{true} if \ccVar\ represents a valid instance of 
   \ccClassTemplateName. In particular, the functions checks the
...
\ccSeeAlso
    ...
    \ccc{checking validity}\lcTex{ 
      (\ccRefPage{CGAL::is_valid})} \\
    ...
    \ccc{removal functions}\lcTex{ 
      (\ccRefPage{CGAL::remove_edge}, \ccRefPage{CGAL::remove_vertex})} \\
    \ccc{overlaying arrangements}\lcTex{ 
      (\ccRefPage{CGAL::overlay})} \\
    \ccc{input/output functions}\lcTex{ 
      (\ccRefPage{CGAL::read},\ccRefPage{CGAL::write})} \\
