...
\ccRefName\ uses \ccc{Kernel::Point_2} as its point type. However, it
does {\sl not} use \ccc{Kernel::Segment_2} as its curve type, as one can
...
nested \ccc{X_monotone_curve_2} and \ccc{Curve_2} types (in this case both
types refer to the same class, as {\sl every} line segement is (weakly)
$x$-monotone) are however fully convertable to the \ccc{Kernel::Segment_2}.
