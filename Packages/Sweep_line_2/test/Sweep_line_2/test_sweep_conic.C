...
public:
  ...
  void ReadCurve(std::ifstream & is, Curve_2 & cv)
  {
    ...
    {  
      ...
      {
	is_circle = true;
	circle = Int_circle_2 (Int_point_2 (CfNT(x0), CfNT(y0)),
			       CfNT(a*b));
      }
      else
      {
	...
	u = CfNT(-2*x0*b_sq);
	v = CfNT(-2*y0*a_sq);
	w = CfNT(x0*x0*b_sq + y0*y0*a_sq - a_sq*b_sq);
      }
    ...
    else if (type == 's' || type == 'S')
    {
      ...
      cv = Curve_2(Int_segment_2 (source, target));
    }
