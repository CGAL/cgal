...
\begin{ccRefFunction}[CGAL::]{sweep_to_construct_planar_map_2}  %% add template arg's if necessary
...
holding the resulting \ccc{Planar map}, if \ccStyle{result} holds a
non empty \ccc {Planar map} before calling to
\ccc{sweep_to_construct_planar_map} - then \ccStyle{result} will be
...
mentioned above. This function is also updating the \ccc{Planar map}
...
\ccc{Planar map}, even when there are no interior intersectrions
