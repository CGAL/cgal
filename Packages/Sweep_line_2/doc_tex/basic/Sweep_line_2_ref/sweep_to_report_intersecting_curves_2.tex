% +------------------------------------------------------------------------+
% | Reference manual page: sweep_to_report_intersecting_curves_2.tex
% +------------------------------------------------------------------------+
% | 17.02.2002   Author
...
\begin{ccRefFunction}{sweep_to_report_intersecting_curves_2}  %% add template arg's if necessary
...
portions using the \ccc{make_x_monotone} function of
\ccc{SweepLineTraits_2}. 
The spliting points are treated as new endpoints rather than 
intersection points.

\ccInclude{sweep_to_report_intersecting_curves_2.h}

\ccFunction{
   template <class InputIterator, class SweepLineTraits_2, class OutputIterator>
   void  
   sweep_to_report_intersecting_curves_2(InputIterator curves_begin, 
  ...
  {Reports all subsets of curves meeting in a common point.
   ...
   The parameter \ccStyle{intersecting_curves} is an inserter iterator which 
   will eventually point to all the reported subsets as above. 
   \ccc{sweep_to_report_intersecting_curves_2} supports
...
\begin{enumerate}
   ...
   \ccc{pair<Traits::Point_2, list<Traits::X_curve_2>}
\end{enumerate}
...
\ccIncludeExampleCode{Sweep_line_2/example7.C}

And the output of the program looks like this:

\ccIncludeExampleCode{Sweep_line_2/example7.cout}
