...
(\ccc{Min_circle_2<Traits>}) is constructed from points 
...
(\ccc{Min_sphere_d<Traits>}) and spheres for spheres 
(\ccc{Min_sphere_of_spheres_d<Traits>}), there
...
In addition to computing the smallest sphere enclosing a set
of points, \cgal\ offers a class \ccc{Min_sphere_of_spheres_d<Traits>}
which computes the smallest sphere enclosing a set of spheres.
...
Given a set of points in the plane, the class \ccc{Largest_empty_iso_rectangle_2<T>}
