...
protected:
  ...
  typedef typename Planar_map_with_intersections_2<Planar_map>
                                                        Pmwx;
  ...
  typedef CGAL::Map_overlay_2<Subdivision,MapOverlay_change_notification>
                                                                MapOverlay; 
...
protected:
                                               ...
                                               const MapOverlay& map_overlay)
  {
    ...
    for (v_iter = pm1.vertices_begin(); v_iter != pm1.vertices_end(); ++v_iter)
    {
      ...
      Halfedge_const_handle h =
          map_overlay.subdivision().locate(v_iter->point(), lt);
                                                     ...
                                                     map_overlay)
  {
      ...
      for (Halfedge_const_handle h_iter2 = pm2.halfedges_begin(); 
           h_iter2 != pm2.halfedges_end(); ++h_iter2, ++h_iter2)
        {
          ...
          for (typename std::list<Point>::iterator p_iter = points.begin(); 
            ...
            Locate_type lt;
            Halfedge_const_handle h =
                map_overlay.subdivision().locate(*p_iter, lt);
            CGAL_assertion(lt == Subdivision::VERTEX);
    ...
    std::cout << "Check_that_intersections_are_in_map_overlay -- passed"
              << std::endl;
  }
                                                ...
                                                const MapOverlay& map_overlay)
  {
    ...
    for (Halfedge_const_iterator h_iter = pm.halfedges_begin();
         h_iter != pm.halfedges_end(); ++h_iter, ++h_iter)
      {
        ...
        Point t = rightmost(h_iter->source()->point(),
                            h_iter->target()->point());
        ...
        while (!finish_traversal)
          {
            ...
            Halfedge_around_vertex_const_circulator halfedge =
            ...
            do {
#ifdef OVL_DEBUG_TEST
              std::cout << "walking around vertex: "<< halfedge->curve()
                        << std::endl;
#endif
              // finding the first halfedge which is part of h_iter->curve()
              // and continues from the right of vertex->point().
            ...
            CGAL_assertion(CGAL::compare_lexicographically_xy(
                                 rightmost(halfedge->source()->point(),
                                           halfedge->target()->point()), 
                                 vertex->point()) == CGAL::LARGER &&
                           traits.curves_overlap(halfedge->curve(),
                                                 h_iter->curve()));
            
            // marking the corresponding
            halfedges[halfedge.operator->()] = halfedge;          
            // halfedge from the overlay.
            halfedges[halfedge->twin().operator->()] = halfedge->twin();

            // the intersection points: p1=s and p2 is the next intersection 
            Point p1,p2;
            // point on h_iter->curve();
            CGAL_assertion(traits.nearest_intersection_to_right
                           (halfedge->curve(),
            ...
            CGAL_assertion(CGAL::compare_lexicographically_xy(s,next_point) ==
                           CGAL::SMALLER);

            // reaching the right endpoint of h_iter->curve().
            if (next_point == t)
  ...
  {
    const MapOverlay_change_notification * notifier =
        map_overlay.change_notification();
    //Pm_file_writer<Subdivision>  pm_writer1(std::cout, pm1);
    //Pm_file_writer<Subdivision>  pm_writer2(std::cout, pm2);
    //Pm_file_writer<Subdivision>  pm_writer(std::cout,
    // map_overlay.subdivision());
    ...
    for (Face_const_iterator f_iter = map_overlay.subdivision().faces_begin(); 
         f_iter != map_overlay.subdivision().faces_end(); ++f_iter)
      {
        ...
        Face_const_handle  face_creator1 =
            notifier->get_first_face_above(f_iter);
        Face_const_handle  face_creator2 =
            notifier->get_second_face_above(f_iter);
        ...
        // asserting that all halfedges along the holes of f_iter points to the
        // same faces.
        ...
        if (!f_iter->is_unbounded()){
          ...
          do {
            ...
            CGAL_assertion(notifier->get_first_face_above(face_halfedge) ==
                           face_creator1);
            CGAL_assertion(notifier->get_second_face_above(face_halfedge) ==
                           face_creator2);
            // a queue to hold all faces involed with hit.
  ...
  {
    // Checking vertices.
    for (Vertex_const_iterator v_iter =
    ...
    // Checking halfedges.
    for (Halfedge_const_iterator h_iter =
             map_overlay.subdivision().halfedges_begin();
    ...
    for (f_iter = map_overlay.first_creator().subdivision().faces_begin();
         f_iter != map_overlay.first_creator().subdivision().faces_end(); +
             +f_iter)
      CGAL_assertion(faces.find(&*f_iter) != faces.end());

    for (f_iter = map_overlay.second_creator().subdivision().faces_begin();
         f_iter != map_overlay.second_creator().subdivision().faces_end();
         ++f_iter)
      CGAL_assertion(faces.find(&*f_iter) != faces.end());
...
public:
  void start(char * filename,
             CGAL::Map_overlay_base<Subdivision,
             MapOverlay_change_notification>* ovl_alg)
  {
    ...
    check_that_vertices_are_in_map_overlay(subdivision1, subdivision2,
                                           map_overlay);
    
     // Check that intersections vertices are indeed in the map overlay
    check_that_intersections_are_in_map_overlay(subdivision1, subdivision2,
                                                map_overlay);

    // Check that halfedges are indeed in the map overlay.
    check_that_halfedges_are_in_map_overlay(subdivision1, subdivision2,
                                            map_overlay);
