...
template <class Curve_iterator_, class Traits_, class Point_plus_, class X_curve_plus_>
...
protected:
  ...
  public:
    ...
    typedef std::vector<Point_plus>             Points_container; //substituting list with In_place_list for optimization.
    ...
    {
      // first at all comparing pointers to accelarate this operatiot.
      if (this == &rep)
        return true;
  ...
  public:
    ...
    Curve_node() : Handle_for_Curve_node_rep() {
#ifdef CGAL_MAKE_PROFILING
      std::cout<<"allocating handle for DEFAULT Curve_node_rep"<<endl;
#endif
    }
    
    Curve_node(const X_curve_plus& cv) : Handle_for_Curve_node_rep(Curve_node_rep(cv)) {
#ifdef CGAL_MAKE_PROFILING
      std::cout<<"allocating handle for Curve_node_rep(cv)" << cv << endl;
#endif
      
      //PTR = allocator.allocate(1);
      //allocator.construct(ptr(), Curve_node_rep(cv));
      
      //PTR = new Curve_node_rep(cv);
    }
    
    Curve_node(const X_curve_plus& cv, const Point& p) : Handle_for_Curve_node_rep(Curve_node_rep(cv,p)) {
#ifdef CGAL_MAKE_PROFILING
      std::cout<<"allocating handle for Curve_node_rep(cv,p)" << cv <<" "<< p << endl;
#endif
      //PTR = allocator.allocate(1);
      //allocator.construct(ptr(), Curve_node_rep(cv, p));

      //PTR = new Curve_node_rep(cv, p);
    }

    Curve_node(const X_curve_plus& cv, const Point_plus& p) : Handle_for_Curve_node_rep(Curve_node_rep(cv,p)){
#ifdef CGAL_MAKE_PROFILING
      std::cout<<"allocating handle for Curve_node_rep(cv,p_plus)" << cv <<" "<< p.point() << endl;
#endif
      //PTR = allocator.allocate(1);
      //allocator.construct(ptr(), Curve_node_rep(cv, p));
    ...
    Curve_node(const Curve_node& cv_node) : Handle_for_Curve_node_rep(cv_node) {
#ifdef CGAL_MAKE_PROFILING
      std::cout<<"allocating handle for cv_node" << endl;
#endif
    }

    ~Curve_node() { //cout<<"~Curve_node()"<<endl; 
    }
  ...
  public:
    ...
    //substituting list with In_place_list for optimization.
    ...
    Intersection_point_node(const Curve_node_& cv1, 
      ...
      Comparison_result result = traits.curve_compare_at_x_right (cv1.get_curve(), 
                                                                  cv2.get_curve(), 
                                                                  ref_point.point());
    ...
    void merge(const Self& point_node){
      ...
      for ( ; cv_iter != curves.end() &&  
        ...
        if (is_left(traits.curve_source(cv_iter->get_curve()), 
          ...
          Comparison_result result = traits.curve_compare_at_x_left (cv_iter->get_curve(), 
                                                                     point_node_cv_iter->get_curve(), 
                                                                     intersect_p.point());
          if (result == LARGER){
            merged_left_curves.push_back(*cv_iter);
  ...
  public:
    ...
    inline  bool operator()(const Curve_node& cv1, const Curve_node& cv2) const { 
      Traits traits;
    ...
    Comparison_result curve_node_compare_at_x(const Curve_node &cv1, 
      ...
      if ( traits.curve_is_vertical(cv1.get_curve()) ){
        ...
        if (traits.curve_target(cv1.get_curve()) == 
          ...
          Curve_point_status p_status = traits.curve_get_point_status(cv2.get_curve(), 
                                                                      traits.curve_target(cv1.get_curve()) );
      ...
      if (!update_first_cv && !update_second_cv) 
        result = traits.curve_compare_at_x_right (cv1.get_curve(), cv2.get_curve(),  // making an optimization attemp:
        rightmost(cv1.get_rightmost_point().point(), 
        cv2.get_rightmost_point().point()) );
      
      else if (update_first_cv && !update_second_cv)
        result = traits.curve_compare_at_x_right (first_cv, cv2.get_curve(), 
                                                  rightmost(cv1.get_rightmost_point().point(), 
                                                            cv2.get_rightmost_point().point()));
      else if (!update_first_cv && update_second_cv)
        result = traits.curve_compare_at_x_right (cv1.get_curve(),second_cv, 
                                                  rightmost(cv1.get_rightmost_point().point(), 
                                                            cv2.get_rightmost_point().point()));
      ...
      if (result == EQUAL){
        if (!update_first_cv && !update_second_cv)
          ...
          result = traits.curve_compare_at_x (first_cv, cv2.get_curve(), 
                                              rightmost(cv1.get_rightmost_point().point(), 
                                                        cv2.get_rightmost_point().point()));
        else if (!update_first_cv && update_second_cv)
          result = traits.curve_compare_at_x (cv1.get_curve(),second_cv, 
                                              rightmost(cv1.get_rightmost_point().point(), 
                                                        cv2.get_rightmost_point().point()));
        else                       // update_first_cv && update_second_cv is true.
      ...
      if ( result == EQUAL && traits.curve_is_vertical(update_first_cv? first_cv: cv1.get_curve()) ){
        // if first_cv is vertical and its source tangent to second_cv - 
        // it means that first_cv is above second_cv.
        Curve_point_status p_status = traits.curve_get_point_status(update_second_cv? second_cv: cv2.get_curve(), 
                                                                    traits.curve_source(update_first_cv? first_cv: cv1.get_curve()));
        if (p_status == Traits::ON_CURVE)
          result = LARGER;
        else
          result = SMALLER;
      }
      else if (result == EQUAL && traits.curve_is_vertical(update_second_cv? second_cv: cv2.get_curve())){

        // if second_cv is vertical and its source tangent to first_cv - 
        // it means that second_cv is above first_cv.
        Curve_point_status p_status = traits.curve_get_point_status(update_first_cv? first_cv: cv1.get_curve(), 
                                                                    traits.curve_source(update_second_cv? second_cv: cv2.get_curve()));
        if (p_status == Traits::ON_CURVE)
          result = SMALLER;
...
private:
  ...
  typedef  std::pair<const Point,Intersection_point_node>  pair_Point_Intersection_point_node; 
  typedef  std::pair<const Curve_node,X_curve_plus>        pair_Curve_node_X_curve_plus;
  
public:
  typedef typename std::map<Point, Point_plus, less_xy<Point>, CGAL_ALLOCATOR(pair_Point_Point_plus) > 
                                                         Vertices_points_plus;
  typedef typename std::map<Point,Intersection_point_node,less_xy<Point>, CGAL_ALLOCATOR(pair_Point_Intersection_point_node) >
                                                                  Event_queue;
  typedef typename std::set<Curve_node, less_yx<Curve_node>, CGAL_ALLOCATOR(Curve_node) > 
                                                                  Status_line; 
...
protected:
                          ...
                          Intersection_point_node& point_node)
  {
    ...
    // a container to hold the point_node curve nodes ordered as they should on status.
    typedef typename std::set<Curve_node, less_yx<Curve_node>, CGAL_ALLOCATOR(Curve_node) > Local_status_line; 
    typedef typename Local_status_line::iterator                          Local_status_line_iterator; 
    Local_status_line  local_status;  // reserve the size of the point_node enamating curves.
    ...
    if (cv_iter != point_node.curves_begin())
      --cv_iter;
    for ( ; hint == status.end() && cv_iter != point_node.curves_end(); ++cv_iter)
    ...
    if (hint != status.end()){
      ++hint;  // getting the first curve node on status above the highest *cv_iter of curve_node.
      if (hint == status.end())
    ...
    while (hint != status.end()){
      for (cv_iter = point_node.curves_begin() ; cv_iter != point_node.curves_end(); ++cv_iter)
        if (*hint == *cv_iter){
    ...
    /*
    if (hint_in_point_node){
      // starting the loop from the end since we are interseted with the higher curve on sweep and not the lowest, 
      // and since the curves on an Intersection node are ordered in counterclock wise order we make our 
      // transition this way. The opposite way leads to a bug (cgal test: big overlap) since an interator 
      // of the Intersection node curve may be calculated instead of the higher iterator on the status 
      // right after the highest curve of the Intersection node.

      cv_iter = point_node.curves_end();
      if (cv_iter != point_node.curves_begin())
        --cv_iter;
      for ( ; hint == status.end() && cv_iter != point_node.curves_begin(); --cv_iter)
        hint = std::find(status.begin(), status.end(), *cv_iter);
    ...
    for (cv_iter = point_node.curves_begin(); cv_iter != point_node.curves_end(); ++cv_iter){
      ...
      // trying to use general find in order to force it to use operator == of Curve node. We only care here if *cv_iter
      // is on the status, we don't care where it should be put on status according less_yx predicate.
      // Since we work with handles only address comparing will be done.
...
#ifdef  CGAL_SWEEP_LINE_DEBUG  
        // for debug!
        less_yx<Curve_node>  pred;
        if (local_status_hint != local_status.end()) {
          if (!pred( *cv_iter, *local_status_hint))
            cout<< cv_iter->get_curve()<<" "<< local_status_hint->get_curve()<< "  "<< 
              cv_iter->get_rightmost_point().point() <<" are in wrong order on local sweep "<<endl;
          else
          ...
          if ( !pred( *(--local_status_hint), *cv_iter) )
            ...
            cout<< "local status hint is local_status.end() and is helping"<<endl;
          ++local_status_hint;
          // end debug.
        }
#endif
      ...
      if (CGAL::compare_lexicographically_xy (event_point, 
                                              traits.curve_target(cv_iter->get_curve()) ) == CGAL::SMALLER){
        local_status_hint = local_status.insert(local_status_hint, *cv_iter);
        
        //if (local_status_hint == local_status.end())
        //  cout << "ERROR - returned value of insert to local status is local_status.end()"<<endl;
      }
    }
    ...
    if (local_status.empty()) {
      ...
      if (!(point_node_first_on_status || point_node_last_on_status) 
          && hint != status.begin())  // if the neighbor of the curves enamting from point node is not the last one or the first one on status
        if (check_status_neighbors_intersections(event_queue, status, --hint, event_point))
    ...
    Status_line_iterator new_cv_node_begin = status.insert(hint, *(local_status.begin()) );
#ifdef  CGAL_SWEEP_LINE_DEBUG 
    cout<<"bogi2"<<endl;
#endif
    ...
    for (++local_status_iter; local_status_iter != local_status.end(); ++local_status_iter){
       ...
       if (hint != status.end()){
         ...
         if (hint != status.end()) {
           if (!pred( *local_status_iter, *hint))
             cout<< local_status_iter->get_curve() << " " << hint->get_curve() << " "<<
               local_status_iter->get_rightmost_point().point() <<" are in wrong order on local sweep "<<endl;
           else 
    ...
    for (new_cv_node_it = new_cv_node_begin; new_cv_node_it != new_cv_node_end; ++new_cv_node_it){
      ...
      // only if it's the first one, we will also check the preseding curve node adjancy.
                                             ...
                                             const Point &event_point)
  {
    ...
    // debugging for measuring time only.
    //bool  curves_intersect = segment_intersect_to_right(cv1.get_curve(), cv2.get_curve(), event_point, xp2);
    //xp3 = xp2;
    // end debugging.
    ...
    if (curves_tangent || curves_intersect){  
      ...
      if ( curves_tangent 
        ...
        if (xp_event == event_queue.end())
          xp_event = event_queue.insert(Event_queue_value_type(xp1, 
                                 Intersection_point_node(cv1, cv2, xp1))).first;
        else{
          // have to check whether the event is a new event. 
          // (we might calculated this point before).
          for ( Curve_node_iterator cv_iter = xp_event->second.curves_begin(); 
      ...
      //check_status_neighbors_t.stop();
      //std::cout<<"The time required to check status neighbors : "<< check_status_neighbors_t.time()<<std::endl;
