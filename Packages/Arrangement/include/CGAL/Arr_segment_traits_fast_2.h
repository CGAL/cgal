...
//
// file          : include/CGAL/Arr_segment_traits_fast_2.h
// package       : arr (1.03)
// source        :
// revision      :
// revision_date :
...
#include <CGAL/Pm_segment_traits_fast_2.h>
...
class Arr_segment_traits_fast_2 : public Pm_segment_traits_fast_2<Kernel_>
{
public:
...
public:
  ...
  /*! do_intersect_to_right() compares the location of the intersection
   ...
   * larger than pt, that is above or to the right of pt but not on pt.    
   */
                             ...
                             const Point_2 & pt) const 
  {
    ...
    if (assign(seg, res)) {
      ...
      return ((compare_xy(seg.source(), pt) == LARGER) ||
              (compare_xy(seg.target(), pt) == LARGER));
    }
