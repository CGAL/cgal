               ...
               std::ostream& o) 
{
  ...
  typedef typename Arrangement::Overlap_const_circulator                   Overlap_const_circulator;
  ...
  Cn_index               cn_index(arr.curve_node_begin(), arr.curve_node_end()); // creating curve indices.

  
  // creating the edge node indices table.
  for (Curve_const_iterator cv_iter = arr.curve_node_begin(); cv_iter != arr.curve_node_end(); cv_iter++){
  ...
  for (Curve_const_iterator cv_iter = arr.curve_node_begin(); cv_iter != arr.curve_node_end(); cv_iter++, number_of_curves++);
  writer.write_comment("number of curves");
  writer.write_value(number_of_curves);
  
  unsigned int curve_index = 0;
  for (Curve_const_iterator cv_iter = arr.curve_node_begin(); cv_iter != arr.curve_node_end(); cv_iter++, curve_index++){
    ...
    std::vector<Scv_index> scv_index;  // making a table of indices for all scv levels.
    unsigned int i;
    ...
    for (i = 0; i < cv_iter->number_of_sc_levels(); i++){
      ...
      for (sc_iter = cv_iter->level_begin(i); sc_iter != cv_iter->level_end(i); 
           sc_iter++, number_of_subcurves++);
      writer.write_value(number_of_subcurves);
      
      for (sc_iter = cv_iter->level_begin(i); sc_iter != cv_iter->level_end(i); sc_iter++){
    ...
    for (Edge_const_iterator edge_count_iter = cv_iter->edges_begin(); edge_count_iter != cv_iter->edges_end(); 
         edge_count_iter++,  number_of_edge_nodes++);
    
    writer.write_value(number_of_edge_nodes);
    
    writer.write_comment("----------------------- Edge nodes childrens:");
    for (Edge_const_iterator edge_iter = cv_iter->edges_begin(); edge_iter != cv_iter->edges_end(); edge_iter++){
      ...
      Edge_const_iterator edge_past_end_child = Edge_const_iterator(edge_iter->children_end());
