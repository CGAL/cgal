...
// set the colo of the unbounded face (its the same as the background color of the tab)
...
bool merge_segments_precondition(Pm_point_2 s, Pm_point_2 t, Pm_point_2 s1, Pm_point_2 t1 ,int s_degree , int t_degree)
{
	return
		(!((s1 == s && t1 == t) || (s1 == t && t1 == s)) 			&& //  cannot be the same curve as closest_curve
		(s1 == s || t1 == t || s1 == t || t1 == s)       			&& // must be one shared point
        (!(((s == s1 || s == t1) && s_degree != 2)       			||
		    ((t == s1 || t == t1) && t_degree != 2)))    			&& // vertex degree must be exactly 2
	    ...
	    (t == t1 && ((t.x() < s.x() && s1.x() < t.x())   			||
		            (t.x() > s.x() && s1.x() > t.x())))  			|| // checking x-monotone
		((s.x() == s1.x())&&( t.x() == t1.x()) && (s.x()==t.x()))));  // dealing with vertical curves
}
...
void visit_face_rec( Face_handle &f, Function func )
{
	...
	for(hit= f->holes_begin() ; hit!=f->holes_end() ; ++hit)
	{
		...
		visit_ccb_faces(inner_face,func); // TODO: how to pass the colors vector?
	}
    ...
    if(e->button() == Qt::LeftButton && is_pure(e->state()))
    {
      ...
      {	    
             ...
             hei != m_curves_arr.halfedges_end(); ++hei) 
        {
          ...
          CGAL::Object res =
            m_traits.nearest_intersection_to_right(split_curve, xcurve,p_right);
          if (CGAL::assign(p1, res))
            break;
        }	
...
{
  std::cout << "curve_has_same_direction \n" << cc->curve() <<" \n" 
	  << cc->curve().source() << "\n"
	  << cc->source()->point() << "\n";
...
bool is_curve_and_halfedge_same_direction (const Halfedge_handle&  he , const Xcurve & cv)
{
 ...
 */
  void fill_face(Qt_widget_demo_tab<Segment_tab_traits> * w , Face_handle f)
  {
	  ...
	  Ccb_halfedge_circulator cc=f->outer_ccb();
        do {
			Halfedge he = *cc;
			Coord_type x = CGAL::to_double(he.source()->point().x());
			Coord_type y = CGAL::to_double(he.source()->point().y());
	  ...
	  std::cout << "overlay 2\n";
	   if (f->does_outer_ccb_exist())  // f is not the unbounded face
	   {
         ...
         do {
			...
			{
				std::cout<<" overlay 3.1\n";
				temp_he = original_he;
			}
			else
			{
				std::cout<<" overlay 3.2\n";
	   ...
	   {  
	      std::cout << "update unbounded face color\n";
 ...
 //   Halfedge_handle original_he = d.halfedge_handle;
	...
	bool move_event)
  {
    ...
    {	
	  ...
	  if ( merge_segments_precondition<Pm_point_2>(s,t, s1,t1,vis->degree(),vit->degree()) && 
		   ker.compare_slope_2_object()(Kernel::Segment_2 (s, t),
									   Kernel::Segment_2 (s1, t1)) == CGAL::EQUAL)
	  { 		  
        Xcurve & xcurve = hei->curve();
	...
	{	  
      ...
      Curve_data d = h->curve().get_data();
	  if(is_curve_and_halfedge_same_direction(h , h->curve()))
	    d.halfedge_handle = h;
	  else
	  ...
	  Qt_widget_demo_tab<Segment_tab_traits> *w)
  {
	...
	// split he1 into curves : c1 ,c2 and store halfedge e1 (halfedge of c1)
	...
	// update c1
	Curve_data d1 = e1->curve().get_data();

	if(is_curve_and_halfedge_same_direction(e1 , c1))
	...
	// update c2
	Curve_data d2 = e1->next()->curve().get_data();
	if(is_curve_and_halfedge_same_direction((Halfedge_handle)e1->next() , c1))
    ...
    if (f->does_outer_ccb_exist())  // f is not the unbounded face
	  ...
	  Ccb_halfedge_circulator cc=f->outer_ccb();
        do {
			...
			{
			  for( pt_itr = cv.rbegin() , ++pt_itr ; pt_itr != cv.rend() ; ++pt_itr)
			  {
				Coord_type x = CGAL::to_double((*pt_itr).x());
				Coord_type y = CGAL::to_double((*pt_itr).y());
				Coord_point coord_source(x , y);
