...
\ccDefinition
    ...
    The \ccc{Point_2} type is \ccc{Point_2<Cartesian<NT> >}. The  
    \ccc{Curve_2} and \ccc{X_monotone_curve_2}
...
\ccConstructor{Curve_2(const Circle_2<Cartesian<NT> >& c);}{constructs
   a curve with a CGAL circle (source and target are identical and
   initialized to the leftmost point on the circle).}
\ccConstructor{Curve_2(const NT& x, const NT& y, const NT& r2);}{constructs a
...
\ccConstructor{Curve_2(const NT& x, const NT& y, const NT& r2, const Point_2& src,
   const Point_2& trgt);}{constructs a
...
\ccMethod{const Point_2& source() const;}{returns the source point of the arc.}
\ccMethod{const Point_2& target() const;}{returns the target point of the arc.}
