...
\ccMethod{bool nearest_intersection_to_right(const X_monotone_curve_2& c1,
	 ...
	 {finds the nearest intersection point (or points) of \ccc{c1} and
	  \ccc{c2} lexicographically to the right of \ccc{pt} and not 
          including \ccc{pt} itself (with one exception explained below):
           \begin{itemize}
           \item If the intersection of the two curves is a point to the
	   right of \ccc{pt}, it is returned through both \ccc{p1} and 
           \ccc{p2}.
	   \item If the intersection of \ccc{c1} and \ccc{c2} is an
	   ...
	   respectively.
	   \end{itemize}
	  Returns \ccc{true} if \ccc{c1} and \ccc{c2} do intersect to the 
          right of \ccc{pt}, \ccc{false} otherwise.}
...
\ccMethod{bool nearest_intersection_to_left(const X_monotone_curve& c1,
                                            ...
                                            Point_2& p1, Point_2& p2);}
	 {finds the nearest intersection point (or points) of \ccc{c1} and
	  \ccc{c2} lexicographically to the left of \ccc{pt} and not including
	  \ccc{pt} itself (with one exception explained below).
           \begin{itemize}
           \item If the intersection of the two curves is a point to the
	   left of \ccc{pt}, it is returned through both \ccc{p1} and \ccc{p2}.
	   \item If the intersection of \ccc{c1} and \ccc{c2} is an
	   \ccc{X_monotone_curve_2}, that is, they overlap at infinitely many
