...
called the \ccc{infinite vertex}.
...
on the right side of the oriented line from \ccc{va} to \ccc{vb}.}
\ccGlue
...
to the face with  \ccc{v1}, \ccc{v2} and a\ccc{v3} 
as vertices. This function is NOT YET implemented.} 
...
\ccThree{Vertex_iterator}{t.finite_vertices_begin()x}{}
\ccMethod{Vertex_iterator finite_vertices_begin() const;}{Starts at an arbitrary finite vertex}
\ccGlue
\ccMethod{Vertex_iterator finite_vertices_end() const;}{Past-the-end iterator}

\ccMethod{Edge_iterator finite_edges_begin() const;}{Starts at an arbitrary finite edge}
\ccGlue
\ccMethod{Edge_iterator finite_edges_end() const;}{Past-the-end iterator}

\ccMethod{Face_iterator finite_faces_begin() const;}{Starts at an arbitrary finite face}
\ccGlue
\ccMethod{Face_iterator finite_faces_end() const;}{Past-the-end iterator}
...
\ccThree{Vertex_iterator}{t.all_vertices_begin()x}{}
\ccMethod{Vertex_iterator all_vertices_begin() const;}{Starts at an arbitrary  vertex}
\ccGlue
\ccMethod{Vertex_iterator all_vertices_end() const;}{Past-the-end iterator}

\ccMethod{Edge_iterator all_edges_begin() const;}{Starts at an arbitrary edge}
\ccGlue
\ccMethod{Edge_iterator all_edges_end() const;}{Past-the-end iterator}

\ccMethod{Face_iterator all_faces_begin() const;}{Starts at an arbitrary face}
\ccGlue
\ccMethod{Face_iterator all_faces_end() const;}{Past-the-end iterator}
...
of its neighbor \ccc{f.neighbor(i)}.}
...
\ccMethod{Vertex* insert_first(Vertex* v);} {creates the first 
vertex and returns a pointer to it.}
\ccGlue
\ccMethod{Vertex* insert_second(Vertex* v);} {creates the second finite
vertex and returns a pointer to it.}
\ccMethod{Vertex* insert_in_edge(Vertex* v, Face* f, int i);} {adds a
vertex \ccc{v} splitting 
edge \ccc{i} of face \ccc{f}. Return a  pointer to \ccc{v}.}
\ccGlue
\ccMethod{Vertex* insert_in_face(Vertex* v,Face* f);} {adds a vertex
...
which decreases by one the dimension of the triangulation.
...
\ccHeading{Other Access Functions}
\ccMethod{int  mirror_index(int i) const;}
{index of \ccVar as a neighbor of \ccc{\ccVar.neighbor(i)}}
\ccGlue
\ccMethod{Vertex* mirror_vertex(int i) const;}
{vertex opposite of \ccc{\ccVar.neighbor(i)} to \ccVar.}
...
An hidden point can appear as vertex only when the
...
\subsubsection{Requirements for the face base class of a regular triangulation}
...
n0, void* n1, void* n2)} {Constructor setting the incident verticesand 
neighborinf faces.}
...
{ Inserts the line segment s whose endpoints are the vertices va and
vb as a constraint e. The triangles intersected by s
are removed and new ones are created. 
\ccPrecond The relative interior of s does not
intersect the relative interior of another constrained edge.  
\ccPrecond  va and vb are distinct vertices of t.  }

\ccMethod{ void insert(const Vertex_handle & va, const Vertex_handle & vb,
	    Face_handle & fr, int & i);}
{Same as above. In addition,  returns the face incident to edge e
and to the right of e and the index i of the vertex of fr opposite to
e, i.e.  e=(fr,i).}	    

\ccMethod{void insert(const Vertex_handle & va, const Vertex_handle & vb,
	    Face_handle & fr, int & i, List_edges & new_edges);}
{Same as above. In addition,  the edges that are created are put in list
new\_edges.}
...
C or N depending whether edge (f,i) is constrained or not.}
...
constrained edges which tries to as much Delaunay as possible.
This means that the triangulation do no longer fulfills the empty
circle property but a weaker property. To state this property,
...
to provide the \ccc{in_circle} test as the geometric traits
...
As a constrained Delaunay triangulation is not a Delaunay
