...
when used in conjonction with a geometric traits
...
\label{Section_2D_Constrained_triangulation_plus}

The class \ccc{Constrained_triangulation_plus_2<Tr>}
provides a constraint triangulation with an additional data
...
handle intersection of constraints and uses an exact number type,
...
is specially designed to avoid cascading in the computation of
intersecting points.

\subsection{Example : Building a triangulated arrangement of constraint}

The following code inputs a set of intersecting constraint segments
and count the number of constrained edges of the
