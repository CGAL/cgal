...
\ccParameters
The class \ccRefName\ depends on  two template parameters. The first one
\ccc{Traits} expects a model of the \ccc{Triangulation_traits_2}
concept described on page~\ccRefPage{Triangulation_traits_2}.
The second parameter expects a model of the
\ccc{Triangulation_data_structure_2} described
 on page~\ccRefPage{Triangulation_data_structure_2}.
Dy befault, the second template parameter is instanciated by
...
\ccGlue
\ccNestedType{Tds}{the triangulation data structure type.}
...
\ccPredicates
The class \ccRefName\ provides method to test
...
which is a witness of the point's location (see the description of the
...
The following fonctions are mainly intended to be used in conjonction
with the \ccc{find_conflicts()} member fonctions of Delaunay and constrained 
Delaunay triangulations to perfoem insertions.
...
\ccc{CGAL::Triangulation_default_data_structure} \\
\ccc{CGAL::Triangulation_data_structure_using_list} \\
