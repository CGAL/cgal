...
handle\ ccc{f}
points to a face incident \ccc{e} and \ccc{li}  is the
...
index \ccc{li}
has no meaning.
\ccc{OUSIDE_CONVEX_HULL} if the point lies within the afffine hull of
the triangulation but outside its convex hull. In this case, 
the face handle \ccc{f} points to any infinite face which is a {\em witness} of the
query point location.For a two dimensional triangulation, it means
...
{Same as above. In addition, parameters \ccc{lt} and \ccc{li} are set respectively
