...
and Section~\ref{Section_2D_Triangulations_Software_Design} presents
and the overall software
design of the 2D triangulation package. 
...
At last, Section~\ref{Section_2D_Triangulations_Hierarchy}
described a hierarchical data structure for
fast point location queries and
Section~\ref{Section_2D_Triangulations_Flexibility} 
...
- two facets are either disjoint or share a lower dimensional
face (edge or vertex).\\
...
as a simplicial complex, according to the following definitions.

A simplicial complex is a set $T$  of simplices such that~\\
- any face of a simplex in $T$ is a simplex in $T$ \\
- two simplices in $T$ are either disjoints or  share
  a subface.

The dimension $d$ of a  simplicial complex is the 
maximal dimension of all its simplices. \\
...
adjacent if they share a $(d-1)$ dimensinal subface.
...
The union of all simplices in $T$ is called the domain of $T$.
...
simplicial complex with dimension 2 that is pure,
...
{\bf The set of represented faces}
The  triangulations of \cgal\
are such that their domain always covers the convex hull
of their vertices \footnote{Triangulations covering a given 
bounded part of the plane can nevertheless be represented
through constrained od Delaunay constrained triangulations.
See sections~\ref{Section_2D_Triangulations_Constrained},
\ref{Section_2D_Triangulations_Constrained_Delaunay} and
~\ref{Section_2D_Triangulations_Constrained_Plus}.}
Therefore a triangulation  can be viewed as a planar partition of the plane
...
\begin{figure}
\begin{ccTexOnly}
\begin{center} \IpeScale{100} \Ipe{infinite_vertex.ipe} \end{center}
\end{ccTexOnly}
...
Thus the set of represented faces is topologically
equivalent to a two-dimensional sphere. This is extended
...
With the infinite\_vertex, the set of faces representing 
a triangulation of dimension 1 is
topologically equivalent to a $1$-sphere i.e. to a circle.
A triangulation of dimension 0 is reduced to a single point.
It is represented  as a $0$-sphere, i.e. with two vertices
corresponding to the single point and the infinite vertex.


{\bf A representation based on faces and vertices}
...
\subsection{Examples of the Use of a  Triangulation Hierarchy}
\label{Subsection_2D_Triangulations_Basic_Example}
