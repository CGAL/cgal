...
providing  the  type
\ccc{Weighted_point} and the \ccc{power-test}  predicate on those 
weighted points.
A weighted point is basically 
a point augmented with a scalar weight or it can be seen as a circle
represented trough its center and radius.
The  \ccc{power-test} on weighted points
is the fundamental test to build regular triangulations 
as  the \ccc{in-circle} test is the fundamental test
...
\ccc{Bare_point operator() ( Weighted_point p,
		     Weighted_point q,
		     Weighted_point r);} }
...
\ccc{ Line_2  operator() ( Weighted_point p, Weighted_point q);} }
