...
\lcTex{described \ccRefPage{TriangulationDataStructure_2::Vertex}
...
\ccTypes
\ccThree{typedef std::pair<Face_handle,int>xx}{Edge; }{}
\ccThreeToTwo
...
the dimension, the number of vertices, the number of faces,
the vertices
the indices of the vertices in  each face, 
then the indices of the neighbors of each cell, 
where the index of an item corresponds to its order in the ouput.
When dimension $<$ 2, the same information is stored
