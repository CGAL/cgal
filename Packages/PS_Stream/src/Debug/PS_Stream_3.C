...
{
#ifdef DEBUG
  cout << " JE SUIS DANS PS_STREAM_3 UTILISE" << endl;
#endif
double xn=d.dx()/norme(d.dx(),d.dy(),d.dz());
double yn=d.dy()/norme(d.dx(),d.dy(),d.dz());
double zn=d.dz()/norme(d.dx(),d.dy(),d.dz());
...
#ifdef DEBUG
  cout << "Point of view before transformation : " << _point_of_view
       << endl;
#endif
  _point_of_view=_transformation(_point_of_view); 
#ifdef DEBUG
  cout << "Point of view after transformation : " << _point_of_view << endl;
  cout << "Light before transformation : " << _light << endl;
#endif
  _light=_transformation(_light);
#ifdef DEBUG  
cout << "Light after transformation : " << _light << endl;
#endif
}
...
#ifdef DEBUG
  for (int i=0;i<(int)_scene.size();i++) {
    cout << "Face no " << _scene[i].number_of_the_facet() <<
      "\tCouleur : " << _scene[i].color() << "\t -> zmin : " << _scene[i].zmin() << endl;
  }
#endif
  return _scene;
}
...
#ifdef DEBUG
   cout << "##################### FACE "
	<<_scene[i].number_of_the_facet() << " ##########################" <<endl;
#endif
   Plane3 P1(_scene[i][0].first_point(),_scene[i][0].second_point(),_scene[i][_scene[i].number_of_edge()-1].first_point());
#ifdef DEBUG
   cout << "Coefficients Plan P1 : " << P1.a() << " " << P1.b() << " " << P1.c() << " " << P1.d() << "\n" << endl;
#endif   
   //Creation of the normal vector of the plane
   Vector3 v_ortho(P1.orthogonal_vector());
   Vector3 normal_normalise(v_ortho.x(),v_ortho.y(),v_ortho.z(),norme(v_ortho.x(),v_ortho.y(),v_ortho.z()));
#ifdef DEBUG   
cout << "Vecteur Normal a P1 : " << v_ortho << " | Normalise : " <<
  normal_normalise <<endl;
#endif
...
#ifdef DEBUG
   cout << "Vecteur pov " << v_pov <<  " | Normalise : " <<
     pov_normalise << endl;
#endif
...
#ifdef DEBUG
cout << "Vecteur light " << v_light << " | Normalise : " << light_normalise << endl;
#endif
   
 double angle = CGAL::to_double(normal_normalise*pov_normalise);
 
#ifdef DEBUG
 double angle_non_normalise = v_ortho*v_pov;
 cout << "Angle entre le vecteur pov et normal non normalises : " << angle_non_normalise << endl;
 cout << "Angle entre le vecteur pov et normal normalises : " << angle << "\n"<<endl;
#endif   
   
 double angle_light = light_normalise*normal_normalise;
#ifdef DEBUG 
cout << "cos(alpha) = " << angle_light << endl;
#endif  
   
//The face is visible if angle>0
 if (angle>0) {//The facet is visible
#ifdef DEBUG
   cout << "PS_Facet number : " << _scene[i].number_of_the_facet()
	<< " -> VISIBLE with angle : " << angle << endl;
   cout << "Gray Level : " << angle_light << endl;
#endif
   _scene[i].set_gray_level(angle_light);
   scene_visible.push_back(_scene[i]);
 }
 else {//The facet is invisible
#ifdef DEBUG
  cout << "PS_Facet number : " << _scene[i].number_of_the_facet()
	<< " -> INVISIBLE with angle : " << angle << endl;
#endif
 }
#ifdef DEBUG
 cout << "\n" << endl; 
...
#ifdef DEBUG
cout << "\n############## CUTTING ################\n" << endl;
cout << "Parametre : " << "i=" <<i << " | j="<< j <<endl;
cout << "liste face.size = " << liste_face.size() << endl;
//The index i give the facet which is going to be cut and the j the face which intersect facet i
cout << "Je suis dans cut et je coupe la face number : " <<
    liste_face[i].number_of_the_facet() << " de couleur " << liste_face[i].color() << endl;
#endif
...
#ifdef DEBUG
 cout << "Plan P1 : " << P1 << endl;cout << "Plan P2 : " << P2 << endl;
 cout << " P1 -> face number " << liste_face[i].number_of_the_facet() 
      << " de couleur : " << liste_face[i].color()  << endl;
 cout << " P2 -> face number " << liste_face[j].number_of_the_facet() 
      << " de couleur : " << liste_face[j].color()  << endl;
 cout << "Do intersect -> " << do_intersect(P1,P2) << endl;
#endif
...
#ifdef DEBUG
cout << "la.0 et la 1 " << l.point(0) << l.point(1) <<endl;
#endif
...
#ifdef DEBUG
   cout << "facet1.size" << facet1.size() << endl;
   cout << "AVT CUT  liste_face " << liste_face.size() << endl;
#endif
...
#ifdef DEBUG
   cout << "APT CUT  facet1.size() " << facet1.size() << endl;
#endif
   liste_face=facet1;
   return 1;
 }
...
#ifdef DEBUG  
  cout << "P: ax+by+cz+d=0 " << Pi << endl;
  cout << "a=" << a << " ,b=" << b << " ,c="<< c << " ,d=" << d << endl;
  cout << "Number of vertices in the face j : " << facej.number_of_edge() << endl;
#endif  
  vector<double> v;
  double result;
...
void PS_Stream_3::depth_sort() {
...
#ifdef DEBUG
  cout << "\n############## DEPTH SORT ################\n" << endl;
  cout << "Scene nbface : " << _scene.size() << endl;
#endif  
  ...
  if (_scene.size() > 1) {
    ...
    int i=0;int j=1;
#ifdef DEBUG
    cout << "DEBUT \nListe_face " << liste_face.size() << "  liste_face_triee : " << liste_face_triee.size() << endl;
    cout << "i : " << i << " j : " << j << endl;
#endif    
...
#ifdef DEBUG
     cout << "Je compare la face : " << liste_face[i].number_of_the_facet();
     cout << " de couleur " << liste_face[i].color() << " zmin " <<  liste_face[i].zmin();
     cout << " et la face : " << liste_face[j].number_of_the_facet();
     cout  << " de couleur " << liste_face[j].color() << " zmin : " << 	liste_face[j].zmin() <<  endl;
#endif
	if (liste_face[i].number_of_the_facet() == liste_face[j].number_of_the_facet()) {
#ifdef DEBUG	  
 	  cout << "Je compare deux faces coplanaires !!!!" << endl;
#endif
	  //Coplanar faces so I don't cut , I try with the next facet.
	  j++;}
	else {
	  if (liste_face[i].five_test(liste_face[j])) {
#ifdef DEBUG
 	    cout << "Je suis dans cas normal \n" << endl;
#endif
	    j++;}
	  else {
...
#ifdef DEBUG
 		cout << "detection cycle  !!!" << endl;
 		cout << "Je suis dans cut pour cycle " << endl;
#endif
...
#ifdef DEBUG
	      cout << "Je compare dans l'autre sens " << endl;
#endif	
	      if(liste_face[j].two_test(liste_face[i])) {//Swap P and Q
#ifdef DEBUG		
 		cout << " Je suis dans swap " << endl;
#endif  
...
#ifdef DEBUG
 		cout << "Nb face dans liste_face : " << liste_face.size();
 		cout << " et dans liste_face_triee : " <<  liste_face_triee.size() << endl;
 		cout << "Liste_face contient desormais : " << endl; 
 		for (int p=0;p< (int)liste_face.size();p++) {
 		  cout << liste_face[p].number_of_the_facet() << "\t" ;
 		}
 		cout << "###########################################\n" << endl;
#endif
		i=0;j=1;
...
#ifdef DEBUG
      cout << "J'insere la face " << liste_face[i].number_of_the_facet(); 
      cout << " de couleur " << liste_face[i].color() << " zmin : " << liste_face[i].zmin() << endl;
#endif
      liste_face_triee.push_back(liste_face[i]);
      liste_face.erase(&liste_face[i]);
#ifdef DEBUG
      cout << "Puis je l'efface \n" << endl;
      cout << "Nb face dans liste_face : " << liste_face.size(); 
      cout  << " et dans liste_face_triee : " <<  liste_face_triee.size() << endl;
      cout << "Liste_face contient desormais : " << endl;
      for (int p=0;p< (int)liste_face.size();p++) {
	cout << liste_face[p].number_of_the_facet() << "\t" ;
      ...
      cout << "\nListe Face Triee contient : " << endl;
      for (int q=0;q< (int)liste_face_triee.size();q++) {
	cout << liste_face_triee[q].number_of_the_facet() << "\t" ;
      }
      cout << "#################################################\n" << endl;
#endif
    }
...
#ifdef DEBUG
    cout << "Nombre de Face Visible inferieur ou egal a 1 " << endl;
#endif
  }
}
...
#ifdef DEBUG
      cout << "Gray level : " << face.gray_level() << endl;  
#endif
...
vector<PS_facet_3> PS_Stream_3::triangulation_2_to_facet(triangulation_2& tr) {
  ...
  #ifdef DEBUG
  cout << "Number of faces in the triangulation : " << nbface << endl; 
  #endif
return vfacet;
}
