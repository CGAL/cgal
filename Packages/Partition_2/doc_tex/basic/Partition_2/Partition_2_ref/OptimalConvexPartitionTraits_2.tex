...
used with the function \ccc{optimal_convex_partition_2}.
...
\ccNestedType{Construct_segment_2}{Predicate object type that provides
\ccc{Segment_2 operator()(Point_2 p, Point_2 q)}, which constructs and
returns the segment defined by the points $p$ and $q$.}
\ccNestedType{Construct_ray_2}{Predicate object type that provides
...
functions that create instances of the above predicate object types
must exist:
...
%The following function is required by \ccc{is_convex_2}, which is used
%to test the convexity postcondition:
%\ccIndexSubitem[C]{optimal_convex_partition_2}{postconditions}
%\ccIndexGlobalFunction{is_convex_2}
%
%\ccSetThreeColumns{CGAL_Oriented_side}{}{\hspace*{8.5cm}}
%\ccMemberFunction{
%    bool lexicographically_yx_smaller(const Point_2& p,
%                                      const Point_2& q) const;
%}
%{
%Returns \ccc{true} iff $p_y < q_y$ or $p_y = q_y$ and $p_x < q_x$.
%}
