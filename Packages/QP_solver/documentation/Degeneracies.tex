...
\marginpar{variable index issue: section adapted}
...
\marginpar{variable index issue: subsection adapted}
...
constraints}
\marginpar{no variable index issue}
...
%\end{equation} 
\marginpar{may the mapping $\sigma: S \rightarrow I$ still be arbitrary?}
...
$A_{i}$, $i \in J=\sigma(art \cap B_{O})$ are linear combinations of the equality
constraints $A_{i}$, $i \in E \setminus J$.
...
the term $\frac{\varepsilon^{i+1}}{p_{x_{i}}}$ is unique to
...
\nonumber \\
  &&
\frac{\tilde{\nu}}{\tilde{q}_{x_{l}}}\varepsilon^{l+1}
\end{eqnarray}
...
Definitions~(\ref{def:n_mu_j_Q_1}) and~(\ref{def:n_x_i_Q_1}).
...
\begin{algorithm}
\caption{Perturbed Ratio Test 1\_0, $\check{t}(0, B)$}
...
\Else
  ...
  \Repeat
    \State $T_{k}^{\prime} \gets \emptyset,
      ...
      \quad q_{min} \gets 0$
    \ForAll{$i \in T_{k}$}
      \State $c_{x_{i,k}} \gets n_{x_{i}}^{(Q_{1})}(B)[k]$
    ...
    \EndFor
    \If{$ \neg leaving$}
      \State $c_{j,k} \gets n_{\mu_{j}}^{(Q_{1})}(B)[k]$
      \If{$c_{j,k} * q_{min} < \nu *c_{min}$}
        \State \textbf{return} $\emptyset$
      \EndIf
...
\Else
    ...
    \Repeat
	  ...
	  \quad p_{min} \gets 0$
        \ForAll{$i \in T_{k}$}
            \State $c_{x_{i,k}} \gets n_{x_{i}}^{(Q_{2})}(\hat{B})[k]$
	    \If{$i < n$} \Comment{$x_{i}$ is original variable}
