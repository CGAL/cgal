...
In case of the first event, if the entering variable $x_{j}$ takes the value
of its lower or upper bound, we implicitly add the constraint $x_{j}=l_{j}$ or
$x_{j}=u_{j}$ to $(UQP(\hat{B}^{u}))$ by removing $j$ from $\hat{B}$. The pivot
step is then completed.
