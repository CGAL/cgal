...
\begin{algorithmic}
\Function{ratio\_test\_1\_\_t\_i}{$B_{O}, B_{S}, dir$}
\State $i_{min} \gets -1, \quad     d_{min} \gets 1, \quad  q_{min} \gets 0$
\If{$dir=1$}
    \ForAll{$i \gets 0,  \left|B_{O}\right| - 1$}
        \If{$q_{B_{O}}[i] > 0 \wedge f_{i}^{l}=1$}
            \If{$d_{min}*q_{B_{O}}[i] > (x_{B_{O}}[i]-l_{i})*q_{min}$}
                \State $i_{min} \gets B_{O}[i],
                    \quad d_{min} \gets x_{B_{O}}[i]-l_{i},
                    \quad q_{min} \gets q_{B_{O}}[i]$
            \EndIf
        \ElsIf{$q_{B_{O}}[i] < 0 \wedge f_{i}^{u}=1$}
            \If{$d_{min}*-q_{B_{O}}[i] > (u_{i}-x_{B_{O}}[i])*q_{min}$}
                \State $i_{min} \gets B_{O}[i],
                        \quad d_{min} \gets u_{i}-x_{B_{O}}[i],
                        \quad q_{min} \gets -q_{B_{O}}[i]$
            \EndIf
        \EndIf
    \EndFor
...
\Else
    \ForAll{$i \gets 0,  \left|B_{O}\right| - 1$}
        \If{$q_{B_{O}}[i] > 0 \wedge f_{i}^{u}=1$}
            \If{$d_{min}*q_{B_{O}}[i] > (u_{i}-x_{B_{O}}[i])*q_{min}$}
                \State $i_{min} \gets B_{O}[i],
                    \quad d_{min} \gets u_{i}-x_{B_{O}}[i],
                    \quad q_{min} \gets q_{B_{O}}[i]$
            \EndIf
        \ElsIf{$q_{B_{O}}[i] < 0 \wedge f_{i}^{l}=1$}
            \If{$d_{min}*-q_{B_{O}}[i] > (x_{B_{O}}[i]-l_{i})*q_{min}$}
                \State $i_{min} \gets B_{O}[i],
                    \quad d_{min} \gets x_{B_{O}}[i]-l_{i},
                    \quad q_{min} \gets -q_{B_{O}}[i]$
...
\end{algorithmic}
%\end{algorithm}
In contrast to the Ratio Test Step~1 for the standard form we can distinguish three differences: 
\begin{itemize}
\item entering variables are no longer only increased
\item basic vars may have lower as well as upper bounds
\item the entering variable itself may run into a bound
\end{itemize}
...
\end{eqnarray}
\end{lemma}
In contrast to pricing with the standard form, where all nonbasic variables have zero values and can only be increased, nonbasic original variables may 
\begin{equation}
\mathcal{E}:=\{k \in N \left|\right. b(k)=-1 \wedge \mu_{k} < 0 \vee
  b(k)=0 \wedge \mu_{k} \neq 0 \vee b(k)=1 \wedge \mu_{k} > 0 \}
\end{equation}
