...
\ccConstructor{snap_rounding(typename std::list<Segment_2>::const_iterator first,typename std::list<Segment_2>::const_iterator beyond,NT pixel_size,bool do_isr = true,int 
...
or Snap Rounding. The forth parameter denotes the number of KD trees to use (see cite ISR for details). The default,
$1$, is ussually the best one to use.
}

\ccConstructor{snap_rounding(NT pixel_size,bool do_isr = true,int 
inp_number_of_kd_trees = 1);}{
