...
as a finite precision approximation technique. Iterated Snap Rounding (ISR, for short)
is a modification of SR in which each vertex is at least
...
is a modification of SR which makes a vertex and a non-incident
edge well separated (the distance between each is at least
half-the-width-of-a-pixel). However, the guaranteed quality of the
approximation in ISR degrades. For more details on ISR see
\cite{isr-2002}.
...
If the parameter value is {\ccc true}, then the output is represented in an integer
grid just as defined by the SR algorithm. Otherwise, the plane is tiled
with unit pixels with size {\ccc pixel\_size} and each vertex of the output is
...
\ccPrecond{\ccc{pixel_size} must have a positive value and \ccc{number_of_kd_trees} must be a positive integer.}
