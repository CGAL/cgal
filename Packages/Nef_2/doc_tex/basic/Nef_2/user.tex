...
interior, its boundary, and its closure, but also composed operations
...
\begin{ccTexOnly}
\begin{center}
\scalebox{0.5}{\includegraphics{Nef_2_ref/halfplane.ps}}
\hspace{1cm}
\scalebox{0.5}{\includegraphics{Nef_2_ref/complex.ps}}
\end{center}
\end{ccTexOnly}
...
boundary are at infinity.}\label{nefexamples}    
\begin{ccHtmlOnly}
<CENTER>
...
Following the above definition the data type \ccc{Nef_polyhedron_2<T>}
allows construction of elementary Nef polyhedra and the binary and
unary composition by the mentioned set operations.
...
\ccIncludeExampleCode{Nef_2/nef_construction.C}
...
\ccc{Nef_polyhedron_2<T>}.

Note that Nef polyhedra have input and output operators that allow to
...
object (vertex, edge, face) is attributed by a boolean value that
referes to set inclusion or exclusion.

The plane map is defined by the interface data type
\ccc{Nef_polyhedron_2<T>::Const_decorator}. Embedding the vertices by
...
is bounded symbolically by an axis paralled square box of infimaximal
...
propositions shall be enough at this point. Due to the fact that the
infimaximal box is included into the plane map the vertices and edges
...
During exploration box objects can be tracked down by the interface of
\ccc{Nef_polyhedron_2<T>::Explorer} that is derived from
\ccc{Nef_polyhedron_2<T>::Const_decorator} and adds just the box
exploration functionality to the interface of the latter.
...
the simple planar affine kernels.  Actually it holds that (pseudo type
equation)
\begin{ccExampleCode}
...
\begin{ccAdvanced}
For its notions and requirements see \ccc{ExtendedKernelTraits_2}.
\end{ccAdvanced}
