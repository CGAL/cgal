...
\begin{ccRefClass}{Const_decorator}\ccCreationVariable{D}

\ccSection{Topological plane map exploration}

\ccDefinition

An instance \ccc{D} of the data type \ccc{Const_decorator} is a 
...
\ccConstructor{Const_decorator(const Plane_map& P)}{
constructs a plane map decorator exploring \ccc{P}. }
...
\ccMethod{const GenPtr& info(Vertex_const_handle v) ;}{
  returns a generic information slot. }

\ccMethod{const GenPtr& info(Halfedge_const_handle e) ;}{
  returns a generic information slot. }

\ccMethod{const GenPtr& info(Face_const_handle f) ;}{
  returns a generic information slot. }
