...
plane which is the result of forming complements and intersections
...
creates a Nef polyhedron \ccc{N} containing the halfplane
...
Boundary b = INCLUDED)}{
creates a Nef polyhedron \ccc{N} from the simple polygon
...
returns \ccc{N} $\cup$ \ccc{N1}. Note that ``union'' is keyword of C++
...
\ccMethod{Nef_polyhedron_2<T> symmetric_difference(const Nef_polyhedron_2<T>& N1) ;}{
...
binary operations \emph{intersection}, \emph{join},
...
\ccMethod{Object_handle locate(const Point& p, Location_mode m = DEFAULT) ;}{
returns a generic handle \ccc{h} to an object (face, halfedge, vertex) 
...
returns a handle \ccc{h} with \ccc{N.contains(h)} which can be
...
\ccMethod{Object_handle ray_shoot_to_boundary(const Point& p, 
const Direction& d, Location_mode m = DEFAULT) ;}{
returns a handle \ccc{h} which can be converted to a
...
returns a decorator object which allows read-only access of
...
\ccc{d} until an obstacle is met. In a minimum weight triangulation of
...
CGAL::Nef_polyhedron_2< CGAL::Filtered_extended_homogeneous<RT> >::Point
   == CGAL::Homogeneous<RT>::Point_2
\end{ccExampleCode}
The same holds for the types \ccc{Line_2} and \ccc{Direction_2} in the
