...
class Hash_map : private leda::ch_map<E> {
...
public:
  ...
  public:
    ...
    element_iterator(item p) : it(p) {}
    element_iterator() : it(0) {}
    bool  operator==( const element_iterator& i) const 
    { return ( it == i.it); }
    bool  operator!=( const element_iterator& i) const 
    { return !(*this == i); }
    const E& operator*() const { return Base::inf(it); }
    const E* operator->() const { return &Base::inf(it); }
    element_iterator& operator++() 
    { it=Base::next_item(it); return *this; }
...
element_iterator begin() { return element_iterator(Base::first_item()); }
/*{\Mop returns the element iterator pointing to the first element 
stored.}*/
element_iterator end() { return element_iterator();}
