...
The \ccc{Voronoi_diagram_2<DG,VT,AP>} class is parameterized by
...
The \ccc{Voronoi_diagram_2<DG,VT,AP>} class has been
...
Finally, depending on the Voronoi traits passed to the Voronoi diagram
...
functor required by the adaptor in order to access geoemtric
information in the Delaunay graph that is needed by the
\ccc{Voronoi_diagram_2<G,VT,AP>} class.
In particular, it provides functors for accessing sites in the Delaunay
graph and constructing Voronoi vertices given their dual faces in the
Delaunay graph.
...
traits concept. The point location and nearest sites queries of the
\ccc{Voronoi_diagram_2<DG,VT,AP>} class use internally this nearest
site query functor.

In this package we provide four Voronoi traits classes, all of which
support nearest site queries:
\begin{itemize}
\item
  The \ccc{Apollonius_graph_Voronoi_traits_2<AG2>} class: it
  provides the Voronoi traits for adapting Apollonius graphs.
\item
  The \ccc{Delaunay_triangulation_Voronoi_traits_2<DT2>} class: it
  provides the Voronoi traits for adapting Delaunay triangulations.
\item
  The \ccc{Regular_triangulation_Voronoi_traits_2<RT2>} class: it
  provides the Voronoi traits for adapting regular triangulations.
\item
  The \ccc{Segment_Delaunay_graph_Voronoi_traits_2<SDG2>} class: it
  provides the Voronoi traits for adapting segment Delaunay graphs.
\end{itemize}
...
construct its Voronoi diagram representation. 
The manner that we treat such issues in this package in a generic way
...
ways for answering this question. The first one is called the
...
Delaunay graph has edges and potentially faces that geometrically are
degenerate. By that we mean that the Voronoi diagram provided by the
adaptor can have Voronoi edges of zero length or Voronoi faces/cells
of zero area. Such features may not be desirable and ideally we would
...
\item
  ...
  adapting of an Apollonius graph to an Apollonius diagram.
\item
  The \ccc{Delaunay_triangulation_degeneracy_removal_policy_2<DT2>} class: it
  provides an adaptation policy for removing degeneracies when
  adapting of a Delaunay triangulation to a point Voronoi diagram.
\item
  The \ccc{Regular_triangulation_degeneracy_removal_policy_2<RT2>} class: it
  provides an adaptation policy for removing degeneracies when
  adapting of a regular triangulation to a power diagram 

\item
  The \ccc{Segment_Delaunay_graph_degeneracy_removal_policy_2<SDG2>} class: it
  provides an adaptation policy for removing degeneracies when
  adapting of a segment Delaunay graph to a segment Voronoi diagram.
\end{itemize}
...
sites. Computing whether a degenerate Voronoi edge is really
degenerate implies computing the outcome of a predicate in a
...
Voronoi traits and a copy of the adaptation policy are stored in it.
The various Voronoi traits we provide are empty classes. The major
time and space efficiency issues arise from the various
implementations of the adaptation policies.
%
