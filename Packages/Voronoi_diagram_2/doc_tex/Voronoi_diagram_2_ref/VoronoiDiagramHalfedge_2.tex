...
%
\ccThree{typedef typename Delaunay_graph::Edge}{Delaunay_edge;}{}
\ccThreeToTwo
\ccNestedType{Delaunay_graph}{A type for the Delaunay graph. It must be a
  model of the \ccc{DelaunayGraph_2} concept.}
\ccGlue
\ccTypedef{typedef typename Delaunay_graph::Edge Delaunay_edge;}{A type for
...
the Delaunay graph corresponding to the defining site above of
the Voronoi edge.}
\ccGlue
\ccMethod{Delaunay_vertex_handle down();}{returns a handle to the vertex
in the Delaunay graph corresponding to the defining site below of
the Voronoi edge.}
\ccGlue
