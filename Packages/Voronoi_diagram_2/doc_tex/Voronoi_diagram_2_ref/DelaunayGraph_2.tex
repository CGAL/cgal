...
first template parameter of the \ccc{Voronoi_diagram_adaptor_2<DG,VT>}
...
\ccGlue
\ccTypedef{typedef std::pair<Face_handle,int> Edge;}{A type for the
...
\ccCreation
In addition to the default and constructors, as well as the assignment
operator, the following constructors are required.
\ccThree{DelaunayGraph_2}{dg(It begin, It beyond)+}{}
\ccThreeToTwo
%
\ccConstructor{template<class It> DelaunayGraph_2(It begin, It beyond);}
{Constructor that takes an iterator range. The value type of the
  iterator must be \ccc{Site_2}.}
\ccGlue
\ccConstructor{template<class It> DelaunayGraph_2(It begin, It beyond,
  Voronoi_traits tr);}
{Constructor that takes an iterator range and an instance of the
  Voronoi traits. The value type of the iterator must be \ccc{Site_2}.}
