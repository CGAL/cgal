...
thought of as occupying the four major directions \ccStyle{up},
\ccStyle{down}, \ccStyle{left} and \ccStyle{right}, where
\ccStyle{up} means above the horizontal axis (on the positive
side of the axis), \ccStyle{down} means below the horizontal axis (on the
negative side of the axis), \ccStyle{left} means towards minus
infinity and \ccStyle{right} means towards plus infinity. Notice that
then the Voronoi vertices, that is the endpoints of the Voronoi edge,
are defined by the generator triplets $\{$\ccStyle{down},
\ccStyle{up}, \ccStyle{left}$\}$ (this is the source of the Voronoi
edge) and $\{$\ccStyle{up}, \ccStyle{down}, \ccStyle{right}$\}$
(this is the target  of the Voronoi edge). Notice that \ccStyle{up}
and \ccStyle{down} can never correspond to the site at infinity. If
either \ccStyle{left} or \ccStyle{right}, but not both, are the
site an infinity, then the Voronoi edge is essentially a bisecting
ray, whereas if both \ccStyle{left} and \ccStyle{right} are the site at
infinity the Voronoi edge is the entire bisector of \ccStyle{up}
and \ccStyle{down}. The following methods provide access to these
four defining sites:

\ccThree{Site_2}{e.right()+}{}
\ccThreeToTwo
%
\ccMethod{Site_2 up();}{return the defining site above of
  the Voronoi edge.}
\ccGlue
\ccMethod{Site_2 down();}{return the defining site below of
  the Voronoi edge.}
\ccGlue
\ccMethod{Site_2 left();}{return the defining site to the left of
  the Voronoi edge.
\ccPrecond{\ccc{has_source()} must be \ccStyle{true}.}}
\ccGlue
\ccMethod{Site_2 right();}{return the defining site to the right of
  the Voronoi edge.
