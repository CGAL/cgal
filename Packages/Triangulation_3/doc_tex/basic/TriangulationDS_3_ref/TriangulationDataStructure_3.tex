...
To ensure all the \textbf{flexibility} of the class \ccc{Triangulation_3}
described in 
...
\ccGlue
\ccTypedef{typedef pair<Cell_handle, int>  Facet;}{\ccc{(c,i)} is the facet
...
copy_tds(const TriangulationDataStructure_3 & tds1, Vertex_handle v = NULL);}
...
\ccMethod{Vertex_handle insert_increase_dimension(Vertex_handle star = NULL);}
...
\ccMethod{void remove_decrease_dimension(Vertex_handle v);}
...
\ccVar.\ccc{degree(v)} $=$ \ccVar.\ccc{number_of_vertices()} $-1$.}
}

\ccMethod{Cell_handle remove_from_simplex(Vertex_handle v);}
...
operation is exactly the reciprocal to \ccVar.\ccc{insert_in_cell(v)}.
...
\ccMethod{Vertex_handle create_vertex();}
{Creates a vertex and adds it to the triangulation data structure.}

\ccMethod{Cell_handle create_cell();}
...
\ccMethod{Cell_handle create_cell(Vertex_handle v0, Vertex_handle v1, Vertex_handle v2, Vertex_handle v3);}
...
\ccMethod{template <class CellIt> void delete_cells(CellIt begin, CellIt end);}
{Calls \ccc{delete_cell} over an iterator range.}
