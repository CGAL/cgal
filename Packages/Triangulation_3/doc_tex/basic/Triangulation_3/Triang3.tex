...
also shows two other classes: \ccc{Triangulation_utils_3}, which provides
a set of tools operating on the indices of vertices in cells\lcTex{
(\ccRefPage{CGAL::Triangulation_utils_3})}, and \ccc{Triangulation_hierarchy_3},
which implements a hierarchy of triangulations suitable for speeding up point
location.
...
\subsection{The Triangulation Data Structure}
...
initially given a fake TDS parameter, which is then \textit{rebound} to the real
TDS type by and inside the TDS itself.  Rebinding is performed by a nested
...
it gets slightly more involved because the base class has to be rebound as
well:
...
\subsubsection{Backward compatility}

Starting with \cgal\ release 3.0, the design has been changed in order to
