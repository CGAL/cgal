...
elementary incidence and adjacency (and eventually geometric or other)
...
the eventual geometric triangulation built on top of it (the upper layer, not
described here).
\end{itemize}
...
and \ccc{Triangulation_cell_base_with_info}, which allow to add one field of
a user defined type, and give access to it.
...
concepts \ccc{TriangulationVertexBase_3} and \ccc{TriangulationCellBase_3}
concepts described in \ccRefPage{TriangulationVertexBase_3} and
\ccRefPage{TriangulationCellBase_3}.
\end{itemize}
...
which is the rebound vertex or cell base class\footnote{It is logically
equivalent to a mecanism which does not exist yet in the C++ language:
\textit{template typedef} or \textit{template aliasing}}.
