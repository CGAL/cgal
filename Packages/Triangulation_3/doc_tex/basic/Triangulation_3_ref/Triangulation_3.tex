...
The second argument has a default value, which is
...
\ccc{operator*} and \ccc{operator->}. Iterators and circulators are
...
of the iterator range \ccc{[first,last)} of value type \ccc{Point}.} 
...
duplication does not modify the original.}
...
is connected, its boundary is connected, and \ccc{p} lies inside the
hole, which is star-shaped wrt \ccc{p}}.}
