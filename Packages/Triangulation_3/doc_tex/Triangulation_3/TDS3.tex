...
\ccVar.\ccc{dimension()} $<3$ then the set is empty.
...
\ccVar.\ccc{number_of_vertices()} $<2$ then the set is empty.
\ccPrecond{\ccc{v} $\neq$ \ccc{NULL}, \ccVar.\ccc{is_vertex(v)} and
the optional argument \ccc{c} is a cell having \ccc{v} as vertex.}}
...
\protect \ccc{Triangulation_data_structure_3<Vb,Cb>} }
...
\label{TDS3-sec-class-Base}
	...
	\subsection{The Base Class \protect \ccc{Triangulation_vertex_base_3<Traits>} }
...
\ccInclude{CGAL/Triangulation_vertex_base_3.h}
	...
	\subsection{The Base Class \protect \ccc{Triangulation_cell_base_3<Traits>}}
