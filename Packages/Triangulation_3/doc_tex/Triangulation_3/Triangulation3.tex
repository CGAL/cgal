...
{The triangulation \ccc{tr} is duplicated, and modifying one after the 
...
\ccc{Triangulation_cell_3<Traits,Tds>}.
...
\ccConstructor{\ccClassName(Vertex_handle v0,	
    	                    Vertex_handle v1,
                            Vertex_handle v2,
                            Vertex_handle v3);}
...
Let $\widehat{S}$ be a set of weighted points in $\R^3$. Let
$\widehat{p}=(p,w_p), p\in\R^3, w_p\in\R$ and 
$\widehat{z}=(z,w_z), z\in\R^3, w_z\in\R$ be two weighted points. 
A weighted point
$\widehat{p}=(p,w_p)$ can also be seen as a sphere of center $p$ and
radius $w_p$. 
The \textit{power product} between $\widehat{p}$ and $\widehat{z}$ is
defined as 
\[\Pi(\widehat{p},\widehat{z}) = {\|{p-z}\|^2-w_p^2-w_z^2}\]
where $\|{p-z}\|$ is the Euclidean distance between $p$ and $z$. 
 $\widehat{p}$ and $\widehat{z}$
are said to be \textit{orthogonal} if $\Pi{(\widehat{p}-\widehat{z})}
...
\textit{power sphere}. A sphere $\widehat{z}$ is said to be
\textit{regular} if $\forall \widehat{p}\in\widehat{S},
\Pi{(\widehat{p}-\widehat{z})}\geq 0$.

A triangulation of $\widehat{S}$ is \textit{regular} if the power spheres
of all simplices are regular. 

The regular triangulation of
$\widehat{S}$ is in fact the projection onto $\R^3$ of the convex hull 
of the four-dimensional points $(p,\|p-O\|^2-w_p),
\widehat{p}=(p,w_p)\in\widehat{S}$. 
Note that all points of $\widehat{S}$ do not
...
point $p\in\R^3$ or the weighted point $\widehat{p}=(p,w_p)$.
...
\ccc{p} of weighted points $\widehat{p}=(p,w_p)$}
\ccGlue
...
\ccConstructor{\ccClassName(const Regular_triangulation_3<Traits,Tds> & dt1)}
{Copy constructor.}
...
Let us remark that 
\[\Pi(\widehat{p}-\widehat{z}) > 0\]
is equivalent to\\
\centerline{\ccc{p} lies outside the sphere of center \ccc{z} and radius
$\sqrt{w_p^2+w_z^2}$.}\\
...
- \ccc{ON_BOUNDED_SIDE} if $\Pi(\widehat{p}-\widehat{z(c)})<0$ where
$\widehat{z(c)}$ is the power sphere of \ccc{c}. For an
infinite cell this means either that \ccc{p} lies strictly in the half
space limited by its finite facet, or that the angle
between \ccc{p} and the power circle of the *finite* facet of \ccc{c}
is greater than $\pi/2$. \\  
- \ccc{ON_BOUNDARY} if p is orthogonal to the power sphere of \ccc{c}
i.e. $\Pi(\widehat{p}-\widehat{z(c)})=0$. For an infinite cell this means
that \ccc{p} is orthogonal to the power circle of its *finite* facet.\\ 
- \ccc{ON_UNBOUNDED_SIDE} if $\Pi(\widehat{p}-\widehat{z(c)})>0$
i.e. the angle between the weighted point \ccc{p} and the power sphere
of \ccc{c} is less than $\pi/2$ or if they do not intersect (for an
infinite cell this means that \ccc{p} is not in the preceding two cases).
\ccPrecond{\ccc{\ccVar.dimension()} $=3$.}}
...
\ccc{f}, it returns:\\
\ccc{ON_BOUNDED_SIDE} if $\Pi(\widehat{p}-\widehat{z(e)})<0$, where
$\widehat{z(e)}$ is the power segment of \ccc{e} in the line supporting
\ccc{e},\\ 
\ccc{ON_BOUNDARY} if $\Pi(\widehat{p}-\widehat{z(e)})=0$,\\
\ccc{ON_UNBOUNDED_SIDE} if $\Pi(\widehat{p}-\widehat{z(e)})>0$ .
\ccPrecond{\ccc{\ccVar.dimension()} $\geq 2$.}}
...
\ccc{ON_BOUNDED_SIDE} if $\Pi(\widehat{p}-\widehat{z(c)})<0$, where
$\widehat{z(c)}$ is the power segment of the edge represented by
\ccc{c},\\
\ccc{ON_BOUNDARY} if $\Pi(\widehat{p}-\widehat{z(c)})=0$,\\
\ccc{ON_UNBOUNDED_SIDE} if $\Pi(\widehat{p}-\widehat{z(c)})>0$ .
\ccPrecond{\ccc{\ccVar.dimension()} $= 1$.}}
...
\ccPrecond{\ccc{p,q,r,test} are coplanar and \ccc{p,q,r} are not collinear.}} 
...
{Let $\widehat{z(p,q,r,s)}$ be the power sphere of the weighted points 
$(p,q,r,s)$. Returns \\
...
(which is equivalent to $\Pi(\widehat{t},\widehat{z(p,q,r,s)} >0$)),\\
...
with the power circle $\widehat{z(p,q,r)}$.
...
\ccMethod{Oriented_side power_test(const Weighted_point &p,
			   const Weighted_point &q,
			   const Weighted_point &t) const;}
{Same for collinear points, where $\widehat{z(p,q)}$ is the
...
type for point $p$ of a weighted point $\widehat{p}=(p,w_p)$.}
\ccGlue
\ccTypedef{typedef Weighted_point <Bare_point, Weight> Weighted_point;}{}

Unless specified, the type \ccc{Weight} for weights is \ccc{R::RT}. 
