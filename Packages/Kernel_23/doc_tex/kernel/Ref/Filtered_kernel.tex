...
predicates given an exact kernel \ccc{EK} and a filtering kernel
\ccc{FK}.  The geometric objects and constructions are exactly those
of the kernel \ccc{CK}, which means that there is no type equality
between, say \ccc{Point_2<Kernel>} and \ccc{Kernel::Point_2}, when
\ccc{Kernel} is \ccClassTemplateName. Hence, you should always use
\ccc{Kernel::Point_2} instead of \ccc{Point_2<Kernel>}.
