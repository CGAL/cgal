...
\ccHidden \ccConstructor{Quotient(const Quotient<NT> &q);}
 	    {copy constructor.}
\ccGlue
\ccConstructor{Quotient(int i)}
            {introduces the quotient \ccStyle{i/1}.}
\ccGlue
\ccConstructor{Quotient(const NT& n)}
...
If \ccc{CGAL_PROVIDE_STL_PROVIDED_REL_OPS} is defined,
the comparison operations $<=,\ >,\ >=,\ !=$ are provided, too.
...
\ccImplementation 
The implementation of the quotient class template largely follows the implementation
of the class \ccc{leda\_rational} in \leda.
