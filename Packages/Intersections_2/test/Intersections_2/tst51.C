...
void one_pair(line_t const & line, trian_t const & trian)
{
    ...
    point_t point;
/*
    CGAL::Line_2_Triangle_2_pair<TestR> pair(&line, &trian);
    switch (pair.intersection_type()) {
    case CGAL::Line_2_Triangle_2_pair<TestR>::SEGMENT:
	...
	cout<<'\n';
	break;
    case CGAL::Line_2_Triangle_2_pair<TestR>::POINT:
	cout<<"Point intersection.\n";
	pair.intersection(point);
	print(point);
	cout<<'\n';
	break;
    case CGAL::Line_2_Triangle_2_pair<TestR>::NO:
	cout<<"No intersection.\n";
	break;
    }
*/

    if (CGAL::do_intersect(line, trian))
	;

    CGAL::Object result = CGAL::intersection(line, trian);
