...
\begin{ccHtmlClassFile}{computing_maximum_area_inscribed_k_gon.html}
  ...
  \ccc{RandomAccessIC} is fixed to \ccc{std::vector<Point_2>::iterator}
...
\begin{ccHtmlClassFile}{computing_maximum_perimeter_inscribed_k_gon.html}
  ...
  \ccc{std::vector<Point_2>::iterator} where \ccc{Point_2} is the
  value type of \ccc{RandomAccessIC}.
...
\begin{ccAdvanced}
  ...
  \ccHtmlNoClassToc\ccHtmlNoClassIndex\begin{ccClass}{Exp_traits}
    ...
    \item On compilers not supporting member function templates,
      \ccc{RandomAccessIC} is fixed to
      \ccc{std::vector<Point_2>::iterator} and \ccc{OutputIterator} is
      fixed to \ccc{std::vector<int>::reverse_iterator}.
