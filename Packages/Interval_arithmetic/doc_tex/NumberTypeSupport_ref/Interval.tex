...
\begin{ccRefClass}{Interval_nt<bool Protected = true>}
\label{interval}
...
The boolean template parameter \ccc{Protected} provides a way to make faster
computations by avoiding rounding mode switches.  The default value,
\ccc{true}, is the safe way, and takes of proper rounding mode changes.
When specifying \ccc{false}, the user has to take care about positionning
the rounding mode towards plus infinity before doing any computations with
the interval class.
...
\ccTypes
The class \ccc{Interva_nt} defines the following types:
\ccThree{typedef std::exception unsafe_comparison;}{Tetrahedronxxxxx}{}
\ccThreeToTwo
...
\ccSetThreeColumns{xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx}{}{\hspace*{8cm}}
