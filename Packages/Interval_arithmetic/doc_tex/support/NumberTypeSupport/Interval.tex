...
\item When optimizing, compilers usually propagate the value of variables when
      ...
      with the default rounding mode, which is wrong.  This kind of problems
      can be avoided globally by setting the flag
      \ccc{CGAL_IA_STOP_CONSTANT_PROPAGATION}.  However, this slows down the
      code noticeably and is usually not useful, thus the workaround is not
      activated by default.  A simple way to stop this optimization in a
      particular case is to put the constant in a global, non static,
