...
Several functions \ccc{CGAL_to_Interval_nt(NT)} provide a cast from all
supported numerical type, to an interval \ccc{CGAL_Interval_nt}
...
\item \ccc{CT} is the construction and storage type.  The only data member of
      the class \ccc{CGAL_Filtering<CT,ET>} is the \ccc{value}, of type
      \ccc{CT}.
...
the type \ccc{CGAL_Filtering<CT,ET>}.  It's a partial specialisation, which
...
\item convert the entries to intervals using
    \ccc{CGAL_to_Interval_nt_advanced(CT)}.
...
\item if an exception is thrown (the filter failed), convert the original
    entries using \ccc{CGAL_to_Exact_type<ET>(CT)}.
...
to produce the files \ccc{CGAL/Filter/predicates_on_ftC2.h} (resp. ftC3, rtH2,
rtH3).
