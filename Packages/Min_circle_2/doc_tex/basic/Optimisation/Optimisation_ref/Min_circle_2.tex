...
% Subsec.: 2D Smallest Enclosing Circle
% -----------------------------------------------------------------------------
...
\ccHtmlNoClassToc
\begin{ccClassTemplate}{Min_circle_2<Traits>}
\ccSubsection{2D Smallest Enclosing Circle}
...
smallest area enclosing a finite set of points in the two-dimensional
...
We provide a traits class implementation using the 2D \cgal-kernel as
...
\ccTexHtml{Section~\ref{sec:min_circle_2_traits_req}}{Section&nbsp;<A HREF="requirements_Min_circle_2.html"><IMG SRC="../gif/cc_ref_up_arrow.gif" ALT="reference arrow" WIDTH="10" HEIGHT="10"></A>}.

% -----------------------------------------------------------------------------
\ccHeading{Guidelines and Hints}

For ultimate speed, use the representation type \ccc{Cartesian<float>}
or \ccc{Cartesian<double>}. For exact number types \ccc{NT}, we
recommend the homogeneous representation \ccc{Homogeneous<NT>}, even
if \ccc{NT} is a field type and supports divsions; the latter are
usually slower than the other arithmetic operations, and it pays off
to avoid them by using the homogeneous representation.

Because an object of type \ccClassTemplateName\ is of non-constant
size, copy and assignment are expensive. Use references whenever
possible.

% -----------------------------------------------------------------------------
\ccTypes

\ccSetThreeColumns{typedef Traits::Circle}{Circle;}{}
...
\ccSetTwoColumns{Min_circle_2<Traits>:: Support_point_iterator}{}
...
\emph{Note:} In case a compiler does not support member templates yet,
we provide specialized constructors instead. In the current release
there are constructors for C arrays (using pointers as iterators), for
the \stl\ sequence containers \ccc{vector<Point>} and \ccc{list<Point>}
and for the \stl\ input stream iterator \ccc{istream_iterator<Point>}.

\ccHidden
\ccConstructor{ Min_circle_2( const Point*   first,
                              const Point*   last,
                              bool           randomize = false,
                              Random&        random    = default_random,
                              const Traits&  traits    = Traits());}{
...
\ccHidden
\ccConstructor{ Min_circle_2( list<Point>::const_iterator  first,
...
\ccHidden
\ccConstructor{ Min_circle_2( istream_iterator<Point,ptrdiff_t>  first,
...
\ccUnchecked
\ccHidden
\ccConstructor{ Min_circle_2( const Min_circle_2<Traits>&);}{
        copy constructor.}

\ccHidden
\ccConstructor{ ~Min_circle_2( );}{
        destructor.}

\ccUnchecked
\ccHidden
\ccMemberFunction{ Min_circle_2<Traits>&
                   operator = ( const Min_circle_2<Traits>&);}{
        assignment operator.}
...
\newlineByHand
        on the boundary of, or properly outside of \ccVar, resp.}
...
\emph{Note:} In case a compiler does not support member templates yet,
we provide specialized \ccc{insert} functions instead. In the current
release there are \ccc{insert} functions for C arrays (using pointers
as iterators), for the \stl\ sequence containers \ccc{vector<Point>}
and \ccc{list<Point>} and for the \stl\ input stream iterator
...
\begin{ccAdvanced}
  ...
  \end{itemize}
  Using the traits class implementation for the 2D \cgal-kernel with
  exact arithmetic as described in
...
\ccc{Min_sphere_d}
  \ccTexHtml{(Section~\ref{sec:min_sphere_d_spec})}{}.
...
in~\cite{gs-seceg-98}. If randomization is chosen, the creation time
