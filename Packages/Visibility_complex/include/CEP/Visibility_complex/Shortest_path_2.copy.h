...
public:
    ...
    typedef Base::Edge_handle                          Edge_handle;
...
__shortest_path_2(_Edge_handle s , Visibility_complex_2<_Gtr,_It>* /*V*/) // warning V never used
{
    ...
    typedef Visibility_complex::Vertex_iterator         Vertex_iterator;
	...
	Antichain::Vertex_handle v = a.next_vertex(s->object());
	Antichain::Edge_handle   e = a.next_edge(s->object());

	if (v != 0) {
	    Exact_NT d = da + v->weight();
	    if (v->distance() == -1 || d < v->distance()) {
		if (v->distance() != Exact_NT(-1)) {
		    Queue::iterator vit = X.find(Atom(v));
	...
	if (e != 0) {
		...
		if (e->distance() != Exact_NT(-1)) {
		    Queue::iterator eit = X.find(Atom(e));
	       ...
	       OutputIterator result, Visibility_complex_2<_Gtr,_It>* V)
{
    ...
    typedef Antichain::Vertex                           Vertex;
    typedef Visibility_complex_2<_Gtr,_It>              Visibility_complex;
    typedef Antichain::Edge_iterator                    Edge_iterator;
    typedef Antichain::Edge_handle                      Edge_handle;
    // Stop if two disks intersect ---------------------------------------------
    if (!V->is_valid()) return Exact_NT(-1);
    // Find edges two edges with opposite sign on t ----------------------------
    Antichain::Edge_handle ept(0),emt(0);
		...
		OutputIterator result, Visibility_complex_2<_Gtr,_It>* V)
{
    ...
    Antichain::Edge_handle eps(0),ems(0);
    Edge_iterator e = V->antichain()->edges_begin();
    while (eps == 0 || ems == 0) {
	CGAL_precondition(e != V->antichain()->edges_end());
		    ...
		    OutputIterator result, Visibility_complex_2<_Gtr,_It>* V)
{
    ...
    typedef Visibility_complex_antichain<_Gtr,_It>  Antichain;
    typedef Antichain::Vertex                       Vertex;
    typedef Antichain::Disk_handle               Disk_handle;
    // Stop if two disks intersect ---------------------------------------------
    if (!V->is_valid()) return result;
    // Find edges on s ---------------------------------------------------------
    Antichain::Edge_handle ep(0),em(0);
    Antichain::Edge_iterator ei = V->antichain()->edges_begin();
    while (ep == 0 || em == 0) {
    ...
    int i = 0;
    for (std::vector<Disk_handle>::iterator eh  = points.begin(); 
					       eh != points.end() ; ++eh , ++i) 
...
// -----------------------------------------------------------------------------
/*
// af: put this in a comment because for bcc there is an ambiguity 
//     between all the shortest_path_2 functions with 9 arguments

template < class InputIterator  , class ConstraintIt, 
	   class _Disk_handle, class OutputIterator , 
	   class _Gtr , class _It>
OutputIterator
shortest_path_2(InputIterator first ,InputIterator last ,
		ConstraintIt  firstc,ConstraintIt  lastc,
		_Disk_handle s   ,_Disk_handle t  ,
		OutputIterator result, _Gtr , _It)
{
    Visibility_complex_2<_Gtr,_It> V(first,last,firstc,lastc);
...
template < class InputIterator , class ConstraintIt, class OutputIterator , 
	   class _R, class _Gtr , class _It>
OutputIterator
shortest_path_2(InputIterator first, InputIterator last ,
		ConstraintIt  firstc,ConstraintIt  lastc,
		const Point_2<_R>& p, const Point_2<_R>& q,
		OutputIterator result, _Gtr , _It )
{
    // -------------------------------------------------------------------------
    typedef typename _Gtr::Exact_NT                     Exact_NT;
