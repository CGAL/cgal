// GyrovizView_3.cpp : implementation of the CGyrovizView_3 class
//
...
IMPLEMENT_DYNCREATE(CGyrovizView_3, CView)

BEGIN_MESSAGE_MAP(CGyrovizView_3, CView)
	ON_WM_CREATE()
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
  ...
  ON_COMMAND(ID_RENDER_TRIANGLES, &CGyrovizView_3::OnRenderTriangles)
  ON_UPDATE_COMMAND_UI(ID_RENDER_TRIANGLES, &CGyrovizView_3::OnUpdateRenderTriangles)
  ON_COMMAND(ID_RENDERCOFF_RAYS, &CGyrovizView_3::OnRendercoffRays)
  ON_UPDATE_COMMAND_UI(ID_RENDERCOFF_RAYS, &CGyrovizView_3::OnUpdateRendercoffRays)
  ON_COMMAND(ID_RENDERCOFF_INSIDE, &CGyrovizView_3::OnRendercoffInside)
  ON_UPDATE_COMMAND_UI(ID_RENDERCOFF_INSIDE, &CGyrovizView_3::OnUpdateRendercoffInside)
END_MESSAGE_MAP()

// CGyrovizView_3 construction/destruction

CGyrovizView_3::CGyrovizView_3()
{
	// OpenGL
	m_hGLContext = NULL;
	m_GLPixelIndex = 0;
...
BOOL CGyrovizView_3::PreCreateWindow(CREATESTRUCT& cs)
{
...
// CGyrovizView_3 drawing

void CGyrovizView_3::OnDraw(CDC* /*pDC*/)
{
	CGyrovizDoc_3* pDoc = GetDocument();
	ASSERT_VALID(pDoc);
	if (!pDoc)
		return;
...
// CGyrovizView_3 diagnostics

#ifdef _DEBUG
void CGyrovizView_3::AssertValid() const
{
	CView::AssertValid();
}

void CGyrovizView_3::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}

CGyrovizDoc_3* CGyrovizView_3::GetDocument() const // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CGyrovizDoc_3)));
	return (CGyrovizDoc_3*)m_pDocument;
}
#endif 
...
//********************************************
BOOL CGyrovizView_3::SetWindowPixelFormat(HDC hDC)
{
...
//********************************************
BOOL CGyrovizView_3::CreateViewGLContext(HDC hDC)
{
...
// CGyrovizView_3 message handlers

int CGyrovizView_3::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CGyrovizView_3::OnPaint()
{
    ...
    if (pDoc->is_dt2())
    {
      arcball_sphere = pDoc->get_dt2().region_of_interest();
    }

    else 
    {
      arcball_sphere = pDoc->get_dt3().region_of_interest();
    }
    ...
    if(m_view_delaunay_edges)
    {
      if (pDoc->is_dt2())
      {
        pDoc->get_dt2().gl_draw_soup_edges(0,0,255,1.0f);
      }
      else
        pDoc->get_dt3().gl_draw_3D_delaunay_triangulation(0,191,255,1.0);
    }
    ...
    if(m_view_delaunay_triangles)
    {
      if (pDoc->is_dt2())
      {
        pDoc->get_dt2().gl_draw_soup_triangles(0,255,0);
      }
      else 
        pDoc->get_dt3().gl_draw_3D_delaunay_triangulation(0,191,255,1.0);
    }
    ...
    if(m_view_coff_rays)
      pDoc->get_dt3().gl_draw_3D_rays(255,0,0);
...
void CGyrovizView_3::OnLButtonDown(UINT nFlags, CPoint point)
{
	SetCapture();
...
void CGyrovizView_3::OnLButtonUp(UINT nFlags, CPoint point)
{
	ReleaseCapture();
	m_LeftButtonDown = FALSE;
	m_moving = false;
...
void CGyrovizView_3::OnMButtonDown(UINT nFlags, CPoint point)
{
	SetCapture();
...
void CGyrovizView_3::OnMButtonUp(UINT nFlags, CPoint point)
{
	ReleaseCapture();
	m_MiddleButtonDown = FALSE;
	m_moving = false;
...
void CGyrovizView_3::OnRButtonDown(UINT nFlags, CPoint point)
{
	SetCapture();
...
void CGyrovizView_3::OnRButtonUp(UINT nFlags, CPoint point)
{
	ReleaseCapture();
	m_RightButtonDown = FALSE;
	m_moving = false;
...
// Mouse move callback
void CGyrovizView_3::OnMouseMove(UINT nFlags, CPoint point)
{
  if (m_KeyboardFlags != nFlags) // update keyboard status and redraw arcball sphere
  {
	    m_KeyboardFlags = nFlags;
...
BOOL CGyrovizView_3::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	m_KeyboardFlags = nFlags;
...
void CGyrovizView_3::OnSize(UINT nType, int cx, int cy)
{
...
void CGyrovizView_3::OnRenderTriangles()
{
	m_view_delaunay_triangles = !m_view_delaunay_triangles;
	InvalidateRect(NULL,FALSE);
}
void CGyrovizView_3::OnUpdateRenderTriangles(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_delaunay_triangles);
}

void CGyrovizView_3::OnRenderDelaunayedges()
{
	m_view_delaunay_edges = !m_view_delaunay_edges;
	InvalidateRect(NULL,FALSE);
}
void CGyrovizView_3::OnUpdateRenderDelaunayedges(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_delaunay_edges);
}

void CGyrovizView_3::OnRenderPoints()
{
	m_view_delaunay_vertices = !m_view_delaunay_vertices;
	InvalidateRect(NULL,FALSE);
}
void CGyrovizView_3::OnUpdateRenderPoints(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_delaunay_vertices);
}

void CGyrovizView_3::OnArcballReset()
{
  m_pArcball->reset();
  InvalidateRect(NULL,FALSE);
}

void CGyrovizView_3::OnRenderArcball()
{
  m_view_arcball = !m_view_arcball; 
  InvalidateRect(NULL,FALSE);
}
void CGyrovizView_3::OnUpdateRenderArcball(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(m_view_arcball);
}


void CGyrovizView_3::OnRendercoffRays()
{
  m_view_coff_rays = !m_view_coff_rays; 
  InvalidateRect(NULL,FALSE);
}

void CGyrovizView_3::OnUpdateRendercoffRays(CCmdUI *pCmdUI)
{      
  //if (!pDoc->is_dt2())
...
void CGyrovizView_3::OnRendercoffInside()
{
  m_view_coff_inside = !m_view_coff_inside; 
  InvalidateRect(NULL,FALSE);
}

void CGyrovizView_3::OnUpdateRendercoffInside(CCmdUI *pCmdUI)
{
//  if (!pDoc->is_dt2())
