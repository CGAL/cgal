...
BEGIN_MESSAGE_MAP(CGyrovizView_2, CView)
	...
	ON_COMMAND(ID_RENDER_CONSTRAINTS, &CGyrovizView_2::OnRenderConstraints)
	ON_UPDATE_COMMAND_UI(ID_RENDER_CONSTRAINTS, &CGyrovizView_2::OnUpdateRenderConstraints)
END_MESSAGE_MAP()
...
void CGyrovizView_2::resize(UINT nType, int cx, int cy)
{
	...
	double w = (double)cx;
	double h = (double)cy;
	if(w/h > image_dim_x/image_dim_y)
	{
		double ratio = (w/h) / (image_dim_x/image_dim_y);
		m_xmin = image_dim_x*(1.0-ratio)/2.0;
		m_xmax = image_dim_x*(1.0+ratio)/2.0;
		m_ymin = 0;
		m_ymax = image_dim_y;
	}
	...
	{
		double ratio = (h/w) / (image_dim_y/image_dim_x);
		m_xmin = 0;
		m_xmax = image_dim_x;
		m_ymin = image_dim_y*(1.0-ratio)/2.0;
		m_ymax = image_dim_y*(1.0+ratio)/2.0;
	}
...
void CGyrovizView_2::OnPaint()
{
	...
	{
		//::glColor3ub(255,0,0);
		pDoc->get_cdt2().gl_draw_2D_constrained_edges(255,0,0,2/*,pDoc->m_cimg_seg_image,1*/);
	}
	...
	if(m_view_2D_delaunay_triangulation)
	{
		...
		/*pDoc->get_cdt2().gl_draw_2D_delaunay_triangles(0,255,0);*/
		pDoc->get_cdt2().gl_draw_2D_constrained_delaunay_triangles(0,255,0,2);
	}
...
void CGyrovizView_2::OnMouseMove(UINT nFlags, CPoint point)
{
	CGyrovizDoc_2* pDoc = GetDocument();
...
void CGyrovizView_2::OnRenderConstraints()
{
	m_view_constraints = ! m_view_constraints;
	InvalidateRect(NULL,FALSE);
}

void CGyrovizView_2::OnUpdateRenderConstraints(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(m_view_constraints);
}
