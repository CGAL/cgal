...
\ccFunction{OrientableNormal_3 pca_normal_estimation(const typename Kernel::Point_3& query, Tree& tree, unsigned int KNN);}
{
Estimate normal direction using linear least squares fitting of a plane on the K nearest neighbors.
Precondition: KNN $>$= 2.
}
\ccGlue
\begin{description}
...
\item[Parameters: ]
\begin{description}
\item[query]3D point whose normal we want to compute \item[tree]KD-tree \item[KNN]number of neighbors \end{description}
\end{description}
\ccGlue
...
\ccc{pca_normal_estimation} (const typename \ccc{Kernel::Point_3}\& query, Tree\& tree, unsigned int KNN);  \\
  \\
template$<$  \\
