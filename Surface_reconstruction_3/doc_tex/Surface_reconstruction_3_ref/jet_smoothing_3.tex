...
The \ccc{CGAL::jet_smoothing_3()} function exists in four flavors.
...
\ccFunction{Kernel::Point_3 jet_smoothing_3(const typename Kernel::Point_3& query, Tree& tree, const unsigned int KNN, const unsigned int degre_fitting, const unsigned int degree_monge);}
{
...
typename Tree$>$  \\
\ccc{Kernel::Point_3}  \\
\ccc{jet_smoothing_3} (const typename \ccc{Kernel::Point_3}\& query, Tree\& tree, const unsigned int KNN, const unsigned int \ccc{degre_fitting}, const unsigned int \ccc{degree_monge});  \\
  \\
template$<$  \\
