...
\item \ccc{Lightweight_vector_3<Geom_traits>}: 3D vector allocated only if not (0,0,0).
\item \ccc{Orientable_normal_3<Geom_traits>}: Normal (oriented or not).
Inherits from \ccc{Vector_3<Geom_traits>} and contains an "is normal oriented?" flag.
\item \ccc{Point_with_normal_3<Geom_traits, Normal_3>}: 3D position + normal.
\end{itemize}
