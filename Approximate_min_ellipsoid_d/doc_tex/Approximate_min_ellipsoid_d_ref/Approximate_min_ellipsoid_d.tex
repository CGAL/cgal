...
\ccc{RandomAccessIterator} that is used to iterate over the Cartesian
center coordinates of the computed ellipsoid, see
\ccc{center_cartesian_begin()}.}
\ccNestedType{Axes_lengths_iterator}{A model of STL concept
\ccc{RandomAccessIterator} that is used to iterate over the lengths of
the semiaxes of the computed ellipsoid, see
\ccc{axes_lengths_begin()}.}  \ccNestedType{Axes_lengths_iterator}{A
model of STL concept \ccc{RandomAccessIterator} that is used to
iterate over the Cartesian coordinates of the direction of a fixed
...
approximation ellipsoid, you can use the following functions
\ccc{center_cartesian_begin()}, \ccc{axes_lengths_begin()}, and
\ccc{axis_direction_cartesian_begin()}. In constrast to the above
access functions \ccc{achieved_epsilon()}, \ccc{defining_matrix()},
...
\ccMemberFunction{Center_coordinate_iterator center_cartesian_begin();}{
  ...
  The returned point is a floating-point approximation to the
	...
	\ccc{\ccVar.is_full_dimensional()} is \ccc{true}.}
...
\ccMemberFunction{Axes_lengths_iterator axes_lengths_begin();}{
    ...
    \ccc{\ccVar.is_full_dimensional()} is \ccc{true}, and
    $d\in\{2,3\}$.}
...
\ccc{\ccVar.is_full_dimensional()} is \ccc{true}, and $d\in\{2,3\}$,
and $0\leq i < d$.}
