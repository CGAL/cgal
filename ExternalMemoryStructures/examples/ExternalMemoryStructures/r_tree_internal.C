...
//
// file          : include/CGAL/R_Tree/examples/ExternalMemoryStructures/r_star_tree_internal.C
...
int main() {
  ...
  R_Tree_Inst r_tree("__star_tree.head","__star_tree.dat", "__star_leaf_data.dat");
  ...
  std::cerr << "\n Check for elements that intersects source[1].key=(1,3,1,3)\n";
  ...
  std::cerr << "\n Check for elements that intersect source[1].key=(1,3,1,3)\n";
  if(!r_tree.find_key_intersect(source[1].key))
    {
      std::cerr << "\n no key intersect of"; 
      traits.dump(traits.build(source[1]));
