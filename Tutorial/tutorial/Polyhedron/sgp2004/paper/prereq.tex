...
programming paradigm\/}~\cite{cgal:ms-aogl-94} %,cgal:sl-stl-95
% and in a few places also design patterns~\cite{Gamma:1995:DP}. 
...
%See also the
%recent textbooks on template programming
%techniques~\cite{Alexandrescu:2001:MCD,cgal:vj-ctcg-03}.
...
% Important related issues, for example visualization, are
%supported with standard formats and interfaces.

% An overview of \cgal\ is given in~\cite{Kettner04Handbook}. 
The design of \cgal\ is thoroughly covered
in~\cite{fgkss-dccga-00,bksv-agppd-00}. 
% and generic programming aspects are discussed
% in~\cite{bksv-agppd-00}. 
...
% traversal simplifies to mere pointer traversal. The edge split can be
% done in two ways, either assigning a halfedge to each incident facets,
% or assigning a halfedge to each incident vertex~\cite{Weiler85}, these
% are duals of each other. Combining both splits leads to the quad-edge
% data structure~\cite{Guibas:1983:PMG} that, deploying an additional
% bit per quad-edge and per referring pointer, can also represent
% non-orientable 2-manifold surfaces. A detailed comparison of these
...
%The quad-edge data structure is the theoretically more appealing data
...
% % item, i.e., direct expressions such as
% % \begin{lstlisting}
% % facet_handle->halfedge()->opposite()->facet()
% % \end{lstlisting}
...
% An iterator allows both referring to an entity (as a \CC\ pointer) 
