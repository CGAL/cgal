...
int main(int argc, char** argv) {
  ...
  const Point_3 p0(200, -200, -200), p0_new(400, -400, -400);
  const Point_3 p1(200, 200, -200), p1_new(400, 400, -400);
  const Point_3 p2(-200, 200, -200), p2_new(-400, 400, -400);
  const Point_3 p3(-200, -200, -200), p3_new(-400, -400, -400);

  const Point_3 p4(-200, -200, 200), p4_new(-400, -400, 400);
  const Point_3 p5(200, -200, 200), p5_new(400, -400, 400);
  const Point_3 p6(200, 200, 200), p6_new(400, 400, 400);
  const Point_3 p7(-200, 200, 200), p7_new(-400, 400, 400);

  CGAL::make_hexahedron(p0, p1, p2, p3, p4, p5, p6, p7, quad_cage,
    CGAL::parameters::do_not_triangulate_faces(false));

  FT vol_before = CGAL::Polygon_mesh_processing::volume(sm);
  ...
  std::vector<Point_3> source_cube{ p0, p1, p2, p3,
                                   p4, p5, p6, p7 };
  ...
  for (Surface_mesh::Vertex_index v : vertices(deformed)) {
    ...
    Point_3 orig = CGAL::Barycentric_coordinates::apply_barycentric_coordinates_3(source_cube, coords);
    assert((vertex_val - orig).squared_length() < 0.0000001); // inexact construction kernel, so we use a tolerance
    Point_3 p = CGAL::Barycentric_coordinates::apply_barycentric_coordinates_3(target_cube, coords);

    put(vertex_point_map, v, p);
  }

  FT vol_after = CGAL::Polygon_mesh_processing::volume(deformed);

  assert(std::abs(vol_after / vol_before - 8) < 0.001); // inexact construction kernel, so we use a tolerance
