...
{
  CGAL_FOREACH(Mesh::Halfedge_index hh, CGAL::halfedges_around_source(h, m))
  {
    if (!is_border(hh, m))
      triangles.insert(face(hh,m));
  }
  CGAL_FOREACH(Mesh::Halfedge_index hh, CGAL::halfedges_around_target(h, m))
  {
    if (!is_border(hh, m))
      triangles.insert(face(hh,m));
  }
...
{
  CGAL_FOREACH(Mesh::Face_index f, triangles)
  {
    CGAL_FOREACH(Mesh::Halfedge_index h, CGAL::halfedges_around_face(halfedge(f, m), m))
    {
...
void test(const char* fname)
{
  ...
  CGAL_FOREACH(Mesh::Halfedge_index h, halfedges(m))
  {
    ...
    if (CGAL::Euler::does_satisfy_link_condition(edge(h, m), m))
    {
      ...
      CGAL_FOREACH(const Mesh::Vertex_index& v, profile.link())
      {
    ...
    CGAL_FOREACH(const Profile::Triangle& t, profile.triangles())
    {
