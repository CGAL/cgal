...
\ccc{TSM} defines the type of the surface.
It must be a model of \ccc{TriangulatedSurfaceMesh}

\ccc{ShouldStop} defines the type of the stop-condition policy.
It must be a model of \ccc{StopCondition} concept
...
\ccc{VertexIsFixedMap} defines the type of the property map which 
indicates whether a vertex is fixed or not.
It must be a model of \ccc{VertexIsFixedMap} concept
...
\ccc{EdgeExtraPtrMap} defaults to \ccc{CGAL::Edge_extra_pointer_map_stored<TSM>()}.

\ccc{VertexIsFixedMap} defaults to \ccc{CGAL::Vertex_is_fixed_map_always_false<TSM>()}.

\ccc{SetCollaspeData} defauls to \ccc{CGAL::Surface_mesh_simplification::Edge_collapse::LindstromTurk_set_partial_collapse_data<TSM>}.

\ccc{GetCost} defaults to \ccc{CGAL::Surface_mesh_simplification::Edge_collapse::Cached_cost<TSM>}.

\ccc{GetPlacement} defaults to \ccc{CGAL::Surface_mesh_simplification::Edge_collapse::LindstromTurk_placement<TSM>}
