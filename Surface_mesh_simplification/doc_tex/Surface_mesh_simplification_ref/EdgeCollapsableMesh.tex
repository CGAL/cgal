...
\ccPrecond This function requires \ccc{mesh} to be an oriented 2-manifold with or without boundarties. Furthermore, the undirected edge \ccc{v0v1,v1v0} must satisfy the {\em link condition} 
...
\item One of $\{$\ccc{v0,v1}$\}$ is no longer in \ccc{mesh} while the other is kept. Let \ccc{vr} be the one removed and \ccc{vk} be the one kept.
...
\item For all edges \ccc{ie} in \ccc{in_edges(vr,mesh)}, \ccc{target(ie,mesh) == vk} and \ccc{source(opposite_edge(ie),mesh) == vk}.
