...
namespace internal {
    ...
    if (Binary)
      {
        ...
        Value_type out = get(map, *it);
        stream.write (reinterpret_cast<char*>(&out), sizeof (Value_type));
      }
    else
      stream << get (map, *it);
  }
  ...
  void output_properties (std::ostream& stream,
                          ...
                          cpp11::tuple<PropertyMap, Ply::Property<T>... >& current)
  {
    property_write<Binary> (stream, it, std::get<0>(current));
    if (!Binary)
      stream << std::endl;
  }
  ...
  void output_properties (std::ostream& stream,
                          ...
                          std::pair<PropertyMap, Ply::Property<T> >& current)
  {
    property_write<Binary> (stream, it, current.first);
    if (!Binary)
      stream << std::endl;
  }
                          ...
                          PropertyHandler&& ... properties)
  {
    ...
    if (!Binary)
      stream << " ";
    output_properties<Binary> (stream, it, next, properties...);
  }
  ...
  PropertyHandler&& ... properties)
{
  ...
  // Write header
  stream << "ply" << std::endl
         << (Binary ? "format binary_little_endian 1.0" : "format ascii 1.0") << std::endl
  ...
  {
    Ply::internal::output_properties<Binary> (stream, it, properties...);
  }
...
write_ply_points_with_properties(
  ...
  PropertyHandler&& ... properties)
{
  return write_ply_points_with_properties<true> (stream, first, beyond, properties...);
}
...
write_ply_points_and_normals(
  ...
  VectorMap normal_map) ///< property map: value_type of ForwardIterator -> Vector_3. 
{
  return write_ply_points_with_properties<Binary> (stream, first, beyond,
...
{
  return write_ply_points_and_normals<true>(
    stream,
    first, beyond,
    point_map,
    normal_map);
...
write_ply_points(
  ...
  PointMap point_map) ///< property map: value_type of OutputIterator -> Point_3.
{
  return write_ply_points<true> (stream, first, beyond, point_map);
}
/// @endcond
