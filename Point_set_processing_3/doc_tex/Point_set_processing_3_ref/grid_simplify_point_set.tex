...
\begin{description}
\item \ccc{first}: iterator over the first input point. \item \ccc{beyond}: past-the-end iterator over the input points. \item \ccc{point_pmap}: property map ForwardIterator -$>$ \ccc{Point_3} \item \ccc{epsilon}: tolerance value when merging 3D points. \item \ccc{kernel}: geometric traits. \end{description}
}
\ccGlue
