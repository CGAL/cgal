...
\ccc{CGAL::jet_smooth_point_set()} smoothes a point set by fitting for each point a jet surface and projecting it onto the jet. The default jet surfaces are quadrics.
...
\begin{description}
\item \ccc{first}: iterator over the first input point. \item \ccc{beyond}: past-the-end iterator over the input points. \item \ccc{point_pmap}: property map ForwardIterator -$>$ \ccc{Point_3}. \item \ccc{k}: number of neighbors. \item \ccc{kernel}: geometric traits. \end{description}
}
\ccGlue
