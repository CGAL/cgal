...
\ccFunction{template<typename ForwardIterator, typename PointPMap, typename Kernel> ForwardIterator remove_outliers(ForwardIterator first, ForwardIterator beyond, PointPMap point_pmap, unsigned int k, double removed_percentage, const Kernel& kernel);}
{
...
\begin{description}
\item \ccc{first}: iterator over the first input point. \item \ccc{beyond}: past-the-end iterator. \item \ccc{point_pmap}: property map ForwardIterator -$>$ \ccc{Point_3} \item \ccc{k}: number of neighbors. \item \ccc{removed_percentage}: percentage of points to remove. \item \ccc{kernel}: geometric traits. \end{description}
}
\ccGlue
