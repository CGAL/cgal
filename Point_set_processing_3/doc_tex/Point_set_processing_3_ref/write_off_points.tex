...
\ccc{CGAL::write_off_points()} saves the [first, beyond) range of points (positions + normals) to a .off ASCII stream.
...
\ccFunction{template<typename ForwardIterator, typename PointPMap, typename NormalPMap, typename Kernel> bool write_off_points(std::ostream& stream, ForwardIterator first, ForwardIterator beyond, PointPMap point_pmap, NormalPMap normal_pmap, const Kernel& kernel);}
{
