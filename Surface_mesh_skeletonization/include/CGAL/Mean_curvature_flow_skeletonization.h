...
public:
 ...
 void set_max_triangle_angle(double value)
  ...
  {
    fixed_points.clear();
    CGAL_FOREACH(vertex_descriptor vd, vertices(m_tmesh))
    {
      if (vd->is_fixed)
  ...
  void poles(std::vector<Point>& max_poles)
  {
    ...
    int cnt = 0;
    CGAL_FOREACH(vertex_descriptor v, vertices(m_tmesh))
    {
...
private:
  ...
  void init(const TriangleMesh& tmesh)
  {
    ...
    CGAL_FOREACH(const Vertex_pair& vp, v2v)
    ...
    CGAL_FOREACH( vertex_descriptor vd, vertices(m_tmesh) )
      vd->id()=i++;
    i=0;
    CGAL_FOREACH( halfedge_descriptor hd, halfedges(m_tmesh) )
      hd->id()=i++;
  ...
  void assemble_LHS(typename SolverTraits::Matrix& A)
  {
    ...
    CGAL_FOREACH(vertex_descriptor vd, vertices(m_tmesh))
    {
      ...
      double diagonal = 0;
      CGAL_FOREACH(edge_descriptor ed, in_edges(vd, m_tmesh))
      {
  ...
  std::size_t split_flat_triangles()
  {
    ...
    CGAL_FOREACH(edge_descriptor ed, edges(m_tmesh))
    {
    ...
    // now split the edge
    std::size_t cnt = 0;
    CGAL_FOREACH(edge_descriptor ed, edges_to_split)
    {
      halfedge_descriptor ei = halfedge(ed, m_tmesh);
      halfedge_descriptor ej = opposite(ei, m_tmesh);
      int ei_id = static_cast<int>(get(m_hedge_id_pmap, ei));
  ...
  void compute_voronoi_pole()
  {
    ...
    CGAL_FOREACH(const Pair_type& p, points)
    {
...
std::size_t Mean_curvature_flow_skeletonization<TriangleMesh, Traits_, VertexPointMap_, SolverTraits_>::collapse_short_edges()
{
  ...
  do{
    ...
    while(!edges_to_collapse.empty())
    {
      ...
      CGAL_FOREACH(edge_descriptor oed, out_edges(v, m_tmesh))
