...
public:
  ...
  void extract_skeleton(Graph& curve)
  {
    ...
    for (int i = 0; i < id; ++i)
    {
      ...
      CGAL_FOREACH(int vid, record[orig_id])
      {
    ...
    CGAL_FOREACH(vertex_descriptor vd, vertices(hg))
    {
...
private:
  void init()
  {
    ...
    idx = 0;
    CGAL_FOREACH(edge_descriptor ed, edges(hg))
    {
    ...
    int face_id = 0;
    CGAL_FOREACH(face_descriptor fd, faces(hg))
    {
      CGAL_FOREACH(halfedge_descriptor hd, halfedges_around_face(halfedge(fd,hg), hg))
      {
    ...
    CGAL_FOREACH(vertex_descriptor vd, vertices(hg))
    {
      ...
      CGAL_FOREACH(edge_descriptor ed, in_edges(vd, hg))
      {
        halfedge_descriptor hd = halfedge(ed, hg);
  ...
  {
    CGAL_FOREACH(int fid, edge_to_face[eid])
    {
  ...
  void check_edge()
  {
    CGAL_FOREACH(halfedge_descriptor hd, halfedges(hg))
    {
