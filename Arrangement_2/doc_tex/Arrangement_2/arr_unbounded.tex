...
Previous chapters dealt only with arrangements of line segments,
namely of bounded curves. Such arrangements always have one unbounded
face that contains all other arrangement features. In this section
...
Let us consider the arrangement induced by the two lines $y = x$ and
...
In general we do not have to access the source (or the target) of
halfedge if it lies at infinity, since this vertex is not associated
with any valid point. Similary, calling \ccc{arr.number_of_vertices()}
for an arrangement instance \ccc{arr} counts only the vertices
associated with finite points, and disregards vertices at
infinity (and the range \ccc{[vertices_begin(), vertices_end())}
contains only finite vertices).
One can obtain the number of vertices at infinity by
calling \ccc{arr.number_of_vertices_at_infinity()}.

As we have already mentioned, arrangements of unbounded curves
usually have more than one unbounded face. The function
\ccc{arr.number_of_unbounded_faces()} returns the number of unbounded
arrangement faces (thus, \ccc{arr.number_of_faces() -
arr.number_of_unbounded_faces()} is the number of bounded faces).
...
of unbounded faces. The value-type of this iterator is of course
...
$x$-monotone unbounded curves, provided that they interior-disjoint
from any subcurve that already exists in the arrangement. For example,
if we wish to insert a ray $r$ emanating from $(0,0)$ in the direction
of $(1,0)$, to the arrangement of $y = -x$ and $y = x$, we can use
the functions \ccc{arr.insert_from_left_vertex()}, as the left
...
the existance of collinearity is forced and verified as follows. A line
...
faces, namely a fictitous face lying outside of the imaginary bounding
...
We note that \ccc{arr.number_of_edges()} does not count the number of
...
at infinity do traverse fictitious halfegdes. For example, it is possible
