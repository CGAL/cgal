...
Observe that we have four extra vertices at infinity that do not lie
...
respectively. Introducing these four fictitious vertices, enables us
to label each of the fictitious edges as lying on the top, the bottom,
...
Observe that the \ccc{arr.number_of_vertices()} does not count the
number of vertices at infinity. Call
\ccc{arr.number_of_vertices_at_infinity()}, to obtain this number.
Similarly, \ccc{arr.number_of_edges()} does not count the number of
fictitious edges, (which is always
\ccc{arr.number_of_vertices_at_infinity() + 4}), and
\ccc{arr.number_of_faces()} does not count the number of fictitious
faces. The vertex, halfedge, edge, and face iterators defined by the
\ccc{Arrangement_2} class only iterate over true features of the
arrangement. Namely, vertices at infinity and fictitious halfedges
and faces are ignored. On the other hand, the
