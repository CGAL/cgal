...
//                (based on CORE_algebraic_number_traits.h 
                   by Ron Wein <wein@post.tau.ac.il> 
...
public:
	...
	//typedef leda::polynomial<Integer>		Polynomial;
	...
	*/
				   ...
				   unsigned int degree) const
	{
		...
		for (int i = 0; i <= degree; i++ ) cp[i]=coeffs[i];
		Polynomial   poly(cp);
...
/*	bool construct_polynomial (const Rational *coeffs,
				 ...
				 Polynomial& poly, Integer& poly_denom) const
	{
		...
		while (leda::sign (coeffs[degree]) == ZERO)
		{
		  if (degree == 0)
		return (false);
		  degree--;
...
*/
   ...
   */
	...
	//		oi = roots.begin();
		...
		//to rational polynomial
		LEDA_VECTOR<Rational> cp_rat(degree + 1);
		for (int i = 0; i <= degree; i++ ) cp_rat[i]=Rational(poly[i]);
		leda::polynomial<Rational>   poly_rat(cp_rat);

		//to real polynomial
		LEDA_VECTOR<Algebraic> cp_real(degree + 1);
		for (int i = 0; i <= degree; i++ ) cp_real[i]=Algebraic(poly[i]);
		leda::polynomial<Algebraic>   poly_real(cp_real);

		// Compute the real-valued roots of the polynomial.
		leda::isolating<leda::rational>	sturm (poly_rat);
		const unsigned int			n_roots = sturm.number_of_real_roots();

		//square free test 
		Polynomial g = leda::poly_gcd(poly, diff(poly));
		bool is_square_free = g.degree() == 0 ? true : false;
		//

		for (int j = 1; j <= n_roots; j++)
		{
		  // Get the j'th real-valued root.
			*oi = leda::diamond( j, poly_real, leda::real::isolating_algorithm , is_square_free);
		  ++oi;
		}
