  ...
  */
  ...
  bool is_valid(const Traits* traits) const
  {
	...
	if (b) {
      ...
      {
        if (right_top_neighbour() && 
            (! traits->curve_equal(right_top_neighbour()->top(),top())) ||
            left_top_neighbour() && 
            (! traits->curve_equal(left_top_neighbour()->top(),top())) ||
            right_bottom_neighbour() &&
            (! traits->curve_equal(right_bottom_neighbour()->bottom(),
                                   bottom())) ||
            left_bottom_neighbour() &&
            (! traits->curve_equal(left_bottom_neighbour()->bottom(),
                                   bottom())) ||
            right_top_neighbour() &&
            ...
            traits->is_degenerate(*left_bottom_neighbour()))
        {
          ...
          CGAL_warning(!(right_top_neighbour() &&
                         (! traits->curve_equal(right_top_neighbour()->top(),
                                                top()))));
          CGAL_warning(!(left_top_neighbour() &&
                         (! traits->curve_equal(left_top_neighbour()->top(),
                                                top()))));
          CGAL_warning(!(right_bottom_neighbour() &&
                         (! traits->curve_equal(right_bottom_neighbour()->
                                                bottom(),bottom()))));
          CGAL_warning(!(left_bottom_neighbour() &&
                         (! traits->curve_equal(left_bottom_neighbour()->
                                                bottom(),bottom())) ));
          CGAL_warning(!(right_top_neighbour() &&
                         traits->is_degenerate(*right_top_neighbour())));
          CGAL_warning(!(left_top_neighbour() &&
