...
\begin{figure}
\lcTex{\psfig{file=tools_overview.ips,width=\textwidth}%
...
formatting for chapter authors or a release number\footnote{This is
  ...
  as it is available with the \Backslash {\tt RCSdef} or \Backslash {\tt
    RCSdefDate} macros from Section~\ref{sectionRCSmacros}.}.  They
are inactive by default (since the official \cgal\ manual is written
without them), but they can be activated using the boolean values
\verb+\ccTagChapterAuthor+ and \verb+\ccTagChapterRelease+, as
described in Section~\ref{sectionCustomizeMisc}. The example assumes both
values set to \verb+\ccTrue+ and the manual release in
\verb+\a+ and the manual date in \verb+\b+.  
...
\lcTex{  M \= \hspace*{77mm} \= \kill}
  ...
  \> \verb+\ccChapterAuthor{Jones}+      \>  {\em Jones}
\end{tabbing}
...
\lcTex{  M \= CCimplementationNNN \= IsxModelxforxthexConceptxxx \= \kill}
  ...
  \> \verb+\ccRefines+         \>  {\bf Refines}     \> 
                                                     concept names\\
  \> \verb+\ccRequirements+    \>  {\bf Reqruiements}     \> 
...
current default creates a section title as in the example above. The
other style creates a new page for each item and draws a tab marker at
the side margin indicating the category of this item. Use the
\verb+\ccNewRefManualStyle+ macro as follows to switch to the second
style.  The tab markers use the side margins of \LaTeX. The dimensions
of the side margins need to be set accordingly.
...
The macro \verb+\ccRefPageBegin+\ccIndexEntry{RefPageBegin} is called just 
before the section
title of a reference page is formatted. The macro \verb+\ccRefPageEnd+\ccIndexEntry{RefPageEnd}
is called after all formatting of the reference page is finished.
These two macros are user-customizable and empty by default. The
following redefinition of these macros places each reference page on
its own page. Note that this redefinition is superfluous if the second
style for reference pages has been selected.
...
\begin{figure}[tbp!]
\lcTex{\psfig{file=horizontal_struct.ips,width=\textwidth}%
...
\begin{figure}
\lcTex{\psfig{file=vertical_struct.ips,width=\textwidth}%
...
authors or release number of a chapter.  They are switched off by
default and can be activated setting the tags
\verb+\ccTagChapterAuthor+ and \verb+\ccTagChapterRelease+ to
\verb+\ccTrue+. The macro \verb+\ccTagDefaults+ resets them to
\verb+\ccFalse+.
