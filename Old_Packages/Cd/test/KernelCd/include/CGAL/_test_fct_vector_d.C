...
_test_fct_vector_d(const R& )
{
 ...
 CGAL::Vector_d<R>  v0(CGAL::NULL_VECTOR);  // ( 0, 0, 0)
 CGAL::Vector_d<R>  v1(n1, n2, n3, n4);    // ( 6,-2, 3)
 CGAL::Vector_d<R>  v2(n5, n6, n7, n8);    // ( 3,-6,12)
 CGAL::Vector_d<R>  v3(n5, n10, n9);       // ( 9,-8,15)
 CGAL::Vector_d<R>  v4(n8, -n2, -n5);      // ( 3, 4,-9)
 CGAL::Vector_d<R> mv4(-n8, n2, n5);       // (-3,-4, 9)

 CGAL::Vector_d<R>  v10( n8,-n2,-n5, n4);  // (1.5,2,-4.5)
 CGAL::Vector_d<R>  v11(-n6, n11,-n12, n8);// ( 6, 8, -18)
 ...
 assert( CGAL::Vector_d<R>( n1, n2, n3) == RT(2) * v1 );
 assert( CGAL::Vector_d<R>( n5, n6, n7) == RT(3) * v2 );
 assert( v2 / RT(3) == CGAL::Vector_d<R>( RT(1), -n4, -n2) );
 assert( (CGAL::Vector_d<R>&)(RT(3) * (v2 / RT(3))) == v2 );
 assert( (v2 * RT(3)) / RT(3) == v2 );
#else // no multiplication scalar * vector ( only from the right side )
 assert( v1 * v2 == FT(66) );
 assert( v1 * v0 == FT(0) );
 assert( CGAL::Vector_d<R>( n1, n2, n3) == v1 * RT(2));
 assert( CGAL::Vector_d<R>( n5, n6, n7) == v2 * RT(3));
 assert( v2 / RT(3) == CGAL::Vector_d<R>( RT(1), -n4, -n2) );
 assert( (v2 * RT(3)) / RT(3) == v2 );
 assert( (v2 / RT(3)) * RT(3) == v2 );
 ...
 CGAL::Vector_d<R> evx( n5, n0, n0, n5);
 CGAL::Vector_d<R> evy( n0, n5, n0, n5);
 CGAL::Vector_d<R> evz( n0, n0, n5, n5);
 ...
 RT ns3(60);
 RT ns4(126);
 CGAL::Vector_d<R> vs1(-n6,ns2,ns1,-n8);   // (-6,-63,-30)
 assert( CGAL::cross_product(v1,v2) == vs1 );
 CGAL::Vector_d<R> vs2(n1,ns4,ns3,n4);     // (6,63,30)
