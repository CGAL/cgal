...
void test_Halfedge_data_structure_using_vector() {
    ...
    hds.reserve(2,4,4);
    CGAL_assertion( hds.capacity_of_facets() == 4);
    CGAL_assertion( D.is_valid( hds, false, 4));
...
int main() {
    test_Halfedge_data_structure_using_vector();
    return 0;
}
// EOF //
