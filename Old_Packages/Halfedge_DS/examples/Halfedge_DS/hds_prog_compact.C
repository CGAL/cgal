...
public:
    ...
    void*       opposite()       {
        const std::size_t SIZE = sizeof( My_halfedge);
        if ( nxt & 1)
    ...
    void*       next()           { return (void*)(nxt & (~ std::size_t(1)));}
    const void* next() const     { return (void*)(nxt & (~ std::size_t(1)));}
    ...
    void  set_opposite( void* g) {
        char* h = (char*)g;
        CGAL_assertion( std::size_t( abs( h - (char*)this)) 
                        == sizeof(My_halfedge));
        if ( h > (char*)this)
            nxt |= 1;
        else
    ...
    void  set_next( void* h)     {
        CGAL_assertion( ((std::size_t)h & 1) == 0);
        nxt = ((std::size_t)(h)) | (nxt & 1);
    }
