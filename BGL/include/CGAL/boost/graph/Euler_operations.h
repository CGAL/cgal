...
/** 
 * Creates an isolated triangle with border edges in `g` having `p0`, `p1`, and `p2` as points.
...
/** 
 * Creates an isolated quadrangle with border edges in `g` having `p0`, `p1`, `p2`, and `p3` as points.
 * \returns the non-border halfedge which has the target vertex associated with `p0`.
 **/ 
template<typename Graph, typename P>
typename boost::graph_traits<Graph>::halfedge_descriptor
...
{
  return make_tetrahedron(p0, p1, p2, p3, g);
}
/// @endcond
