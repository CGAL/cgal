...
The triangulations of \cgal\ are all models of the concepts \ccc{BidirectionalGraph},
\ccc{VertexListGraph}, and \ccc{EdgeListGraph}  of the Boost Graph Library \cite{}.
...
\ccNestedType{edge_descriptor;}{The edge descriptor. It is constructible from and convertible to \ccc{Triangulation::Edge}.
              The egde descriptor is not a simple typedef, but a proper class,  because in an undirected graph
...
\ccNestedType{out_edge_iterator}{An iterator for the outgoing edges incident to a vertex. Its value type is \ccc{edge_descriptor}.}
\ccNestedType{in_edge_iterator}{An iterator for the incoming edges incident to a vertex. Its value type is \ccc{edge_descriptor}.}
\ccNestedType{adjacency_iterator}{An iterator for the vertices adjacent to a vertex. Its value type is \ccc{vertex_descriptor}.}
