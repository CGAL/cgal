% +------------------------------------------------------------------------+
% | Reference manual page: HalfedgeDS_vertex_max_base_with_id.tex
% +------------------------------------------------------------------------+
% | 22.03.1999   Lutz Kettner
% | Package: HalfedgeDS
% | 
% +------------------------------------------------------------------------+

%%RefPage: end of header, begin of main body
% +------------------------------------------------------------------------+


\begin{ccRefClass}{HalfedgeDS_vertex_max_base_with_id<Refs>}

\ccDefinition
  
The class \ccRefName\ is a model of the \ccc{HalfedgeDSVertex}
concept. \ccc{Refs} is an instantiation of a \ccc{HalfedgeDS}.  It is
equivalent to \ccc{CGAL::HalfedgeDS_vertex_base< Refs, CGAL::Tag_true>}
with an added integer field which can be used to index vertices
in {\sc Bgl} algorithms.. 
The class contains support for the point and the required type definitions.
It can be used for deriving own vertices.

Note that the user is in charge to set the index correctly before
running a graph algorithm.

\ccInclude{CGAL/HalfedgeDS_vertex_max_base_with_id.h}

\ccIsModel

\ccRefConceptPage{HalfedgeDSVertex}

\ccThree{const Point&;;}{point() const;MMMMMMMMMMMM}{}
\ccThreeToTwo

\ccCreation
\ccCreationVariable{v}

\ccConstructor{HalfedgeDS_vertex_max_base_with_id();}{default constructor.}

\ccMethod{int id() const;}
{Returns the index.}
\ccGlue
\ccMethod{int& id();}
{Returns a reference to the index stored in the vertex.}

\ccSeeAlso

\ccRefConceptPage{HalfedgeDS<Traits,Items,Alloc>}\\
\ccRefConceptPage{HalfedgeDSItems}\\
\ccRefConceptPage{PolyhedronItems_3}\\
\ccRefIdfierPage{CGAL::HalfedgeDS_min_items}\\
\ccRefIdfierPage{CGAL::HalfedgeDS_halfedge_min_base<Refs>}\\
\ccRefIdfierPage{CGAL::HalfedgeDS_face_min_base<Refs>}\\
\ccRefIdfierPage{CGAL::HalfedgeDS_vertex_base<Refs>}

\end{ccRefClass}


