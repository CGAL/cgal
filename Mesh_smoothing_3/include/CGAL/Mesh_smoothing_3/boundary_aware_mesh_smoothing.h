...
)
{
    ...
    for (const auto& f : ref.facets_in_complex()) {
        facets_by_patch[ref.surface_patch_index(f)].push_back(f);
    }

    std::map<Curve_index, std::vector<Edge>> edges_by_curve;
    for (const auto& e : ref.edges_in_complex()) {
        edges_by_curve[ref.curve_index(e)].push_back(e);
    }

    using Facet_to_triangle_map = Mesh_smoothing_3::internal::Facet_to_triangle_property_map<C3t3>;
    using Facet_to_point_map = Mesh_smoothing_3::internal::Facet_to_point_property_map<C3t3>;
    using Edge_to_segment_map = Mesh_smoothing_3::internal::Edge_to_segment_property_map<C3t3>;
    using Edge_to_point_map = Mesh_smoothing_3::internal::Edge_to_point_property_map<C3t3>;
    ...
    std::map<Surface_patch_index, Facet_tree> facet_trees;
    for (auto& kv : facets_by_patch) {
        facet_trees.try_emplace(kv.first, kv.second.begin(), kv.second.end());
    }

  std::map<Curve_index, Edge_tree> edge_trees;
    for (auto& kv : edges_by_curve) {
        edge_trees.try_emplace(kv.first, kv.second.begin(), kv.second.end());
    }

    if (verbose) std::cout << "Built " << facet_trees.size() << " facet AABB trees and "
                           << edge_trees.size() << " edge AABB trees." << std::endl;
    ...
    smoother.set_boundary_query([&](const Point_3& pt, Surface_patch_index id, double /*radius*/) {
        ...
        return std::make_tuple(closest_point, normal, 1.0);
    });

    smoother.set_curves_query([&](const Point_3& pt, Curve_index id, double /*radius*/) {
        ...
        return std::make_tuple(closest_point, direction, 1.0);
    });
