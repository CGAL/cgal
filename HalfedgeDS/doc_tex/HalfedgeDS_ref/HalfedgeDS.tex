...
\ccHeading{Faces with Several Boundaries}
...
{\XHDS
While the storage of \ccc{halfedge_cycle} items is organized by the \ccRefName\ , faces contain 
local sequences of \ccc{Halfedge_cycle_handle} elements, not of \ccc{Halfedge_cycle} elements. Thus,
a \ccRefName\ defines a \ccc{Halfedge_cycle_iterator} while a face defines a
\ccc{Halfedge_cycle_handle_iterator}.
}
...
\ccGlue
\ccNestedType{\XHDS Halfedge_cycle}{\XHDS model of \ccc{HalfedgeDSHalfedgeCycle} concept.}
...
\ccGlue
\ccNestedType{\XHDS Halfedge_cycle_iterator}{\XHDS iterator over all \ccc{halfedge_cycles}.}
...
\ccGlue
\ccNestedType{\XHDS Supports_halfedge_halfedge_cycle}{\XHDS \ccc{Halfedge::halfedge_cycle()}.}
\ccGlue
\ccNestedType{Supports_face_halfedge}{\ccc{Face::halfedge()}.}
\ccGlue
\ccNestedType{\XHDS Supports_face_halfedge_cycle}{\XHDS \ccc{Face::outer_halfedge_cycle()}.}
\ccGlue
\ccNestedType{\XHDS Supports_halfedge_cycle_face}{\XHDS \ccc{Halfedge_cycle::face()}.}
\ccGlue
...
\ccc{Halfedge}, \ccc{Halfedge_cycle}, and \ccc{Face} types defined in \ccRefName. If they 
differ, they are derived from the user provided item types. We denote the
user item types with \ccc{Vertex_base}, \ccc{Halfedge_base}, 
\ccc{Halfedge_cycle_base}, and \ccc{Face_base} in the following. The fully qualified name for 
...
\ccFunction{static Face_const_handle 
  HalfedgeDS::face_handle( const Face_base* f);}{}

\ccFunction{\XHDS static Halfedge_cycle_handle HalfedgeDS::halfedge_cycle_handle( Halfedge_cycle_base* f);}{}
\ccGlue
\ccFunction{\XHDS static Halfedge_cycle_const_handle HalfedgeDS::halfedge_cycle_handle( const Halfedge_cycle_base* f);}{}
...
\ccGlue
\ccConstructor{HalfedgeDS( size_type v, size_type h, size_type f, size_type c );}
    {storage reserved for $v$ vertices, $h$ halfedges, $f$ faces, and $c$ halfedge cycles (if provided).}
...
\ccGlue
\ccMethod{void reserve( size_type v, size_type h, size_type f, size_type c );}
    {reserves storage for $v$ vertices, $h$ halfedges, $f$ faces and $c$ halfedge cycles (if provided).
...
\ccGlue
\ccMethod{\XHDS Size size_of_halfedge_cycles() const;}{\XHDS number of \ccc{halfedge_cycles}.}
\ccGlue
...
\ccGlue
\ccMethod{\XHDS Size capacity_of_halfedge_cycles() const;}{\XHDS space reserved for \ccc{halfedge_cycles}.}
\ccGlue
...
\ccGlue
\ccMethod{\XHDS Halfedge_cycle_iterator      halfedge_cycles_begin();}{\XHDS iterator over all \ccc{halfedge_cycles}.}
\ccGlue
\ccMethod{\XHDS Halfedge_cycle_iterator      halfedge_cycles_end();}{}
...
\ccMethod{\XHDS Halfedge_cycle_handle halfedge_cycles_push_back( const Halfedge_cycle& c);}
    {\XHDS appends a copy of $c$ to \ccc{hds}. Returns a handle of the new cycle.}
...
\ccMethod{\XHDS void halfedge_cycles_pop_front();}{\XHDS 
    removes the first \ccc{halfedge_cycle} if supported and
    \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}

\ccMethod{\XHDS void halfedge_cycles_pop_back();}{\XHDS 
    removes the last \ccc{halfedge_cycle}  if supported and
    \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}

\ccMethod{\XHDS void halfedge_cycles_erase( Halfedge_cycle_handle c);}{\XHDS 
    removes the \ccc{halfedge_cycle} $c$ if halfedge\_cycles are supported and
    \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}

\ccMethod{\XHDS void halfedge_cycles_erase( Halfedge_cycle_handle first, Halfedge_cycle_handle last);}{\XHDS 
    removes the range of \ccc{halfedge_cycles} $[\ccc{first},\ccc{last})$ if are 
    supported and \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}
...
\ccGlue
\ccMethod{\XHDS void halfedge_cycles_clear();}{\XHDS removes all \ccc{halfedge_cycles}.}
