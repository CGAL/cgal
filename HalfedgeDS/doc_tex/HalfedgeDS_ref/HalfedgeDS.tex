...
and vice versa, or via a \ccc{ccb} object common to incident faces and halfedges.
}
...
{\bf\ttfamily
While the storage of \ccc{ccb} items is organized by the \ccRefName\ , faces contain 
local sequences of \ccc{Ccb_handle} elements, not of \ccc{Ccb} elements. Thus,
a \ccRefName\ defines a \ccc{Ccb_iterator} while a face defines a
\ccc{Ccb_handle_iterator}.
}
...
\ccGlue
\ccNestedType{\bf\ttfamily Ccb}{\bf\ttfamily model of \ccc{HalfedgeDSCcb} concept.}
...
\ccGlue
\ccNestedType{\bf\ttfamily Ccb_iterator}{\bf\ttfamily iterator over all ccbs.}
...
\ccGlue
\ccNestedType{\bf\ttfamily Supports_halfedge_ccb}{\bf\ttfamily \ccc{Halfedge::ccb()}.}
\ccGlue
\ccNestedType{Supports_face_halfedge}{\ccc{Face::halfedge()}.}
\ccGlue
\ccNestedType{\bf\ttfamily Supports_face_ccb}{\bf\ttfamily \ccc{Face::outer_ccb()}.}
\ccGlue
\ccNestedType{\bf\ttfamily Supports_ccb_face}{\bf\ttfamily \ccc{Ccb::face()}.}
\ccGlue
...
\ccFunction{\bf\ttfamily static Ccb_handle HalfedgeDS::ccb_handle( Ccb_base* f);}{}
\ccGlue
\ccFunction{\bf\ttfamily static Ccb_const_handle HalfedgeDS::ccb_handle( const Ccb_base* f);}{}
...
\ccConstructor{HalfedgeDS( size_type v, size_type h, size_type f, size_type b = 16 );}
    {storage reserved for $v$ vertices, $h$ halfedges, $f$ faces, and $b$ ccbs.}
...
\ccMethod{void reserve( size_type v, size_type h, size_type f, size_type b = 16 );}
    {reserves storage for $v$ vertices, $h$ halfedges, $f$ faces, and $b$ ccbs.
...
\ccGlue
\ccMethod{\bf\ttfamily Size size_of_ccbs() const;}{\bf\ttfamily number of ccbs.}
\ccGlue
...
\ccGlue
\ccMethod{\bf\ttfamily Size capacity_of_ccbs() const;}{\bf\ttfamily space reserved for ccbs.}
\ccGlue
...
\ccGlue
\ccMethod{\bf\ttfamily Ccb_iterator      ccbs_begin();}{\bf\ttfamily iterator over all ccbs.}
\ccGlue
\ccMethod{\bf\ttfamily Ccb_iterator      ccbs_end();}{}

% +-----------------------------------+
\ccHeading{Insertion}
\ccTagFullDeclarations

Note that the vertex-related, face-related and ccb-related member functions may
not be provided for a \ccRefName\ that does not support vertices,
faces or ccbs respectively.
...
\ccMethod{\bf\ttfamily Ccb_handle ccbs_push_back( const Ccb& b);}
    {\bf\ttfamily appends a copy of $b$ to \ccc{hds}. Returns a handle of the new ccb.}
...
\ccMethod{\bf\ttfamily void ccbs_pop_front();}{\bf\ttfamily 
    removes the first ccb if ccbs are supported and
    \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}

\ccMethod{\bf\ttfamily void ccbs_pop_back();}{\bf\ttfamily 
    removes the last ccb.}

\ccMethod{\bf\ttfamily void ccbs_erase( Ccb_handle b);}{\bf\ttfamily 
    removes the ccb $b$ if ccbs are supported and
    \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}

\ccMethod{\bf\ttfamily void ccbs_erase( Ccb_handle first, Ccb_handle last);}{\bf\ttfamily 
    removes the range of ccbs $[\ccc{first},\ccc{last})$ if ccbs are 
    supported and \ccc{Supports_removal} $\equiv$ \ccc{CGAL::Tag_true}.}
...
\ccGlue
\ccMethod{\bf\ttfamily void ccbs_clear();}{\bf\ttfamily removes all ccbs.}
