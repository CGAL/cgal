...
\ccGlue
\ccMethod{\bf\ttfamily Ccb_handle ccbs_push_back( const Ccb& b);}{\bf\ttfamily 
    appends a copy of $b$ to \ccc{hds} if ccbs are supported.
    Returns a handle of the new ccb, or \ccc{Ccb_handle()} otherwise.}
...
\ccMethod{Halfedge_handle create_loop();}{
    ...
    consisting of a single closed edge, one vertex, two faces 
    {\bf\ttfamily and two outer ccbs}
...
\ccMethod{Halfedge_handle create_segment();}{
    ...
    {\bf\ttfamily and one outer ccbs}
    (if supported respectively).}
\ccGlue
\ccMethod{\bf\ttfamily Halfedge_handle create_isolated_vertex();}{\bf\ttfamily
    returns handle of a halfedge from a newly created loop in \ccc{hds}
    consisting of a single closed edge belonging to one ccb if supported
...
\ccMethod{void erase_face( Halfedge_handle h);} {removes the
   ...
   {\bf\ttfamily along with the \ccc{ccb} containing them}
...
\ccMethod{void erase_connected_component( Halfedge_handle h);}
    {removes the vertices, halfedges, faces {\bf\ttfamily and ccbs}
...
\ccMethod{Halfedge_handle make_hole( Halfedge_handle h);}
   {removes the face incident to \ccc{h} from \ccc{hds} and creates a hole.
    ...
    {\bf\ttfamily \ccCommentHeading{Note} \ccc{h->ccb()} {\em is not} removed from \ccc{hds}}.
   }

\ccMethod{\bf\ttfamily Ccb_handle host_halfedge( Halfedge_handle h);}
   {\bf\ttfamily Associates \ccc{h} with \ccc{b}, a new \ccc{ccb} item created from \ccc{hds}.
    Returns \ccc{b}.
    \ccPrecond \ccc{h != Halfedge_handle()} and \ccc{h->ccb() == Ccb_handle()}.}

\ccMethod{\bf\ttfamily Ccb_handle host_halfedge( Halfedge_handle h, const Ccb& b );}
   {\bf\ttfamily Associates \ccc{h} with a copy of $b$.
...
\ccMethod{Halfedge_handle fill_hole( Halfedge_handle h);}
   ...
   {\bf\ttfamily adds \ccc{h->ccb()} to the outer sequence of ccbs of the new face.}.
    Returns \ccc{h}.
    \ccPrecond \ccc{h != Halfedge_handle()} and \ccc{h->is_border()}.}

\ccMethod{Halfedge_handle fill_hole( Halfedge_handle h, const Face& f);}
   {fills the hole incident to \ccc{h} with a copy of face $f$.
   {\bf\ttfamily adds \ccc{h->ccb()} to the outer sequence of ccbs of the copy of $f$}.
    Returns \ccc{h}.
    \ccPrecond \ccc{h != Halfedge_handle()} and \ccc{h->is_border()}.}
...
\ccMethod{Halfedge_handle split_face( Halfedge_handle h, Halfedge_handle g);}
    {splits the face incident to \ccc{h} and \ccc{g} into two faces
     ...
     \ccc{g} respectively. \ccc{h} and \ccc{g} must belong to the same \ccc{ccb}.
     The second (new) face {\bf\ttfamily and \ccc{ccb}}
     obtained from \ccc{hds} are a copy of the first face {\bf\ttfamily and \ccc{ccb}}.
     Returns \ccc{h->next()} after the operation, i.e., the new diagonal.
     The new face is to the right of the new diagonal
     {\bf\ttfamily (thus the new \ccc{ccb} contains \ccc{g})}
     , the old face is to the left 
     {\bf\ttfamily (and the old \ccc{ccb} contains \ccc{h})}.
     The time is proportional 
     to the distance from \ccc{h} to \ccc{g} around the face.
     {\bf\ttfamily If \ccc{h->face()} contains any other ccbs besides \ccc{h->ccb()},
     they are kept in \ccc{h->face()}. It is the responsability of the upper level 
...
\ccMethod{Halfedge_handle join_face( Halfedge_handle h);}
    {joins the two faces incident to $h$. The face incident to
     \ccc{h->opposite()} {\bf\ttfamily and its hosting \ccc{ccb}}
...
\ccMethod{Halfedge_handle create_center_vertex( Halfedge_handle h);}
    {barycentric triangulation of \ccc{h->face()}. Creates a new vertex,
     ...
     {\bf\ttfamily and \ccc{h->ccb()}} into triangles. 
     \ccc{h} remains incident to the original face {\bf\ttfamily
     and belonging to the original ccb}, all other triangles 
     are copies of this face {\bf\ttfamily and ccb}. 
     ...
     {\bf\ttfamily If \ccc{h->face()} contains any other ccbs besides \ccc{h->ccb()},
     they are kept in \ccc{h->face()}. It is the responsability of the upper level
     data struct to migrate other ccbs to the other faces if applicable.}
...
\ccMethod{Halfedge_handle erase_center_vertex( Halfedge_handle g);}
    {reverses \ccc{create_center_vertex}. Erases the
     ...
     {\bf\ttfamily and \ccc{g->ccb()}} remains. 
