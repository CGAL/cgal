...
and
\ccc{Supports_halfedge_ccb} $\equiv$  \ccc{CGAL::Tag_false}

For ccb items use:

\ccc{Supports_halfedge_face} $\equiv$  \ccc{CGAL::Tag_false}
and
\ccc{Supports_halfedge_ccb} $\equiv$  \ccc{CGAL::Tag_true}
...
\ccMethod{Halfedge_const_handle next() const;}
    {the next halfedge along the ccb.}
\ccGlue
...
\ccMethod{Halfedge_const_handle prev() const;}
    {the previous halfedge along the ccb.}
\ccGlue
...
\ccHeading{Operations available if \ccc{Supports_halfedge_ccb} $\equiv$ 
           \ccc{CGAL::Tag_true}}

\ccMethod{\bf\ttfamily Ccb_handle       ccb();}{}
\ccGlue
\ccMethod{\bf\ttfamily Ccb_const_handle ccb() const;}
    {\bf\ttfamily the host ccb of \ccVar.}
\ccGlue
\ccMethod{\bf\ttfamily void set_ccb( Ccb_handle b);}{\bf\ttfamily 
    sets the host ccb to $b$.}

\ccHeading{Operations available if \ccc{Supports_halfedge_face} $\equiv$ \ccc{CGAL::Tag_true} 
          $or$ 
          ( \ccc{Supports_halfedge_ccb} $\equiv$ \ccc{CGAL::Tag_true}
          $and$
          \ccc{Supports_ccb_face} $\equiv$ \ccc{CGAL::Tag_true}
          ) }
