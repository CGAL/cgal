...
functions in \ccc{Vertex}, \ccc{Halfedge}, \ccc{Face} and
\ccc{Ccb} respectively. Halfedges are required to provide a reference to the
...
\ccc{HalfedgeDS_items_2} does not support ccbs.}
...
{\bf\ttfamily
The following example program uses the default halfedge data structure
with \ccc{HalfedgeDS_items_with_ccb_2}, enabling support for
...
\ccc{CGAL::HalfedgeDS_min_items} does not support ccbs.}
