...
\ccMethod{L_iterator get_l() const;}{returns an iterator for the
  ...
  $\ccVar.\ccc{l()}[j]$ is not accessed.\ccPrecond if 
  $\ccVar.\ccc{fl()}[j]=\ccVar.\ccc{fu()}[j]=\ccc{true}$, then
  $\ccVar.\ccc{l()}[j]\leq \ccVar.\ccc{u()}[j]$}
...
\ccMethod{U_iterator get_u() const;}{returns an iterator over the
  ...
  $\ccVar.\ccc{u()}[j]$ is not accessed. \ccPrecond if 
  $\ccVar.\ccc{fl()}[j]=\ccVar.\ccc{fu()}[j]=\ccc{true}$, then
  $\ccVar.\ccc{l()}[j]\leq \ccVar.\ccc{u()}[j]$}
