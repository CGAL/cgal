...
of a class that is a model of the concept \ccc{QuadraticProgramInterface} 
...
\ref{sec:first-qp} below. The input data may be of any given number 
...
The concept \ccc{QuadraticProgramInterface} (as well as the
...
\section{How to enter and solve a program}
\label{sec:QP-first}
...
advantagous if you already have the data (explicitly, or implictly
...
Figure \ref{fig:QP-first_qp} shows a picture. The picture
...
internal computations. For an example 
how to work with the input type \ccc{double}, we refer to 
Section \ref{sec:QP-iterators}. 
...
{\bf Note:} in specifying $D$, we have to provide the entries
of the matrix $2D$. This is a common feature of almost all
...
The following program again solve our running example from above,
...
{\bf Note:} This example shows an interesting feature of this approach:
...
package offers dedicated models and solution methods for these special
cases.
...
\ccc{NonnegativeLinearProgramInterface} rather than 
...
\section{Customizing the solver}
