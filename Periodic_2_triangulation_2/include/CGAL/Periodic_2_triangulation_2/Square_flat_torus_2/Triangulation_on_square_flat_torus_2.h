...
#ifndef CGAL_P2T2_TRIANGULATION_ON_FLAT_TORUS_2_H
#define CGAL_P2T2_TRIANGULATION_ON_FLAT_TORUS_2_H
...
CGAL_GENERATE_MEMBER_DETECTOR(set_offsets);
...
class Triangulation_on_square_flat_torus_2
...
public:
  ...
  /// The rectangle type
  typedef typename Geom_traits::Iso_rectangle_2                             Iso_rectangle;
  ...
  /// The vertex type
  typedef typename Triangulation_data_structure::Vertex                     Vertex;

  /// Size type (an unsigned integral type)
  typedef typename Triangulation_data_structure::size_type                  size_type;
  /// Difference type (a signed integral type)
  typedef typename Triangulation_data_structure::difference_type            difference_type;
  ...
  typedef Face_iterator                                                     Faces_iterator;
...
protected:
  ...
  typedef std::unordered_map<Vertex_handle, Virtual_vertex>                 Virtual_vertex_map;
  typedef typename Virtual_vertex_map::const_iterator                       Virtual_vertex_map_it;

  /// Vector contains virtual copies with offset `off`, stored at position: i=3*off[0]+off[1]-1
  typedef std::unordered_map<Vertex_handle, std::vector<Vertex_handle> >    Virtual_vertex_reverse_map;
...
public:
  ...
  enum Locate_type
  {
    VERTEX = 0, /// The query point lies on a vertex
    EDGE, /// The query point lies on an edge
    FACE, /// The query point lies on a face

    EMPTY, /// The query point lies outside the affine hull of the triangulation,
...
public:
  ...
  /// Constructor
  Triangulation_on_square_flat_torus_2(const Gt& gt) : _gt(gt), _tds(), _cover(make_array(1, 1)) { }
  Triangulation_on_square_flat_torus_2(const Domain& domain)
    : Triangulation_on_square_flat_torus_2(Gt(domain))
  { }

  /// Copy constructor
  Triangulation_on_square_flat_torus_2(const Triangulation_on_square_flat_torus_2<Gt, Tds>& tr)
  {
    copy_triangulation(tr);
  }

  /// Assignment
  Triangulation_on_square_flat_torus_2 &operator=(const Triangulation_on_square_flat_torus_2& tr)
  {
    copy_triangulation(tr);
    return *this;
  }
  ...
  void copy_triangulation(const Triangulation_on_square_flat_torus_2& tr)
  {
    _tds.clear();
    _gt = tr.geom_traits();
    _cover = tr._cover;
  ...
  void swap(Triangulation_on_square_flat_torus_2& tr)
  {
    _tds.swap(tr._tds);
  ...
  virtual ~Triangulation_on_square_flat_torus_2() { }
  ...
  int dimension() const
  {
    return (number_of_vertices() == 0) ? -2 : 2;
  }
  ...
  /// \name Offset helper functions

public:
  const Offset& extract_offset(const Offset& off) const { return off; }

  /// Creates an offset from a bit pattern.
  Offset extract_offset(const int i) const
  {
  ...
  {
    const Offset o_ct(_cover[0] * o_t.x(), _cover[1] * o_t.y());
    return o_c + o_ct;
  }
                             ...
                             const Face_handle nb, const int j) const
  {
    ...
    CGAL_triangulation_precondition(f->neighbor(i) == nb);
    CGAL_triangulation_precondition(nb->neighbor(j) == f);
    CGAL_triangulation_precondition(f->vertex(cw(i)) == nb->vertex(ccw(j)));

    return extract_offset(nb->offset(ccw(j))) - extract_offset(f->offset(cw(i)));
  }

  /// [Undoc] Returns the offset of nb==f->neighbor(i) with respect to f.
  /// Get the offset between the origins of the internal offset coordinate
  ...
  {
    const Face_handle nb = f->neighbor(i);
  ...
  Offset get_offset(const Face_handle f, const int i) const
  {
    if(is_1_cover())
      return extract_offset(f->offset(i));

    Virtual_vertex_map_it it = virtual_vertices.find(f->vertex(i));
    if(it != virtual_vertices.end())
      return combine_offsets(it->second.second, extract_offset(f->offset(i)));
    else
      return combine_offsets(Offset(), extract_offset(f->offset(i)));
  }

  /// [Undoc] Returns the offset of the vertex if we are not on the 1-cover.
  Offset get_offset(const Vertex_handle vh) const
  {
    if(is_1_cover())
      return Offset();
  ...
  ///
  /**
   * - f->offset(i) is a bit tuple encapsulated in an integer. Each bit
   *   represents the offset in one direction --> 2-cover!
   * - extract_offset(int) decodes this again.
   ...
   */
  void get_vertex(const Face_handle f, const int i, Vertex_handle& vh, Offset& off) const
  {
    off = combine_offsets(Offset(), extract_offset(f->offset(i)));
    vh = f->vertex(i);
  ...
  void get_vertex(const Vertex_handle vh_i, Vertex_handle& vh, Offset& off) const
  {
    const Virtual_vertex_map_it it = virtual_vertices.find(vh_i);

    if(it == virtual_vertices.end())
    {
      // if vh_i is not contained in virtual_vertices, then it is in the original domain.
      vh = vh_i;
  ...
  template <typename FH> // that's 'Face_handle', but we need SFINAE so gotta be a template
  void set_offsets(FH f, int o0, int o1, int o2,
                   typename std::enable_if<
                     internal::has_set_offsets<
                       typename std::iterator_traits<FH>::value_type,
                       void(int, int, int)>::value>::type* = nullptr)
  {
    ...
    for(int i=0; i<2; ++i)
    {
      const int min_off = (std::min)((std::min)(off0[i], off1[i]), off2[i]);
      if(min_off != 0)
      {
        off0[i] -= min_off;
        off1[i] -= min_off;
  ...
  void set_offsets(FH f, const Offset& o0, const Offset& o1, const Offset& o2,
                   typename std::enable_if<
                     internal::has_set_offsets<
                       typename std::iterator_traits<FH>::value_type,
                       void(int, int, int)>::value>::type* = nullptr)
  ...
  void set_offsets(FH f, int o0, int o1, int o2,
                   typename std::enable_if<
                     !internal::has_set_offsets<
                       ...
                       void(int, int, int)>::value>::type* = nullptr)
  {
    set_offsets(f, extract_offset(o0), extract_offset(o1), extract_offset(o2));
  }
  ...
  Point point(const Vertex_handle v) const { return point(periodic_point(v)); }
  Point point(const Face_handle f, int i) const { return point(periodic_point(f, i)); }
  ...
  {
    return CGAL::filter_iterator(vertices_end(), P2T2::internal::Domain_tester<Self>(this), vertices_begin());
  }
  /// past-the-end iterator over the non-virtual vertices
  Unique_vertex_iterator unique_vertices_end() const
  {
    return CGAL::filter_iterator(vertices_end(), P2T2::internal::Domain_tester<Self>(this));
  }
  ...
  Bounded_side bounded_side(const Point& p0, const Point& p1, const Point& p2, const Point& p) const
  {
    ...
    const Orientation o1 = orientation(p0, p1, p);
    const Orientation o2 = orientation(p1, p2, p);
    const Orientation o3 = orientation(p2, p0, p);
                            ...
                            const Offset& o0, const Offset& o1, const Offset& o2, const Offset& o) const
  {
    ...
    const Orientation orient1 = orientation(p0, p1, p, o0, o1, o);
    const Orientation orient2 = orientation(p1, p2, p, o1, o2, o);
    const Orientation orient3 = orientation(p2, p0, p, o2, o0, o);
  ...
  Oriented_side oriented_side(Face_handle f, const Point& p, const Offset& o,
                              const int v0o, const int v1o, const int v2o) const
  {
    int cumm_off = v0o | v1o | v2o;
    for(int i=0; i<4; ++i)
    {
      if((cumm_off | ((~i) & 3)) == 3)
      {
        const Offset off_q = combine_offsets(o, extract_offset(i));
        Bounded_side bs = bounded_side(f->vertex(0)->point(), f->vertex(1)->point(), f->vertex(2)->point(), p,
                                       get_offset(f, 0), get_offset(f, 1), get_offset(f, 2), off_q);
  ...
  Oriented_side oriented_side(Face_handle f, const Point& p, const Offset& o,
                              const Offset& v0o, const Offset& v1o, const Offset& v2o) const
  {
    const int min_off_x = (std::min)((std::min)(v0o.x(), v1o.x()), v2o.x());
    const int min_off_y = (std::min)((std::min)(v0o.y(), v1o.y()), v2o.y());
    const int max_off_x = (std::max)((std::max)(v0o.x(), v1o.x()), v2o.x());
    const int max_off_y = (std::max)((std::max)(v0o.y(), v1o.y()), v2o.y());

    for(int i=min_off_x; i<=max_off_x; ++i)
    {
      for(int j=min_off_y; j<=max_off_y; ++j)
      {
        const Offset off_q = combine_offsets(o, Offset(i, j));
        Bounded_side bs = bounded_side(f->vertex(0)->point(), f->vertex(1)->point(), f->vertex(2)->point(), p,
                                       get_offset(f, 0), get_offset(f, 1), get_offset(f, 2), off_q);
        if(bs != ON_UNBOUNDED_SIDE)
          return (bs == ON_BOUNDARY ? ON_ORIENTED_BOUNDARY : ON_POSITIVE_SIDE);
  ...
  Oriented_side oriented_side(Face_handle f, const Point& p, const Offset& o) const
  {
    ...
    {
      // the orientation of the vertices is assumed to be counter clockwise
      CGAL_triangulation_assertion_code(Offset off0 = get_offset(f, 0);)
      CGAL_triangulation_assertion_code(Offset off1 = get_offset(f, 1);)
      CGAL_triangulation_assertion_code(Offset off2 = get_offset(f, 2);)
      CGAL_triangulation_assertion(orientation(p0, p1, p2, off0, off1, off2) == LEFT_TURN);

      // return position of point p with respect to the oriented triangle p0p1p2
      return oriented_side(f, p, o, f->offset(0), f->offset(1), f->offset(2));
    }
  }

  void side_of_face_orientations(const Point& q, const Offset& off,
                                 Face_handle f, const int v0o, const int v1o, const int v2o,
                                 bool& found, Orientation& o0, Orientation& o1, Orientation& o2) const
  {
    int cumm_off = v0o | v1o | v2o;
    for(int i=0; (i < 4) && (!found); ++i)
    {
      ...
      {
        o0 = o1 = o2 = RIGHT_TURN;
        const Offset off_q = combine_offsets(off, extract_offset(i));

        if(((o0 = orientation(q, f->vertex(1)->point(), f->vertex(2)->point(),
                              off_q, get_offset(f, 1), get_offset(f, 2))) != RIGHT_TURN) &&
           ((o1 = orientation(f->vertex(0)->point(), q, f->vertex(2)->point(),
                              get_offset(f, 0), off_q, get_offset(f, 2))) != RIGHT_TURN) &&
           ((o2 = orientation(f->vertex(0)->point(), f->vertex(1)->point(), q,
                              get_offset(f, 0), get_offset(f, 1), off_q)) != RIGHT_TURN))
        {
          found = true; // also breaks
        }
      }
    }
  }
  ...
  void side_of_face_orientations(const Point& q, const Offset& off,
                                 Face_handle f, const Offset& v0o, const Offset& v1o, const Offset& v2o,
                                 bool& found, Orientation& o0, Orientation& o1, Orientation& o2) const
  {
    const int min_off_x = (std::min)((std::min)(v0o.x(), v1o.x()), v2o.x());
    const int min_off_y = (std::min)((std::min)(v0o.y(), v1o.y()), v2o.y());
    ...
    for(int i=min_off_x; i<=max_off_x && (!found); ++i)
    {
      ...
      {
        o0 = o1 = o2 = RIGHT_TURN;
        const Offset off_q = combine_offsets(off, Offset(i, j));
  ...
  /// Testing where the point (q,off) lies w.r.t. the face f
  Bounded_side side_of_face(const Point& q, const Offset& off, Face_handle f, Locate_type &lt, int& li) const
  {
#ifdef CGAL_DEBUG_P2T2
    std::cout << "Side of face(" << q << ", " << off << ", " << &*f << ")" << std::endl;
    ...
    Orientation o0 = ZERO, o1 = ZERO, o2 = ZERO;
    if(f->has_zero_offsets() && is_1_cover())
    {
    ...
    else // Special case for the periodic space.
    {
      ...
      bool found = false;
      side_of_face_orientations(q, off, f, f->offset(0), f->offset(1), f->offset(2), found, o0, o1, o2);
  ...
  Offset get_location_offset(const ConflictTester& tester, Face_handle f, bool& found,
                             const int v0o, const int v1o, const int v2o) const
  {
    // Main idea seems to just test all possibilities.
    const int cumm_off = v0o | v1o | v2o;
    for(int i=0; i<4; ++i)
    {
      // Check if for each bit position: bit of i <= bit of cumm_off
      if(((cumm_off | (~i)) & 3) == 3)
      {
        const Offset o = extract_offset(i);
        if(tester(f, o))
        {
          std::cout << "found with " << o << std::endl;
          found = true;
          return o;
        }
      }
  ...
  template<class ConflictTester>
  Offset get_location_offset(const ConflictTester& tester, Face_handle f, bool& found) const
  {
#ifdef CGAL_DEBUG_P2T2
    std::cout << "get location offset " << tester.point() << ", off: " << tester.get_offset() << std::endl;
#endif
    ...
    {
      return get_location_offset(tester, f, found, f->offset(0), f->offset(1), f->offset(2));
    }
  }

  // In this overload, we know we must find the offset
  template<class ConflictTester>
  Offset get_location_offset(const ConflictTester& tester, Face_handle f) const
  {
    bool found = false;
  ...
  void set_best_possible_offset(const Point& query, Offset& off_query,
                                const int v0o, const int v1o, const int v2o) const
  {
    int cumm_off = v0o | v1o | v2o;
  ...
  void set_best_possible_offset(const Point& /*query*/, Offset& /*off_query*/,
                                const Offset& /*v0o*/, const Offset& /*v1o*/, const Offset& /*v2o*/) const
  {
    // @todo not sure what to do in the generic case
  }

  /// Do a remembering heuristic walk to locate point (query,o_p)
  Face_handle march_locate_2D(Face_handle f,
                              const Point& query, const Offset& o_p,
                              Offset& off_query,
                              Locate_type& lt, int& li) const
  {
...
#ifdef CGAL_DEBUG_P2T2
    std::cout << "                   ###########" << std::endl;
    std::cout << "March locate: " << query << " off_p: " << o_p << " = " << construct_point(query, o_p) << std::endl;
    std::cout << "p2t2: " << number_of_vertices() << " nv " << number_of_faces() << " nf" << std::endl;
    std::cout << "p2t2 (tds): " << _tds.number_of_vertices() << " nv " << _tds.number_of_faces() << " nf" << std::endl;
#endif
    ...
    // Give the point the best start-offset possible // @todo gp2t2
    if(is_1_cover() && !f->has_zero_offsets())
      set_best_possible_offset(query, off_query, f->offset(0), f->offset(1), f->offset(2));
    ...
    for(;;)
    {
#ifdef CGAL_DEBUG_P2T2
      std::cout << "------" << std::endl;
      std::cout << "current face: " << f->vertex(0)->point() << " (" << &*(f->vertex(0)) << ") / "
                                    << f->vertex(1)->point() << " (" << &*(f->vertex(0)) << ") / "
                                    << f->vertex(2)->point() << " (" << &*(f->vertex(0)) << ")" << std::endl;
      std::cout << "vertex offs: " << get_offset(f->vertex(0)) << " / "
                                   << get_offset(f->vertex(1)) << " / "
      ...
      std::cout << "previously on the negative side of " << point(f, ccw(prev_index)) << " 0 / "
                                                         << point(f, cw(prev_index)) << " 0" << std::endl;
      ...
      if(!simplicity_criterion)
      {
        ...
        else
        {
          ...
          for(int i=0; i<3; ++i)
            offs[i] = extract_offset(f->offset(i));
        }
      }
      ...
      if(prev == Face_handle())
      {
        ...
        {
          o[ccw(prev_index)] = orientation(*p[ccw(prev_index)], *p[cw(prev_index)], query,
                                           offs[ccw(prev_index)], offs[cw(prev_index)], off_query);
        }
      ...
      if(left_first)
      {
        ...
        {
          o[prev_index] = orientation(*p[prev_index], *p[ccw(prev_index)], query,
                                      offs[prev_index], offs[ccw(prev_index)], off_query);
        }
...

      {
        ...
        {
          o[cw(prev_index)] = orientation(*p[cw(prev_index)], *p[prev_index], query,
                                          offs[cw(prev_index)], offs[prev_index], off_query);
        }
  ...
  Face_handle locate(const Point& p, const Offset& o_p,
                     Offset& lo,
                     ...
                     Face_handle start = Face_handle()) const
  {
    ...
    return locate(p, o, unused_off_query, lt, li, start);
  }
...
public:
  ...
  /// \name Delaunay/regular insertions and removal
...
protected:
  ...
  {
#ifdef CGAL_DEBUG_P2T2
    std::cout << "find_conflicts: " << tester.point() << " off: " << current_off << std::endl;
#endif
...
//      vit->clear_offset();
    ...
    *it.second++ = d;
    ...
    std::cout << "face points: " << point(f, 0) << " 0 / "
                                 ...
                                 << point(f, 2) << " 0 / "
                                 << point(f, 0) << " 0" << std::endl;
    std::cout << "current_off: " << current_off << std::endl;
#endif
...
#ifdef CGAL_DEBUG_P2T2
        std::cout << "neighbor: " << point(test, 0) << " 0 / "
                                  << point(test, 1) << " 0 / "
                                  << point(test, 2) << " 0 / "
        ...
        *it.first++ = Edge(f, i);
        for(int j=0; j<3; ++j)
        {
          ...
          std::cout << "marking " << f->vertex(j)->point() << " (" << point(f, j)
                    << ") with offset " << extract_offset(f->offset(j)) << " - " << current_off2
                    << " = " << extract_offset(f->offset(j)) - current_off2 << std::endl;
#endif
...
#endif
            f->vertex(j)->set_offset(extract_offset(f->offset(j)) - current_off2);
  ...
  Vertex_handle create_initial_triangulation(const Point& p)
  {
    ...
    for(int i=0; i<_cover[0]; ++i)
    {
      for(int j=0; j<_cover[1]; ++j)
      {
        ...
        for(int f=0; f<2; ++f)
        {
          faces[i][j][f]->set_vertices(
            vir_vertices[(i + vertex_ind[f][0][0]) % _cover[0]][(j + vertex_ind[f][0][1]) % _cover[1]],
            vir_vertices[(i + vertex_ind[f][1][0]) % _cover[0]][(j + vertex_ind[f][1][1]) % _cover[1]],
            vir_vertices[(i + vertex_ind[f][2][0]) % _cover[0]][(j + vertex_ind[f][2][1]) % _cover[1]]);

          set_offsets(faces[i][j][f],
                      offset & (vertex_ind[f][0][0] * 2 + vertex_ind[f][0][1] * 1),
                      offset & (vertex_ind[f][1][0] * 2 + vertex_ind[f][1][1] * 1),
                      offset & (vertex_ind[f][2][0] * 2 + vertex_ind[f][2][1] * 1));

          faces[i][j][f]->set_neighbors(
            faces[(i + _cover[0] + neighb_ind[f][0][0]) % _cover[0]]
                   [(j + _cover[1] + neighb_ind[f][0][1]) % _cover[1]][neighb_ind[f][0][2]],
            faces[(i + _cover[0] + neighb_ind[f][1][0]) % _cover[0]]
                   [(j + _cover[1] + neighb_ind[f][1][1]) % _cover[1]][neighb_ind[f][1][2]],
            faces[(i + _cover[0] + neighb_ind[f][2][0]) % _cover[0]]
                   [(j + _cover[1] + neighb_ind[f][2][1]) % _cover[1]][neighb_ind[f][2][2]]);
        }
      }
    }
...
protected:
  ...
  Vertex_handle periodic_insert(const Point& p, const Offset& o,
                                ...
                                Face_handle f,
                                const ConflictTester& tester,
                                PointHider& hider,
                                CoverManager& cover_manager,
                                Vertex_handle vh = Vertex_handle())
  {
#ifdef CGAL_DEBUG_P2T2
    std::cout << "Periodic insert: " << p << " Offset: " << o << " Curr off: " << current_off << std::endl;
#endif
    ...
    if(!tester.test_initial_face(f, current_off))
    {
    ...
    {
      std::cout << "f in conflict: " << &*f << std::endl;
      std::cout << point(f, 0) << " 0 "
                << point(f, 1) << " 0 "
                << point(f, 2) << " 0 "
                << point(f, 0) << " 0" << std::endl;
    }
    ...
    for(Face_handle f : nbs)
    {
      Offset offs[3];
      for(int i=0; i<3; ++i)
        offs[i] = f->vertex(i)->offset();

      set_offsets(f, offs[0], offs[1], offs[2]);
    }
                                ...
                                Vertex_handle vh = Vertex_handle())
  {
    ...
    CGAL_triangulation_assertion_code(Locate_type lt_assert; int i_assert;)
    CGAL_triangulation_assertion(side_of_face(tester.point(), o,
                                              f, lt_assert, i_assert) != ON_UNBOUNDED_SIDE);

    tester.set_offset(o);

    // Choose the periodic copy of tester.point() that is in conflict with f
    bool found = false;
    const Offset current_off = get_location_offset(tester, f, found);
                                   ...
                                   CoverManager& cover_manager)
  {
    ...
    // @todo re-enable this check, with a kind of domain.is_canonical(p);
...
//    CGAL_triangulation_assertion((domain().ymin() <= p.y()) && (p.y() < domain().ymax()));
    ...
    for(int i=0; i<_cover[0]; ++i)
    {
      for(int j=0; j<_cover[1]; ++j)
      {
        ...
        {
#ifdef CGAL_DEBUG_P2T2
          std::cout << "Insert copy (" << i << ", " << j << ")" << std::endl;
#endif
          Offset plo; // @todo, should be able to re-use that in the get_location_offset...
  ...
  template <class InputIterator, class ConflictTester, class PointHider, class CoverManager>
...
protected:
  // This is just adapted from the 3D version, there is likely a simpler way in 2D
  template <class PointRemover, class CoverManager>
  bool periodic_remove(Vertex_handle v,
                       PointRemover& remover,
                       CoverManager& cover_manager,
                       const bool abort_if_cover_change = false)
  {
    // Construct the set of vertex pairs on the boundary with the edge just behind
    typedef PointRemover                                             Point_remover;
    typedef typename Point_remover::FaceE_handle                     FaceE_handle;
    typedef typename Point_remover::VertexE_handle                   VertexE_handle;
    typedef typename Point_remover::EdgeE                            EdgeE;
    typedef typename Point_remover::VertexE_pair                     VertexE_pair;
    typedef typename Point_remover::Finite_facesE_iterator           Finite_facesE_iterator;
    typedef typename Point_remover::Vertex_pair_EdgeE_map            Vertex_pair_EdgeE_map;
    ...
    std::vector<Face_handle> hole;
    hole.reserve(16);
    Vertex_pair_Edge_map outer_map;
    Vertex_pair_EdgeE_map inner_map;
    ...
    for(typename std::vector<Face_handle>::iterator fit=hole.begin(); fit!=hole.end(); ++fit)
    {
      ...
      for(int j=0; j<3; ++j)
      {
        if((*fit)->vertex(j) != v)
        {
          tmp_vertices.insert((*fit)->vertex(j));
          vh_off_map[(*fit)->vertex(j)] = int_to_off((*fit)->offset(j))
                                          - int_to_off((*fit)->offset((*fit)->index(v)));
        }
      }
    }
    ...
    {
      typedef typename Point_remover::Triangulation_R2::Point TRPoint;
      CGAL_triangulation_assertion(get_offset(vertices[i]) + combine_offsets(Offset(), vh_off_map[vertices[i]])
                                   == combine_offsets(get_offset(vertices[i]), vh_off_map[vertices[i]]));
      TRPoint trp = std::make_pair(vertices[i]->point(),
                                   combine_offsets(get_offset(vertices[i]), vh_off_map[vertices[i]]));

      VertexE_handle vh = remover.tmp.insert(trp, fh);
      vmap[vh] = vertices[i];
    ...
    // Construct the set of vertex pairs of tmp
    // We reorient the vertex pair so that it matches those from outer_map
    // Also note that we use the vertices of *this, not of tmp
    for(Finite_facesE_iterator it=remover.tmp.finite_cells_begin(); it!=remover.tmp.finite_cells_end(); ++it)
    {
      VertexE_pair vt_aux;
      for(int i=0; i<3; ++i)
      {
        EdgeE e = std::make_pair(it, i);
        vt_aux = std::make_pair(it->vertex(ccw(i)), it->vertex(cw(i)));
        if(vmap.is_defined(vt_aux.first) && vmap.is_defined(vt_aux.second))
        {
          Vertex_pair vt(vmap[vt_aux.second], vmap[vt_aux.first]);
          make_canonical(vt);
          inner_map[vt] = e;
        }
      }
    ...
    typedef std::tuple<Face_handle, int, Face_handle> Neighbor_relation;
    ...
    {
      typename Vertex_pair_Edge_map::iterator oit = outer_map.begin();
      typename Vertex_pair_Edge_map::value_type o_vp_e_pair = *oit;
      Face_handle o_fh = o_vp_e_pair.second.first;
      unsigned int o_i = o_vp_e_pair.second.second;

      typename Vertex_pair_EdgeE_map::iterator iit = inner_map.find(o_vp_e_pair.first);

      CGAL_triangulation_assertion(iit != inner_map.end());
      typename Vertex_pair_EdgeE_map::value_type i_vt_f_pair = *iit;
      FaceE_handle i_fh = i_vt_f_pair.second.first;
      unsigned int i_i = i_vt_f_pair.second.second;

      // create a new cell to glue to the outer surface
      Face_handle new_fh = _tds.create_face();
      new_faces.push_back(new_fh);
      new_fh->set_vertices(vmap[i_fh->vertex(0)], vmap[i_fh->vertex(1)], vmap[i_fh->vertex(2)]);
      set_offsets(new_fh, vh_off_map[vmap[i_fh->vertex(0)]],
                          vh_off_map[vmap[i_fh->vertex(1)]],
                          vh_off_map[vmap[i_fh->vertex(2)]]);

      // Update the cover management
      if(cover_manager.update_cover_data_during_management(new_fh, new_faces, abort_if_cover_change))
      {
      ...
      nr_vec.emplace_back(o_fh, o_i, new_fh);
      nr_vec.emplace_back(new_fh, i_i, o_fh);
      ...
      for(unsigned int i=0; i<3; ++i)
      {
        if(i != i_i)
        {
          Edge e = std::make_pair(new_fh, i);
          Vertex_pair vt = std::make_pair(new_fh->vertex(ccw(i)), new_fh->vertex(cw(i)));
          make_canonical(vt);
          std::swap(vt.second, vt.third);
          typename Vertex_pair_Edge_map::iterator oit2 = outer_map.find(vt);
          ...
          {
            // glue the faces
            typename Vertex_pair_Edge_map::value_type o_vp_e_pair2 = *oit2;
            Face_handle o_fh2 = o_vp_e_pair2.second.first;
            int o_i2 = o_vp_e_pair2.second.second;
            nr_vec.emplace_back(o_fh2, o_i2, new_fh);
            nr_vec.emplace_back(new_fh, i, o_fh2);
    ...
    for(unsigned int i=0; i<nr_vec.size(); ++i)
      nr_vec[i].template get<0>()->set_neighbor(nr_vec[i].template get<1>(),
                                                nr_vec[i].template get<2>());

    // Output the hidden points.
    for(typename std::vector<Face_handle>::iterator hi=hole.begin(), hend=hole.end(); hi!=hend; ++hi)
      remover.add_hidden_points(*hi);

    _tds.delete_vertex(v);
    _tds.delete_faces(hole.begin(), hole.end());
  ...
  template <class PointRemover, class ConflictTester, class CoverManager>
              ...
              CoverManager& cover_manager)
  {
    ...
    if(!is_1_cover())
    {
      ...
      CGAL_triangulation_assertion(vhrem.size() == 8);

      virtual_vertices_reverse.erase(v);

      for(int i=0; i<8; ++i)
      {
        periodic_remove(vhrem[i], remover, cover_manager);
        virtual_vertices.erase(vhrem[i]);
        CGAL_triangulation_expensive_assertion(is_valid());
      }

      periodic_remove(v, remover, cover_manager);
    }
    else
    {
      periodic_remove(v, remover, cover_manager);
      if(!is_1_cover())
        remove(v, remover, tester, cover_manager);
    }
  }
...
protected:
  ...
  void copy_multiple_covering(const Triangulation_on_square_flat_torus_2& tr)
  {
    ...
    for(Vertex_iterator vit2 = vertices_begin(); vit2 != vertices_end(); ++vit2)
    {
      if(vit2->offset() != Offset())
      {
        ...
        virtual_vertices.emplace(vit2, std::make_pair(*vlist_it, off));
...
public:
  ...
  void convert_to_1_sheeted_covering()
  {
    ...
    // ###################################################################
    {
      ...
      for(Face_iterator it = all_faces_begin(); it != all_faces_end(); ++it)
      {
        ...
        if(it->tds_data().processed())
          continue;

        // Redirect neighbors: Only neighbors that are marked have to be substituted
        // by one of their periodic copies. The unmarked neighbors stay the same.
        for(int i=0; i<3; ++i)
        {
          if(!it->neighbor(i)->tds_data().processed())
            continue;
          ...
          for(int j=0; j<3; ++j)
          {
            ...
            else
            {
              ...
              nbv[j] = virtual_vertices_reverse.find(vert[j]) ->second[nbo[0] * 3 + nbo[1] - 1];
            }
          }

          // Find the new neighbor by its 3 vertices
          ...
          new_neighbor_relations.emplace_back(it, i, new_neighbor);
        }
      }
    ...
    // ###################################################################
    {
      ...
      for(Face_iterator it = all_faces_begin(); it != all_faces_end(); ++it)
      {
        ...
        CGAL_triangulation_assertion(extract_offset(it->offset(0)) == off[0]);
        CGAL_triangulation_assertion(extract_offset(it->offset(1)) == off[1]);
        CGAL_triangulation_assertion(extract_offset(it->offset(2)) == off[2]);
      }
    }
  ...
  void convert_to_9_sheeted_covering()
  {
        ...
        vit != original_vertices.end(); ++vit)
    {
      ...
      for(int i=0; i<3; ++i)
      {
        for(int j=0; j<3; ++j)
        {
          ...
          virtual_vertices.emplace(v_cp, std::make_pair(*vit, Offset(i, j)));
        }
      }
        ...
        original_vertices.begin(); vit != original_vertices.end(); ++vit)
    {
      ...
      off_v.push_back(extract_offset(fff->offset(v_index)));
    }
        ...
        fit != original_faces.end(); ++fit)
    {
      ...
      for(int i=0; i<3; ++i)
      {
        ...
        vvoff[i] = extract_offset((*fit)->offset(i));
      }
        ...
        fit != original_faces.end(); ++fit)
    {
      for(int i=0; i<3; ++i)
      {
        ...
        Offset vvoff = extract_offset((*fit)->offset(i));
        if(!vvoff.is_null())
        {
        ...
        fit != original_faces.end(); ++fit)
    {
      ...
      for(int i=0; i<3; ++i)
        off[i] = extract_offset((*fit)->offset(i));
...
public:
  ...
  bool is_virtual(Vertex_handle v) const
  {
    ...
    return (virtual_vertices.count(v) != 0);
  }
  ...
  const std::vector<Vertex_handle>& periodic_copies(const Vertex_handle v) const
  {
    ...
    CGAL_triangulation_precondition(virtual_vertices.count(v) == 0);
    CGAL_triangulation_assertion(virtual_vertices_reverse.find(v) != virtual_vertices_reverse.end());
  ...
  bool is_valid(Face_handle f, bool verbose = false, int level = 0) const
  {
    ...
    CGAL::Orientation o = orientation(*p[0], *p[1], *p[2], off[0], off[1], off[2]);
    if(o != POSITIVE)
    {
      if(verbose)
      {
        ...
        std::cout << construct_point(*p[0], off[0]) << " 0 "
                  << construct_point(*p[1], off[1]) << " 0 "
                  << construct_point(*p[2], off[2]) << " 0" << std::endl;
        std::cout << "Check result without offset: " << orientation(construct_point(*p[0], off[0]),
                                                                    construct_point(*p[1], off[1]),
                                                                    construct_point(*p[2], off[2])) << std::endl;
      }
  ...
  bool has_self_edges(Face_handle f) const
  {
    CGAL_triangulation_assertion((f->vertex(0) != f->vertex(1)) || (f->offset(0) != f->offset(1)));
    CGAL_triangulation_assertion((f->vertex(0) != f->vertex(2)) || (f->offset(0) != f->offset(2)));
    CGAL_triangulation_assertion((f->vertex(1) != f->vertex(2)) || (f->offset(1) != f->offset(2)));
    return ((f->vertex(0) == f->vertex(1)) ||
...
bool
test_next(const Triangulation_on_square_flat_torus_2<GT, Tds1>& t1,
          const Triangulation_on_square_flat_torus_2<GT, Tds2>& t2,
          typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Face_handle f1,
          typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Face_handle f2,
          std::map<typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Face_handle,
          typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Face_handle >& Cmap,
          std::map<typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Vertex_handle,
          typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Vertex_handle >& Vmap)
{
  ...
  typedef Triangulation_on_square_flat_torus_2<GT, Tds1>                     Tr1;
  typedef Triangulation_on_square_flat_torus_2<GT, Tds2>                     Tr2;
...
std::istream& operator>>(std::istream& is, Triangulation_on_square_flat_torus_2<Gt, Tds>& tr)
{
  return tr.load(is);
}

template<class Gt, class Tds>
std::ostream& operator<<(std::ostream& os, Triangulation_on_square_flat_torus_2<Gt, Tds>& tr)
{
  return tr.save(os);
}

template < class GT, class Tds1, class Tds2  >
bool operator==(const Triangulation_on_square_flat_torus_2<GT, Tds1>& t1,
                const Triangulation_on_square_flat_torus_2<GT, Tds2>& t2)
{
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Vertex_handle        Vertex_handle1;
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Face_handle          Face_handle1;
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Vertex_handle        Vertex_handle2;
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Vertex_handle        Vertex_iterator2;
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Face_handle          Face_handle2;
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds2>::Face_circulator      Face_circulator2;

  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Point                Point;
  typedef typename Triangulation_on_square_flat_torus_2<GT, Tds1>::Offset               Offset;
...
#define CGAL_INCLUDE_FROM_TRIANGULATION_ON_FLAT_TORUS_2_H
#include <CGAL/Periodic_2_triangulation_2/Square_flat_torus_2/Periodic_2_triangulation_dummy_12.h>
#undef CGAL_INCLUDE_FROM_TRIANGULATION_ON_FLAT_TORUS_2_H

} //namespace CGAL

#endif // CGAL_P2T2_TRIANGULATION_ON_FLAT_TORUS_2_H
