...
/*
class cgal_SHOT : public CGAL::Classification::Feature_base
{
    pcl::PointCloud<pcl_SHOT>::Ptr shotFeatures;
public:
    cgal_SHOT(const pcl::PointCloud<pcl_SHOT>::Ptr shotFeatures)
        : shotFeatures(shotFeatures)
    {
        this->set_name("SHOT");
    }

    std::vector<float> value(std::size_t pt_index)
    {
        std::vector<float> values;
        //shotFeatures->at(pt_index).descriptor
        float* desc = shotFeatures->at(pt_index).descriptor;
        std::size_t length = (std::size_t) shotFeatures->at(pt_index).descriptorSize;
        std::copy(desc, desc + length, std::back_inserter(values));
        return values;
    }
...
int main (int argc, char** argv)
{
    ...
    for (std::size_t i = 0; i < pts.size(); ++i) {
        cloud->at(i) = pcl::PointXYZ(pts.at(i).x(), pts.at(i).y(), pts.at(i).z());
    }
