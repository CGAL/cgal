...
#define COMPUTE_SKELETON true
#define COMPUTE_BETTI false
...
int main(int argc, char** argv)
{
    ...
    if (COMPUTE_COVERAGE) {
        ...
        //features.add<MyCoverage>(pts, pts.point_map(), wrap, generator.radius_neighbors(4));
        //return EXIT_SUCCESS;
        t.reset(); t.start();
    ...
    if (COMPUTE_SKELETON) {
        ...
        //using MySkeleton = CGAL::Classification::Feature::Skeleton<Kernel, Point_set, Point_set::Point_map>;
        t.reset(); t.start();
        //auto skeleton_features = CGAL::Classification::Feature::compute_skeleton_features<Kernel, Point_set, Pmap>(pts, pts.point_map(), wrap);
        //features.add<CGAL::Classification::Feature::Skeleton_branches>(std::get<0>(skeleton_features));
        //features.add<CGAL::Classification::Feature::Skeleton_cycles>(std::get<1>(skeleton_features));
        using MySkeleton = CGAL::Classification::Feature::Skeleton<Kernel, Point_set, Pmap, Mesh, CGAL::Parallel_if_available_tag>;
        features.add_multidimensional_feature2<MySkeleton>(pts.number_of_points(), 2, pts, pts.point_map(), wrap);
        std::cout << "done in " << t.time() << " second(s)" << std::endl;
    }

    if (COMPUTE_COMPACTNESS) {
        std::cout << "Computing compactness feature..." << std::endl;
        using MyCompactness = CGAL::Classification::Feature::Compactness<Kernel, Point_set, Point_set::Point_map>;
        t.reset(); t.start();
    ...
    if (COMPUTE_BETTI) {
        ...
        //using MyBettiNumbers = CGAL::Classification::Feature::BettiNumbers<Kernel, Point_set, Point_set::Point_map>;
        t.reset(); t.start();
        auto bbox = CGAL::bounding_box(CGAL::make_transform_iterator_from_property_map(pts.begin(), pts.point_map()),
            CGAL::make_transform_iterator_from_property_map(pts.end(), pts.point_map()));
        using Planimetric_grid = Classification::Planimetric_grid<Kernel, Point_set, Pmap>;
        for (std::size_t i = 0; i < generator.number_of_scales(); ++i) {
            std::cout << "  scale: " << i << std::endl;
            Planimetric_grid grid(pts, pts.point_map(), bbox, generator.grid_resolution(i));
            auto betti_numbers = CGAL::Classification::Feature::compute_betti_numbers<Kernel, Point_set, Point_set::Point_map, Planimetric_grid>(pts, pts.point_map(), grid, generator.radius_neighbors(i), wrap);
            features.add_with_scale_id<CGAL::Classification::Feature::Betti_0>(i, std::get<0>(betti_numbers));
            features.add_with_scale_id<CGAL::Classification::Feature::Betti_1>(i, std::get<1>(betti_numbers));
            features.add_with_scale_id<CGAL::Classification::Feature::Betti_2>(i, std::get<2>(betti_numbers));
        }
        std::cout << "done in " << t.time() << " second(s)" << std::endl;
    }
    ...
    if (COMPUTE_FRACTAL_DIMENSIONALITY) {
        ...
        t.reset(); t.start();
        for (std::size_t i = 0; i < generator.number_of_scales(); ++i) {
            auto fractal_dimensionality = CGAL::Classification::Feature::compute_fractal_dimensionality<Kernel, Point_set, Point_set::Point_map>(pts, pts.point_map(), generator.radius_neighbors(i));
            std::vector<float> d0 = std::get<0>(fractal_dimensionality);
            features.add_with_scale_id<CGAL::Classification::Feature::Fractal_0>(i, std::get<0>(fractal_dimensionality));
            features.add_with_scale_id<CGAL::Classification::Feature::Fractal_1>(i, std::get<1>(fractal_dimensionality));
            features.add_with_scale_id<CGAL::Classification::Feature::Fractal_2>(i, std::get<2>(fractal_dimensionality));
        }
        std::cout << "done in " << t.time() << " second(s)" << std::endl;
    }
