...
typedef Variational_medial_axis_skeleton_item_priv Priv;


typedef Scene_surface_mesh_item::Face_graph Mesh;

// TODO: update
struct Variational_medial_axis_skeleton_item::vertex{
  int id;
  double x;
  double y;
  double z;
...
struct Variational_medial_axis_skeleton_item_priv{
  ...
  Variational_medial_axis_skeleton_item_priv(const CGAL::Three::Scene_interface* scene_interface,
                                             const Scene_surface_mesh_item* sm_item,
                                             std::size_t nb_spheres,

                                             Variational_medial_axis_skeleton_item* ebi)
    : vmas(*sm_item->face_graph())
  {
...
//    const Mesh& mesh = *mesh_item->face_graph();
    ...
    vmas.init();
    vmas.compute(CGAL::parameters::number_of_spheres(nb_spheres).concurrency_tag(CGAL::Parallel_if_available_tag())/* .lambda(l) */);

    auto skeleton = vmas.export_skeleton();

    vertices.resize(skeleton.vertices().size());
    for (std::size_t i=0; i<skeleton.vertices().size(); ++i)
      vertices[i].set(skeleton.vertices()[i].center(), i );
  ...
  ~Variational_medial_axis_skeleton_item_priv(){
    delete frame;
  ...
  mutable std::vector<Variational_medial_axis_skeleton_item::vertex> vertices;
  std::vector<Variational_medial_axis_skeleton_item::vertex*> selected_vertices;
  CGAL::Variational_medial_axis<Mesh, CGAL::Epick> vmas;
  ...
  Variational_medial_axis_skeleton_item* item;
  QPoint picked_pixel;
...
Variational_medial_axis_skeleton_item::Variational_medial_axis_skeleton_item(const CGAL::Three::Scene_interface* scene_interface,
                                                                             const Scene_surface_mesh_item *sm_item,
                                                                             std::size_t nb_spheres)
{
  d = new Variational_medial_axis_skeleton_item_priv(scene_interface, sm_item, nb_spheres, this);
...
QString Variational_medial_axis_skeleton_item::toolTip() const
{
  std::stringstream ss;
/*
...
QMenu* Variational_medial_axis_skeleton_item::contextMenu()
{
...
void Variational_medial_axis_skeleton_item::drawSpheres(Viewer_interface *viewer, const QMatrix4x4 f_matrix ) const
{
  GLdouble d_mat[16];
  QMatrix4x4 mv_mat;
...
/*
void Variational_medial_axis_skeleton_item::drawPath() const
{
...
void Variational_medial_axis_skeleton_item::draw(Viewer_interface *viewer) const
{
  if(!isInit(viewer))
    initGL(viewer);
  if ( getBuffersFilled() &&
...
void Variational_medial_axis_skeleton_item::compute_bbox() const
{
...
void Variational_medial_axis_skeleton_item_priv::computeElements() const
{
  vertex_edges.clear();
  vertex_faces.clear();
  normal_faces.clear();
...
bool Variational_medial_axis_skeleton_item::supportsRenderingMode(RenderingMode m) const {
  return m==FlatPlusEdges;
}

Scene_item::ManipulatedFrame*
Variational_medial_axis_skeleton_item::manipulatedFrame()
{
  return d->frame;
}

void Variational_medial_axis_skeleton_item::highlight(Viewer_interface *viewer)
{
  d->ready_to_hl = true;
  viewer->makeCurrent();
  int type = -1, id = -1;
  ...
  if(type !=-1)
  {
    ...
    case 0:
    {
      ...
      //draw
      d->hl_type = Variational_medial_axis_skeleton_item_priv::VERTEX;
      for(CGAL::QGLViewer* v : CGAL::QGLViewer::QGLViewerPool())
      {
    ...
    default:
      d->hl_type = Variational_medial_axis_skeleton_item_priv::NO_TYPE;
      break;
    }
  }
  else
...
void Variational_medial_axis_skeleton_item::clearHL()
{
...
void Variational_medial_axis_skeleton_item_priv::reset_selection()
{
  selection_on = false;
  CGAL::QGLViewer* viewer = *CGAL::QGLViewer::QGLViewerPool().begin();
...
bool Variational_medial_axis_skeleton_item::eventFilter(QObject *obj, QEvent *event)
{
  if(!visible())
    return false;
...
void Variational_medial_axis_skeleton_item_priv::draw_picking(Viewer_interface* viewer)
{
...
void Variational_medial_axis_skeleton_item_priv::update_points(const QVector3D &dir)
{
/*   CGAL::qglviewer::AxisPlaneConstraint::Type prev_cons = constraint.translationConstraintType();
  constraint.setTranslationConstraintType(CGAL::qglviewer::AxisPlaneConstraint::FREE);
  for(Variational_medial_axis_skeleton_item::vertex*  selected_vertex: selected_vertices )
  {
...
void Variational_medial_axis_skeleton_item_priv::picking(int& type, int& id, Viewer_interface *viewer)
{
  viewer->makeCurrent();
  type = -1;
  id = -1;
...
void Variational_medial_axis_skeleton_item::drawHl(Viewer_interface* viewer)const
{
  QMatrix4x4 f_matrix;
  for (int i=0; i<16; ++i){
    f_matrix.data()[i] = (float)d->frame->matrix()[i];
  ...
  if(d->hl_type == Variational_medial_axis_skeleton_item_priv::VERTEX)
  {
    Tc* tc = getTriangleContainer(Priv::S_Spheres);
...
void Variational_medial_axis_skeleton_item::invalidateOpenGLBuffers()
{
  compute_bbox();
...
void Variational_medial_axis_skeleton_item::computeElements() const
{
  d->computeElements();
...
void Variational_medial_axis_skeleton_item::initializeBuffers(Viewer_interface *v) const
{
...
void Variational_medial_axis_skeleton_item::connectNewViewer(QObject *o)
{
