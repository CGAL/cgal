    ...
    public CGAL::Three::Scene_transparent_interface
{
  ...
  Q_PLUGIN_METADATA(IID "com.geometryfactory.CGALLab.TransparentInterface/1.0")
...
public:
  ...
  Scene_edit_box_item* clone() const { return nullptr; }
  ...
  bool performPicking(const QPoint& picked_pixel, int& type, int& id, float& depth, CGAL::Three::Viewer_interface* viewer);
  void setHighlightedHandle(int type, int id);
  void beginHandleDrag(int type, int id, const QPoint& pos, CGAL::Three::Viewer_interface* viewer);
  void dragHandle(const QVector3D& delta);
  void endHandleDrag();
  void cancelDrag();
