...
load(QFileInfo fileinfo, bool& ok, bool add_to_scene){
  ...
  // Open file
  std::ifstream in(fileinfo.filesystemFilePath(), std::ios::in | std::ios::binary);
  if(!in) {
    std::cerr << "Error! Cannot open file " << fileinfo.filesystemFilePath() << std::endl;
    ok = false;
  ...
  if (!CGAL::IO::read_polygon_soup(fileinfo.filesystemFilePath(), points, triangles))
  {
...
save(QFileInfo fileinfo,QList<CGAL::Three::Scene_item*>& items)
{
  ...
  std::ofstream out(fileinfo.filesystemFilePath(), std::ios::out | std::ios::binary);
  if ( choice == tr("Binary") )
