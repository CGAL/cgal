...
protected:
  ...
  typedef CGAL_SEGMENT_DELAUNAY_GRAPH_LINF_2_NS::Internal::Project_site_2<Vertex>
  Proj_site;
...
protected:
  ...
  // the edge list
  typedef typename
  CGAL_SEGMENT_DELAUNAY_GRAPH_LINF_2_NS::Internal::Which_list<Edge,List_tag>::List 
  List;
...
public:
  ...
  void swap(Segment_Delaunay_graph_Linf_2& sdg) {
    DG::swap(sdg);
    pc_.swap(sdg.pc_);
    isc_.swap(sdg.isc_);
  }
...
protected:
  ...
  void copy(Segment_Delaunay_graph_Linf_2& other);
  void copy(Segment_Delaunay_graph_Linf_2& other, Handle_map& hm);
...
public:
  ...
  //inline Polygonal_chain_Linf_2 bisector(co
...
protected:
  ...
  // philaris: new oriented_side function to support Linf
  inline Oriented_side
  oriented_side(const Storage_site_2& s1, const Storage_site_2& s2,
...
std::istream& operator>>(std::istream& is,
			 Segment_Delaunay_graph_Linf_2<Gt,D_S,LTag>& sdg)
{
  sdg.file_input(is);
  return is;
}
...
std::ostream& operator<<(std::ostream& os,
			 const Segment_Delaunay_graph_Linf_2<Gt,D_S,LTag>& sdg)
{
  sdg.file_output(os);
  return os;
}
