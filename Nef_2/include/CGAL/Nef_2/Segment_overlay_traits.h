...
public:
  ...
  void process_event() 
  {
    ...
    for( CGAL_LEDA_SCOPE::seq_item sitl = YS.pred(sit_succ); 
	 sitl != sit_pred; sitl = YS.pred(sitl) ) {
...
*/
  ...
  typedef std::pair<event_iterator, bool>                event_return;
  ...
  typedef CGAL::Unique_hash_map<ss_iterator, event_iterator> Y2X;
  typedef CGAL::Unique_hash_map<ISegment, ISegment> Y2Y;
  typedef CGAL::Unique_hash_map<event_iterator, ss_iterator> X2Y;
  typedef CGAL::Unique_hash_map<ss_iterator, Halfedge_handle> AssocEdgeMap;
  ...
  stl_seg_overlay_traits(const INPUT& in, OUTPUT& G, const GEOMETRY& k) 
    : its(in.first), ite(in.second), GO(G), K(k), 
    XS(lt_pnts_xy(K)), SLcmp(p_sweep,&sl,&sh,K), YS(SLcmp), 
    y2x(XS.end()), x2y(YS.end()), y2y(&sl),
  ...
  {    
	...
	event_return  er = XS.insert(event_pair(q,0)); // only done if none existed!!!
	x2y[er.first] = sit0;
	y2x[sit0] = er.first;	
  ...
  typedef std::pair<ISegment, Halfedge_handle>           ss_pair;
  typedef std::list<ss_pair>                             SweepStatus;
  // typedef std::deque<ss_pair>                             SweepStatus;
  typedef typename SweepStatus::iterator               ss_iterator;
  typedef CGAL::Unique_hash_map<ISegment, event_iterator> Y2X;
  typedef CGAL::Unique_hash_map<ISegment, ISegment>        Y2Y;
  typedef CGAL::Unique_hash_map<event_iterator, ISegment>  X2S;
  typedef CGAL::Unique_hash_map<ISegment, ss_iterator>     S2Y;
  ...
  stl_seg_overlay_traits(const INPUT& in, OUTPUT& G, const GEOMETRY& k) 
    : its(in.first), ite(in.second), GO(G), K(k), 
    XS(lt_pnts_xy(K)), SLcmp(p_sweep,&sl,&sh,K), x2s(0), SQ(lt_pnts_xy(K)) 
  {}
  ...
  { 
    ...
    for( sit3 = YS.begin(); sit3 != YS.end(); ++sit3 )
      out << sit3->first << "  y2y:" << y2y[sit3->first] << std::endl;
    return out.str();
  }
  ...
  bool collinear(ss_iterator sit1, ss_iterator sit2) const
  { if(sit1 == YS.begin() || sit1 == --YS.end() || 
       sit2 == YS.begin() || sit2 == --YS.end()) return false;
  ...
  {    
    ...
    if ( or0 <= 0 && or1 >= 0  ) { 
      Point_2 q = K.intersection(s0,s1);
      // TODO: hash existing element
      event_return  er = XS.insert(event_pair(q,0)); // only done if none existed!!!
      if(er.second) {
	x2s[er.first] = sit0->first;
	s2y[sit0->first] = sit0;
	assert(sit0 != YS.end());
      }
      y2x[sit0->first] = er.first;
    }
  } 
  ...
  void initialize_structures()
  {
    ...
    */
    ...
    // insert a lower and an upper sentinel segment to avoid special
    // cases when traversing the Y-structure
    YS.push_back(ss_pair(&sl,Halfedge_handle()));
    YS.push_back(ss_pair(&sh,Halfedge_handle()));
    CGAL_NEF_TRACEN("end of initialization\n");
  }
  ...
  void process_event() 
  {
    ...
    Segment_2 s_sweep = K.construct_segment(p_sweep,p_sweep);
    seg_pair sp(s_sweep, ite);
    ISegment itmp = x2s[event];
    if(itmp == 0) {
      sit_succ = ++YS.begin();
      // sit_succ = 
      //	std::upper_bound(++YS.begin(), --YS.end(), &sp, SLcmp);
      // TODO: is while loop more effective than upper_bound?
      //       try together with deque
			
      while(sit_succ != YS.end() &&
	    !SLcmp(&sp, sit_succ->first)) {
	++sit_succ;
      }
      sit = sit_succ; --sit;      
    } else {
      assert(itmp != YS.begin()->first);
      assert(itmp != (--YS.end())->first);
      sit_succ = s2y[itmp];
      assert(sit_succ->first == itmp);
      
      do {
	sit = sit_succ;
	++sit_succ;
      }	while(y2x[sit->first] == event ||
	      y2y[sit->first] == sit_succ->first);
    }
    ...
    CGAL_assertion(sit == YS.begin() || orientation(sit,p_sweep) == 0);
	  ...
	  ss_iterator sit_next(sit); --sit_next;
	  overlapping = 
	    y2y[sit_next->first] == sit->first;
	  CGAL_assertion(overlapping == collinear(sit_next, sit));
	    ...
	    CGAL_NEF_TRACEN("ending segment "<<s);
	    if ( overlapping ) {
	      y2y[sit_next->first] = y2y[sit->first];
	    }
	    YS.erase(sit);
	    GO.ending_segment(v,original(s));
          } else { // passing segment, take care of the node here!
	  ...
	  ++sit_next;
		...
		overlapping || 
		(orientation(sit, p_sweep) == 0)));
	// TODO: does y2x work effectively?
	...
	while( sit != sit_succ) {
	  ss_iterator sub_first(sit), 
	    sub_last(sit), succ_sub_last(sub_last);
	  ++succ_sub_last;
	  
	  while(y2y[sub_last->first] == succ_sub_last->first) {
	    ++sub_last; ++succ_sub_last;
	  }
	  
	  if(sub_last != sub_first) {
	    ++sub_last;
	    std::reverse(sub_first,sub_last);
	  }
	...
	ss_iterator sitx;
	//	for(sitx = sit_first; sitx != sit_succ; ++sitx)
	//	  std::cerr << "before " << sitx->first << std::endl;
	std::reverse(sit_first,sit_succ);
	//	for(sitx = sit_first; sitx != sit_succ; ++sitx)
	//	  std::cerr << "after  " << sitx->first << std::endl;	
	for(sitx = sit_first; sitx != sit_succ; ++sitx)
	  s2y[sitx->first] = sitx;
      } // if (sit != ss_iterator() )
      ...
      while ( next_it != SQ.end() && 
              ( next_seg = next_it->second, p_sweep == source(next_seg)) ) {
        CGAL_NEF_TRACEN("inserting "<<next_seg);
	ss_iterator s_sit(sit_pred);
	do{ ++s_sit; } 
	while(s_sit != YS.end() && 
	      SLcmp(s_sit->first, next_seg));

	ss_iterator ys_tmp =
	  YS.insert(s_sit, ss_pair(next_seg,Halfedge_handle()));
	// Line needed when YS is a deque: s_sit = ys_tmp+1;

	if(s_sit != --YS.end() &&
	   orientation(s_sit, source(next_seg)) == 0 &&
	   orientation(s_sit, target(next_seg)) == 0) {
	  y2y[ys_tmp->first] = s_sit->first;
	}

	ss_iterator p_sit(ys_tmp); --p_sit;
	if ( YS.begin() != p_sit &&
	     orientation(p_sit, source(next_seg) ) == 0 &&
	     orientation(p_sit, target(next_seg) ) == 0 ) {
	  y2y[p_sit->first] = ys_tmp->first;
	}

	s2y[ys_tmp->first] = ys_tmp;
	x2s[XS.find(target(next_seg))] = ys_tmp->first;
	assert(++ys_tmp == s_sit);
	assert(ys_tmp != YS.end());
        GO.starting_segment(v,original(next_seg));
        // delete minimum and assign new minimum to next_seg
        SQ.erase(SQ.begin());
        next_it = SQ.begin();
      ...
      for( --sit_curr; sit_curr != sit_pred; 
        ...
        if (y2y[sit_curr->first] == sit_prev->first) { // overlapping
	  assert(collinear(sit_curr, sit_prev));
