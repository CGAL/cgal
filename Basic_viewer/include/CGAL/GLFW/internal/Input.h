...
private:
  std::unordered_map<int, bool> m_pressed_keys, m_holding_keys, m_consumed_keys;
  std::unordered_map<ActionEnum, bool> m_started_actions, m_activated_actions;

  std::unordered_map<ActionEnum, std::string> m_action_description;
  std::deque<Action> m_key_actions;
  Eigen::Vector2f m_cursor, m_cursor_old, m_cursor_delta;
...
double Input::get_scroll_yoffset()
{
  double ret = m_yoffset;
  m_yoffset = 0;
  return ret;
}
...
void Input::add_action(Key_data keys, ActionEnum action)
{
  ...
  while (it != m_key_actions.end())
  {
    if (it->keys.priority < keys.priority)
    {
      m_key_actions.insert(it, act);
      return;
    }
    it++;
  }
...
void Input::on_key_event(int key, int scancode, int action, int mods)
{
  if (action == GLFW_PRESS)
  {
    ...
    m_holding_keys[key] = true;
  }

  if (action == GLFW_RELEASE)
  {
    m_holding_keys.erase(key);
  }
}

void Input::on_cursor_event(double xpos, double ypos)
{
  m_cursor_delta << xpos - m_cursor.x(), ypos - m_cursor.y();
...
auto last_click = std::chrono::high_resolution_clock::now();
int last_btn_clicked = -1;

constexpr auto ms = std::chrono::milliseconds(250);
...
void Input::on_mouse_btn_event(int btn, int action, int mods)
{
  ...
  if (action == GLFW_PRESS)
  {
    ...
    const std::chrono::duration<double> dt = time - last_click;
    ...
    m_pressed_keys[btn] = true;
    m_holding_keys[btn] = true;
  }

  if (action == GLFW_RELEASE)
  {
    m_holding_keys.erase(btn);
  }
}
...
void Input::handle_events(const double deltaTime)
{
  ...
  for (auto& [k, action] : m_key_actions)
  {
    ...
    {
      m_consumed_keys[k.key1] = true;

      if (k.hold)
      {
        m_activated_actions[action] = true;
