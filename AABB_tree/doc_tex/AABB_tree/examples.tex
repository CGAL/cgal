...
In the following example, a set of 3D triangles is stored in a list. The AABB primitive wraps a triangle as \ccc{datum} and an iterator in the list as \ccc{id}. We compute the number of input triangles intersected by a line query, and the closest point from a query point to the input triangles.
...
The following example computes the number of intersections between a 3D ray and a set of facets of a triangle polyhedral surface. The AABB primitive wraps a facet handle as \ccc{id} and generates a 3D triangle on the fly, each time its method \ccc{datum} is called.
