...
\ccNestedType{Data}{Type of input objects.}
...
Usually, the \ccc{Primitive} type is a wrapper around a \ccc{Handle}. Assume for instance that the input objects are the triangle faces of a mesh stored as a \ccc{CGAL::Polyhedron}, the Primitive type would be a wrapper around the \ccc{Face_handle}  type of the polyhedron. The \ccc{Data} would be \ccc{Face},
 the \ccc{Id} would be \ccc{Face_handle}, \ccc{data()} would dereference the \ccc{Face_handle}, and \ccc{id()} would return the \ccc{Face_handle} itself. 
