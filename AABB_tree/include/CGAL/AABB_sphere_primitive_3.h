...
#ifndef CGAL_AABB_SPHERE_PRIMITIVE_3_H_
#define CGAL_AABB_SPHERE_PRIMITIVE_3_H_
...
namespace internal {
  ...
  struct Point_on_sphere_iterator_property_map{
    //classical typedefs
    ...
    typedef Point_on_sphere_iterator_property_map<GeomTraits, Iterator> Self;

    inline friend reference
    get(Self, key_type it)
    {
      const typename GeomTraits::Point_3 c = it->center();
      const typename GeomTraits::FT r = approximate_sqrt(it->squared_radius());
 ...
 * Primitive type that uses as identifier an iterator with a 3D sphere as `value_type`.
...
class AABB_sphere_primitive_3
#ifndef DOXYGEN_RUNNING
  : public AABB_primitive<  Iterator,
                            Input_iterator_property_map<Iterator>,
                            internal::Point_on_sphere_iterator_property_map<GeomTraits, Iterator>,
                            Tag_false,
                            CacheDatum >
#endif
{
...
public:
  ...
  AABB_sphere_primitive_3(Iterator it) : Base(it){}
};
