...
\begin{ccRefClass}{Dispatch_output_iterator<V,O>}
  ...
  \ccParameters
  \ccc{V} must be a \ccc{CGAL::tuple} of the types of values to be accepted and dispatched.
  \ccc{O} must be a \ccc{CGAL::tuple} of the types of corresponding output iterators.
  ...
  \ccTypedef{typedef V Value_types_tuple;}{}
  \ccGlue
  \ccTypedef{typedef O Iterators_tuple;}{}

  \ccCreation

  \ccTwo{Exptyset_iterator()}{}

  \ccConstructor{Dispatch_output_iterator(O...o);}{Constructor taking all the output iterators.}

  \ccMemberFunction{const Iterators_tuple& get_iterator_tuple() const;}
...
\begin{ccRefClass}{Dispatch_or_drop_output_iterator<V,O>}
  ...
  \ccConstructor{Dispatch_or_drop_output_iterator(O...o);}{Constructor taking all the output iterators.}
...
\begin{ccRefClass}{Twotuple<T>}
  ...
  Please use \ccc{CGAL::array} instead.
...
\begin{ccRefClass}{Triple<T1, T2, T3>}
  ...
  \ccc{boost::tuple} or \ccc{CGAL::array} instead for new uses.
