...
int main()
{
  const std::filesystem::path file_cube = CGAL::data_file_path("meshes/cube.off");
  std::cout << "Testing close mesh " << file_cube << std::endl;
  Mesh mesh_cube;
  if (!load_mesh(file_cube, mesh_cube) || !test_shape(mesh_cube, 6))
    return EXIT_FAILURE;

  const std::filesystem::path file_cube2 = CGAL::data_file_path("meshes/cube-ouvert.off");
