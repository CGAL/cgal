...
\ccc{iso} and then its \ccc{operator()}  implemenents
the function \ccc{sign of (f(x,y,z) - iso)}.
...
\ccc{ImageIO}. An executable that uses \ccRefName\ must be linked with the ImageIO
library. This library is shipped with \cgal\ in the
\ccc{examples/Surface_mesher/} subdirectory.

The library \ccc{ImageIO} and therefore \ccRefName\ support
