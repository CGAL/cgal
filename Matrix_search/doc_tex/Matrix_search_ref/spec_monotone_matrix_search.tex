...
\begin{ccRefFunction}{monotone_matrix_search}
  ...
  \begin{ccAdvanced}
    ...
    \item \ccc{Matrix} is a model for
      \ccc{MonotoneMatrixSearchTraits}.
...
\begin{ccRefClass}{Dynamic_matrix<M>}
  \begin{ccAdvanced}
    ...
    \ccRequirements \ccc{M} is a model for \ccc{BasicMatrix}.
    ...
    \ccMemberFunction{void replace_column( int old, int new);}{replace
      column \ccc{old} with column number \ccc{new}.  \ccPrecond{
      $0 \le$ \ccc{old}, \ccc{new} $<$ \ccc{number_of_columns()}.}}
...
\begin{ccRefConcept}{MonotoneMatrixSearchTraits}
  \begin{ccAdvanced}
    \ccCreationVariable{m}\ccTagFullDeclarations
    
    \ccDefinition The concept \ccRefName\ is a refinement of
    \ccc{BasicMatrix} and defines types and operations needed to
...
\begin{ccRefConcept}{BasicMatrix}
  \begin{ccAdvanced}
    \ccCreationVariable{m}\ccTagFullDeclarations
