...
/// \name Has Models 
CONVERROR Check if this needs to be spread\n/// `CGAL::Triangulation_data_structure<Dimensionality, TriangulationDSVertex, TriangulationDSFullCell>` The concept `TriangulationDataStructure` also defines a type for describing facets of the triangulation with codimension 1: Vertices and full cells are manipulated via <I>handles</I>. Handles support the usual two dereference operators and `operator->`. Requirements for `Vertex` and `Full_cell` are described in concepts `TriangulationDataStructure::Vertex` and `TriangulationDataStructure::FullCell` . CONVERROR ADVANCED Vertices, facets and full cells can be iterated over using <I>iterators</I>. Iterators support the usual two dereference operators and `operator->`.
/// @{
...
CONVERROR Check if this needs to be spread\n/// CONVERROR HtmlOnly needs treatment <center> <img border=0 src="./fig/insert-in-cell.png" align="middle" alt="The effect of insert_in_full_cell()"> </center> CONVERROR EndHtmlOnly CAPTION Insertion in a full cell, \f$ \cd=2\f$ \anchor triangulationfiginsertfullcell CONVERROR HtmlOnly needs treatment <center> <img border=0 src="./fig/insert-in-face.png" align="middle" alt="The effect of insert_in_face()"> </center> CONVERROR EndHtmlOnly CAPTION Insertion in face, \f$ \cd=3\f$ \anchor triangulationfiginsertface CONVERROR HtmlOnly needs treatment <center> <img border=0 src="./fig/insert-in-hole.png" align="middle" alt="The effect of insert_in_hole()"> </center> CONVERROR EndHtmlOnly CAPTION Insertion in a hole, \f$ \cd=2\f$ \anchor triangulationfiginserthole CONVERROR HtmlOnly needs treatment <center> <img border=0 src="./fig/insert-increase-dim.png" align="middle" alt="The effect of insert_increase_dimension()"> </center> CONVERROR EndHtmlOnly CAPTION Insertion, increasing the dimension from \f$ \cd=1\f$ to \f$ \cd=2\f$ \anchor triangulationfiginsertincreasedim CONVERROR ADVANCED
/// @{
...
share the vertex `v` (see Figure \ref triangulationfiginsertfullcell). 
...
`Face f`. Returns a handle to the newly created `Vertex` (see 
Figure below \ref triangulationfiginsertface). 
*/ 
...
(see Figure \ref triangulationfiginserthole). 
...
/// \name Vertex removal 
CONVERROR Check if this needs to be spread\n/// CONVERROR HtmlOnly needs treatment <center> <img border=0 src="./fig/collapse-face.png" align="middle" alt="The effect of collapse_face()"> </center> CONVERROR EndHtmlOnly CAPTION Collapsing an edge in dimension \f$ \cd=3\f$, `v` is returned \anchor triangulationfigcollapseface CONVERROR ADVANCED CONVERROR DEBUG
/// @{
...
\relates TriangulationDataStructure 
*/ 
istream & operator>>(istream & is, TriangulationDataStructure & 
tds); 
...
\relates TriangulationDataStructure 
*/ 
ostream & operator<<(ostream & os, const TriangulationDataStructure 
& tds); 
