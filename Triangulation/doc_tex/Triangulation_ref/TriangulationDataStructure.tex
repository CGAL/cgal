...
\note{OD: je vire la precondition (is-valid est pas vraiment une
  precondition, puisque c'est garanti par la structure hors fonctions advanced).}
}
\ccMethod{template< typename OutputIterator > OutputIterator
...
\note{SH: Peut-être mettre cette fonction dans la doc de la classe? ou
nulle-part. La doc de \ccc{TriangulationFace} est sûrment suffisante.}%
}
