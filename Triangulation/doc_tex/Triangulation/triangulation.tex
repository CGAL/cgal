...
It always holds that $-2\leq\ad\leq\cd$ and $0<D$.


\note{ I remove some comments about 3D vs dD which are not exact.
in T3D package in degenerate dimension
\ccc{Cell} is actually used with the same meaning as here 
(a $d$-face  and not a $D$-face.}
...
\ccc{Triangulation_ds_full_cell<TDS, TDSFullCellStoragePolicy>} (typically a
