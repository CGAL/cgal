...
TDSVertex, TDSSimplex>} models an abstract triangulation: vertices in this
...
TDSSimplex>}.  For simplicity, we use the abbreviation \tds.
...
A \tds\ also knows the \textbf{current dimension} of its maximal simplices,
...
The \ccc{Triangulation_data_structure<Dimensionality, TDSVertex, TDSSimplex>}
...
\ccc{TDSSimplex} template parameter.
\end{itemize}
...
The \ccc{TDSVertex} and \ccc{TDSSimplex} parameters to the class template
...
\ccc{Triangulation_ds_simplex<TDS, TDSSimplexStoragePolicy>}, which, as one
...
\ccc{Triangulation_ds_simplex<TDS, TDSSimplexStoragePolicy>} (typically a
