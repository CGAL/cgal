...
\begin{ccRefConcept}{DelaunayComplexTraits}
...
\ccc{Delaunay_complex<DCTraits, PCDS>}. It brings the geometric ingredient to
...
Point_d & p)}.\\
Let Aff be the affine subspace spanned by the points in range
\ccc{[start,end)}. The operator behaves in the same way as the above predicate
\ccc{Side_of_oriented_sphere_d}, but operates in the affine subspace Aff. It
is guaranteed that the affine subspace Aff is given a consistent orientation
...
subspace Aff doesn't change.
\ccPrecond \ccc{std::distance(start,end)>=3} and
\ccc{std::distance(start,end)<=start->dimension()+1}. The points in range
...
\ccConstructor{DelaunayComplexTraits();}{The default constructor.}
