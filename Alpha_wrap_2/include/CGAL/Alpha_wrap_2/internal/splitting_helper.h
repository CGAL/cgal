...
//
#ifndef CGAL_ALPHA_WRAP_2_INTERNAL_SPLITTING_HELPER_H
#define CGAL_ALPHA_WRAP_2_INTERNAL_SPLITTING_HELPER_H
...
struct AABB_tree_splitter_traits
{
  ...
  using BPMB = internal::Vector_property_map<CGAL::Bbox_2>;
  ...
  using DPMB = internal::Vector_property_map<Triangle_2>; // Datum pos --> Datum
  ...
  using AABB_traits = CGAL::AABB_traits_2<GT, Primitive, BPM>;
...
protected:
  ...
  CGAL::Bbox_2 compute_bbox(const P& ap0, const P& ap1, const P& ap2)
  {
  ...
  void split_and_insert_datum(const Triangle_2& tr,
                              AABB_tree& tree,
                              const GT& gt)
  {
    ...
    using AFT = AK::FT;
    using APoint_2 = AK::Point_2;

    using APL = std::pair<APoint_2, AFT>; // point and length of the opposite edge
    using AT = std::array<APL, 3>;

    const std::size_t data_size = m_dpmb.range().size();
    put(m_dpmb, data_size, tr);

    auto vertex = gt.construct_vertex_2_object();
    ...
    if(!subdivide || m_sq_alpha == 0.) // no splits
    {
      ...
      put(m_bpm, id, gt.construct_bbox_2_object()(tr));
...
//                    << "Datum: [" << get(m_bpm, id) << "]" << std::endl;
    ...
    const APoint_2 ap0 = k2ak(p0);
    const APoint_2 ap1 = k2ak(p1);
    const APoint_2 ap2 = k2ak(p2);
    ...
    while(!to_treat.empty())
    {
      ...
      if(max_sql > sq_bound)
      {
        ...
        if(i == 0)
        {
          ...
          const APoint_2 amp = CGAL::midpoint(apl1.first, apl2.first);
        ...
        else if(i == 1)
        {
          ...
          const APoint_2 amp = CGAL::midpoint(apl2.first, apl0.first);
          to_treat.push(CGAL::make_array(std::make_pair(apl0.first, CGAL::squared_distance(apl1.first, amp)),
                                         std::make_pair(apl1.first, CGAL::squared_distance(amp, apl0.first)),
                                         std::make_pair(amp, apl2.second)));
        ...
        else // i == 2
        {
          ...
          const APoint_2 amp = CGAL::midpoint(apl0.first, apl1.first);
          to_treat.push(CGAL::make_array(std::make_pair(apl2.first, CGAL::squared_distance(apl0.first, amp)),
                                         std::make_pair(apl0.first, CGAL::squared_distance(amp, apl2.first)),
                                         std::make_pair(amp, apl1.second)));
          to_treat.push(CGAL::make_array(std::make_pair(apl1.first, CGAL::squared_distance(apl2.first, amp)),
...
#endif // CGAL_ALPHA_WRAP_2_INTERNAL_SPLITTING_HELPER_H
