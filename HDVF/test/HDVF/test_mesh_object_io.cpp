...
typedef HDVF::Mesh_object_io<Traits>::Point Point;

int main() {
    // ------ Test constructor from vectors of nodes and Io_cell_types

    std::vector<Point> nodes1, nodes2;
    std::vector<HDVF::Io_cell_type> cells1, cells2;

    // (nodes1, cells1) : "pure" mesh
    // (nodes2, cells2) : heterogeneous simplicial complex
    nodes1.push_back(Point(0,0,0));
    nodes1.push_back(Point(0,1,0));
    nodes1.push_back(Point(0,0,1));
    nodes2 = nodes1;
    nodes2.push_back(Point(0,1,1));
