...
int main() {
    CGAL::HDVF::Simplex s1({1,2}), s2({1,2,3});
    std::vector<CGAL::HDVF::Simplex> bnd, bnd2 ;
    ...
    bnd = s1.boundary() ;
    bnd2.push_back(CGAL::HDVF::Simplex({2})) ;
    bnd2.push_back(CGAL::HDVF::Simplex({1})) ;
    ...
    bnd = s2.boundary() ;
    bnd2.clear();
    bnd2.push_back(CGAL::HDVF::Simplex({2,3})) ;
    bnd2.push_back(CGAL::HDVF::Simplex({1,3})) ;
    bnd2.push_back(CGAL::HDVF::Simplex({1,2})) ;
    assert(bnd.size() == bnd2.size());
    for (int i=0; i<bnd.size(); ++i)
        assert(bnd.at(i) == bnd2.at(i)) ;
    ...
    CGAL::HDVF::Simplex s3({3,2,1},true);
    assert(s2 == s3);
    ...
    for (CGAL::HDVF::Simplex::const_iterator it = s2.cbegin(); it != s2.cend(); ++it)
        assert(*it == i++);
