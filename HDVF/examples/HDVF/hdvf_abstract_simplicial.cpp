#include <iostream>
#include <fstream>

#include <CGAL/Simple_cartesian.h>
#include <CGAL/Zp.h>
#include <CGAL/Z2.h>
#include <CGAL/HDVF/Hdvf_traits_3.h>
#include <CGAL/HDVF/Mesh_object_io.h>
#include <CGAL/HDVF/Simplicial_chain_complex.h>
#include <CGAL/HDVF/Geometric_chain_complex_tools.h>
#include <CGAL/HDVF/Hdvf.h>
#include <CGAL/OSM/OSM.h>

namespace HDVF = CGAL::Homological_discrete_vector_field;

//typedef int Coefficient_ring;
//typedef CGAL::Z2 Coefficient_ring;
typedef CGAL::Zp<5, char, true> Coefficient_ring;

typedef CGAL::Simple_cartesian<double> Kernel;
typedef HDVF::Hdvf_traits_3<Kernel> Traits;

int main(int argc, char **argv)
{
#if 1
    //    using Complex = HDVF::Simplicial_chain_complex<Coefficient_ring,Traits> ;
    using Complex = HDVF::Abstract_simplicial_chain_complex<Coefficient_ring> ;
    using HDVF_type = HDVF::Hdvf<Complex> ;

    std::string filename ;
    if (argc > 2) std::cerr << "usage: hdvf_abstract_simplicial simp_file" << std::endl;
    else if (argc == 1) filename  = "data/three_triangles_2.simp";
    else filename = argv[1];

    // Load simp object
    HDVF::Mesh_object_io<Traits> mesh ;
    mesh.read_simp(filename);

    mesh.print_infos();

    // Build simplicial chain complex
    Complex complex(mesh);

    std::cout << complex;

    //    // Build empty HDVF
    // Without dimension restriction
    //    HDVF_type hdvf(complex, HDVF::OPT_FULL) ;
    // With computation restricted to dimension 1
    HDVF_type hdvf(complex, HDVF::OPT_FULL, 1) ;

    // Compute a perfect HDVF
    //    hdvf.compute_perfect_hdvf();
    hdvf.compute_rand_perfect_hdvf();
    hdvf.write_hdvf_reduction("tmp/hdvf_init.hdvf");

//    hdvf.read_hdvf_reduction("tmp/hdvf_init.hdvf");

    // Output HDVF to console
    hdvf.write_flags(std::cout, -1);
    hdvf.write_matrices(std::cout, -1);

//    // M operation
//    {
//        bool found;
//        std::vector<HDVF::Cell_pair> pairs(hdvf.find_pairs_M(1, found));
//        if (found) {
//            hdvf.M(pairs.at(0));
//            std::cout << "===========" << std::endl;
//            hdvf.write_flags(std::cout, -1);
//            hdvf.write_matrices(std::cout, -1);
//        }
//    }

//    // W operation
//    {
//        bool found;
//        std::vector<HDVF::Cell_pair> pairs(hdvf.find_pairs_W(1, found));
//        if (found) {
//            hdvf.W(pairs.at(0));
//            std::cout << "===========" << std::endl;
//            hdvf.write_flags(std::cout, -1);
//            hdvf.write_matrices(std::cout, -1);
//        }
//    }

//    // MW operations
//    {
//        bool found;
//        std::vector<HDVF::Cell_pair> pairs(hdvf.find_pairs_MW(1, found));
//        if (found) {
//            hdvf.MW(pairs.at(0));
//            std::cout << "===========" << std::endl;
//            hdvf.write_flags(std::cout, -1);
//            hdvf.write_matrices(std::cout, -1);
//        }
//    }

    // Save HDVF to .hdvf file
    hdvf.write_hdvf_reduction("tmp/test.hdvf") ;

#endif
    return 0;
}
