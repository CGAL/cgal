...
// in-between.
  ...
  public:
    ...
    Halfedge_handle weld_edge(Halfedge_handle h, Halfedge_handle g) {
      // weld two edges such that the topology relationship between
      // the two incident facet is build.
      // the endpoints of g are set to the endopoints of h
      // return the new welded halfedge hnew
      Assert_compile_time_tag(Tag_true(), Supports_removal());
      /*CGAL_precondition(h->is_border());
      CGAL_precondition(g->is_border());*/
      Halfedge_handle hprev, hnext, gprev, gnext;
      if (h->next() != g) {
        gprev = find_prev(g);
        hnext = h->next();
        vertices_erase(get_vertex(gprev));
        // set the vertex of halfedges
        set_vertex(gprev, get_vertex(h));
        Halfedge_handle gi = g->opposite();
        while (gi != gprev) {
          set_vertex(gi, get_vertex(h));
          gi = gi->next()->opposite();
        }
        // set the prev and next of haledges
        gprev->HBase::set_next(hnext);
        set_prev(hnext, gprev);
        h->HBase::set_next(g);
        set_prev(g, h);
      }
      if (g->next() != h) {
        gnext = g->next();
        hprev = find_prev(h);
        vertices_erase(get_vertex(g));
        // set the vertex of halfedges
        set_vertex(g, get_vertex(hprev));
        Halfedge_handle gi = gnext->opposite();
        while (gi != g) {
          set_vertex(gi, get_vertex(hprev));
          gi = gi->next()->opposite();
        }
        // set the prev and next of halfedges
        hprev->HBase::set_next(gnext);
        set_prev(gnext, hprev);
        g->HBase::set_next(h);
        set_prev(h, g);
      }
      CGAL_precondition(h->next() == g && g->next() == h);
      hprev = find_prev(h->opposite());
      gprev = find_prev(g->opposite());
      hnext = h->opposite()->next();
      gnext = g->opposite()->next();
      hprev->HBase::set_next(gnext);
      set_prev(gnext, hprev);
      gprev->HBase::set_next(hnext);
      set_prev(hnext, gprev);
      // remove the old edges, and add the new edge
      hds->edges_erase(h);
      hds->edges_erase(g);
      Halfedge_handle hnew = hds->edges_push_back(Halfedge(), Halfedge());
      insert_tip(hnew->opposite(), hprev);
      insert_tip(hnew, gprev);
      set_face(hnew, get_face(hprev));
      set_face(hnew->opposite(), get_face(gprev));
      set_face_halfedge(hnew);
      set_face_halfedge(hnew->opposite());
      set_vertex_halfedge(hnew);
      set_vertex_halfedge(hnew->opposite());
