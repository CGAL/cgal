...
In \cgal, \blas\ is required by the \ccc{Jet_fitting_3} and \ccc{Ridges_3}
packages only.
...
\texttt{APPLEVECLIB, SUNPERFORMANCECC, SUNPERFORMANCEGCC, SGICOMPLIB,
...
no native \blas\ implementation (see Section~\ref{sec:taucs-setup}).

Usually, \atlas\ setup is automatically deduced from \blas\ and \taucs\ setup.
...
In \cgal, \lapack\ is required by the \ccc{Jet_fitting_3} and \ccc{Ridges_3}
packages only.

You can download the official release from \lapackpage.
Alternatively, installing \taucs\ provides \lapack\ (see Section~\ref{sec:taucs-setup}).
...
In \cgal, it is used to improve (a lot) the computations within
...
or \atlas\ as \taucs\ requires \blas\ plus \lapack\ or \atlas). For
...
TAUCSFREEBSDLAPACK, TAUCSFREEBSDATLAS, TAUCSDARWIN, TAUCSIRIXCC32 or TAUCSIRIXCC64}
(depending of your platform and BLAS library).
