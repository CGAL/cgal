...
\begin{center}\index{cgal!config_vars}\index{cgal configuration variables}
  \renewcommand{\arraystretch}{1.3}
  \gdef\lcTabularBorder{2}
  \begin{tabular}{|l|l|l|l|} \hline
    \textbf{variable}  & \textbf{description} & \textbf{possible values} & \textbf{default value}
    \\\hline\hline
    {\tt CMAKE_BUILD_TYPE}  & Indicates type of build & Debug/Release & Debug
    \\\hline
    {\tt CMAKE_CXX_FLAGS} & C++ flags for both Debug and Release modes. & space-separated list of flags & platform-dependent, determined by cmake
    \\\hline
    {\tt CMAKE_CXX_FLAGS_RELEASE} & C++ flags for Release mode only. & space-separated list of flags & platform-dependent, determined by cmake
    \\\hline
    {\tt CMAKE_CXX_FLAGS_DEBUG} & C++ flags for Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_SHARED_LINKER_FLAGS} & Linker flags for shared libraries on both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_SHARED_LINKER_FLAGS_RELEASE} & Linker flags for shared libraries on Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_SHARED_LINKER_FLAGS_DEBUG} & Linker flags for shared libraries on Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_MODULE_LINKER_FLAGS} & Linker flags for static libraries on both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_MODULE_LINKER_FLAGS_RELEASE} & Linker flags for static libraries on Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_MODULE_LINKER_FLAGS_DEBUG} & Linker flags for static libraries on Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_EXE_LINKER_FLAGS} & Linker flags for executables on both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_EXE_LINKER_FLAGS_RELEASE} & Linker flags for executables on Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_EXE_LINKER_FLAGS_DEBUG} & Linker flags for executables on Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_CXX_FLAGS} & Additional C++ flags for both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_CXX_FLAGS_RELEASE} & C++ flags for Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_CXX_FLAGS_DEBUG} & C++ flags for Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_SHARED_LINKER_FLAGS} & Linker flags for shared libraries on both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_SHARED_LINKER_FLAGS_RELEASE} & Linker flags for shared libraries on Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_SHARED_LINKER_FLAGS_DEBUG} & Linker flags for shared libraries on Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_MODULE_LINKER_FLAGS} & Linker flags for static libraries on both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_MODULE_LINKER_FLAGS_RELEASE} & Linker flags for static libraries on Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_MODULE_LINKER_FLAGS_DEBUG} & Linker flags for static libraries on Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_EXE_LINKER_FLAGS} & Linker flags for executables on both Debug and Release modes. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_EXE_LINKER_FLAGS_RELEASE} & Linker flags for executables on Release mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CGAL_EXE_LINKER_FLAGS_DEBUG} & Linker flags for executables on Debug mode only. & space-separated list of flags & platform-dependent
    \\\hline
    {\tt CMAKE_CXX_COMPILER} & C++ compiler to use. This is used only if Makefiles are generatede. & full path to the compiler executable & platform-dependent
    \\\hline
    
  \end{tabular}
\end{center}

\subsection{Used to configure only the \cgal\ libraries}
...
\subsection{Used to configure only programs linking to \cgal\}
...
\begin{center}\index{cgal!config_vars}\index{cgal configuration variables}
  ...
  \begin{tabular}{|l|l|l|l|} \hline
    ...
    {\tt WITH\_CGAL\_PDB}    & {\em CMake} & Whether to configure CGAL_PDB component along with the main library & TRUE/FALSE & TRUE
    \\\hline\hline
    {\tt CMAKE_BUILD_TYPE}   & {\em CMake} & Whether to configure CGAL_ImageIO component along with the main library & TRUE/FALSE & TRUE
    \\\hline
