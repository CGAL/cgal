...
inline CGAL::Color get_random_color(CGAL::Random &random) {
  CGAL::Color res;
  do {
    res = CGAL::Color(random.get_int(0, 256), random.get_int(0, 256),
...
public:
  ...
  Basic_viewer_qt(QWidget *parent, const char *title = "",
                  bool draw_vertices = false, bool draw_edges = true,
                  bool draw_lines = true, bool draw_faces = true,
                  bool use_mono_color = false, bool inverse_normal = false)
      : CGAL::QGLViewer(parent), m_draw_vertices(draw_vertices),
        m_draw_edges(draw_edges), m_draw_lines(draw_lines),
        m_draw_faces(draw_faces), m_flatShading(true),
        m_use_mono_color(use_mono_color), m_inverse_normal(inverse_normal),
        m_size_points(7.), m_size_edges(3.1), m_size_lines(3.1),
        m_vertices_mono_color(200, 60, 60), m_edges_mono_color(0, 0, 0),
        m_lines_mono_color(0, 100, 0), m_faces_mono_color(60, 60, 200),
        m_ambient_color(0.6f, 0.5f, 0.5f, 0.5f),
        m_are_buffers_initialized(false),
        m_buffer_for_mono_points(&arrays[POS_MONO_POINTS], NULL,
                                 &m_bounding_box, NULL, NULL, NULL),
        m_buffer_for_colored_points(&arrays[POS_COLORED_POINTS], NULL,
                                    &m_bounding_box, &arrays[COLOR_POINTS],
                                    NULL, NULL),
        m_buffer_for_mono_segments(&arrays[POS_MONO_SEGMENTS], NULL,
                                   &m_bounding_box, NULL, NULL, NULL),
        m_buffer_for_colored_segments(&arrays[POS_COLORED_SEGMENTS], NULL,
                                      &m_bounding_box, &arrays[COLOR_SEGMENTS],
                                      NULL, NULL),
        m_buffer_for_mono_lines(&arrays[POS_MONO_LINES], NULL, &m_bounding_box,
                                NULL, NULL),
        m_buffer_for_colored_lines(&arrays[POS_COLORED_LINES], NULL,
                                   &m_bounding_box, &arrays[COLOR_LINES], NULL,
                                   NULL),
        m_buffer_for_mono_faces(&arrays[POS_MONO_FACES], NULL, &m_bounding_box,
                                NULL, &arrays[FLAT_NORMAL_MONO_FACES],
                                &arrays[SMOOTH_NORMAL_MONO_FACES]),
        m_buffer_for_colored_faces(&arrays[POS_COLORED_FACES], NULL,
...
protected:
  void compile_shaders() {
    ...
    QOpenGLShader *fragment_shader_p_l =
  ...
  void initialize_buffers() {
    ...
    buffers[bufn].bind();
    buffers[bufn].allocate(
        arrays[POS_MONO_POINTS].data(),
    ...
    buffers[bufn].bind();
    buffers[bufn].allocate(
        arrays[POS_COLORED_POINTS].data(),
    ...
    buffers[bufn].bind();
    buffers[bufn].allocate(
        arrays[POS_COLORED_SEGMENTS].data(),
    ...
    if (m_flatShading) {
      buffers[bufn].allocate(
          arrays[FLAT_NORMAL_MONO_FACES].data(),
          static_cast<int>(arrays[FLAT_NORMAL_MONO_FACES].size() *
                           sizeof(float)));
    } else {
      buffers[bufn].allocate(
          arrays[SMOOTH_NORMAL_MONO_FACES].data(),
    ...
    if (m_flatShading) {
      buffers[bufn].allocate(
          arrays[FLAT_NORMAL_COLORED_FACES].data(),
          static_cast<int>(arrays[FLAT_NORMAL_COLORED_FACES].size() *
                           sizeof(float)));
    } else {
      buffers[bufn].allocate(
          arrays[SMOOTH_NORMAL_COLORED_FACES].data(),
  ...
  void attrib_buffers(CGAL::QGLViewer *viewer) {
    ...
    if (bb == bounding_box()) // Case of "empty" bounding box
    {
      ...
      bb =
          bb + Local_point(1, 1, 1).bbox(); // To avoid a warning from Qglviewer
    } else {
      bb = bounding_box();
    }

    QVector4D position((bb.xmax() - bb.xmin()) / 2, (bb.ymax() - bb.ymin()) / 2,
                       bb.zmax(), 0.0);
  ...
  virtual void draw() {
    ...
    if (m_draw_vertices) {
      ...
      rendering_program_p_l.setUniformValue("point_size",
                                            GLfloat(m_size_points));
      glDrawArrays(GL_POINTS, 0,
                   static_cast<GLsizei>(arrays[POS_MONO_POINTS].size() / 3));
      vao[VAO_MONO_POINTS].release();
      ...
      rendering_program_p_l.setUniformValue("point_size",
                                            GLfloat(m_size_points));
      glDrawArrays(GL_POINTS, 0,
                   static_cast<GLsizei>(arrays[POS_COLORED_POINTS].size() / 3));
      vao[VAO_COLORED_POINTS].release();
    ...
    if (m_draw_edges) {
      ...
      glDrawArrays(GL_LINES, 0,
                   static_cast<GLsizei>(arrays[POS_MONO_SEGMENTS].size() / 3));
      vao[VAO_MONO_SEGMENTS].release();
      ...
      glDrawArrays(
          GL_LINES, 0,
          static_cast<GLsizei>(arrays[POS_COLORED_SEGMENTS].size() / 3));
      vao[VAO_COLORED_SEGMENTS].release();
    ...
    if (m_draw_lines) {
      ...
      glDrawArrays(GL_LINES, 0,
                   static_cast<GLsizei>(arrays[POS_MONO_LINES].size() / 3));
      vao[VAO_MONO_LINES].release();
      ...
      glLineWidth(m_size_lines);
      glDrawArrays(GL_LINES, 0,
                   static_cast<GLsizei>(arrays[POS_COLORED_LINES].size() / 3));
      vao[VAO_COLORED_LINES].release();
    ...
    if (m_draw_faces) {
      ...
      glDrawArrays(GL_TRIANGLES, 0,
                   static_cast<GLsizei>(arrays[POS_MONO_FACES].size() / 3));
      vao[VAO_MONO_FACES].release();
      ...
      glDrawArrays(GL_TRIANGLES, 0,
                   static_cast<GLsizei>(arrays[POS_COLORED_FACES].size() / 3));
      vao[VAO_COLORED_FACES].release();
  ...
  virtual void init() {
    ...
    setKeyDescription(::Qt::Key_G,
                      "Switch between flat/Gouraud shading display");
    ...
    setKeyDescription(::Qt::Key_Plus + ::Qt::ControlModifier,
                      "Increase size of vertices");
    setKeyDescription(::Qt::Key_Minus + ::Qt::ControlModifier,
                      "Decrease size of vertices");
    setKeyDescription(::Qt::Key_PageDown,
                      "Increase light (all colors, use shift/alt/ctrl for one "
                      "rgb component)");
    setKeyDescription(::Qt::Key_PageUp,
                      "Decrease light (all colors, use shift/alt/ctrl for one "
                      "rgb component)");
    ...
    this->camera()->setSceneBoundingBox(
        CGAL::qglviewer::Vec(bb.xmin(), bb.ymin(), bb.zmin()),
  ...
  virtual void keyPressEvent(QKeyEvent *e) {
    ...
    if ((e->key() == ::Qt::Key_E) && (modifiers == ::Qt::NoButton)) {
      ...
      displayMessage(
          QString("Draw faces=%1.").arg(m_draw_faces ? "true" : "false"));
      update();
      ...
      displayMessage(
          QString("Mono color=%1.").arg(m_use_mono_color ? "true" : "false"));
      update();
    } else if ((e->key() == ::Qt::Key_N) && (modifiers == ::Qt::NoButton)) {
      m_inverse_normal = !m_inverse_normal;
      displayMessage(QString("Inverse normal=%1.")
                         .arg(m_inverse_normal ? "true" : "false"));
      negate_all_normals();
      redraw();
    } else if ((e->key() == ::Qt::Key_V) && (modifiers == ::Qt::NoButton)) {
      m_draw_vertices = !m_draw_vertices;
      displayMessage(
          QString("Draw vertices=%1.").arg(m_draw_vertices ? "true" : "false"));
      update();
  ...
  virtual QString helpString(const char *title) const {
    ...
    text += "Double clicks automates single click actions: A left button "
            "double click aligns the closer axis with the camera (if close "
            "enough). ";
...
template <class T> void draw(const T &, const char *, bool) {
  std::cerr
      << "Impossible to draw because CGAL_USE_BASIC_VIEWER is not defined."
      << std::endl;
}

template <class T> void draw(const T &, const char *) {
  std::cerr
      << "Impossible to draw because CGAL_USE_BASIC_VIEWER is not defined."
      << std::endl;
}
