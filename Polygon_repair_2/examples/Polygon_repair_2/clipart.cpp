...
//    }
    ...
    start_time = clock();
    Polygon_repair_2::Triangulation::Face_handle search_start;
    for (auto const& edge: edges_to_insert) {
      Polygon_repair_2::Triangulation::Vertex_handle va = pr.triangulation().insert(edge.first, search_start);
      Polygon_repair_2::Triangulation::Vertex_handle vb = pr.triangulation().insert(edge.second, va->face()); // vb is likely close to va
