...
void test_struct(){
  Node* data_struct = generate_nodes( 5);
  {
    ...
    { // Open own scope to hide local variables.
        ...
        CGAL_test_assert( ! CGAL::is_empty_range( i, start));
        ...
        CGAL_test_assert( i == start);  // Equality checked.
    ...
    { // Open own scope to hide local variables.
        ...
        (void)k2;
        CGAL_test_assert( k2.key == 1);
        Struct_circulator::pointer         p1 = &k1;
        (void)p1;
        CGAL_test_assert( (*p1).key == 1);
        k1.key = 3;
        CGAL_test_assert( k1.key == 3);
        CGAL_test_assert( k2.key == 3);
        CGAL_test_assert( (*p1).key == 3);
        k1.key = 6;
        CGAL_test_assert( k1.key == 6);
        CGAL_test_assert( k2.key == 6);
        CGAL_test_assert( (*p1).key == 6);
...
void test_array() {
  {
    ...
    { // Open own scope to hide local variables.
        ...
        jj = 4 + jj;
        CGAL_test_assert( jj == start);
        Circulator ij = jj - 5;
        CGAL_test_assert( ij == start);
    ...
    { // Open own scope to hide local variables.
        ...
        CGAL_test_assert( start - i == 1 ||  start - i == -1);
        CGAL_test_assert( i - start == 1 ||  i - start == -1);
