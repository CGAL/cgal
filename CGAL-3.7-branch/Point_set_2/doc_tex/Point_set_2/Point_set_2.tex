...
The \ccc{CGAL::Point_set_2} class in this section is inherited
from the two-dimensional \cgal\ {\em Delaunay Triangulation} data type.

The \ccc{CGAL::Point_set_2} class depends on two template parameters \ccc{T1} and \ccc{T2}.
They are used as template parameters for the \ccc{CGAL::Delaunay_triangulation_2}
class \ccc{CGAL::Point_set_2} is inherited from. \ccc{T1} is a model for the
geometric traits and \ccc{T2} is a model for the triangulation data structure that the Delaunay triangulation
expects.

The \ccc{CGAL::Point_set_2} class supports the following kinds of queries:
\begin{itemize}
...
\ccIncludeExampleCode{Point_set_2/range_search.cpp}
