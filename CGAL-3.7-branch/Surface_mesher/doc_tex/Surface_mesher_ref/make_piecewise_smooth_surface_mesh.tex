\RCSdef{\RCSpiecewisesmoothsurfacemesherRev}{$Id$}
\RCSdefDate{\RCSpiecewisesmoothsurfacemesherDate}{$Date$}
% |
...
\ccInclude{CGAL/make_piecewise_smooth_surface_mesh.h}
...
\ccc{PiecewiseSmoothSurfaceMeshComplex_2InTriangulation_3},
...
of the concept \ccc{PiecewiseSmoothSurfaceMeshTraits_3}.
...
of \ccc{make_piecewise_smooth_surface_mesh},  the surface type  is given  
...
\item In the second overloaded version of \ccc{make_piecewise_smooth_surface_mesh}, 
the surface mesh generator traits type is provided
by the  template parameter \ccc{PiecewiseSmoothSurfaceMeshTraits_3}
...
that is  a model of \ccc{PiecewiseSmoothSurfaceMeshTraits_3}
...
\ccc{Surface_mesher_traits_generator_3<Surface>} for polyhedral surfaces
(\ccc{Polyhedral_surface_3<Traits>}), that can represent piecewise
...
the output surface mesh. The template parameter \ccc{EdgesCriteria} has to be a model
of the concept \ccc{PiecewiseSmoothSurfaceMeshEdgesCriteria_3}.
The argument of type  \ccc{EdgesCriteria} passed to the surface
...
\ccc{PiecewiseSmoothSurfaceMeshEdgesCriteria_3} \\
