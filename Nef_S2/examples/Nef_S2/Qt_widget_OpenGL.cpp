...
namespace CGAL {
    ...
    Qt_widget_OpenGL::~Qt_widget_OpenGL() {}
    ...
    void Qt_widget_OpenGL::slotNextObject() {
        if (object_.size() < 2) return;
        object_index_ = (object_index_+1) % object_.size();
        if (!label_[object_index_].empty()) {
            std::cout << "Displaying: '" << label_[object_index_] <<"'\n";
        }
        // controlMenu(RESET_CONTROL);
        object_[object_index_]->init();
        updateGL();
    }

    void Qt_widget_OpenGL::slotPrevObject() {
        if (object_.size() < 2) return;
        object_index_ = (object_index_+object_.size()-1) % object_.size();
        if (!label_[object_index_].empty()) {
            std::cout << "Displaying: '" << label_[object_index_] <<'\n';
        }
        // controlMenu(RESET_CONTROL);
    ...
    void Qt_widget_OpenGL::controlMenu(int index) {
    ...
    void Qt_widget_OpenGL::renderMenu(int index) {
        object_[object_index_]->set_style(index);
        updateGL();
    }

    void Qt_widget_OpenGL::optionsMenu(int index) {
        object_[object_index_]->toggle(index);
        updateGL();
    }
