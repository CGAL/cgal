...
void SM_io_parser<Decorator_>::read() 
{
  if ( !check_sep("Plane_map_2") )  
    CGAL_error("SM_io_parser::read: no embedded_PM header.");
  if ( !(check_sep("vertices") && (in >> vn)) ) 
    CGAL_error("SM_io_parser::read: wrong vertex line.");
  if ( !(check_sep("edges") && (in >> en) && (en%2==0)) )
    CGAL_error("SM_io_parser::read: wrong edge line.");
  if ( !(check_sep("loops") && (in >> ln)) )
    CGAL_error("SM_io_parser::read: wrong loop line.");
  if ( !(check_sep("faces") && (in >> fn)) )
    CGAL_error("SM_io_parser::read: wrong face line.");
  ...
  for(i=0; i<vn; i++) {
    if (!read_vertex(SVertex_of[i]))
      CGAL_error("SM_io_parser::read: error in node line");
  }
  for(i=0; i<en; i++) {
    if (!read_edge(Edge_of[i]))
      CGAL_error("SM_io_parser::read: error in edge line");
  }
  if ( ln == 2 ) {
  ...
  for(i=0; i<fn; i++) {
    if (!read_face(SFace_of[i]))
      CGAL_error("SM_io_parser::read: error in face line");
  }
}
