...
struct Lambda
{
    ...
    Lambda(int zero_one)
      : approx(zero_one), is_zero(zero_one == 0), is_one(zero_one == 1), is_exact(true)
    {}

    Lambda(Approx approx, const Point& circle_center, distance_t radius, const Point& line_start, const Point& line_end, bool is_start)
        : approx(approx), circle_center(circle_center), line_start(line_start), line_end(line_end), radius(radius), is_zero(false), is_one(false), is_exact(false), is_start(is_start)
    {}
  ...
  bool operator!=(const Lambda& other) const
  {
    ...
    bool operator<(const Lambda& other) const
    {
        if ((is_zero && other.is_zero) || (is_one && other.is_one)) return false;
        if ((is_zero && (!other.is_zero)) || (!is_one && other.is_one)) return true;
...
namespace CGAL {
  ...
  CGAL::Interval_nt<> to_interval(const Lambda& lambda)
  {
...
public:
        ...
        std::string to_string() const {
          ...
          // AF  avoid +    stream << std::fixed << std::setprecision(10) << (double) point + fraction;
          return stream.str();
        }
