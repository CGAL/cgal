...
In case \ccc{NT} is a model of \ccc{RealEmbeddable} the instance is also 
\ccc{RealEmbeddable}.

For example let \ccc{Integer} be some type representing $\Z$, then 
\ccc{Sqrt_extension<Integer,Integer>} is able to represent $\Z[\sqrt{discr}]$
...
e.g. \ccc{Sqrt_extension<Sqrt_extension<Integer,Integer>,Integer>} is able to represent 
...
%number types such as \ccc{leda_real} or \ccc{CORE::Expr} 
%\footnote{ For a details discussion on these types see 
%\cite{mn-lpcgc-00, klpy-clp-99}} 
%being a model of the superior concept \ccc{FieldWithSqrt} should 
...
%On the other hand, an object of \ccc{Sqrt_extension} represents an extension 
%by one square root only.  
...
with respect to the algebraic category of \ccc{NT}.
