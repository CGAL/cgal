...
\ccMethod{Gmpfr abs(Precision_type p);}
        ...
        and rounded using the default rounding mode. If \ccc{p} is not
        specified, the precision used is the maximum between \ccVar
...
%        {Returns the cubic root of \ccVar, with precision \ccc{p},
%        and rounded using the default rounding mode. If \ccc{p} is not
%        specified, the precision used is the maximum between \ccVar
%        's precision and the default.}

\ccMethod{Gmpfr kthroot(int k,Precision_type p);}
        {Returns the k-th root of \ccVar, with precision \ccc{p}, and
        rounded using the default rounding mode. If \ccc{p} is not
        specified, the precision used is the maximum between \ccVar
        's precision and the default.}

\ccMethod{Gmpfr square(Precision_type p);}
        {Returns the square of \ccVar, with precision \ccc{p}, and rounded
        using the default rounding mode. If \ccc{p} is not specified,
...
\ccMethod{bool is_square();}
        {Returns \ccc{true} iff \ccVar~is a the square of a number
        representable by an object of this type.}

\ccMethod{bool is_square(const Gmpfr &y);}
        {Returns \ccc{true} iff \ccVar~is the the square of \ccc{y}.}
...
\ccFunction{std::istream& operator>>(std::istream& in, Gmpfr& f);}
        ...
        \ccc{Gmpfr}.  The number can be an integer in base 10, as well
        as a floating-point number in the same base. After being read,
        it is rounded using the default rounding mode to the precision
        of \ccc{f}.}
