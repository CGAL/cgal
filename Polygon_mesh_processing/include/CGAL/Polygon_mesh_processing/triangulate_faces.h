...
OutputIterator
face_triangulation(typename boost::graph_traits<PolygonMesh>::face_descriptor f,
                   PolygonMesh& pmesh,
                   OutputIterator out,
                   const NamedParameters& np = parameters::default_values())
{
  ...
  //Face_info
  typedef typename internal::Triangulate_modifier<PolygonMesh, VPMap, Kernel, internal::Default_visitor<PolygonMesh>>::Face_info Face_info;

  //CDT
  typedef typename boost::graph_traits<PolygonMesh>::halfedge_descriptor halfedge_descriptor;
  typedef CGAL::Projection_traits_3<Kernel>   P_traits;
  typedef CGAL::Triangulation_vertex_base_with_info_2<halfedge_descriptor,
                                                      P_traits>        Vb;
  typedef CGAL::Triangulation_face_base_with_info_2<Face_info,
                                                    P_traits>          Fb1;
  ...
  if(cdt.dimension() != 2 || cdt.number_of_vertices() != original_size)
    return out;
      ...
      fit != end; ++fit)
  {
    if(!is_external(fit))
    {
      ...
      halfedge_descriptor h1 = fit->info().e[1];
      halfedge_descriptor h2 = fit->info().e[2];
      *out++=std::make_tuple(target(h0, pmesh), target(h1, pmesh), target(h2,pmesh));
    }
  }
