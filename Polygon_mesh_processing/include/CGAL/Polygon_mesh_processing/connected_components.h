                                        ...
                                        , const NamedParameters& np)
{
  ...
  CGAL_FOREACH(std::size_t i, components_to_keep)
    cc_to_keep.insert(i);
  ...
  CGAL_FOREACH(face_descriptor f, faces(pmesh)){
    if (get(fcm, f) == 1){
      CGAL_FOREACH(halfedge_descriptor h, halfedges_around_face(halfedge(f, pmesh), pmesh)){
                                ...
                                , const CGAL_PMP_NP_CLASS& np)
{
  ...
  CGAL_FOREACH(face_descriptor f, components_to_remove)
                             ...
                             , const CGAL_PMP_NP_CLASS& np)
{
  ...
  std::vector<std::size_t> cc_to_keep;
  CGAL_FOREACH(face_descriptor f, components_to_keep)
