...
{
    angle_remeshing(pmesh, faces(pmesh), edges(pmesh), np);
}

template<typename PolygonMesh>
void angle_remeshing(PolygonMesh& pmesh)
{
    angle_remeshing(pmesh, faces(pmesh), edges(pmesh), parameters::all_default());
}
...
void area_remeshing(PolygonMesh& pmesh, const FaceRange& faces, const EdgeRange& edges, const NamedParameters& np)
{
    ...
    ECMap ecmap = (boost::is_same<ECMap, internal::Edge_constraint_map<PolygonMesh, EdgeRange> >::value)
    ? choose_param(get_param(np, internal_np::edge_is_constrained),
                   internal::Edge_constraint_map<PolygonMesh, EdgeRange>(edges)) // to fix this
    : choose_param(get_param(np, internal_np::edge_is_constrained),
                   internal::Edge_constraint_map<PolygonMesh, EdgeRange>());
...
{
    area_remeshing(pmesh, faces(pmesh), edges(pmesh), np);
}

template<typename PolygonMesh>
void area_remeshing(PolygonMesh& pmesh)
{
    area_remeshing(pmesh, faces(pmesh), edges(pmesh), parameters::all_default());
}

template<typename PolygonMesh, typename NamedParameters> // to fix: use constraits
