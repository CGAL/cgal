...
namespace internal {
template <class FT>
FT get_maximum_radius(int dimensions, int sample_size, FT domain_size) {
  ...
  default:
    ...
    for (int d = d_start; d <= dimensions; d += 2) c *= FT(2) * CGAL_PI / FT(d);
...
template <class FT>
FT get_minimum_radius(int inputSize, int outputSize, FT beta, FT gamma, FT r_max) {
...
void poisson_eliminate2(PointRange points, unsigned int number_of_points, OutputIterator out, const NamedParameters& np = parameters::default_values()) {
  ...
  auto tile_point = [&tiling_points, &bb, &r_max, &ambient_dimension](const Point& p, std::size_t dim = 0) {
    auto do_tiling = [&tiling_points, &bb, &r_max, &ambient_dimension](const auto& self, const Point& p, std::size_t dim) -> void {
      ...
      if (bb.min_coord(dim) > (*(it + dim) - r_max)) {
        FT v = 2 * bb.min_coord(dim) - (*(it + dim));
        Point p2;
      ...
      if (bb.max_coord(dim) < (*(it + dim) + r_max)) {
        FT v = 2 * bb.max_coord(dim) - (*(it + dim));
        Point p2;
        internal::copy_and_replace(p, p2, dim, v);
        tiling_points.emplace_back(p2);
        if (dim + 1 < ambient_dimension)
