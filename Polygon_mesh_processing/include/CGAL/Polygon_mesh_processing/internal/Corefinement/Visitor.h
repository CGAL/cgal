...
//   CGAL_FOREACH(halfedge_descriptor hc,halfedges_around_target(h,tm))
...
// }
  ...
  struct Face_boundary{
                                      ...
                                      TriangleMesh& tm)
    {
      for (int i=0; i<3; ++i)
      {
        ...
        h = next(h, tm);
        CGAL_FOREACH(std::size_t id, node_ids_array[i])
        {
    ...
    std::map<Node_id,CDT_Vertex_handle>& id_to_CDT_vh)
  {
    if (node_id < number_coplanar_vertices){
      ...
      if (it_neighbors!=coplanar_constraints.end())
      {
        ...
        CGAL_FOREACH(Node_id id,it_neighbors->second)
        {
  ...
  {
    CGAL_FOREACH(Node_id id, node_ids)
    {
      ...
      if (!neighbors.empty())
      {
        ...
        CGAL_FOREACH(Node_id id_n,neighbors)
        {
                 ...
                 const VertexPointMap& vpm2)
  {
    ...
    //   Face_boundaries& face_boundaries=mesh_to_face_boundaries[&tm];
            ...
            ++it_node_2_hedge)
      {
        ...
        if ( !neighbors.empty() )
        {
          CGAL_FOREACH(Node_id node_id, neighbors)
          {
      ...
      it=on_edge.begin(); it!=on_edge.end(); ++it)
    {
                                         ...
                                         ++it2)
      {
        ...
        CGAL_FOREACH(std::size_t node_id, node_ids)
        {
      ...
      it=on_face.begin(); it!=on_face.end(); ++it)
    {
            ...
            it!=on_face_map.end();++it)
      {
        ...
        if (it_fb!=face_boundaries.end()) //if f not a triangle?
        {
          ...
          for (int i=0;i<3;++i){
            ...
            if (!ids_on_edge.empty()){ //is there at least one node on this edge?
              ...
              CGAL_FOREACH(Node_id id, ids_on_edge)
              {
        ...
        CGAL_FOREACH(const Node_id_pair& node_id_pair, constrained_edges)
        {
