    ...
    const Mesh_to_map_node&)
  {
    ...
    std::size_t nb_patches_tm1 =
      PMP::connected_components(tm1,
                                bind_maps(fids1,&tm1_patch_ids[0]),
    ...
    std::size_t nb_patches_tm2 =
      PMP::connected_components(tm2,
                                bind_maps(fids2,&tm2_patch_ids[0]),
                                params::edge_is_constrained_map(
                                    is_marked_2)
            ...
            it_end=an_edge_per_polyline.end(); it!=it_end;++it)
    {
      ...
      if ( is_border_edge(h1, tm1) ){
        ...
        else
        {
          ...
          CGAL_assertion(
              ( index_p1 == -1 ? nodes.to_exact(get(vpm1,p1)): nodes.exact_node(index_p1) ) !=
              ( index_q1 == -1 ? nodes.to_exact(get(vpm2,q1)): nodes.exact_node(index_q1) )
          &&
              ( index_p2 == -1 ? nodes.to_exact(get(vpm1,p2)): nodes.exact_node(index_p2) ) !=
              ( index_q1 == -1 ? nodes.to_exact(get(vpm2,q1)): nodes.exact_node(index_q1) )
          &&
              ( index_p1 == -1 ? nodes.to_exact(get(vpm1,p1)): nodes.exact_node(index_p1) ) !=
              ( index_q2 == -1 ? nodes.to_exact(get(vpm2,q2)): nodes.exact_node(index_q2) )
          &&
              ( index_p2 == -1 ? nodes.to_exact(get(vpm1,p2)): nodes.exact_node(index_p2) ) !=
              ( index_q2 == -1 ? nodes.to_exact(get(vpm2,q2)): nodes.exact_node(index_q2) )
          );
