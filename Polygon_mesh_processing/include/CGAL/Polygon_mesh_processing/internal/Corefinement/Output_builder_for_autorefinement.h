...
// functions called by the intersection visitor
    ...
    const Mesh_to_map_node&)
  {
    ...
    CGAL_FOREACH(const Pair_type& p, all_intersection_edges_map)
    {
    ...
    CGAL_FOREACH(edge_descriptor ed, inter_edges_to_remove)
    ...
    CGAL_FOREACH(std::size_t& patch_id, patch_ids)
    ...
    // the same status.
    CGAL_FOREACH(edge_descriptor e, edges_no_longer_on_intersection)
