                                             ...
                                             bool merge_points=true)
{
  ...
  CGAL_FOREACH(edge_descriptor ed, edges(pm))
  {
    if (is_border(ed, pm))
    {
  ...
  CGAL_FOREACH(const Halfedge_pair& p, matching_hedges)
  {
    ...
    {
      bool skip=false;
      CGAL_FOREACH(halfedge_descriptor h, halfedges_around_source(p.first, pm))
      ...
      CGAL_FOREACH(halfedge_descriptor h, halfedges_around_target(p.first, pm))
        if ( get(vpm, target(h, pm)) == get(vpm, source(h, pm)) )
        {
          // ignore that edge
          skip=true;
  ...
  CGAL_FOREACH(const Halfedge_pair& p, halfedges_to_stitch)
  {
    ...
    CGAL_FOREACH( halfedge_descriptor h, hedges)
    {
