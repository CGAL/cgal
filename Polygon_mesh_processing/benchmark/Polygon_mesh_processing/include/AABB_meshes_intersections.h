                                         ...
                                         const NamedParameters2& np2 = parameters::default_values())
{
  ...
  using Bbox_map_1 = typename boost::property_map<TriangleMesh1, Face_bbox_tag>::const_type;
  using Bbox_map_2 = typename boost::property_map<TriangleMesh2, Face_bbox_tag>::const_type;
  using Ref_point_map_1 = typename boost::property_map<TriangleMesh1, Face_ref_point_tag>::const_type;
  using Ref_point_map_2 = typename boost::property_map<TriangleMesh2, Face_ref_point_tag>::const_type;

  using Kernel = typename Traits_1::geom_traits;
  CGAL::Cartesian_converter<Kernel, Epick> to_epick;

  Bbox_map_1 bbmap1 = get(Face_bbox_tag(), tm);
  Bbox_map_2 bbmap2 = get(Face_bbox_tag(), tm);
  Ref_point_map_1 rpm1 = get(Face_ref_point_tag(), tm);
  Ref_point_map_2 rpm2 = get(Face_ref_point_tag(), tm);
  ...
  if constexpr(std::is_same_v<Concurrency_tag, Parallel_tag>)
  {
    tbb::parallel_for(std::size_t(0), faces(tm1).size(), [&](std::size_t i){
      face_descriptor f(i);
      put(bbmap1, f, face_bbox(f, tm1));
      put(rpm1, f, to_epick(get(vpm1, target(halfedge(f, tm1), tm1))) );
    });

    tbb::parallel_for(std::size_t(0), faces(tm2).size(), [&](std::size_t i){
      face_descriptor f(i);
      put(bbmap2, f, face_bbox(f, tm2));
      put(rpm2, f, to_epick(get(vpm2, target(halfedge(f, tm2), tm2))) );
    });
  }
  else
#endif
