...
//
// $URL: svn+ssh://scm.gforge.inria.fr/svn/cgal/trunk/Segment_Delaunay_graph_Linf_2/include/CGAL/Segment_Delaunay_graph_Linf_site_2.h $
// $Id: Segment_Delaunay_graph_Linf_site_2.h 56667 2010-06-09 07:37:13Z sloriot $
// 
//
...
#ifndef CGAL_SEGMENT_DELAUNAY_GRAPH_LINF_SITE_H
#define CGAL_SEGMENT_DELAUNAY_GRAPH_LINF_SITE_H
...
template <class Gt>
class Segment_Delaunay_graph_Linf_site_2 
{
public:
...
protected:
  ...
  typedef Segment_Delaunay_graph_Linf_site_2<Geom_traits>  Self;

public:
  Segment_Delaunay_graph_Linf_site_2() : type_(0) {}
...
operator<<(std::ostream& os, 
	   const Segment_Delaunay_graph_Linf_site_2<R>& s)
{
  if (!s.is_defined())
    return os << "u";
  if (s.is_point())
...
operator>>(std::istream &is,
	   Segment_Delaunay_graph_Linf_site_2<R>& t)
{
  typedef Segment_Delaunay_graph_Linf_site_2<R>   Site_2;
...
Stream&
operator<<(Stream& str, Segment_Delaunay_graph_Linf_site_2<R>& t)
{
  if ( t.is_defined() ) {
    if ( t.is_point() ) {
      str << "p " << t.point();
...
#endif // CGAL_SEGMENT_DELAUNAY_GRAPH_LINF_SITE_H
