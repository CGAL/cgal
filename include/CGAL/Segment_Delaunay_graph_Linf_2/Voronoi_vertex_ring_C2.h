...
private:
  ...
  Sign incircle_p_no_easy(const Site_2& st, PPS_Type ) const
  {
    ...
    if (crude != ZERO) {
      ...
      std::cout << "debug refining in incircle_p_no_easy PPS pqr=("
        << p_ << ", " << q_ << ", " << r_ << "), " 
        << "t=" << t
        << std::endl;
      // here crude == ZERO, so
  ...
  Sign incircle_p_no_easy(const Site_2& st, PSS_Type ) const
  {
    ...
    if (s_Q != ZERO) {
      ...
      std::cout << "debug refining in incircle_p_no_easy PSS pqr=("
        << p_ << ", " << q_ << ", " << r_ << "), "
        << "t=" << t
        << std::endl;
      // here crude == ZERO, so
  ...
  Sign incircle(const Line_2& l, PPP_Type) const
  {
    ...
    if (cr != other) {
      std::cout << "incircle l PPP instead of 0 returning " << other <<
        std::endl;
    }
  ...
  Sign incircle(const Line_2& l, PPS_Type) const
  {
    ...
    // here cr == EQUAL == ZERO, so
    // we might have to refine
    std::cout << "debug refining in incircle l PPS pqr=("
      << p_ << ", " << q_ << ", " << r_ << "), " 
      << "hp(x,y)=" << hp.x() << ' ' << hp.y() 
      << ", l: " << l.a() << ' ' << l.b() << ' ' <<  l.c() 
      << ", u(x,y,z)= " << ux_ << ' ' << uy_ << ' ' << uz_  
    ...
    if (cr != other) {
      std::cout << "incircle l PPS instead of 0 returning " << other <<
        std::endl;
    }
  ...
  Sign incircle(const Line_2& l, PSS_Type) const
  {
    ...
    // here cr == EQUAL == ZERO, so
    // we might have to refine
    std::cout << "debug refining in incircle l PSS pqr=("
      << p_ << ", " << q_ << ", " << r_ << "), "
      << "hp(x,y)=" << lhp.x() << ' ' << lhp.y()
      << ", l: " << l.a() << ' ' << l.b() << ' ' <<  l.c()
      << ", u(x,y,z)= " << ux_ << ' ' << uy_ << ' ' << uz_
    ...
    if (cr != other) {
      std::cout << "incircle l PSS instead of 0 returning " << other <<
        std::endl;
    }
  ...
  Sign incircle(const Line_2& l, SSS_Type) const
  {
    ...
    if (cr != other) {
      std::cout << "incircle l SSS instead of 0 returning " << other <<
        std::endl;
    }
  ...
  Sign incircle_s_no_easy(const Site_2& t, Type type) const
  {
    ...
    std::cout << "debug incircle_s_no_easy d2=" << d2 << std::endl;
