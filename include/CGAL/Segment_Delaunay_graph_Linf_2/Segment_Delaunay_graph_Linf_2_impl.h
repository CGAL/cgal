...
Segment_Delaunay_graph_Linf_2(const Segment_Delaunay_graph_Linf_2& other)
  : DG(other.geom_traits())
{
  Segment_Delaunay_graph_Linf_2&
    non_const_other = const_cast<Segment_Delaunay_graph_Linf_2&>(other);
...
// assignment operator
template<class Gt, class ST, class D_S, class LTag>
typename Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::Self&
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
...
template<class Gt, class ST, class D_S, class LTag>
typename Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::Vertex_handle
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
	      ...
	      Vertex_handle vnearest)
{
  ...
  if ( s != NEGATIVE ) {
    ...
    do {
      ...
      //std::cout << "debug interior e=" << e << std::endl; 
  ...
  //std::cout << "debug insert_point2 init cf region" << std::endl;
  initialize_conflict_region(start_f, l);


  //std::cout << "debug insert_point2 expand cf region" << std::endl;
#ifdef CGAL_SDG_NO_FACE_MAP
  ...
  //std::cout << "debug insert_point2 retriangulate" << std::endl;
#ifdef CGAL_SDG_NO_FACE_MAP
...
find_faces_to_split(const Vertex_handle& v, const Site_2& t) const
{
  ...
  do {
	...
	sv_ep = ff1->vertex(ccw_v)->site();
        os1 = oriented_side(sitev_supp, sv_ep, sitev_supp, t);
      } else {
	...
	sv_ep = ff1->vertex( cw_v)->site();
        os1 = oriented_side(sv_ep, sitev_supp, sitev_supp, t);
      }
    } else {
	...
	sv_ep = ff2->vertex(ccw_v)->site();
        os2 = oriented_side(sitev_supp, sv_ep, sitev_supp, t);
      } else {
	...
	sv_ep = ff2->vertex( cw_v)->site();
        os2 = oriented_side(sv_ep, sitev_supp, sitev_supp, t);
      }
    } else {
			...
			Vertex_handle vnearest)
{
  ...
  do {
	...
	Storage_site_2 sss = split_storage_site(ss, ssvp, true);
        ...
        // philaris: sss1 is unused-but-set-variable
...
template<class Gt, class ST, class D_S, class LTag>
typename Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::Vertex_list
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
...
{
  //std::cout << "debug retriangulate entering v="
  //  << v->site() << std::endl;
...
template<class Gt, class ST, class D_S, class LTag>
typename Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::size_type
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
...
template<class Gt, class ST, class D_S, class LTag>
typename Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::Arrangement_type
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
...
print_error_message() const
{
  ...
  std::cerr << "A segment-segment intersection was found."
	    << std::endl;
  std::cerr << "The Segment_Delaunay_graph_Linf_2 class is not configured"
...
template<class Gt, class ST, class D_S, class LTag>
typename Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::Storage_site_2
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
...
void
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
copy(Segment_Delaunay_graph_Linf_2& other)
{
...
void
Segment_Delaunay_graph_Linf_2<Gt,ST,D_S,LTag>::
copy(Segment_Delaunay_graph_Linf_2& other, Handle_map& hm)
{
