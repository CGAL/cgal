...
\tparam Info is the information the user wants to add to a face. Must be `DefaultConstructible` and `Assignable`.
\tparam Gt must be a model of `HyperbolicDelaunayTriangulationTraits_2`.
\tparam Fb must be a model of  `HyperbolicTriangulationFaceBase_2`. Defaults to `Hyperbolic_triangulation_face_base_2<GT>`.


\sa Hyperbolic_triangulation_face_base_2

\cgalModels HyperbolicTriangulationFaceBase_2
*/

template < typename Info, typename Gt, typename Fb >
class Hyperbolic_triangulation_face_base_with_info_2: public Fb {
...
public:
  ...
  /// @{
    /*!
      Returns a constant reference to the info stored in the face.
    */
    const Info& info() const;

    /*!
      Returns a non-constant reference to the info stored in the face.
    */
    Info&       info();
  /// @}
