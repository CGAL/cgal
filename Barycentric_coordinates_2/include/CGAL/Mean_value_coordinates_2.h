 ...
 * with respect to an arbitrary simple polygon. This class is parameterized by `CGAL::Polygon_2` class and `Iterator` class.
 ...
 * Mean Value coordinates can be computed only approximately due to the involved square root operation,
 * and they are necesserily positive only inside the kernel of a star-shaped polygon.
 *
 * \sa `Iterator`
 *
 */
...
public:
    ...
    /// Creates an instance of Mean_value_coordinates_2 class for a provided polygon passed as a reference.
 ...
 * with respect to an arbitrary simple polygon. This class is parameterized by `CGAL::Polygon_2` class and a Container class.
...
public:
    ...
    /// Computes Mean Value coordinates for a query point, which coincides with one of the polygon's vertices, with beforehand known index.
    ...
    /// Computes Mean Value coordinates for a query point on the polygon's boundary with beforehand known index of the edge to which this point belongs.
    ...
    /// Computes Mean Value coordinates for any query point with respect to all the vertices of the polygon.
