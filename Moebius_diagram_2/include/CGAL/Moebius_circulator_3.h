...
class Moebius_circulator_3
{
 public:
  ...
  Edge edge () const 
  { 
    return Edge (_curr, _curr->index(_s), _curr->index (_t)); 
  }

  Vertex_handle source () const { return _s; }
  Vertex_handle target () const { return _t; }
  ...
  bool operator== (const Moebius_circulator_3<Tds> &c) {
    return cell() == c.cell() && source() == c.source() 
      && target() == c.target();
  }
 ...
 protected:
  ...
  static Facet facet (const Cell_handle &c, 
		      const Vertex_handle &s, 
		      const Vertex_handle &t) {
    return Facet (c, next_around_edge (c->index(s), c->index(t)));
  }
    
  static Vertex_handle vertex (const Cell_handle &c, 
			       const Vertex_handle &s, 
			       const Vertex_handle &t) {
    return c->vertex (next_around_edge (c->index(t), c->index(s)));
  }
