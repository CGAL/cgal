...
#include "include/CGAL_ipelets/hyperbolic_octagon_translations.h"
...
const std::string helpmsg[] = {
  ...
  "Produce the images of a set of points under the action of the hyperbolic translation group that identifies opposite sides of the regular hyperbolic octagon with angles pi/4",
  "Produce the images of a set of points under the action of translation 'a'",
  "Produce the images of a set of points under the action of translation 'inv-b'",
  "Produce the images of a set of points under the action of translation 'c'",
  "Produce the images of a set of points under the action of translation 'inv-d'",
  "Produce the images of a set of points under the action of translation 'inv-a'",
  "Produce the images of a set of points under the action of translation 'b'",
  "Produce the images of a set of points under the action of translation 'inv-c'",
  "Produce the images of a set of points under the action of translation 'd'",
  "Test"
};
...
void hyperbolicIpelet::protected_run(int fn)
{
  ...
  read_active_objects(
		      CGAL::dispatch_or_drop_output<Point_2,Circle_2,Segment_2>(
  ...
  case 5: {
    std::list<Point_2> images = compute_images<Kernel>(poincare, pt_list);
    for (std::list<Point_2>::iterator pit = images.begin(); pit != images.end(); pit++) {
      draw_in_ipe(*pit);
    }
    return;
    }
  ...
  {
    int tidx = fn - 6;
    std::list<Point_2> images = compute_images_one_direction<Kernel>(poincare, pt_list, tidx);
    for (std::list<Point_2>::iterator pit = images.begin(); pit != images.end(); pit++) {
      draw_in_ipe(*pit);
    }
    return;
  ...
  case 14: {
    std::list<Point_2> images = compute_images_plane_model<Kernel>(*sg_list.begin());
    for (std::list<Point_2>::iterator pit = images.begin(); pit != images.end(); pit++) {
      draw_in_ipe(*pit);
    }
    return;
