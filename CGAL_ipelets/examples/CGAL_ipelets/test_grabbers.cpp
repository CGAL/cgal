...
int main(int, char*[])
{
  ...
  std::copy(sg_list.begin(), sg_list.end(),CGALi::point_grabber<Kernel>(std::back_inserter(pt_list)));  
  ...
  std::copy(pol_list.begin(), pol_list.end(),CGALi::point_grabber<Kernel>(std::back_inserter(pt_list)));  
  
  assert (pt_list.size()==12);
  
  std::copy(pol_list.begin(), pol_list.end(),CGALi::segment_grabber<Kernel>(std::back_inserter(sg_list)));
  ...
  std::copy(l_cir.begin(), l_cir.end(),CGALi::wpoint_grabber<Kernel>(std::back_inserter(l_wp)));

  std::copy(pt_list.begin(), pt_list.end(),CGALi::wpoint_grabber<Kernel>(std::back_inserter(l_wp)));
