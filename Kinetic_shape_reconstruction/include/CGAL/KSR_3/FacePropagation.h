...
//     if (m_parameters.debug)
//       dump_2d_surface_mesh(m_data, pvertex.first, "before");
    ...
    if (m_parameters.debug) {
      ...
      for (auto cropped : pvertices) {
        if (cropped == m_data.null_pvertex()) {
          std::cout << " - pvertices containing null vertex!" << std::endl;
          continue;
        }
        std::cout << "- cropped: " << m_data.str(cropped) << ", " << m_data.point_3(cropped) << std::endl;
        if (m_data.has_iedge(cropped)) {
          std::cout << " - iedge: " << m_data.iedge(cropped);
          if (m_data.direction(cropped) != CGAL::NULL_VECTOR) {
            Vector_2 to_target = m_data.to_2d(cropped.first, m_data.target(m_data.iedge(cropped))) - m_data.point_2(cropped);
            if (to_target * m_data.direction(cropped) > 0) std::cout << " moving towards target " << m_data.target(m_data.iedge(cropped)) << std::endl;
            else std::cout << " moving towards source " << m_data.source(m_data.iedge(cropped)) << std::endl;
          }
          else std::cout << "frozen at " << m_data.ivertex(cropped) << std::endl;
        }
      }
    }

//     if (m_parameters.debug)
//       dump_2d_surface_mesh(m_data, pvertex.first, "after");
  }
  ...
  ********************************/
  ...
  PVertex crop_pvertex_along_iedge(
    ...
    if (m_parameters.debug) {
      std::cout << "- new pvertices: " << m_data.str(pother) << ": " << m_data.point_3(pother) << std::endl;
      std::cout << " - iedge: " << m_data.iedge(pother) << std::endl;
    }
  ...
  std::vector<PVertex> merge_pvertices_on_ivertex(
    ...
    m_data.support_plane(sp_idx).remove_vertex(front.second); // why are these vertices inserted and then again deleted?
  ...
  void apply_back_border_case(
    ...
    { // crop
      ...
      if (m_parameters.debug) {
        std::cout << "- cropped: " << m_data.str(cropped) << ", " << m_data.point_3(cropped) << std::endl;
        std::cout << " - iedge: " << m_data.iedge(cropped);
        Vector_2 to_target = m_data.to_2d(cropped.first, m_data.target(m_data.iedge(cropped))) - future_point;
        if (to_target * m_data.direction(cropped) > 0) std::cout << " moving towards target " << m_data.target(m_data.iedge(cropped)) << std::endl;
        else std::cout << " moving towards source " << m_data.source(m_data.iedge(cropped)) << std::endl;
      }
  ...
  void apply_open_case(
    ...
    if (crossed_iedges.size() == 1) {
      ...
      if (m_parameters.debug) std::cout << "- cropped: " <<
        m_data.str(cropped) << ", " << m_data.iedge(cropped) << " " << m_data.point_3(cropped) << std::endl;
      CGAL_assertion(m_data.is_correctly_oriented(
    ...
    { // first crop
      ...
      if (m_parameters.debug) {
        std::cout << "- cropped 1: " << m_data.str(cropped) << ", " << m_data.point_3(cropped) << std::endl;
        std::cout << " - iedge: " << m_data.iedge(cropped);
        Vector_2 to_target = m_data.to_2d(cropped.first, m_data.target(m_data.iedge(cropped))) - future_points.front();
        if (to_target * m_data.direction(cropped) > 0) std::cout << " moving towards target " << m_data.target(m_data.iedge(cropped)) << std::endl;
        else std::cout << " moving towards source " << m_data.source(m_data.iedge(cropped)) << std::endl;
      }
      CGAL_assertion(m_data.is_correctly_oriented(
    ...
    { // second crop
      ...
      if (m_parameters.debug) {
        std::cout << "- cropped 2: " << m_data.str(cropped) << ", " << m_data.point_3(cropped) << std::endl;
        std::cout << " - iedge: " << m_data.iedge(cropped);
        Vector_2 to_target = m_data.to_2d(cropped.first, m_data.target(m_data.iedge(cropped))) - future_points.back();
        if (to_target * m_data.direction(cropped) > 0) std::cout << " moving towards target " << m_data.target(m_data.iedge(cropped)) << std::endl;
        else std::cout << " moving towards source " << m_data.source(m_data.iedge(cropped)) << std::endl;
      }
      CGAL_assertion(m_data.is_correctly_oriented(
